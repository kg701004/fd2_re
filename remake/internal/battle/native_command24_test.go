...
func TestExecuteNativeCommand24UsesDerivedStatsAndOneMPDebit(t *testing.T) {
	...
	// MP discount.
	actor := &Unit{Camp: Enemy, OnField: true, X: 0, Y: 0, AP: 100, MP: 30, HasNativeRecordByte6: true, NativeRecordByte6: 2}
...
func TestExecuteNativeCommand24RejectsInvalidBookBeforeMutation(t *testing.T) {
	...
	if _, err := st.ExecuteNativeCommand24(actor, target, rand.New(rand.NewSource(1)), nil); err == nil || actor.MP != 30 || actor.Acted || target.HP != 100 {
...
func TestExecuteNativeCommandDerivedStrikeUsesRecoveredID28Multiplier(t *testing.T) {
	...
	got, err := st.ExecuteNativeCommandDerivedStrike(actor, target, 28, rand.New(rand.NewSource(2)), nil)
