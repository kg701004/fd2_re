...
func TestExecuteNativeCommandHealUsesSelectedRecordAndCapsHP(t *testing.T) {
	...
	got, err := st.ExecuteNativeCommandHeal(actor, target, 13, rand.New(rand.NewSource(2)), nil)
...
func TestExecuteNativeCommandHealRejectsFamilyBoundaryBeforeMutation(t *testing.T) {
	actor := &Unit{MP: 5}
	if _, err := (&State{}).ExecuteNativeCommandHeal(actor, nil, 12, rand.New(rand.NewSource(1)), nil); err == nil || actor.MP != 5 || actor.Acted {
