...
func TestNativeAttackCandidatesAppliesInnerRadiusMarker(t *testing.T) {
	...
	targets, err := NativeAttackCandidates(5, 1, Cell{X: 0, Y: 0}, 3, 2, 0, 2, make([]byte, 5), units)
...
func TestNativeAttackCandidatesCrossSkipsInnerRadius(t *testing.T) {
	...
	targets, err := NativeAttackCandidates(3, 1, Cell{X: 0, Y: 0}, 0x11, 99, 0, 2, make([]byte, 3), units)
...
func TestNativeAttackCandidatesRejectsNegativeInnerRadius(t *testing.T) {
	if _, err := NativeAttackCandidates(1, 1, Cell{}, 1, -1, 0, 2, []byte{0}, nil); err == nil {
