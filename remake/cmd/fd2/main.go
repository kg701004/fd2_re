...
// toggleFullscreen switches between windowed and fullscreen. On Windows it
// prefers a borderless-maximized window sized to the real physical monitor
// (see fullscreen_windows.go) over ebiten.SetFullscreen: that API reports
// the DPI-virtualized monitor video mode when the desktop scale isn't 100%,
// which under-fills the real screen with a large integer-scale letterbox
// (confirmed live: 1706x1066 reported instead of the real 2560x1600 at 150%
// scaling). Platforms without a true-size probe fall back to the ebiten API.
func (g *Game) toggleFullscreen() {
	if g.trueFullscreen {
		g.trueFullscreen = false
		ebiten.SetWindowDecorated(true)
		ebiten.SetWindowSize(g.preFullscreenW, g.preFullscreenH)
		ebiten.SetWindowPosition(g.preFullscreenX, g.preFullscreenY)
		return
	}
	if sw, sh, ok := truePhysicalScreenSize(); ok {
		g.preFullscreenW, g.preFullscreenH = ebiten.WindowSize()
		g.preFullscreenX, g.preFullscreenY = ebiten.WindowPosition()
		g.trueFullscreen = true
