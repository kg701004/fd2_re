...
- IDA Pro、Ghidra 與其他反組譯資料庫必須保留原始函式名、全域名稱、線性
  位址、結構偏移及運算元；語意只能以可搜尋註解或受版控的位址／偏移索引
  附加。不可用推測性改名覆蓋唯一的原始定位資訊。
- 每一項附加語意都必須標示「已證實、強推論、假說或未知」並附證據出處。
  沒有推論等級的自訂名稱、反編譯器變數名或結構欄位名只可導航，不可被
  文件、程式或後續工作階段視為原版事實。
- 即使語意已證實，證據匯出仍須同時顯示原始名稱／位址、附加語意與推論
  等級；工具內別名不得取代原始位元組、直接指令、交叉參照與讀寫端證據。
- 對 `[di-6221h]` 這類沒有符號的相對運算元，不可假設 IDA 改名能涵蓋其
  語意；應保留原運算元，另以偏移索引附加語意、等級與出處。匯出流程應
  自動合併這層受審查的索引，避免只靠操作者記得查詢。
- IDA 的直接交叉參照不涵蓋經指標、暫存器或相對基址完成的間接讀寫。若
  某欄位呈現大量讀取卻幾乎沒有寫入，必須追查取址端與後續間接寫入，不可
  因交叉參照清單缺項就宣稱沒有寫入端。
