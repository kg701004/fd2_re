...
class NativeConstructorProjectionTest(unittest.TestCase):
    def test_high_branch_uses_word_plus_two_times_level(self):
        ...
        self.assertEqual(export_units.native_record_word42_for_portrait(tables, 0x44, 3), 0x0306)
        self.assertEqual(export_units.native_record_word46_for_portrait(tables, 0x44, 3), 21)
    ...
    def test_lower_branch_uses_word_plus_three_and_aux_byte_six(self):
        ...
        self.assertEqual(export_units.native_record_word42_for_portrait(tables, 0, 4), 0x0102 + 12)
        self.assertEqual(export_units.native_record_word46_for_portrait(tables, 0, 4), 0x0203 + 15)

    def test_malformed_or_missing_provenance_fails_closed(self):
        tables = {"tables": {"high_class": table(10, [bytearray(9)])}}
        self.assertIsNone(export_units.native_record_word42_for_portrait(tables, 0x44, 1))
        self.assertIsNone(export_units.native_record_word46_for_portrait(tables, 0x44, 1))
        self.assertIsNone(export_units.native_record_word42_for_portrait({}, 0x44, 1))
        self.assertIsNone(export_units.native_record_word46_for_portrait({}, 0x44, 1))
        self.assertIsNone(export_units.native_record_word42_for_portrait(tables, 0x44, 0))
        self.assertIsNone(export_units.native_record_word46_for_portrait(tables, 0x44, 0))
    ...
    def test_real_fixture_has_expected_raw_dimensions(self):
        ...
        self.assertIsInstance(export_units.native_record_word42_for_portrait(tables, 0x44, 1), int)
        self.assertIsInstance(export_units.native_record_word42_for_portrait(tables, 0, 1), int)
        self.assertIsInstance(export_units.native_record_word46_for_portrait(tables, 0x44, 1), int)
        self.assertIsInstance(export_units.native_record_word46_for_portrait(tables, 0, 1), int)
