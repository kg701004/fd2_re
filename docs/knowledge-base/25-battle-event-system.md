...
> **2026-09-08 補述：本節「未能實錘」的部分，下方 2026-09-07 一節其實已經解決了**——
> 我這輪是從工具端獨立撞上同一件事才回頭發現的，先講清楚歸屬，再講真正新的部分。
>
> **已知、本輪只是獨立再確認一次：** 走 LE fixup 重讀 `org_game` 的跳表 `0x51b91`，index 57/58/76/78 分別是
> `0x35833`/`0x35854`/`0x360b6`/`0x36228`，與 2026-09-07 一節記載的四個值**逐一相符**；
> event 56/57/58/59/62 的表值也全部都是合法 Watcom 函式入口。event 58 的表項值就是
> `0x35854`(`push 0x44; call 0x3702f`，`+0x14` 處 `mov esi, 0x5274E`)。
>
> **真正新的、而且是一個更正：`0x356` 不是 relocation 偏移，是版本差。** 2026-09-07 一節寫
> 「§11.7 讀 raw 檔案 + 校準 vs 本輪讀已 relocate 的 image，兩條獨立路徑互相印證」，把
> `+0x356` 當成 raw→relocated 的重定位常數。逐格實測 `org_game` 的 `0x51b91` 跳表，現行參考版**不是這樣**：
>
> | index | 檔案原始 bytes | raw dword | fixup 解出 | 差值 |
> |---|---|---|---|---|
> | 57 | `33 58 02 00` | `0x25833` | `0x35833` | `0x10000` |
> | 58 | `54 58 02 00` | `0x25854` | `0x35854` | `0x10000` |
> | 76 | `b6 60 02 00` | `0x260b6` | `0x360b6` | `0x10000` |
> | 78 | `28 62 02 00` | `0x26228` | `0x36228` | `0x10000` |
>
> 真正的重定位是**一致的 `+0x10000`**(object 1 基底)，四格全同。而 §11.7 記載的
> `0x354dd`/`0x354fe`/`0x35d60`/`0x35ed2` 加 `0x356` 剛好命中上表右欄——這代表那四個值
> **本身就是舊版的「已重定位」位址**，不是未重定位的檔案 bytes；`0x356` 是舊版→新版的
> **版本差**，與 relocation 無關。兩件不同的事被當成同一件，而且因為兩邊都「算得出來」，
> 一直沒有被發現。
>
> **這個混淆造成了一個真的工具 bug。** `tools/extract_native_treasure_event_rules.py` 把
> `0x35854`(**已經是新版位址**)當成舊版常數，再加一次 `+0x356`，輸出 `0x35baa`——起頭
> `83 C4 04`(add esp,4)，落在指令中段，根本不是函式。版本差本身沒錯(同批的 staging
> helper `0x35822` 與 8 個 gate call 加 `0x356` 後都落在正確位置，實測 8/8 都是
> `E8 call -> 0x10b4e`)，錯的是把它套在一個不該套的常數上。該工具已改成從跳表讀(走
> fixup，不再有任何版本常數)、加上序頭檢查(stack-check 位址不寫死，從同表鄰居多數決
> 反推)與 6 項 `--selftest`，並納入 `verify_generated_artifacts.py` 登錄表。
>
> **順帶更正一個敘述範圍**：「`0x354FE` 是落在 event57 handler 中段的 table artifact」
> 是**舊版**的座標系；在現行 EXE 裡 `0x354FE` 的位元組是 `00 00 00 68 40 01 00 00`，
> 引用時應標明版本，不要與現行版本的位址混用。
...
## 2026-09-07：§11.7「event58/76/78 definitively 是 table artifact」**推翻**，§11.7.5 的衍生懸念同時解開

**背景**：§11.7（2026-08-24）用「直接讀 `0x51b91` 跳表原始 bytes + 4 個錨點校準出 relocation
偏移 `0x356`」的方法，確認 event58/76/78 三個槽位的原始值分別是 `0x354fe`/`0x35d60`/`0x35ed2`，
再以指令邊界回溯判定三者都落在鄰居 handler 中段、**無獨立語意**，結論為 table artifact。副作用是
§6.3「event58 = map25 五選一寶物」的入口位址與觸發歸因被撤回，§11.7.5 因此留下一個衍生懸念：
「map25 寶物 slot 在原版 runtime 真正的觸發路徑」變成未解，且 `0x35854` **查無任何已知靜態呼叫者**。

**本輪用新工具 `tools/image_ref_scan.py`（全 image 原始 bytes 掃描，繞過 `-noanalysis` 下已知
不完整的 `xref_to`；該工具自檢含 2 組正向交叉核對與 2 組反向控制）重查，三個結論全部翻案**：

1. **`0x35854` 並非無人參照**：掃描全 image 得到 call/jmp 各 0 筆、**絕對參照 1 筆於 `0x51c79`**。
   dump 該處前後確認那是一張單調遞增的 code-pointer 表（`0x353fa`→`0x3644e`，4-byte stride，
   起始對齊 `addr % 4 == 1`）——`0x35854` 就是表中的一格。§11.7.5 的「查無靜態呼叫者」是
   `xref_to` 盲區造成的假陰性，不是真的沒有參照。

2. **索引與 §11.7 自己的校準完全吻合**：以 §11.7 記載的基底 `0x51b91` 計算，
   `(0x51c79-0x51b91)/4 = 58`；且 index 57/58/76/78 四格讀出來分別是
   `0x35833`/`0x35854`/`0x360b6`/`0x36228`，**精確等於 §11.7 獨立導出的 raw 值
   `0x354dd`/`0x354fe`/`0x35d60`/`0x35ed2` 各加 `0x356`**。兩條獨立路徑（§11.7 讀 raw 檔案 +
   校準；本輪讀已 relocate 的 image）互相印證，基底與 relocation 常數都不是本輪臆測。

3. **三個「artifact」其實都是有自己序言的獨立函式**（`capstone_probe` 逐指令）：
   - `0x35854`：`push 0x44; call 0x3702f` → 存暫存器 → `mov esi, 0x5274e`。**`0x5274e` 正是
     §11.7.5 指名的 map25 寶物邏輯錨點**，語意直接對上。
   - `0x360b6`：`push 0x30; call 0x3702f` → 讀 `[0x53ad5]`（戰鬥區域狀態表）。
   - `0x36228`：`push 4; call 0x3702f` → `[0x53ad5]+0x14 = 1` → `ret`。
   而 index 57 的 handler（`0x35833`）**在 `0x3584f` 以 `jmp 0x3571a` 乾淨結束**，比 `0x35854`
   早 5 bytes——`0x35854` 不在它體內，「落在鄰居 handler 中段」的判定不成立。

**結論**：§11.7 的「三者 definitively 確認為 table artifact」**撤回**。event58 是真實 handler，
入口 `0x35854`（relocate 後），§6.3「event58 = map25 五選一寶物、透過 event_id 58 觸發」的原始
歸因**恢復成立**（只是入口位址應寫 relocate 後的 `0x35854`，而不是 raw 的 `0x354FE`）。§11.7.5 的
衍生懸念「map25 寶物真正的 runtime 觸發路徑」至此解開：就是事件 dispatcher 經這張跳表的 index 58。
58..89 的「18 個具體行為描述 / 14 個 table artifact」統計需要相應修正（至少這 3 個要從 artifact
移回具體行為那一側）。

**誠實範圍（(a) 已於同日 §19 補完：32 格全部重驗，一格 artifact 都沒有）**：(a) 本輪只逐指令驗了這 3 個槽位與 index 57，其餘 11 個被標為 artifact 的槽位**未
逐一重驗**，但既然判定方法本身在這 3 個上都失效，其餘 11 個應視為**存疑待重驗**而非已確認；
(b) `0x360b6`/`0x36228` 的實際行為與 §10.1 對 event76/78 的舊描述（`[0x53AD5+0x13] += 1` 等）
並不逐字吻合，本輪未展開比對，只確認它們是獨立函式；(c) 跳表基底 `0x51b91` 沿用 §11.7 的數值，
本輪是以「index 57/58/76/78 四格同時吻合」間接佐證，未另行獨立推導基底本身。

## 12. `State.NativeEventState` 旗標區塊:大小 32 bytes(更正 §3.2.4 的「12-byte」)、每章清零、寫入端 [驗]

**2026-09-09,純靜態反組譯(現行參考版 `FD2.EXE`),起因是 `chapter_beats` 抽取器認不出
ch06 戰後 handler 的條件**。這一節同時更正一個既有數字、並補上先前沒有記載的清零點。

**先講位址慣例,否則下面每個位址都會看起來像新東西**:本輪的反組譯輸出把 obj2 的
`0x50000` 基底省掉了,所以 `dword ptr [0x3ad5]` 就是既有記載的 `[0x53AD5]`。判定不是
推測,是拿一個**已知對子**核對出來的:同一段 handler 的 `0x233ba inc dword ptr [0x3c03]`
就是 §1 記載的章節索引 `[0x53c03]`。(這也是為什麼先前用 `0x3ad5` 全庫搜尋查無記載——
搜錯了拼法,不是真的沒人寫過。§10.1、doc11 L163 都早有記載。)

**語意**:`[0x53AD5]` 是**指標**,指向一個 heap 旗標區塊(§3.2.4 已記載這件事);
doc11 L163 把它命名為 `State.NativeEventState`,並記載事件觸發後會標記
`[0x53AD5+event_id]`。

**更正:區塊大小是 32 bytes,不是 12**。§3.2.4 的「12-byte 旗標區塊」是當時 live 探測
**已知的 12 個 map10 寶箱 slot** 的數量,不是配置大小。實際清零點在
`0x20605..0x20616`:

```
0x205f9  push dword ptr [0x3c03]      ; 章節索引
0x205ff  call 0x1088d                 ; 章節 loader(doc26 已知)
0x20604  add esp, 4
0x20607  push 0x20                    ; n = 32
0x20609  push 0                       ; value = 0
0x2060b  push dword ptr [0x3ad5]      ; dest = 旗標區塊
0x20611  call 0x37910                 ; memset(dest, 0, 0x20)
```

也就是說 `0x20611` 這個 `memset` 讓**每次載入章節都把整個 32-byte 旗標區塊清零**。
另有一個獨立佐證不必靠 `0x20611` 這段:
ch06 戰後 handler 讀的是 `[+0x11]`(index 17),**本身就超出 12**。

**寫入端**(全 image 掃 `[0x53AD5]` 載入點後 8 條指令內對該區塊的位元組寫入,共 35 處):
`byte [+0x10]` 13 處、`byte [+0x11]` 5 處、`byte [+3]` 5 處,其餘散在 `[+0]/[+6]/[+9]/[+0xc]/[+0x12]/[+0x13]/[+0x14]`;
寫入指令形如 `0x34cec mov byte ptr [eax + 0x11], 1`、`0x34ead mov byte ptr [eax + 0x10], 2`、
`0x34ed8 inc byte ptr [eax + 0x10]`,位置密集落在 `0x348f4..0x363b5`——也就是本文件的
事件 handler 區。**指標本身只有一個 writer**:`0x25d3d mov dword ptr [0x3ad5], eax`。

**讀取端不只當旗標用**:`0x24f35`/`0x24fb6` 的 `movzx eax, byte ptr [eax + 0xc]` 之後
`add eax,5`/`add eax,8` 再送進 `0x15f84`(dialog),即 `[+0xc]` 被當成**對白索引的偏移**。

**與 chapter_beats 的關係(本輪的起因)**:ch06 戰後 handler `0x232e8` 的條件是**兩個
判準的短路 AND,共用同一個 else 臂**——`0x2332a jne 0x23393`(`[+0x11] == 1`)與
`0x23338 jne 0x23393`(`unit_inactive(43)`)跳到同一個位址。抽取器的 diamond 辨識器
只認得後者,若讓它單獨成立會產出 `if any_unit_inactive([43])`,那會**丟掉事件旗標守衛**
而變成不完整的條件主張,所以它刻意維持扁平(見 `dump_chapter_beats.py` 該處註解與第
(2b) 題)。要讓該章可機械重生,缺的是「條件是連接詞」這個表達能力,不是再放寬 diamond
的判準。

## 13. 等級上限第三層(社群說的「80」)——對 `0x1e292` 窮舉確認不存在,強化 §10.4 既有的保留說法 [驗]

§10.4 記著「在 `0x1e292` 目前只看到兩種分支,未證實存在第三分支,可能是社群近似值而非原生規則」。
本輪(2026-09-10)把「目前只看到」升級成**窮舉**。

`0x1e292` 完整反組譯(經驗值閘):

```
0x1e2be  movzx eax, byte ptr [esi + 0x21]   ; 目前等級
0x1e2c2  movzx edx, byte ptr [esi + 7]      ; 職業 id
0x1e2c9  cmp  dword ptr [0x3ec8], 0 ; je    ; 全域閘
0x1e2d6  test byte ptr [esi + 5], 1 ; jne   ; record[+5] bit0(死亡/隱藏)-> 跳過
0x1e2e0  cmp  edx, 0x1e ; je 0x1e2ea        ; 職業 30
0x1e2e5  cmp  edx, 0x1f ; jne 0x1e2ef       ; 職業 31
0x1e2ea  cmp  eax, 0x63                     ; 上限 99
0x1e2ef  cmp  eax, 0x28                     ; 上限 40
0x1e2f2  je   <exit>                        ; 已達上限 -> 不給經驗
0x1e2fd  call 0x4e821                       ; 給經驗
```

**兩個查詢,兩個都是空的**(把查詢寫出來,因為「查無」的價值完全取決於查了什麼):

1. 全 image 線性解碼,找「讀 `byte ptr [reg + 0x21]`(等級)之後 6 條指令內、對同一暫存器做 `cmp`」
   的立即值分布:`8`×3(`0x2279f`/`0x228e4`/`0x22a14`)、`0x63`×1(`0x1e403`)、`0x14`×1
   (`0x2ae61`,即已知的轉職 Lv>=20 閘)、一筆 `cmp reg,eax`(`0x1e37e`)。**沒有 `0x50`**。
2. 放寬到不看暫存器:全 image 可解出的 `cmp` 共 4126 條,立即值為 `0x50` 的有 **16 條**,
   逐一往回看 48 bytes,**0 條**在此之前讀過 `byte ptr [.. + 0x21]`。(`0x50` 也是常見的畫面
   座標/尺寸常數,16 條本來就不奇怪。)

**結論**:等級上限在原生碼裡只有 99(職業 30/31)與 40(其餘)兩級;社群攻略提到的「80」在
指令層面找不到依據。**範圍**:這是對「等級與立即值比較」的窮舉,不排除某處用非立即值(表格查詢
或暫存器)來實施第三種上限——那種形狀本輪沒有查。
...
## 15. `0x25089` = 全名冊完全恢復(2026-09-10,靜態全文反組譯)

`0x25089` 是剩下的未命名原語之一。**它先前留白的理由被記成「有合併清理風險」——
那是關於「參數個數」的風險,被當成了不能命名的理由**,而本工具的 docstring 自己
就寫著「名稱不會帶動參數個數」,兩者是獨立的主張。本輪把兩件事分開處理。

**本體**(`0x25089..0x250cb`,67 bytes):

```
bl = 0
while bl < [0x53bfb]:                    ; 名冊人數
    rec = [0x53bf7] + bl * 0x50          ; 常駐名冊,stride 0x50
    byte [rec + 5]    = 0                ; 清掉狀態旗標(死亡/隱藏/已行動)
    word [rec + 0x40] = word [rec + 0x42]   ; HP    = HPmax
    word [rec + 0x44] = word [rec + 0x46]   ; MP    = MPmax
    bl++
```

四個欄位與兩個全域都不是本輪新命名:

* `[0x53bf7]` = 常駐名冊 —— PRIM 表的 `0x33499` (`roster_has`) 註解寫明「查我方名冊 `[0x53bf7]`」;
* `[0x53bfb]` = 名冊人數 —— worklist 26 的備戰畫面分析用 `DAT_00053bfb - 1` 當游標範圍;
* `+0x40` = current HP —— doc03 欄位表;`+0x42`/`+0x46` = HP/MP 上限 —— 即
  `NativeRecordWord42/46`,doc56 的等級成長那輪已用它們寫回 HP/MP;
* `+5` 的狀態位元 —— bit0 死亡/隱藏、bit7 已行動,doc13 與 doc25 §13 皆有記載。

**`0x25089` 的參數個數 = 0,由第三種訊號證實**。`derive_native_argcounts` 的兩個呼叫端訊號
(cleanup / 連續 push)在這裡因**合併清理**而判 WEAK。但被呼叫端自己給得出答案:
`0x25089` 的本體**完全沒有讀取 `[esp+N]`**——`push ebx` 之後只用 `bl`/`eax`/`edx`,
全部資料來自全域,最後 `pop ebx; ret`。呼叫端訊號模糊時,讀被呼叫端本體是獨立的第三條路。

**誠實範圍(已於同日 §20 補完:由第二張跳表 index 84/87 兩個 handler 呼叫,不是城鎮 UI 路徑)**:本節只主張這個迴圈做了什麼;它在遊戲流程中何時被呼叫(例如是否等同「旅館/教會全體恢復」)本輪未追,所以名稱只描述機制。
...
## 17. `0x1db65` = 對剛歸零的單位播放死亡演出(2026-09-11)

§14 把 `zero_hp_from`(`0x35f10`)的收尾呼叫 `0x1db65` 列為未展開。補完後,它與
`zero_hp_from` 的關係一目了然。

```
for esi in 0 .. [0x53beb]:                       ; 全部單位
    unit = [0x53a45] + esi*0x50
    x = byte[unit+0] ; y = byte[unit+1]
    if byte[unit+5] & 1: continue                ; **已標記死亡/隱藏的跳過**
    if word[unit+0x40] != 0: continue            ; **HP 非 0 的跳過**
    ...對鏡頭原點 [0x53aa9]/[0x53aad] 與視野 [0x51a87]/[0x51a8b] 做四邊界裁切...
    螢幕位址 = (x-1-camX)*0x18 + [0x53a49] + 0x8088 + (y-1-camY)*列距 - 0xab0
    [esp + ebp*4] = 螢幕位址 ; ebp++             ; **收集進堆疊陣列**
if ebp != 0: ...對收集到的位置進行演出...
```

**它處理的正好是「HP 已歸零、但死亡旗標尚未設起」的那些單位**——而
`zero_hp_from(start)` 做的就是把 `start` 之後所有單位的 `unit[+0x40]` 寫 0、
**不動 `+5`**。兩者因此是一組:前者製造狀態,後者發現並演出。這也解釋了為什麼
`0x35f10` 的四個呼叫端都是「先對話、再歸零」——對話宣告劇情結果,歸零 + 掃描
把它演出來。

裁切與螢幕位址算式與 `0x1c2da`(doc35 §11)逐項相同,是同一套地圖→螢幕座標轉換。

~~**誠實範圍**:`ebp != 0` 之後那段實際演出(`0x1dc67` 起)本輪未展開;
`- 0xab0` 這個常數位移的意義未查。~~ **兩項均已於 §18 補完(2026-09-11 同日)。**

## 18. `0x1dc67` 死亡演出本體,以及 `- 0xab0` 的意義(2026-09-11,補完 §17 的兩個殘留)

### 18.1 `- 0xab0` = 往上 6 條掃描線,而 work buffer 的幾何整組對得起來

§17 把螢幕位址算式抄了下來但沒查 `- 0xab0`。答案只要把同一條算式裡的**列距**先算出來就有了。
反組譯 `0x1dc0b..0x1dc23` 那串移位:

```
edx = ebx                       ; ebx = y-1-camY
edx = ((edx*4 + edx)*4 - edx) << 6      ; = (20-1)*edx*64 = 1216*edx
ebx = edx ; edx <<= 3 ; edx += ebx      ; = 1216*edx*9      = 10944*edx
ecx += edx
ecx -= 0xab0
```

列距 = `10944 = 24 × 456`,即**一格(24 px 高)乘上 work buffer 的 stride 456**。
`456 = 0x1c8` 不是本輪推出來的新數字——doc25 §1339 早就記著 work buffer stride `0x1c8=456`,
而且它在本函式裡**以字面值出現**:`0x1dcc2 push 0x1c8` 與 `0x1dc7a push 0x1c8`
分別傳給 `0x11eee`(地形層)與 `0x11eb0`(present)。

於是 **`0xab0 = 2736 = 6 × 456`**,就是**往上 6 條掃描線**的固定位移。
這與本專案既有的觀察一致:場上單位的圖比一格高,會壓到上面那一格
(見 memory `fd2-re-cursor-tile-verification` 記的「tall sprites overlap the tile above」)。

**順帶,同一條算式裡的另一個常數也一起解開,而且三方對得上**:

| 常數 | 分解 | 意義 |
|---|---|---|
| `0x8088` = 32904 | `456×72 + 72` | 視窗原點在 work buffer 內的 (72, 72) |
| `0x1dc7a` 的 `0x138`/`0xc0` | 312 × 192 | `0x11eb0` 實際搬到 VGA 的視窗大小 |
| `0x1dd52` 的 `0x3706e(0x25680)` | `0x25680 = 153216 = 456 × 336` | work buffer 的配置大小 |

`312 + 72 + 72 = 456`、`192 + 72 + 72 = 336`——**四邊各留 72 px 捲動邊界**,而配置大小
`0x25680` 正好等於 `456 × 336`。三個獨立出現的常數(視窗大小、原點偏移、配置位元組數)
互相封閉,不是湊出來的。目的地 `0xa0504 = 0xa0000 + 320×4 + 4`、stride `0x140 = 320`,
即 VGA 上 (4, 4) 起的 312×192 區塊。

### 18.2 `0x1dc67` 起的演出:13 幀、pose = 幀次 mod 4,結束才設死亡旗標

```
edi = 0
while edi < 0xd:                                   ; 13 幀
    0x11eee(buf+0x8088, 0x1c8, 0xd, 8, camX, camY) ; 地形層重繪
    for esi in 0 .. [0x53beb]:
        if unit[+5] & 1: continue                  ; 已死的跳過
        if word[unit+0x40] == 0:                   ; **正在死的那些**
            byte[unit+3] = edi % 4                 ; 換 pose(§16/857:+3 選姿勢)
        0x127e0(esi)                               ; 單位層繪製(存活與將死的都畫)
    0x129ec()                                      ; 單位層收尾
    0x11eb0(0xa0504, 0x140, buf+0x8088, 0x1c8, 0x138, 0xc0)   ; present
    0x17aa9(1)                                     ; 等 1 tick
    edi++
for esi in 0 .. [0x53beb]:                         ; 演完才動旗標
    if word[unit+0x40] == 0: byte[unit+5] = 1
```

呼叫序列 `0x11eee → 0x127e0 → 0x129ec → 0x11eb0` 與 doc25 §1338 記的既有重繪管線
逐項相同,不是另一套繪製路徑。

**兩個值得記下的細節:**

1. **旗標一定在演出之後才設**,而且 `ebp == 0`(畫面上沒有任何要演的單位)那條捷徑
   (`0x1dc36..0x1dd50`)做的事**只有**設旗標——兩條路徑的終態相同,差別只在有沒有演出。
   這正好補上 §17 說的「`zero_hp_from` 製造狀態、本函式發現並演出」:狀態的**收尾**
   (`+5` bit0)也在本函式,不在 `zero_hp_from`。
2. `byte[unit+3] = edi % 4` 用的是 §16/worklist 857 已定案的 `unit[+3]`(選姿勢),
   **不是** `unit[+4]`(姿勢內的子影格)。13 幀跑 `0..3` 循環三輪多一幀。
   這裡刻意不去推「4 是死亡動畫的張數」——`% 4` 只是這段演出自己選的循環長度,
   本函式沒有任何地方去讀該姿勢實際有幾張圖。

## 19. 58..89 全 32 格重驗:**一格 table artifact 都沒有**,那是位址欄少了 `0x356` 造成的假象(2026-09-11)

2026-09-07 那輪推翻了 58/76/78 三格,並誠實記下「其餘 11 格未逐一重驗……應視為**存疑待重驗**」。
本輪把 32 格全部驗完。結論比預期強:**不只那 11 格不是 artifact,32 格全部都是乾淨的函式入口**,
而「14 個 table artifact」這個分類**整個是單一系統性錯誤的產物**。

### 19.1 錯誤本身:§10 的位址欄是未重定位值

跳表 `0x51b91` 每格存的 dword 是**相對 obj1 的位移**,真正的入口是 `stored + 0x10000`。
§10 的位址欄少了這一步。把兩邊逐列比對,**文件裡出現的 25 列一律低 `0x356`,零例外**
(`0x10000 - 0x356` 正是 §11.7 當年校準出來的那個常數的補數,兩者說的是同一件事)。

於是會發生什麼:少 `0x356` 之後,有些格「剛好」重新同步到某條指令邊界——Ghidra 的
`createFunction` 在任何位址都會硬生一個函式出來,看起來就像乾淨 handler;有些格落在
指令中段,被判成 artifact。**artifact 與否,量的是那個偏移落在哪裡,不是 handler 有沒有語意。**

### 19.2 重驗方法:純位元組樣式,刻意不反組譯

新增 `tools/verify_event_dispatch_table.py`(已登錄產物重生表、突變測試表與 wsl 軸),
產出 `docs/data/event_dispatch_table_58_89.json`。**本工具完全不做線性反組譯**——
doc25 這個錯誤本來就是靠反組譯邊界推論造成的,再用同一種手段複驗沒有意義。改成只比對
Watcom 的兩種固定序頭位元組樣式,`call`/`jmp` 的目標都實際算出來核對:
...
**結果:`CLEAN_PROLOGUE` 28 格 + `TAIL_MERGED_STUB` 4 格 = 32 格,`MID_BODY` 0 格**,
入口位址 `0x35854..0x3644e` 嚴格遞增。

四項 `--selftest`,其中兩項是本節結論的要害:
- **獨立路徑真值**:2026-09-07 那輪是「讀 raw 檔案 + 四錨點校準 relocation」,本工具是
  「讀已重定位 image + `+0x10000`」;兩條路徑對 58/76/78 必須得到同樣的 `0x35854`/`0x360b6`/`0x36228`。
- **負向控制**:把 `0x35854..0x3644e` 這 32 個入口各減 `0x356` 後重跑同一個分類器,判定必須塌掉。實測 **32/32 全部變成 `MID_BODY`**——
  一格巧合重新同步都沒有。沒有這一項,「32 格全是入口」會是平凡結論(一個永遠回傳
  CLEAN 的分類器也能通過)。

### 19.3 85..89 不是 artifact,是**空 handler**

這五格先前全被判為 artifact。實際位元組:

```
0x3642e  68 04 00 00 00   push 4          <- event 85
0x36433  e8 f7 0b 00 00   call 0x3702f
0x36438  c3               ret
0x36439  68 04 00 00 00   push 4 ; eb f3 jmp 0x36433   <- event 86
0x36440  68 04 00 00 00   push 4 ; eb ec jmp 0x36433   <- event 87
0x36447  68 04 00 00 00   push 4 ; eb e5 jmp 0x36433   <- event 88
0x3644e  68 04 00 00 00   push 4 ; eb de jmp 0x36433   <- event 89
```

五格都是**各自獨立、對齊良好的入口**,行為是 `__STK(4); return;` ——**空**。
86..89 共用 85 的 `call + ret` 尾段,是編譯器的 tail-merge。
「登記了但什麼都不做」與「指標是巧合」是兩件完全不同的事,前者是可用的事實。

### 19.4 §10.1/§10.2 的語意描述沒有白費,只是**掛錯 event_id**

既然偏移是常數,每一條舊描述實際描述的是「文件位址落在哪一格 handler 體內」的那一格。
機械換算如下(`0x356` 偏移 + 入口區間查表):

| §10 標為 event | §10 記的位址 | 該 event 的真正入口 | 那段描述實際屬於 |
|---:|---|---|---|
| 62 | `0x35898` | `0x35bee` | event **58** |
| 65 | `0x3599b` | `0x35cf1` | event **59** |
| 67 | `0x35a2f` | `0x35d85` | event **61** |
| 68 | `0x35a48` | `0x35d9e` | event **61** |
| 69 | `0x35ab8` | `0x35e0e` | event **61** |
| 70 | `0x35b05` | `0x35e5b` | event **61** |
| 71 | `0x35b6b` | `0x35ec1` | event **61** |
| 72 | `0x35bf2` | `0x35f48` | event **62** |
| 73 | `0x35c23` | `0x35f79` | event **63** |
| 74 | `0x35c32` | `0x35f88` | event **63** |
| 75 | `0x35c79` | `0x35fcf` | event **64** |
| 76 | `0x35d60` | `0x360b6` | event **66** |
| 77 | `0x35ebe` | `0x36214` | event **70** |
| 78 | `0x35ed2` | `0x36228` | event **71** |
| 79 | `0x35ee6` | `0x3623c` | event **71** |
| 80 | `0x35f5a` | `0x362b0` | event **72** |
| 81 | `0x35f6f` | `0x362c5` | event **72** |
| 83 | `0x36088` | `0x363de` | event **75** |
| 84 | `0x360c0` | `0x36416` | event **76** |
| 85 | `0x360d8` | `0x3642e` | event **76** |
| 86 | `0x360e3` | `0x36439` | event **76** |
| 87 | `0x360ea` | `0x36440` | event **76** |
| 88 | `0x360f1` | `0x36447` | event **76** |
| 89 | `0x360f8` | `0x3644e` | event **76** |

這張表同時**解釋了 §10 自己觀察到但無法說明的幾件事**:
- §10.1 說「event 69 與 event 67 幾乎相同的尾段」——因為兩者**就是同一個函式**(event 61),
  只是進入點差了幾十個 byte。同理 67/68/69/70/71 五條描述全部落在 event 61 一個 handler 裡。
- §10.1 把 event 84 描述成精緻的「連續 4 回合倒數 spawn」,又把 event 76 描述成
  「只有 `0x15F84()` 然後返回,近似 no-op」——**兩者對調了**:那段倒數機制是 event 76 的
  真正行為,而 84..89 那六條描述全部落在 event 76 體內。
- §2241 早就對 event 62 的 `0x35898` 起疑(「往回追溯才找到乾淨序頭」),並留下
  「未比照 §11.7 的標準重新驗證」。現在答案很乾脆:`0x35898` 落在 **event 58** 的 handler 體內,
  event 62 真正的入口是 `0x35bee`。

### 19.5 統計修正與誠實範圍

「58..89 = 18 個具體行為 / 14 個 table artifact」這組數字**整組作廢**,正確的是
**32 格全部是 handler 入口,其中 5 格(85..89)行為為空**。

**誠實範圍**:本節只證明了「每一格都是函式入口、以及舊位址錯在哪裡」,**沒有**重新反編譯
32 個 handler 的語意。§10.1/§10.2 的描述經上表換算後可以重新歸位,但那是把舊結論搬到正確的
event_id 上,不是重新驗證過的語意——要真正確立每格語意,仍須以正確入口重跑一次反編譯,
本輪未做。另外 §10.1 描述的內容是從**函式中段**開始反編譯的,因此每條描述都可能**缺少該
handler 的開頭**(例如 event 61 的前 0x22 bytes),搬過去之後仍是不完整的。

## 20. 追兩個「呼叫端未追」的殘留,結果把跳表的真實規模挖出來(2026-09-11)

本輪要關的是兩條登記在案、彼此無關的誠實範圍:

- §15:`0x25089`(全名冊完全恢復)「在遊戲流程中何時被呼叫(例如是否等同旅館/教會全體恢復)本輪未追」。
- doc35 §14:`0x361b0`「在哪些劇情/戰鬥時機被呼叫本輪未追」。

兩條都是「還沒有人去看」,不是看不了——與 2026-09-10 那批的形狀相同。

### 20.1 先講結果:兩者都只從跳表進入,沒有任何直接呼叫端

`call_scan` 全 image 窮舉:

| 被呼叫者 | 呼叫端 | 所屬函式(以 Watcom `push imm32 ; call __STK` 序頭往回判定) |
|---|---|---|
| `0x361b0` | `0x3614a` / `0x3615c` / `0x36174` | **`0x360b6`** = 事件 76 的 handler |
| `0x361b0` | `0x25694` / `0x256a6` | `0x2548c` |
| `0x361b0` | `0x33efd` | `0x33e3c` |
| `0x25089` | `0x2533e` / `0x25458` | `0x250cc` |
| `0x25089` | `0x25850` / `0x2585b` | `0x25757` |

而 `0x250cc` / `0x25757` / `0x2548c` / `0x33e3c` 這四個父函式,`call_scan` 的直接呼叫端數**全部是 0**。

**`0x361b0` 那三個在 `0x360b6` 裡的呼叫點,是今天 §19.4 的獨立佐證。** §19.4 用「文件位址少 `0x356`」
把 §10.1 標為 event 84 的那段描述重新歸位到 event 76,而該描述寫的是
「兩次 `0x361b0()`+等待;迴圈 3 次 `0x361b0()`+畫面」——本輪用完全不同的方法(全 image `E8` 掃描 +
序頭回推)數出來**恰好 3 個靜態呼叫點,全部落在 `0x360b6..0x36214`**。兩條路徑互不相干卻指向同一格。

### 20.2 「找不到參照」一度是我的查法造成的

四個父函式用 `tools/disasm_le.py refs` 掃絕對參照也是**零筆**。那個零是假的:
`build_fixups` 只走 **obj1(程式段)** 的 fixup 頁,資料段裡的跳表登記它看不到。
**用已知真值驗證這個盲點**:`refs 0x35854`(事件 58,已知登記在 `0x51c79`)同樣回傳空。

改成直接掃 obj2 的 dword(值 = 目標 − `0x10000`,與跳表的存法一致)之後,對照組立刻命中
`0x51c79` ——方法由已知真值確立,四個父函式也各自命中一格。

### 20.3 這一帶其實有**兩張**表,中間夾著兩個變數

以 `0x51b91` 為基底、stride 4,逐格套 §19.2 的序頭判定:

| 範圍 | 內容 |
|---|---|
| index 0..89(`0x51b91`,**90 格**) | 全域事件表。全部是入口(`CLEAN_PROLOGUE` 84 / `TAIL_MERGED_STUB` 6) |
| `0x51cf9`、`0x51cfd` | **不是表格** —— 游標 x/y 備份變數 |
| `0x51d01` 起 **88 格** | **另一張表**,全部是入口(84 / 4)。**不是本輪才發現的**——見 §20.5 |
| `0x51e61` 之後 | 值不落在程式段,表結束 |

**中間那兩格是分界的依據,不是我畫的線**:它們通不過序頭判定(首位元組 `0x00`),而且各有
**5 個程式端參照**、是被當變數讀寫的——寫入端就是今天在道具路徑看到的
`0x1be07 mov [0x51cf9], eax` / `0x1be11 mov [0x51cfd], eax`(存 `[0x53ab1]`/`[0x53ab5]` 的游標座標)。
這一點寫成 `--selftest` 的檢查 (6):**若那兩格也能通過入口判定,「兩張表」就失去依據**,
把 178 格併成一張大表也不會有任何檢查失敗。

**第二張表是指令/行動 dispatch,而且只有兩個 dispatch 點**(`refs 0x51d01` 全 image 2 筆;
它先前已被登記,只是只驗了前 12 筆,見 §20.5):

```
0x1541f  call dword ptr [eax*4 + 0x51d01]   ; AI 路徑(0x14ef0/0x15055 那一叢)
0x1d479  call dword ptr [eax*4 + 0x51d01]   ; 玩家 command ring(FUN_0001cff0 內)
```

兩處結構完全相同(都 `add esp, 0xc` 清三個引數、都緊接 `call 0x1d4f6`)。對照之下
`0x51b91` 有 **8** 個 dispatch 點(例如 `0x1d890 call dword ptr [eax*4 + 0x51b91]`,索引來自 `[0x51a8f]`)。

### 20.4 兩條誠實範圍的答案

- **`0x25089`**:由第二張表 index 84(`0x250cc`)與 87(`0x25757`)兩個 handler 呼叫,各 2 次。
  它**不是**從城鎮選單路徑進入的,而是**指令/行動 dispatch 的下游**。所以「是否等同旅館/教會
  全體恢復」的答案是:若真對應到那個玩法,也必定是經由某個指令 id、不是城鎮 UI 直接呼叫。
- **`0x361b0`**:由事件 76 的 handler(3 處)、第二張表 index 86(`0x2548c`,2 處)與
  index 57(`0x33e3c`,1 處)呼叫。同樣只在事件/指令 dispatch 的下游。

**誠實範圍**:本節把「誰呼叫」窮舉完,**沒有**去解那六個 handler 各自的語意,也**沒有**建立
第二張表的 index → 指令名稱對照(那需要追 `0x1541f`/`0x1d479` 兩處 `eax` 的來源,本輪未做)。
`docs/data/event_dispatch_table_58_89.json` 現在同時收錄兩張表的逐格判定,檔名維持原樣以免
既有引用失效,內容已不只 58..89。

### 20.5 訂正 §20.3 的「未記載」,以及本輪真正暴露的東西(2026-09-11 同日)

§20.3 把 `0x51d01` 寫成「另一張**未記載**的表」。**那是錯的**,而且錯法值得記下來:
`tools/verify_findings.py` 的 `FINDINGS` 裡早就有一筆
...
建於 **2026-09-08**。我查了知識庫的 `.md`,沒查工具裡已登記的結論——**與本節前半在講的
「refs 掃不到資料段」是同一種錯**:查法有盲區,而我把空結果當成了事實。

**真正的問題不是沒記載,是只驗了前 12 筆。** 那筆 finding 的 `probe=12` 是呼叫端設的上限;
`findings` 軸因此每輪都報「相符」,卻從來沒有整張表被檢查過。這正是 doc25 §10 那個 `0x356`
偏移能撐五輪的原因——**`findings` 軸的滿分是對已登記項目的滿分,不是對主張的涵蓋率**。

本輪的處置(用既有機制,不新增機制):

1. `verify_findings.py` 新增 `is_tail_merged_stub()` / `is_handler_entry()`。先前
   `pointer_table_len` 只認嚴格入口,遇到共用尾段殘樁就當「表結束」——`0x51b91` 的殘樁在
   index 5,整張 90 格的表會被報成 5 格。兩者**刻意不合併**:`is_function_entry` 的計數
   541 是已登記結論 `584-entries`,動它會改變一個記錄在案的數字。預設判準維持嚴格,
   所以 `0x524c6` 的 10 不變(該表在 index 10 是 `0x20000`,兩種判準都判非入口)。
2. 新增兩筆 finding 涵蓋**整張**表:`212-event-table` = 90、`212-command-table` = 88。
   兩者由 `findings` 軸的路徑(Ghidra 已重定位映像 + 共用判準)重新導出,與本節用
   「原始檔 + `0x10000`」得到的數字一致——**兩條位元組來源不同的路徑互相印證**。
3. `verify_event_dispatch_table.py` 沒有改成 import `is_function_entry`:那條路徑的位元組
   來源走 Ghidra headless(要 JVM、WSL 下不可用),而本工具刻意保持免 capstone/免 JVM 才能
   待在 `wsl` 軸。改成 `--selftest` 檢查 (7):**斷言兩套實作對 178 格逐格一致**。
   實測 `0x51b91`+`0x51d01` 共 178 格零分歧;WSL 下該檢查乾淨 SKIP。比只留一份更強,也是 `verify_findings` 自己對
   `image_ref_scan.py` 用的做法。

**誠實範圍**:這只補上了這兩張表的涵蓋率。「知識庫裡還有多少位址主張沒有任何 finding 列」
仍然無人測量——`findings` 軸報的 17/17 依舊是分母未知的滿分。
