...
| UI-04 | Target/range | 武器 min/max reach、法術 range/AOE、不可用目標灰化、確認／取消 | partial；battle native command candidate→confirm 已接 selected raw record；`NativeAttackCandidates` 保留 `0x14237` caller-specific geometry 但尚無 proven item-row producer。物品 action 現有原版 8-slot selector shell（`0x1b932`）與 ESC/↑↓/Enter trace，Enter 對未閉合 `0x20c6f` effect/target 維持 fail-closed；不能把攻擊距離寫死相鄰格 |
