...
| UI-07 | Postbattle | result → handler → reward/roster cleanup → town/shop/rest/preparation 或 ending；不可預設直連下一戰 | partial；campaign schema 與 bounded menu trace 可表達，`town_ch02→preparation_ch02→story_ch02_pre→battle_ch02` 已有可重播 trace。標準 postbattle 現有玩家 ch04/05/06/09/11/12/14/15/19/25/26/28 共12個節點接入零起算 owner 的 authored binding，另12個維持 blocked。IDA Pro 9.4 直接確認 raw ch05 的完整 JOIN13→spawn3→pan→ACT27→dialog→sync→chapter6 尾段屬玩家第6戰；raw ch25／ch27 分別屬玩家第26／28戰。七個未綁定節點原有的泛用 `sync_party→set_chapter` 會繞過 runtime guard，現已移除；所有未綁定標準節點均以空 beats 失敗即關閉。這批只達 E1，逐關戰間畫面與一般玩家路徑仍不足；直接位址證據見 [`fd2_ch05_post_dispatch_ida.txt`](../data/fd2_ch05_post_dispatch_ida.txt) 與 [`fd2_post26_28_dispatch_ida.txt`](../data/fd2_post26_28_dispatch_ida.txt) |
...
12個 active、12個 blocked，沒有 mapping complete、index mismatch 或 inline bypass。
