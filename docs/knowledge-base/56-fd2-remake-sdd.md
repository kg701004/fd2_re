...
| UI-07 | Postbattle | result → handler → reward/roster cleanup → town/shop/rest/preparation 或 ending；不可預設直連下一戰 | partial；campaign schema 與 bounded menu trace 可表達，`town_ch02→preparation_ch02→story_ch02_pre→battle_ch02` 已有可重播 trace。標準 postbattle 現有玩家 ch04/05/09/11/12/14/15/19/25/26/28 共11個節點接入零起算 owner 的 authored binding，另13個維持 blocked。IDA Pro 9.4 直接確認 raw ch25 是玩家第26戰 handler；raw ch27 的 `0x25464` 會跳到 `0x231df` 共用尾段，確屬玩家第28戰，而不是第27戰天空之鑰成功臂。七個未綁定節點原有的泛用 `sync_party→set_chapter` 會繞過 runtime guard，現已移除；所有未綁定標準節點均以空 beats 失敗即關閉。這批只達 E1，逐關戰間畫面與一般玩家路徑仍不足；直接位址證據見 [`fd2_post26_28_dispatch_ida.txt`](../data/fd2_post26_28_dispatch_ida.txt) |
