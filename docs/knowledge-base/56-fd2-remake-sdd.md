...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Constructor provenance correction: Docker Capstone of `0x10d7f..0x10efc` proves runtime `unit+6` receives FDFIELD roster byte0 and `unit+7/+8` receives roster byte1; the existing editable `map_selector_key` and `battle_fig` fields therefore preserve those two raw sources. A further table trace closes the high-class branch: `0x10da4` computes `FDFIELD b1-0x44`, calls `0x4e4ff`, and that helper returns the 10-byte record at `0x61af9 + index*0xa`; `unit+0x1f/+0x20` are record bytes 0/1, while bytes 2/4/5/6/7/8 feed the other native fields. This is an EXE static table, not a proven DATO resource. The lower branch instead uses the separate static tables at `0x61da1` (24-byte records) and `0x619fd` (7-byte records), with `unit+0x1f/+0x20` coming from the selected 24-byte record bytes 0/1. Until both branch selectors and these raw table records are exported, optional unit/HP admission remains nil; portrait/class must not be used as a substitute.
