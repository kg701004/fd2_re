...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Selected-unit HUD boundary: `0x11cac` calls `0x1acf3` after terrain, range and unit/foreground layers but before the viewport copy. `0x1acf3` returns without drawing unless both raw display bytes `0x51aab` and `0x51aac` are nonzero; it resolves the active unit at cursor globals `0x53ab1/0x53ab5` through `0x12c0d`, then performs its own resource, raw blit and numeric draw sequence. Its first resource is FDOTHER #5 LMI1 entry #130 (`base+0x20e`), independently decoded as 69×34, not the full-screen battle panel entry #22 (149×42). Its panel anchor starts at framebuffer row 157 and applies the evidenced `+0xf2` correction when raw globals `0x53abd>5 && 0x53ab9<3`; the high-level names of those globals and the remaining icon/digit layout remain unassigned. Therefore the current map HUD is explicitly an approximation and must not reuse the full-screen battle-panel asset as proof of native equivalence.
