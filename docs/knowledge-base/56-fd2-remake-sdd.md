...
`cmd/fd2` now owns the playable adapter for this contract. Beat start preflights the complete field, tile-aligned camera, raw actor provenance, selector cache, LUTs, buffers and DAC before publishing a job; rejection leaves the existing map buffers unchanged. Each of the nine indexed presents must receive a real `Draw` acknowledgement, the 500ms tail is 30 update ticks, and each of the 32 cumulative DAC mutations must likewise be drawn before continuation. This preserves every native visual state and prevents a fast update loop from collapsing to the last frame. Because Ebiten's 60Hz host presentation cannot display distinct 5ms/4ms native writes at their original wall-clock cadence, the adapter deliberately uses one host present as the minimum duration and does **not** claim DOS timing parity. The active ch29-post binding can now execute `0x24618`; the enclosing terminal handler remains outside the campaign graph until its later `0x2bce5` ending renderer is complete.
...
2026-07-29 runtime integration narrows that remaining boundary. Direct calls show selector1 at AI dispatcher `0x13E77` and three player completion branches `0x18AEF/0x18B0C/0x18B66`; it is not the selector0 left-step commit. The remake wait completion now owns an event61 gate. Missing D0 queues editable FDTXT2 only. With an explicitly materialized native map runtime, success queues FDTXT3, composes the current 320×200 indexed map, cumulatively blits all 59 FDOTHER#45 frames at the typed base with two BIOS ticks per present, commits D0/entry12/group1, persists JOIN31, then queues the ten FDTXT4 lines. Real-asset Docker regression covers this full adapter and cancellation-safe mutation. This is still E1, not E2: `battle_ch26` has no evidenced dynamic `native_map_view/native_map_hud`, so the unmodified campaign correctly rejects source-frame composition before mutation. Other player actions and the AI selector1 tail remain unbound until their success owners use the same gate.
...
event61。selector1 目前只接入玩家 wait 成功收尾；attack、magic、item
與 AI 的其他成功收尾仍維持失敗即關閉。
