...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Selected-unit HUD boundary: `0x11cac` calls `0x1acf3` after terrain, range and unit/foreground layers but before the viewport copy. `0x1acf3` returns without drawing unless both raw display bytes `0x51aab` and `0x51aac` are nonzero. It first calls `0x12e38` on cursor globals: that helper is a terrain-cell resolver, yielding FDFIELD tile word masked to 10 bits, event low five bits, and the selected four-byte FDSHAP control record; `fdicon.NativeTerrainCursorInfoForCell` preserves this raw contract. Its control byte+1 indexes the verified `0x51a12`/`0x51a2a` terrain AP/DP table: 0→(+5,0), 1/5→(0,0), 2/3→(-5,+10), 4→(-5,-5). `battle.Load` derives the same byte per validated map cell and combat consumes it directly. The now-closed panel geometry is FDOTHER #5 LMI1 #130 (69×34) at `buffer + stride*157 + x`, terrain icon at `+6`, AP signed digits at `stride*8+0x2b`, and DP signed digits at `stride*19+0x2b`; `0x1aeb1` uses #0x83/#0x84 for sign then the native digit path. `x` is the raw static anchor (initial 1; the observed global branches select 0xf2 for `0x53abd>5 && 0x53ab9<3`, or restore 1 when `0x53abd>5 && 0x53ab9>9`). `0x1acf3` then uses `0x12c0d` for an optional active unit icon and HP path. The global meanings and unit-icon/HP resource semantics remain raw. Therefore the current map HUD is explicitly an approximation and must not reuse the full-screen battle-panel asset as proof of native equivalence.
