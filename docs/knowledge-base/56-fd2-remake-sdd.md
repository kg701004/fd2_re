...
| UI-12 | Save/load | scene-safe boundary、campaign cursor、flags、party/inventory/equipment、version/checksum、四槽 selector | partial；自有格式可用。native 已知 `FD2.SAV` rolling-XOR/checksum envelope 與 4×logical records（`+0x312b+i*0xa28`，`0x28` metadata + roster `0xa00`）；metadata `+0`=chapter/`0xff` empty 已閉合，其餘語意未閉合，尚非相容實作 |
