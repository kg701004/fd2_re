...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Correction: the sprite pointer table is no longer opaque. `0x11019(group)` constructs `0x53a61` as twelve pointers per FDICON map-sprite group, and `0x127e0` selects `unit+2 × 12 + pose×3 + cycle`. This is a distinct raw field from battle `0x287b5..0x2884c`, which selects `FIGANI.DAT` by `unit+7 × 3`; an exported visual id may coincide for an evidenced roster record, but must not turn the two ABI fields into aliases. Cycle is global idle/moving animation state, not unit `+4`; that byte offsets the camera-relative placement. The remaining adapter boundary is runtime roster state, remap selection and layer order—not an invented FIGANI or unknown archive mapping.
