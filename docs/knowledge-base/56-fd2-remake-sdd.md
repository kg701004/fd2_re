...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Production steady-frame slice (2026-07-28): ch01 now materializes the explicitly sourced raw selector zero, uses the original party-first/initial-group append constructor order, and consumes regenerated FDFIELD/FDSHAP raw map fields. `nativeBIOSClock` supplies a battle-local signed BIOS low word at the PIT rate; one Update corresponds to the one `0x1297d` call at `0x11cb7`, while terrain phase and the two independent BIOS-word latches consume the same sample. `drawNativeMapFrame` can therefore execute the entire proven neutral `0x11cac` pipeline and present the corrected VGA surface in production. It deliberately falls back for every nonzero selector until the shared overlay/target-legality lifecycle is materialized; cursor artwork/flash is unresolved and the visible white selector remains an explicit approximation.
