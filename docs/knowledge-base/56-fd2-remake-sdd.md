...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Constructor provenance correction: Docker Capstone of `0x10d7f..0x10efc` proves runtime `unit+6` receives FDFIELD roster byte0 and `unit+7/+8` receives roster byte1; the existing editable `map_selector_key` and `battle_fig` fields therefore preserve those two raw sources. `unit+0x1f` is written from the portrait/resource lookup branch (`DATO`-derived data), not from an unlabelled FDFIELD byte. Until that resource byte is exported, optional unit/HP admission must remain nil rather than treating portrait/class as a substitute.
