...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Codec boundary: LMI1 is a directory, not one universal codec. `0x1acf3` sends #130 and its table-value-dependent #0x83/#0x84 entries to `0x4e63d`, the four-mode transparent RLE path. `fdother.ParseLMI1FrameEntry` / `DecodeLMI1FrameResource` preserve that explicit route and regression-decode those three player-archive entries at their verified geometries (69×34, 44×12, 45×12). They intentionally do not reinterpret adjacent LMI1 cells handled by `0x4e916`.
