...
| UI-07 | Postbattle | result → handler → reward/roster cleanup → town/shop/rest/preparation 或 ending；不可預設直連下一戰 | partial；campaign schema 與 bounded menu trace 可表達，`town_ch02→preparation_ch02→story_ch02_pre→battle_ch02` 已有可重播 trace；ch04/ch05/ch08/ch09/ch10/ch11/ch12/ch13/ch18/ch19 post handler 已通過 Docker compiler regression 並接入 authored binding。ch04 的 `0x233c6` layout 已保存 slots0–6、special slot41、camera `(144,96)`，並以 FDTXT_005 index9 的 17+2 句映射驗證；ch05 保存 map5 40-slot frontier、spawn group3=1、pan `(120,336)`、acting resource27(slot34/pose2) 與 FDTXT_006 index6 的 19句；ch08 保存 map8 60-slot frontier、spawn group4=1、pan `(144,24)`、acting resource36(slot47/pose0) 與 FDTXT_009 index4 的 5句；ch11 保存 map11 60-slot frontier、14-slot layout（special slot2 覆寫為 `(10,4,pose0)`）、camera `(336,0)`、acting resource45 的 slot8 special frames 與 FDTXT_012 index3/4 的 3+7句；ch13 保存 map13 70-slot frontier、spawn group1=1、16-slot layout（special slot0 最終 `(0,0,pose0)`）、camera `(288,240)`、acting resource47(slot67/pose2) 與 FDTXT_014 index2/3 的 10+7句；ch09/ch19 acting resources 由 Docker exporter 解碼，ch19 raw handler trace 並釘住入口 frontier83→spawn group1 一筆→slot83；其餘 unbound `postbattle_*` 現在 fail-closed 停住，不會空 beats 直跳 town，逐關 branch 證據仍不足 |
