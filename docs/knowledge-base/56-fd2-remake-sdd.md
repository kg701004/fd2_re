...
The same audit rechecked `0x1b8e7` because it is shared by class-change, item, and post-resolution callers. Its proven body computes a source in the eight 2-byte cells at `record+0x0a`, passes source/destination/length to overlap-safe `0x373c4`, then writes raw `record+0x18=0x80`. Callers do not expose a uniform standalone `(unit, slot)` contract; the effective third stack value participates in source/destination selection. Until that CFG is closed, the remake must not label this as item removal or compact normalized `Inventory` after a shop/church/postbattle action.
