...
| UI-11 | Preparation | 城鎮出發確認／無城鎮記錄詢問、依名冊門檻略過或進入部署、上限（15／19）、取消、最終確認、進戰場 | partial；`0x2cad7` 與 `0x2d093→0x318ad` 的分流已接資料模型。`0x318ad/0x31e80` 的 320×200 三區背景、二位數計數、10 欄角色格、游標、已選彩色／未選灰色及左右一格／上下一列輸入已由原版資源接入；缺原始圖像索引時維持失敗即關閉。局部證據圖為 [`preparation-roster-compositor-partial.png`](../figures/preparation-roster-compositor-partial.png)，不是 DOSBox 截圖或正常戰役存檔。`0x17fc0` 角色狀態、待機計時、進退動畫、最終確認外觀與原版實機差分仍缺；`0x1f42d` 分割滑入原語已閉合但尚未接完整流程 |
