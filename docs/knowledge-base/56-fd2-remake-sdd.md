...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   `State.AppendNativeMapSelectorBatch` now supplies the atomic state seam for that proven order. It owns one per-resource cache and appends only a fully valid batch; party `[9,4]` followed by scripted `[0,2,0]` produces slots `[0,1,2,3,2]`, while a missing-key batch changes neither runtime unit order nor cache. Existing scenario append remains deliberately legacy until all authored/exported chapters carry raw keys, so no UI path is falsely upgraded to native rendering.
