...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Correction: the sprite pointer table is no longer opaque. `0x11019(group)` constructs `0x53a61` as twelve pointers per FDICON map-sprite group, and `0x127e0` selects `unit+2 × 12 + pose×3 + cycle`. This is a distinct raw field from battle `0x287b5..0x2884c`, which selects `FIGANI.DAT` by `unit+7 × 3`. `export_units.py` now writes FDFIELD control `b1` as `battle_fig`, and `battle.Unit.BattleFig` feeds the full-screen FIGANI adapter; the legacy `fig` remains explicitly marked as an unseparated map-`+2` approximation, with a compatibility fallback only for old exports. Thus the two ABI fields can differ in runtime data without being aliased. Cycle is global idle/moving animation state, not unit `+4`; that byte offsets the camera-relative placement. The remaining adapter boundary is the map `+2` export, remap selection and layer order—not an invented FIGANI mapping.
