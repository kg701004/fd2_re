...
| UI-04 | Target/range/item selector | 武器 min/max reach、法術 range/AOE、item兩欄四列、不可用目標灰化、確認／取消 | partial；command/item targets與 observed item effects已閉合。`0x1b9de/0x184c0` 固定 compact occupied prefix、↑↓linear wrap、←→±4、battle type0 confirm gate、`(42/192,103+22r)` layout與raw icon IDs；pure adapters已有。現行八個 raw-hole shell不是original parity，Enter transaction/indexed animation仍fail-closed |
