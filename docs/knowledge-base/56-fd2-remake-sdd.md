...
| UI-11 | Preparation | 城鎮出發確認／無城鎮記錄詢問、依名冊門檻略過或進入部署、上限（15／19）、取消、最終確認、進戰場 | partial；`0x2cad7` 與 `0x2d093→0x318ad` 的分流已接資料模型。`0x318ad/0x31e80` 的 320×200 三區背景、二位數計數、10 欄角色格、游標、已選彩色／未選灰色及左右一格／上下一列輸入已由原版資源接入；游標角色再以既有 `0x17fc0` 原子合成器畫完整狀態。`0x1297d` 的有號 BIOS 低字差值 `>4`／回繞、原始狀態 `0..3` 與可見 `0,1,2,1` 待機週期亦已接正式更新。`0x31d3c` 的穩定確認畫面已按指令順序接入：`0x1956b(0x4b)` 建 FDOTHER #5 對話框與 DATO #75 商店店員、FDTXT `0x292` 由 `0xa951f` 起筆，`0x16559(0)` 再以第0幀覆蓋重疊文字，最後使用 `0x19953` 的 FDOTHER #2 Yes／No。缺任一原始記錄或資源即退回。證據圖 [`preparation-roster-compositor-partial.png`](../figures/preparation-roster-compositor-partial.png) 與 [`preparation-confirmation-compositor-partial.png`](../figures/preparation-confirmation-compositor-partial.png) 都是 E1 原始資源合成，不是 DOSBox 截圖或正常晚期戰役存檔。最終確認的六幀開框、脈動、關框、跨畫面初始相位及原版實機差分仍缺。`0x1f42d` 屬 `0x1a30b` 戰場進入演出，不再列為選人視窗動畫 |
