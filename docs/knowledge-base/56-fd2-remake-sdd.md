...
| UI-04 | Target/range | 武器 min/max reach、法術 range/AOE、不可用目標灰化、確認／取消 | partial；battle native command candidate→confirm 已接 selected raw record；item case0 的 raw two-stage plan（row `+0x10/+0x12/+0x15`、type0x17 inner marker）與 confirmed→final list resolver已閉合。完整 runtime item-row producer、effect renderer/gameplay mapping 未閉合，故 8-slot selector Enter仍 fail-closed；不能把 raw modes改寫為 normalized range/AOE |
