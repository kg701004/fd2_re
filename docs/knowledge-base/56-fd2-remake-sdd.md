...
| UI-10 | Church | revive、class change、費率、候選過濾、確認／取消、缺資料 fail-closed | partial；class path 已對齊 `0x31385→0x31793→0x311DC→0x19953`：Lv>=20、portrait<0x12 且 !=7，三列可見候選、上下 bounded，special>optional>default 自動解析唯一 target，再以左右 Yes/No 確認。`0x31019` 的 FDICON＋四段 FDTXT row、FDOTHER#14 entry16 panel 與 `0x1974c` 六幀 crop schedule已成 indexed compositor；`0x19953` 亦已接 FFFC 動態角色名、FDOTHER#2 cells16/17 opening、48/49與51/52 normal/pulse、四幀 `0x197e5` closing。兩者 final frame 已接 runtime；draw-ack／BIOS pulse cadence、FD2.SAV與未接 callee仍 fail-closed |
