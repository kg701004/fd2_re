...
目前 `cmd/fd2` 的 `any_unit_inactive` 執行器仍在缺少 `HasNativeRecordByte5` 時使用 `OnField/Alive` 作相容 projection；這是明確的 E1 gap，不是 native parity。要升級為 E0，必須在 LOADCH／death／raw byte writer 路徑完整 materialize `NativeRecordByte5`，並讓 predicate 缺 raw 時 fail-closed；在此之前，ch01/ch02 authored handler 只能算可測試腳本切片。
