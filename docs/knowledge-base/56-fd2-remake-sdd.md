...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Native range-overlay contract: `0x122dc` dispatches raw mode 1..5 to an **ordered** table of `0x126f7(x,y,descriptor)` calls. `fdother.NativeRangeOverlayPlacements` preserves all 1/1/5/13/21 calls respectively, including the mode-3 centre descriptor `14` and the mode-5 repeated coordinates with distinct descriptors; it must not normalize them into an inferred diamond or ordinary movement range. `0x126f7` alone owns camera clipping and raw indexed-RLE descriptor blitting. Mode 6 is not a draw table: it writes zero to `[0x53a51 + 4*(cursorX + cursorY*[0x53ac1]) + 7]`; the API exposes only its checked byte-address arithmetic and rejects it as a drawable mode. The descriptor-bank pointer, RLE resource binding, clipping policy, and indexed renderer integration remain unclosed, so GUI range highlights are still not native-equivalent.
