...
3. **Battle interaction**：追 action menu enable gates、weapon reach、spell inventory/targeting、end-turn 判定、HUD anchor；每一項先找 caller/data flow，再改 Go。
   ...
   Codec boundary: LMI1 is a directory, not one universal codec. `0x1acf3` sends #130 and its table-value-dependent decimal #83/#84 entries to `0x4e63d`, the four-mode transparent RLE path. `fdother.ParseLMI1FrameEntry` / `DecodeLMI1FrameResource` preserve their descriptor route and regression-check the three player-archive geometries (69×34, 44×12, 45×12). The generic Go four-mode decoder currently rejects #83/#84 streams, so no successful HUD frame decode or GUI adapter is claimed for them; this remains a fail-closed decoder gap. `fdicon.NativeMapHUDLayoutFor` preserves the six strict 456-stride destinations and rejects an anchor whose 69-pixel frame does not fit the native 320-pixel viewport. These adapters intentionally do not reinterpret adjacent LMI1 cells handled by `0x4e916` or claim an Ebiten renderer.
