...
| raw byte writer / spawn intro | 0x32975(slot)=整 byte覆寫1；0x32999(group)=spawn＋FDOTHER #9 固定12次索引呈現；呼叫端 acting 分離 | 完整 callee body與四個 caller ✅；constructor 0x10eed 寫0 | raw_byte5_write / spawn_intro（adapter 未完成，失敗即關閉） | 部分 |
