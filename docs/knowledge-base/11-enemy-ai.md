...
以下段落保留僅供追溯早期筆記；canonical Docker scan 已確認 `0x15140`、
`0x15356` 沒有 direct caller，且 `0x15140` 不構成已證實的 AI entry。不得把
這些地址、倍率或「dmg≤2」規則當作原版語意或 remake runtime 規格。現行可用
證據請以本檔前述 `0x13A9F`、`0x14EF0`、`0x149F8`、`0x15B77` raw
boundaries 為準；完整權重與執行鏈仍 fail-closed。
