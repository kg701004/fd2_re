...
| 2 | 先走 `0x14EF0`；失敗直接呼叫 `0x14237`，再進入共用收尾；不走 `0x13E9C` 最近座標備援，也不呼叫 `0x13FD4` |
...
`0x13FD4` 回復；合法 IDA 9.4 直接指令與 `0x13FD4` xref 已更正為：
完整 IDA 位址／雜湊記錄見
[`fd2_ai_mode_dispatch_ida.txt`](../data/ida/fd2_ai_mode_dispatch_ida.txt)。

```text
0x14EF0 成功 → 共用收尾
0x14EF0 失敗 → 0x14237 → 共用收尾
```

`0x13FD4` 不是模式 2 的後續函式。IDA 9.4 只找到同一
`sub_13A9F` 內的 direct callers `0x13BC5`、`0x13C0F`、`0x13C84`、
`0x13D58`、`0x13E52`，另有 `0x19082`；沒有 `0x13B5F`。它只在 current HP `+0x40` 不等於
