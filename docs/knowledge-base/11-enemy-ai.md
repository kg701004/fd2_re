...
與欄位契約，且其中關鍵入口已被直接掃描否定，因此不再保留公式內容，避免
後續代理或重製程式誤用。若社群攻略描述相同行為，只能作待驗證線索。
...
1. 靜態閉合 `0x1548E` 完整函式與 callers，記錄輸入／輸出欄位及是否真的
   呼叫 `0x4EE40` 或 `0x4F355`，未閉合前不命名為路徑搜尋。
2. 在固定原版存檔與固定單位上，依序於 `0x1D80B`、`0x13A9F`、
   `0x14EF0`、`0x15AD8` 記錄單位索引、關鍵原始欄位與
   `0x53C23..0x53C3F`。比較只改一格位置或一個候選的兩個狀態。
3. 先證實單位掃描順序、候選數與選定結果的資料來源，再命名目標評分；
   不以現有重製人工智慧輸出補足原版未知值。
