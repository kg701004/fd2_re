...
2. `0x14288→0x1B83D` 及 `0x1429C→0x1B722→0x4E56C` 取得 actor
   的 raw command record；`0x14331→0x4E040` 與
   `0x1434C→0x146D1` 建立候選格集合。
...
2. 追蹤 `0x14237` 的 `0x1B83D→0x1B722` command record 來源與
   `0x146D1` 候選格順序，閉合「可達格」資料契約及同分時的實際順序。
3. 先證實單位掃描順序、候選數與選定結果的資料來源，再命名剩餘 raw 欄位；
   不以現有重製人工智慧輸出補足原版未知值。

## 仍待確認

- `0x14B78` 的最後一個 caller argument 與完整移動／路徑資料契約；
...
`0x1DEBE` 條件、raw `+8`、候選格產生順序、無攻擊目標時的移動分支，以及
