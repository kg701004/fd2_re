...
- event 62 的 `slot: 0` 與實際寫入位置 `[base+3]` 是否一致
  (2026-09-03 反組譯時發現,未改值,見 `extract_native_field_event_rules.py` 註解)

**需要使用者參與才能收斂**
- 字模表 10 組重複對映(同一字對到兩個 glyph index)的目視確認。
  commit `a1851a76` 當時的說法是「own visual confidence too low to commit a fix」
  ——這是**信心門檻**問題,不是技術問題,需要熟悉原版的人拍板。
- `ch03`／`ch06` 秘密商店組合鍵:已排除 5 個假說(見 doc58 續五~續七),
  再往下是**要不要繼續投入**的取捨,不是缺方法。

**已完成(2026-09-03,原列在此)**
- ~~在新版 EXE 上重新錨定 JOIN 建構器表位址~~ → 已用現存 EXE 重生並進版
  `docs/data/native_join_constructor.json`,32 列與舊版逐位元組相同,
  `test_fd2save` 14/14 零 skip
- ~~`apply_hd_composite`／`char_summary` 無參數時安靜 exit 0~~ →
  前者已隨 remake 工具移除,後者已加輸入守衛
- ~~四支被舊版 EXE 擋住的抽取器~~ → 已改為同時支援兩版,逐項重現驗收通過
