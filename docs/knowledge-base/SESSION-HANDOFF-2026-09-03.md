...
> ➡ **已有更新的交接檔:[`SESSION-HANDOFF-2026-09-04.md`](SESSION-HANDOFF-2026-09-04.md)**。
> 本檔 §9 對「確認鍵送不進戰鬥輸入迴圈」的描述**已被否證**(94/94 重測),
> 且該節列的 7 項阻塞不再成立;請以新檔為準。
...
> ⚠ **2026-09-04 更新:本節以下對這道牆的描述已被推翻,保留供追溯。**
> 用原版 DOSBox-X 重驗的結論是**這不是輸入問題**:確認鍵連續約 120 次從標題一路推完 ch01
> 全部開場演出,零掉鍵。真正的原因是量測方法在被提問的那個畫面上壞掉了兩次——
> `wait-settle` 會停在**轉場開始之前**的舊畫面(對確認鍵的傷害遠大於方向鍵,正好造出
> 「方向鍵會到、確認鍵不會到」的假象),而戰場畫面**永遠不會靜止**(單位待機動畫;同一秒
> 連拍三張三個 md5),使 settle 必定 TIMEOUT、`--flag-no-response` 必定為假。
> 三個指令環 gate 也已用活體記憶體否證(我方四人全部通過)。工具已修。
>
> **這道牆是完全否證,不是縮小**:從瀏覽游標按 Enter 已當場斷點命中 `0x18890`
> (EIP=`0x001B4890`),畫面出現正常的移動範圍選格,doc13 §0 的
> `0x117e7 → 0x18890 → 0x115b6 → 0x18d8c` 鏈路在原版上逐段成立。狀態全域同時證實
> 戰鬥正在進行(回合=1、場上 12 單位、游標停在可行動的我方單位上)。
> **因此本節「它擋住的 7 件」不再被這道牆擋住**——那 7 項要重新評估各自真正的前置條件。
> **完整證據與對照見 `48-dosbox-x-debugger-build.md` §9(含 §9.7 對我自己 §9.4/9.5
> 過度推論的更正)。**

**最高槓桿的一項:確認鍵送不進戰鬥輸入迴圈**

這一項擋住下面 7 件事,解掉它一次全開。2026-09-03 把它的形狀縮小了:
**不是「送鍵整體失效」,而是方向鍵會到、確認鍵不會到**,且與 window/xtest 模式、
瞬按/長按(0.4s keydown-keyup)、`Return`/`space`/`KP_Enter` 全都無關。
方向鍵每次都正常移動游標,所以不是視窗焦點或 X server 層的問題。

已排除的混淆(重要,容易重犯):游標可能停在 **camp `0x001` 的客座 NPC** 上
(本輪就是「珊」,狀態卡 HP 150 對不上我方任何人),那種情況按確認鍵當然沒反應,
極易誤記成輸入層失效。**先用狀態卡 HP 與記憶體對照確認游標在誰身上再下判斷。**
本輪已移到索爾(卡片 823 與 `+0x40` 吻合)確認後仍開不了環,所以混淆已排除。

它擋住的 7 件:
- 原版指令環的字模截圖(本輪目標,`glyph 423`/「麻痺術」的最後一分證據)
- M5-F 六項:戰鬥/勝利曲、自動結束回合、治療咒視覺、移動確認輸入丟失、
  ending 演出、ch27 玩家路徑(`fd2_chapter_sweep` 全 30 章都是 `needs_manual_followup`)

**可以做(不需要任何人先決定什麼,也不卡在上面那道牆)**
...
**字模表重複對映:10 組已解 8 組**(2026-09-03)
- 已定案 8 組:468→賣、1070→養、1035→嘗、1581→踹、1274→癮、1366→；、1813→—、585→‥、347→·
  (方法:文本上下文為主,pixel-IoU 經校準證實在此無鑑別力;詳見 doc98 續五)
- **`庫` glyph 423** → 依 doc02 攻略／doc13 反組譯／原始角色資料表三方獨立證據改為 `痺`;
  `command_labels.json` id27 維持使用者裁決的「麻痺術」。原版實機截圖仍缺(卡在上面那道牆)
- **`、` glyph 1188 / 1507 仍未解**:兩者在**同一句**裡都當頓號用
  (「蘊含光[1188]闇[1507]火焰」),點陣一個在左下、一個在置中偏上。
  可能是 `，`／`、` 之分,但兩者上下文都是列舉,**現有證據無法分離**。不猜。

**需要使用者決定要不要繼續投入**
- `ch03`／`ch06` 秘密商店組合鍵:已排除 5 個假說(見 doc58 續五~續七),
  其餘 8 章已實測成立。不是缺方法,是取捨。
