...
- [x] **finale DATO layout call ABI**：official IDA 釘出 `0x2c773→0x168b6` 實參 `(destination=C, stride=0x140, arg8=5, argC=7, arg10=5, arg14=5)`；`0x168b6` 先建立 DATO mouth-frame/grid 到 portrait restore buffer C，後續 `0x4e8af` 才貼到 staging。`native_2c548.json` 與 strict loader 已新增 `dato_layout`，不再把 DATO 說成單張靜態頭像；mouth cadence/完整 indexed paste 仍未接 runtime。
...
- [x] **DATO `0x168b6` raw grid plan**：`Montage.PlanDatoGrid()` 逐一保存 49 次 `sub_1685c` 的 raw resource index/destination byte（固定 12 次、兩組 3×2 loop、5×5 grid），保留 `stride=320,arg8=5,argC=7,arg10=5,arg14=5` 的 exact arithmetic；不替 cell 命名 border/portrait，也未解除 DATO renderer gate。
