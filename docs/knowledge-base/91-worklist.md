...
- [~] **UI-SHOP-RECIPIENT-INPUT-E2**：原版已實測裝備收件者由selection0按Down到selection1、再按Up回selection0，Left/Right不改selection。production現共用純`advanceNativeShopEquipmentRecipient`：bounded Up/Down、horizontal no-op、同tick Up後Down順序與`NativeThreeRowWindow`stateful origin均有直接input regression；helper-level invalid count/selection/start rejection亦有測試，production caller會在索引recipient前fail closed回purchase list。這只閉合E1 production input contract，scroll仍沒有原版E2。selection1高頻原版／remake比較只差商店人物區`(175,90)`、`(176,90)`兩個像素（6個RGB bytes），recipient panel、黃色選列、數值與三列window其餘像素一致；selection0不同原版採樣亦出現同一環境相位差，故不得遮罩或宣稱AE=0。DOSBox runner已有容器實測`waitpixel:x,y,r,g,b,delay,max`，下輪用它同步原版環境相位後再決定是否閉合selection1／scroll E2。
