...
- [x] **RE-AI-RAW-RECORD-1598A**：`NativeAIScoringRecords` 以完整 provenance 建立 detached `0x50` runtime snapshots，補齊 native presentation、`+5/+6/+34..+36` 並拒絕 partial roster；map0 真實匯出資產 anchor 通過。下一步把 `0x1598A` 的 `0x4E040→0x14B16→0x14818` 候選順序接到這份快照，仍不得接 normalized `NextAIPlan`。
- [x] **RE-AI-CANDIDATES-1598A**：`NativeAIScoredCommandCandidateGroups` 已以 command `+3/+4`、原版 cost row、exact grid flags 與 raw `+5/+6` 建立 row-major destination/target-index groups；selector target-code transform 與空 target skip 已保存，map0 identity103→ally `(23,14)` 真實資產 regression 通過。下一步逐 command 將 groups 交給 `0x15B77` family scorer，再以既有 strict score／command-word tie-break 寫出單位級 `[0x53C23]` 結果。
