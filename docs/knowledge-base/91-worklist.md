...
- [~] **RE-CHURCH-RAW-0-17AED**：官方 IDA 9.4 與指令級重讀固定 `0x2ffa5→0x17aed(actor)` 是單參數（Hex-Rays 第二參數為 artifact）：`0x17e0b(actor)` item/status staging、`0x16c57(0)` input wait、`0x1c269(actor,0)` gate、`0x1ceed(actor,-1,...)` command/MP overlay 與 restore/redraw loop；body 未見 persistent roster/inventory/gold/HP/class writer，因此撤回「能力服務」命名。remake 已接 caller 的 `0x2e6b8/0x2ea90` 原版兩欄六人 indexed roster、bounded ±1/±2、6-open/5-close＋source restore；actor confirm 仍在 `0x1ceed` renderer/input 未閉合處 fail-closed，不接 normalized gameplay。
