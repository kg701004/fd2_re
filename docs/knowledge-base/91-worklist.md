...
- [~] class-change church：核心 `campaign.ApplyClassChange` 已依 `0x31602` 實作可重現 RNG（row `[min,max)`）、將新職 AP/DP/DX/MaxHP/MaxMP growth **累加**既有值、MV(+0x3b) 累加、保留 Lv、清 EXP、HP/MP 回滿與轉職道具移除；persistent party 已同步保存 MV。自動 target 解析與左右 Yes/No confirmation 已接，仍需原版實機數值回歸。——**已解**(doc58續八十二,2026-08-31)：5次真實DOSBox-X class-change trial(悠妮portrait9→target0x34/召喚師)實測AP{9,9,10,10,10}/DP{8,8,8,8,8}/DX{4,4,4,4,4}/MaxHP{14,14,16,16,15}/MaxMP{21,24,23,23,21}，全部落在`class_change_growth.json`row idx52的`[min,max)`宣稱range內(AP[9,12)/DP[6,9)/DX[3,5)/HP[12,18)/MP[20,30))，MV固定+2也逐次吻合；未發現任何超出範圍的違規值。——**新疑點，誠實記錄，非本輪修復範圍**(doc58續八十五,2026-08-31)：本輪首次在真實DOSBox-X上打開轉職後的raw service0狀態畫面(續八十二只讀了逐行成長訊息，未讀status畫面)，LV欄位顯示`01`而非預期的`40`，但HP/MP/AP/DX等數量級明顯仍是高等級角色的數值，與「保留Lv」的既有RE結論(本條「保留 Lv」)矛盾。可能是該畫面LV顯示邏輯從EXP即時查表算出(EXP歸零→查表得1)而非讀存的Lv byte，也可能是原版轉職真的會動到某個尚未找到的欄位；remake的`ApplyClassChange`目前完全不寫`u.Lv`。這是一個新的、獨立的疑點，留給下一輪class-change相關RE工作參考，不影響本條AP/DP/DX/MaxHP/MaxMP/MV growth range的既有驗證結論。——**「保留 Lv」勘誤，已用純靜態反組譯解決**(doc32§6.3.1,2026-08-31)：上面疑點的答案是(b)，不是(a)——新版EXE`FD2Analysis3`反組譯找到轉職重算函式(新版位址`FUN_0002ac7d`，透過`xref_to 0x1e529`沿呼叫圖定位，不是位址平移假設)在growth-add×5／MV累加／`0x1b750`衍生重算之後，有一行無條件`MOV byte ptr [ESI+0x21],0x1`(`0x2aded`)，緊接著才是EXP歸零與HP/MP回滿；狀態面板渲染器(`0x17fc0`)的LV欄位是對同一offset的直接讀取，不含任何EXP查表。**本條「保留Lv」的敘述是錯的，應改為「Lv byte 轉職後被寫死成1，AP/DP/DX/MaxHP/MaxMP/MV累加不受影響」**；remake `ApplyClassChange`建議加`u.Lv=1`(未實作，留待下一輪決定)。詳見doc32§6.3.1完整反組譯證據。
