...
- [~] **RE-CHURCH-RAW-SERVICE-LISTS-2E6B8**：官方 IDA 9.4 閉合 `0x2e6b8/0x2df6b` bounded two-column selector。`0x2f8ea` 掃八格 signed flag非負 item，FDTXT510/511/512＋`0x1b8e7→0x1bb8c` 閉合為物品轉交；完整 body不呼叫`0x2f4c6/0x2d516`且不改gold，已刪除舊混線斷言。remake 已接來源／目的原版 roster、`0x2dc55(mode1)` item icon/name/stat/3⁄4 price五位數 indexed list、stateful viewport、6-open/5-close+restore與成功／取消返回來源 loop。目的八格滿載分支也已依 caller 接入：destination roster 關閉後，以 raw `unit[8]+1` 展開 FDTXT506 的 FFFC，6-open、`0x16c57(1)`等待、5-close，再回來源；raw flags／native identity 不完整時 fail-closed。保存[`native-transfer-item-indexed.png`](../figures/native-transfer-item-indexed.png)與[`native-transfer-full-indexed.png`](../figures/native-transfer-full-indexed.png)。仍待 DOSBox E2 visual/input diff，故維持 `[~]`。
