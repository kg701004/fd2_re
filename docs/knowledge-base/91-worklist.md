...
- [~] class-change data/UI bridge：`LoadClassChangeTable`、`NativeClassChangeTarget`、`LoadClassChangeGrowth` 已接；church 先在三列視窗選角色，再依 special>optional>default 自動解析唯一 target，最後以左右 Yes/No confirmation 決定 mutation。`0x31019` row、FDOTHER#14 entry16 panel 與 `0x1974c` 六幀 crop 已成 fail-closed indexed compositor，final frame 已接 runtime；保存 [`native-class-list-indexed.png`](../figures/native-class-list-indexed.png)。待 opening draw-ack cadence、`0x19953` confirmation renderer 與 HIT/EV/DX 實機數值差分。
