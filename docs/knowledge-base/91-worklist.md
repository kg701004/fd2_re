...
- [x] **command 0 core engine slice**：`State.ExecuteNativeCommand0` 只組合已閉合的 ID0 record、two-stage targets、一次 MP debit 與 per-final-target resistance/hit/HP clamp；missing table/flag/candidate/resistance 在 mutation 前 fail-closed。未接 UI、renderer/post-resolution 或其他 command ID。
...
- [~] **native command IDs13..16 healing route**：IDA `0x21AD9/0x21B99/0x2211C/0x22153→0x21B18` 已閉合 generic final target array、`0x1CA89(actor,id)` MP debit、`0x1C8ED→0x1C916` per-target HP restore 及 `+0x42` cap；其 amount 公式同 `9/10 + rand%100/1000`。專用 indexed animation、UI、SFX 和 engine resolver 尚未接入，禁止誤用 IDs0..9 damage executor。
- [~] **native commands 17..19 transient modifiers**：ID17/18/19 handlers 已直接定位 `+0x22/+0x23/+0x24` nonzero gate 與 writer：17 對 derived `+0x48`、18 對 `+0x4a` 做 `__CHP(value*0.15+1)` FPU-rounded increase 並設 2..5 duration，19 對 `+0x4c/+0x4e` 各加 15 並設 duration。這撤回 doc35 將 `+0x48..+0x4e` 稱作 screen coordinates 的衝突斷言；status labels、duration decrement、UI、engine integration 尚未閉合。
- [~] **native commands 20..21 flag-clear/restore route**：`0x22A85/0x22BC6→0x22AA8→0x22AF6` 分別對 `+0x25/+0x26` 做 nonzero gate；成功時以 command record 10 呼 `0x1C916` HP writer 後清 flag，零 flag 只顯示失敗。MP debit 仍以 command20/21 record。兩個 status 名稱與 UI 未閉合；ID22 的 `+0x27` application route 不可混入。
- [~] **native command 22 application route**：`0x22BE1→0x22CDA→0x22D1B` 在 `+0x27==0`、class `+0x20∉{0x19,0x1a}` 且 `rand()%100<50` 時，固定以 `0x1C81F(target,10)` 扣 10 HP，寫 `rand()%4+2` 至 `+0x27`；其他路徑僅失敗顯示。status name/tick、UI、engine integration 未閉合。
- [x] **transient command duration lifecycle**：`0x1A30B` phase driver 依 camp 呼叫 `0x1A866(camp)`；它對 active 同-camp unit 的六個 bytes `+0x22..+0x27` 逐一 decrement，歸零才發 expiry feedback 並 `0x1B750` 重算 derived stats。故 command17..22 的 `rand()%4+2` 是 camp-phase duration，不是 action/frame timer；status name/icon/UI 與 remake integration 仍待。
...
- [~] **native commands 25..27 closure**：ID25 `0x22C04` 以 record25 MP debit，僅在 target `+5 bit0x80` 已設時清除該 action-complete bit；ID26/27 `0x22CBF/0x22E41` 分別復用 ID22 的 application helper 到 `+0x25/+0x26`，同樣有 class/RNG gate、固定 10 HP damage 與 2..5 duration。20↔26、21↔27 的 raw clear/apply pair 已閉合；UI/status labels/engine integration 待。
...
- [~] **native `0x22253` renderer adapter**：已釘死 `0x22547→0x22046` indexed off-screen blit 呼叫鏈與 FDOTHER #81（outer nested `LLLLLL` 18710 bytes、directory first-word `0x12`；nested payload #1=9782 bytes）；`internal/fdother.ArchiveEntry` 已補 nested boundary regression，但 `0x11eee` data selection 與 indexed buffer／resource decoder／PNG or Ebiten adapter 仍缺，`unit_present` 暫維持 fail-closed。
