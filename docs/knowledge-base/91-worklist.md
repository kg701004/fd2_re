...
- [~] **commands 0..8 direct compositor boundary**：重新檢查 `0x1cff0` 可達性後，IDs0..8 是直接 `0x2a6bd`，不是 handler table 的 `0x21227/0x213b7` numeric route。舊 ID0 executor／target UI 已撤除，直到 `0x2a6bd` 的 gameplay state writer 有完整 dataflow；raw grid 對此族保持 fail-closed。
- [x] **shared native damage route IDs9..12**：ID9 直接 `1CA89→1C75E`；IDs10..12 的 `0x21548` 專用 compositor 尾端也直接 `1CA89→per-target 1C75E`。同樣扣 MP、逐 target numeric writer、success acted；engine bounded support 僅 9..12，UI 未擴張。不得因 table 中 IDs0..8 wrapper 存在而宣稱玩家可達。
