...
- [~] **native `0x22253` renderer adapter**：已釘死 `0x22547→0x22046` indexed off-screen blit 呼叫鏈與 FDOTHER #81（outer nested `LLLLLL` 18710 bytes、directory first-word `0x12`；nested payload #1=9782 bytes）。2026-07-26 更正：`0x11eee` 只做背景/tile redraw；descriptor base `0x53a6d` 是 boot `0x111ba(FDOTHER,#3)` 載入，`0x22547` 倒序走 entries 5→0。#81 local payload→call arg/frame decoder、indexed buffer／resource decoder／PNG or Ebiten adapter 仍缺，`unit_present` 暫維持 fail-closed。
