...
- [~] **shared object redraw compositor**：`0x127a9` 的 `0x127e0` 不是單純 loop bookkeeping：active roster entry 以 camera-relative placement 選 24×24 descriptor，走 `0x4deda` raw indexed-RLE 或 `0x4de56` palette-remap-RLE 寫 `0x53a49`；尾端 `0x129ec` 又在同 buffer 疊 map/object layer。**修正待辦**：`0x53a61` 不是未知 archive descriptor；`0x11019(group)` 建立每 group 12 個 pointer，索引為 `group×12 + pose×3 + cycle`，與既有 FDICON map-sprite evidence 一致。cycle 由全域 idle/moving counter 選出，unit+4 僅是 movement offset。仍待實作／驗證 roster-state、palette-remap selector 與 layer ordering；在此之前不得把 `0x22046` passes 或 `unit_present` 接成 native UI。

- [x] **FDICON indexed asset primitive**：`internal/fdicon` 現直接 decode `FDICON.B24` header/offset table/24×24 four-mode RLE，保留透明與 dither spans；`Sprite.BlitAt` 分離 raw (`0x4deda`) 與 256-byte LUT remap (`0x4de56`) 路徑。fixture 與 player-provided 原始 1680-sprite regression 通過；仍未替代 roster/frame/timing/layer adapter。
