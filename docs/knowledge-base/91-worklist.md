...
- [~] **SYNC-PARTY-RAW-IDENTITY-GATE**：更正 `syncPartyFromBattle` 註解與 SDD 斷言：native `0x11506` 以 persistent `+0x08` matching 後 copy full `0x50` bytes；目前 remake 仍以 normalized Fig/party membership projection，因 State 尚未攜帶 raw `+0x08`。不得把現行 sync 宣稱 byte-identical，待 raw identity plumbing 後再接。
...
- [~] **RE-EQUIPMENT-RECALC-1145A**：Docker Capstone 閉合 `0x1145a(persistentIndex)` raw arithmetic：signed base words `+0x37/+0x39/+0x3e`，掃 8 slots；flag byte bit `0x40` set 才以 item byte `+1` 查 `0x4e56c`，累加 effect words `+1/+5/+3/+7` 至 raw destinations `+0x48/+0x4a/+0x4c/+0x4e`。現有 normalized `campaign.RecomputeEquipment` 保持 projection-only；raw item-effect table/record adapter 尚待，不能把 normalized inventory 宣稱 byte-identical。
...
- [~] **RE-RAW-APPLICATION-22D1B**：新增 `battle.ApplyNativeRawApplication`／`ApplyNativeHPDamage`，保存 marker-zero/class `0x19/0x1a` gate、兩次 RNG、`0x1c81f` HP subtract、marker `(rng%4)+2` 與 `8*+0x21` accumulator；presentation/status 維持 fail-closed。
