...
- [~] **UI-VIS-TOWN**：`0x2cd16/0x2cf71/0x11eb0`已閉合3個FDOTHER背景variant、#10 label、FDTXT `0x1ef+selection`、FDICON `0,1,2,1` pulse、6組variant座標與312×192→VGA `(4,4)`；23個town保存raw `native_town_variant`。ch02 variant0／selection0 已取得原版 DOSBox [`pair`](../figures/town-hub-original-vs-remake.png)／[`diff`](../figures/town-hub-pixel-diff.png)，遮蔽角色與標籤 pulse 區後兩幀 RGB MD5 完全相同。尚缺同 tick pulse、variant1/2、selection1–5、左右鍵與secret chord E2。
...
- [x] **UI-08-TOWN-HUB-CH02-E2-SLICE**：在隔離 `/tmp` game sandbox 只以 route patch 跳過戰鬥勝負判定，原版 postbattle handler／campaign gate／town resources 均照常執行；走完20次戰後對話確認，第21次取得 ch02 variant0／selection0 的 [`DOSBox frame`](../figures/town-hub-original-dosbox.png)。與 production remake 無插值縮成320×200後，差異只在未對齊 tick 的角色與標籤 pulse；遮蔽兩區後 raw RGB frame MD5 同為 `d34b124775795f816209e53db603840a`。這不證明 route patch 是原版玩法，也不解除其他 variant/input 的 E2 gate。
