...
- [x] **ch26 success palette-ramp lowering**：Docker Capstone 定義 `0x25052(start,delay)` 為 inclusive `delta=start..0` 的 `0x11df2(0,255,delta)`＋每步 delay；compiler 已 lower immediate start 0..63（ch26 的 5/4/3/2、80ms）並有 descending/zero/invalid regression。這是 palette ramp，不是 generic fade；`0x24618` 與其他 renderer effects 仍 fail-closed。
