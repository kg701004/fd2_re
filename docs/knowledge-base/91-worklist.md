...
- [~] **RE-INVENTORY-COMPACTION-AUDIT**：Docker Capstone 重讀 `0x1b8e7`：它以 unit record `+0x0a` 為來源，依 caller-supplied slot 做 overlap-safe `0x373c4` byte move，最後寫 `record+0x18=0x80`；callers 的第三個 stack argument／source-destination slot 尚未在完整 CFG 中閉合。暫不接成「移除物品」或改寫 runtime inventory，避免破壞 class-change/shop/postbattle flow。
