...
- [~] **native `0x22253` renderer adapter**：已釘死 `0x22547→0x22046` indexed off-screen blit 呼叫鏈與 FDOTHER #81（nested `LLLLLL` entry；18710 bytes、directory first-word `0x12`）；`internal/fdother.ArchiveEntry` 已補 nested boundary regression，但 `0x11eee` data selection 與 indexed buffer／resource decoder／PNG or Ebiten adapter 仍缺，`unit_present` 暫維持 fail-closed。
