...
- [~] **ch15 postbattle raw layout audit**：Docker Capstone 固定 `0x23a0a→0x233c6` 的 slots `0..15`、special raw slot65=`(28,30,pose2)`、camera raw `(22,25)`，X=`[28,27,28,29,30,25,26,27,26,29,30,31,25,26,30,31]`、Y=`[28,27,27,27,27,28,28,28,27,28,28,28,29,29,29,29]`、constant pose0；acting resource49 只有 slot65 pose0/5 beats。handler 另掃 slots66..73，計數 inactive >4 才改 branch，這不是現有 `any_unit_inactive`（any）條件可替代；因此 ch15_post 仍 unbound fail-closed，未把 raw count predicate 猜成 roster/event flag。
