...
- [~] **native foreground-terrain occlusion layer**：Docker trace 已把 `0x129ec` 定為 unit-sprite 後的前景補畫，但修正「每個可見 unit」的簡化說法：它先跳過 `0x1f183(slot)` true 的 raw gate（group `0x1c` 放行；其他 group 的 class `0x13` 或 race `4/5` 跳過），再跳過 `0x3453e` inactive slot。`fdicon.NativeForegroundRedrawEligible` 以 regression 固定兩 gate，`NativeForegroundRedrawCells` 則保留 eligible slot 的精確 `(x,y)`、`(x,y-1)`、移動 pose-neighbour 順序，且故意不提前剪掉 off-map cell（native bounds gate 是 `0x12ac6` 的責任）。`0x12ac6` 僅在 resolved field-tile flag bit7 時寫 `0x53a49+0x8088+(y-cameraY)*24*456+(x-cameraX)*24`，bit8 加 `2*flip`，FDSHAP descriptor 正確取 offset-table `index+1`。`NativeForegroundFrameIndex` 已 regression。raw/alternate path 由 field entry `+3` 分支；`0x4dd52` 已閉合為 24×24 four-mode RLE 的 caller-LUT indexed blitter（非未知 effect）。`0x10937..0x1096f` 再關閉 source provenance：`0x111ba` 以 selected FDSHAP even/odd pair 填 `0x53a5d` image descriptors／`0x53a69` flags；`0x12ac6` 的 LUT selector 亦已閉合為同一 `0x51A97[0x53C1F]`→FDOTHER#3 table。這證實 native 角色會被 map foreground 遮擋；runtime roster bridge、complete layer order 與 GUI adapter 仍待，保持 fail-closed。
