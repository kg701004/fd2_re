...
- [~] **native foreground-terrain occlusion layer**：Docker trace 已把 `0x129ec` 定為 unit-sprite 後的前景補畫：每個可見 unit 走 `0x12ac6(x,y)`、`0x12ac6(x,y-1)`，移動中再按 pose 走一個鄰格；`0x12ac6` 僅在 resolved field-tile flag bit7 時寫 `0x53a49+0x8088+(y-cameraY)*24*456+(x-cameraX)*24`，raw/alternate path 由 field entry `+3` 分支。`0x4dd52` 已閉合為 24×24 four-mode RLE 的 caller-LUT indexed blitter（非未知 effect）。`0x10937..0x1096f` 再關閉 source provenance：`0x111ba` 以 selected FDSHAP even/odd pair 填 `0x53a5d` image descriptors／`0x53a69` flags，palette LUT 仍取 FDOTHER#3 `0x53a6d`。這證實 native 角色會被 map foreground 遮擋；exact descriptor offsets/palette-entry selection、完整 layer order 與 GUI adapter 仍待，保持 fail-closed。
...
- [x] **native terrain frame selector**：`0x11eee` 對 visible FDFIELD cell 取 10-bit tile ID、讀 FDSHAP terrain-control byte；priority 為 bit8→`+2*flip(0x53a40)`，否則 bit10→`+trunc(0x3c0b/2)`，否則 bit4→`+flip`，其餘 base tile，隨後才選 `0x4deda/0x4dcc6`。這是 raw animation ABI，不替 bit 命名；GUI frame scheduler 尚未接。
