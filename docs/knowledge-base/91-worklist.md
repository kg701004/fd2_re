...
- [~] `FD2.SAV` 存檔：高熵 representation 的 transform 尚未解；但 Docker static trace 已固定 `rb/wb FD2.SAV`、全檔 `0x59cb`、四槽 record `+0x312b+i*0x1450`、前 `0xa00`=persistent roster。不得再稱「強加密／無結構」；重製仍用自有格式，native compatibility 低優先。
