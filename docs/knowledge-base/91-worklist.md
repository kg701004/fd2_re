...
- [~] **② 複查 `+0x40`／figure displacement**：**`+0x40=當前HP`**（`0x18c98` 血條 `word[+0x40]×101/word[+0x42]`=HP%）與 `+0x44/+0x46=current/max MP` 已閉合，舊「戰場格 X」已清。`0x29f72` 已重判為 derived AP/DP/HIT/EV、HP、item、terrain/RNG 的 hit/crit/damage resolver，**不是** lunge；`+0x48/+0x4a` 亦是 derived AP/DP。figure displacement 必須另追 `0x2935b` caller／frame metadata，doc35 已同步。
