...
- [~] **shared object redraw compositor**：`0x127a9` 的 `0x127e0` 不是單純 loop bookkeeping：active roster entry 以 camera-relative placement 選 24×24 descriptor，走 `0x4deda` raw indexed-RLE 或 `0x4de56` palette-remap-RLE 寫 `0x53a49`；尾端 `0x129ec` 又在同 buffer 疊 map/object layer。**修正待辦**：`0x53a61` 不是未知 archive descriptor；`0x11019(group)` 建立每 group 12 個 pointer，索引為 `group×12 + pose×3 + frame`，與既有 FDICON map-sprite evidence 一致。仍待實作／驗證 roster-state、palette-remap selector 與 layer ordering；在此之前不得把 `0x22046` passes 或 `unit_present` 接成 native UI。
...
- [x] **FDICON native selector primitive**：`Bank.SpriteFor(group,pose,frame)` 嚴格表達 `0x127e0` 的 `group×12 + pose×3 + frame`（pose 0..3、frame 0..2）並 regression。`Unit.Fig`／`Dir` 不足以猜出 runtime +4 frame，故沒有把它隱式接入 GUI。
