...
- [~] **native commands28/29/31 derived-strike siblings**：同一 `0x276EC` 對玩家 ID28／29／31 分別選 20／12／18 倍率，並經 `0x2B659→0x1CA89(actor,id)` 與同一 final HP delta path；其 ordinary record geometry 可走 `NativeCommandEffectTargets`，`ExecuteNativeCommandDerivedStrike` 已接 strict state-only slice。ID30 也在 `0x2A6BD` 進 `0x276EC` default倍率18，但 `0x1CFF0→0x149F8` 的 globals／axis provenance 尚未關閉；其直譯在同 Y 時仍選 +Y step，與簡化水平 ray 不一致，故撤回未驗證的 ray executor、保持 fail-closed。32..35 走 `0x27FC9`。
  - 下一個 E2 trace：在 ID30 的 `0x1D339/0x149F8` 記錄 confirm 前後 `0x53AB1/0x53AB5`、callee 六個座標參數與每次 `0x12C0D` cell；至少覆蓋同列、同欄、斜向三種 confirm。只有輸出 target indices 與原版畫面一致，才可建立 ray primitive。
