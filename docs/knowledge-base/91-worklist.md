...
- [~] **shared object redraw compositor**：`0x127a9` 的 `0x127e0` 不是單純 loop bookkeeping：active roster entry 以 camera-relative placement 選 24×24 descriptor，走 `0x4deda` raw indexed-RLE 或 `0x4de56` palette-remap-RLE 寫 `0x53a49`；尾端 `0x129ec` 又在同 buffer 疊 map/object layer。待：`0x53a61` descriptor 格式、兩 RLE codec、layer ordering；在此之前不得把 `0x22046` passes 或 `unit_present` 接成 native UI。
