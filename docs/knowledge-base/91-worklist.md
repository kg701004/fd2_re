...
- [~] **0x24618 indexed transition metadata/buffer boundary**：已保存 tile/radial-radius/frame/timing 與 32-step 全 palette brightness ramp（delta 0→62, step2）之 editable schema、binding resolver/compiler regression；`fdother.BuildNativeIndexedTransitionSchedule` 固定 descriptor `9..1`、radius progression、5ms/500ms/4ms cadence，`CopyNativeTransitionBuffer`／`CopyNativeTransitionViewport` 亦固定 `0x24618` 的 full seed 與 `0x11eb0` `(320,456,312,192)` stride/geometry。descriptor decode、`0x22046` pass 執行、indexed double-buffer presentation 仍未接 runtime，故仍 fail-closed。
- [x] **RE-22046-INDEXED-PASS-SEQUENCE**：Docker Capstone 重讀 `0x22046`，新增 `fdother.ApplyIndexedTransitionPass` 保存第一 radial LUT→`0x127a9` middle redraw→第二 radial LUT→centered rectangle LUT 的不可省略順序；三段 geometry 先完整 preflight，缺 redraw/invalid second pass 不修改 buffer。descriptor/double-buffer source 與 Ebiten presentation 仍未接。
