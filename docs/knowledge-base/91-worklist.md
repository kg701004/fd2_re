...
- [~] **native selected-unit HUD (`0x1acf3`)**：它在 `0x11cac` 的 terrain/range/unit+foreground 後、viewport copy 前執行，且須 raw gates `0x51aab`、`0x51aac` 都非零。已釘住 `0x12c0d(cursor 0x53ab1/0x53ab5)` active-unit lookup、row157 anchor 和 `0x53abd>5 && 0x53ab9<3` 時 `+0xf2` correction、resource/raw blit/digit path；兩 global 的高階名稱與完整素材／數字 layout 尚待。**撤回**把現行 map HUD 的 FDOTHER#5 full-screen battle frame 當 native equivalent 的說法；目前只保留可玩 approximation。
