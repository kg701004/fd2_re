...
- [~] **0x24618 indexed transition metadata**：已保存 tile/radial-radius/frame/timing 與 32-step 全 palette brightness ramp（delta 0→62, step2）之 editable schema、binding resolver/compiler regression；新增 `fdother.BuildNativeIndexedTransitionSchedule` 固定 descriptor `9..1`、radius progression、5ms/500ms/4ms cadence。descriptor/double-buffer PNG adapter 尚未完成，故仍 fail-closed。
