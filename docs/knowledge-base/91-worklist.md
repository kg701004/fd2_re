...
- [x] **本輪稽核基線**：24 個標準戰後節點為 19 active／5 blocked；
  ...
  `0x24df2` 的兩個 `0x112a5` immediate 仍未知，不能把它們命名成 JOIN。
...
- [~] **raw ch24 post evidence（玩家第25戰候選；owner 勘誤）**：固定版 FD2.EXE 的 IDA Pro 9.4／Docker Capstone 已共同證實 table index24 的 raw entry `0x24df2`、FDTXT_025 index6/7、PAN raw `(4,16)`→`(96,384)`、raw `0x10b4e(2)`、ACT resource75 與共享 `0x112a5(0x1a)` append 呼叫；Docker acting exporter 解碼為4 beats、slot70/pose2，map24 raw frontier=70。後續 IDA 追查確認 `0x10b4e` 逐筆比對 FDFIELD row `+0x15` 並呼叫 `0x10c50` 建立 runtime record；map24 resource073 的 70 筆列中 group2 恰有1筆，因此 binding 的 `spawn_groups["2"] = 1` 已有可重生的列數證據。這只閉合 materializer cardinality，不把 `0x1a`、`0x1d`、`0x0e` 命名成 JOIN／章節；table index不直接等於玩家戰次，先前誤接 `postbattle_ch24_persist→town_ch25` 已撤回。`postbattle_ch24_persist` 仍 blocked，完整位址與雜湊見 [`fd2_ch24_post_ida.txt`](../data/ida/fd2_ch24_post_ida.txt)。
- [x] **ch24 candidate binding withdrawal**：`0x112a5(0x1a)` 與共享尾段 `0x112a5(0x0e)` 只保存為 raw append，沒有已證實的永久角色身分；因此 `postbattle_ch25_persist→town_ch26` 也維持 blocked，`ch24_post.json` 的兩個未知操作由編譯器失敗即關閉。
