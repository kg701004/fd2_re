...
- [~] **raw ch24 post evidence（玩家第25戰候選；owner 勘誤）**：固定版 FD2.EXE 的 IDA Pro 9.4／Docker Capstone 已共同證實 table index24 的 raw entry `0x24df2`、FDTXT_025 index6/7、PAN raw `(4,16)`→`(96,384)`、raw `0x10b4e(2)`、ACT resource75 與共享 `0x112a5(0x1a)` append 呼叫；Docker acting exporter 解碼為4 beats、slot70/pose2，map24 raw frontier=70。table index不直接等於玩家戰次；map／文字／演出交叉證據支持它是玩家第25戰候選，先前誤接 `postbattle_ch24_persist→town_ch25` 已撤回。`postbattle_ch24_persist` 仍 blocked，`0x1a`、`0x1d` 與 `0x10b4e(2)` 的高階語意維持未知，故不建立正式接線；完整位址與呼叫順序見 [`fd2_ch24_post_ida.txt`](../data/ida/fd2_ch24_post_ida.txt)。
...
- [~] **chapter ending renderer (`0x2bce5`)**：已釘死 FDOTHER `#0x36`（十進位54）、320×200 雙 buffer、palette 0→63/4ms、2000ms hold、chapter26/29 分支文字與 fade-out；前綴、`0x2c548` party-cycle 與 `0x2c194` raw tail 現已有原資源 executor／schedule，仍不可把未閉合的完整 indexed owner／輸入事件／campaign handoff 吞成 generic ending。
...
- [x] **RE-UNIT-PRESENT-FIVE-ARG-ABI**：五個direct callers固定
  ...
  source/destination。
- [~] **CH29-POST-FLOW-WIRING 勘誤**：撤回 `postbattle_ch29_persist→ch29_post` 的錯接；第29戰依零起算 dispatch 應使用 raw `ch28_post`，未獲 activation gate 前保持未綁定並停在 `preparation_ch30` 前。IDA 新增固定 raw handler table：index26→`0x250cc`、index29→`0x25757`；後者 `0x25970→0x2bce5→0x25975` 是 self-loop，前者依 `0x24b14(0x64)` 分成 success exit 與 ending self-loop。這只校正 caller／owner 邊界，不把 table index 命名成玩家戰次；raw `ch29_post` 的 LOADCH/persistent-roster 與 `0x2bce5` 證據保留，正確 owner 必須和一般玩家 `[0x53c03]` trace 及完整終局流程另行閉合。證據見 [`fd2_ch29_terminal_callers_ida.txt`](../data/ida/fd2_ch29_terminal_callers_ida.txt)。
