...
- [~] **UI-SHOP-RECIPIENT-INPUT-E2**：原版已實測裝備收件者由selection0按Down到selection1、再按Up回selection0，Left/Right不改selection；production input目前同為bounded Up/Down、Left/Right no-op，三列window由`NativeThreeRowWindow`計算，但尚缺直接input-level regression。selection1的高頻原版／remake比較只差商店人物區`(175,90)`、`(176,90)`兩個像素（6個RGB bytes），recipient panel、黃色選列、數值與三列window其餘像素一致；selection0不同原版採樣亦出現同一環境相位差，故不得遮罩或宣稱AE=0。DOSBox runner新增已容器實測的`waitpixel:x,y,r,g,b,delay,max`，下輪用它同步原版環境相位後再決定是否閉合selection1／scroll E2。
