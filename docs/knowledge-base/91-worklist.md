...
- [x] **RE-CHURCH-RAW-SERVICE-LISTS-2E6B8**：官方 IDA 9.4 閉合 `0x2e6b8/0x2df6b` 的 bounded two-column selector：左右 ±1、上下 ±2、六項可視窗以兩項為步長滾動、Enter/Space 回 `1`、Esc 回 `-1`。`0x2f8ea` 會掃 selected runtime record 的八格 inventory，保留 signed flag 非負的 raw cells（`0x40` equipped 與 `0x00` ordinary 都保留，只有 `0x80` reserved 排除），再進第二層 list/confirm、`0x2f4c6` feedback、`0x2d516` amount 與 remove/recompute topology；`0x2ffa5` 經 caller-owned list 與唯讀 `0x17aed` presentation。raw index0 已完成 indexed runtime；raw index1 transfer core已接，但第二層 native list/feedback compositor仍 fail-closed。
