...
- [~] **UI-VIS-SHOP**：四個callee與secret selection+BIOS-scan gate皆已接production；ch02 variant5四個service、Right/Left wrap及Escape closing→town selection5均已達原版／production整幀AE=0，並修正service pulse double-`/2`與production返回selection0錯誤。下一gate是variant1/3 DOSBox同狀態diff。
