...
- [~] **native commands32..35 `0x27FC9` compound family**：ID32 `→0x2111A→0x1C75E` numeric、ID33 先清 final target `+0x25..+0x27` 再以固定 `0x320` 走 `0x211A4→0x1C916` restore、ID34 依序 `0x22721/0x22866/0x22997`、ID35 依序用 IDs26/22/27 呼 `0x22D1B` 寫 `+0x25/+0x27/+0x26`。撤回「`0x27FC9`／helpers 未見 `0x1CA89`」：它在 `0x28189` 進 `0x2B659`，後者以 FIGANI container header `byte+4==1` 為條件呼 `0x1CA89(actor,commandID)`。`figani.Animation.HeaderByte4` 現保留此 raw input，但尚未實證 32..35 各自可達 unit/header，故 selector `record+5` MP gate 仍不能推出必扣或免費，也不能沿用 `SpendNativeCommandMP`；transaction dataflow、multi-effect ordering/rollback、UI/SFX 待 runtime／額外 evidence，engine 保持 fail-closed。
