...
- [~] **UI-VIS-TOWN**：`0x2cd16/0x2cf71/0x11eb0`已閉合3個FDOTHER背景variant、#10 label、FDTXT `0x1ef+selection`、FDICON `0,1,2,1` pulse、6組variant座標與312×192→VGA `(4,4)`；23個town保存raw `native_town_variant`。ch02 variant0 [`selection0–5`](../figures/town-hub-six-selections-original-vs-remake.png) 都達原版／remake raw RGB 整幀相同，Left/Right wrap、Shift+F1 hidden reveal及Enter進variant5亦有input E2；共用 glyph shadow 與誤 reset pulse 已修。尚缺variant1/2與secret shop返回town E2。
- [~] **UI-VIS-SHOP**：四個callee與secret selection+BIOS-scan gate皆已接production；ch02 variant5 service0 phase0/2已達原版／production整幀AE=0，並修正service pulse double-`/2`。下一gate是service1–3 input、Escape closing→town restore及variant1/3 DOSBox同狀態diff。
