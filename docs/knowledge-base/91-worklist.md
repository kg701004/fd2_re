...
- [x] **POSTBATTLE-BINDING-GATE-AUDIT**：新增唯讀 `tools/audit_postbattle_binding_gates.py`，逐一依 handler source address 檢查 generated binding 的 `loadch/pan/dialog/act/layout` 覆蓋；目前 22 節點仍 blocked、2 節點 mapping-complete 但僅列 review candidate，禁止把 skeleton 當 active handler。
