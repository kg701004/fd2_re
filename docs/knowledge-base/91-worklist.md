...
- [~] **scenario native command-mask bridge**：`NativeCommandMask` ABI／grid renderer 都已存在，但 scenario party override 尚無 editable raw mask source，故一般 chapter party 的 grid 必須安全退回 legacy Spells；不可由 normalized spell list 反造 raw bytes。待 exporter／scenario schema carry FDFIELD `initial_command_mask`，再以 per-chapter regression 對照 native command IDs。
