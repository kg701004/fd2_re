...
- [x] **ch15 candidate editable CFG**：新增 `handlers/candidates/ch15_post_cfg.json`，保留 raw source addresses 與 nested OR/else branch；候選 binding 仍由 compiler 明確拒絕 active-slot acting，直到 branch context model 完成，不把候選檔誤列為 60 個 exported production handlers。
