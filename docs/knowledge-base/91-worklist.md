...
- [~] **UI-VIS-TOWN**：`0x2cd16/0x2cf71/0x11eb0`已閉合3個FDOTHER背景variant、#10 label、FDTXT `0x1ef+selection`、FDICON `0,1,2,1` pulse、6組variant座標與312×192→VGA `(4,4)`；23個town保存raw `native_town_variant`。ch02 variant0 的 [`selection0/pulse2`](../figures/town-hub-original-vs-remake.png) 與 [`Left→selection1/pulse2`](../figures/town-hub-selection1-original-vs-remake.png) 均達原版／remake raw RGB 整幀相同；共用 glyph shadow 與方向鍵誤 reset pulse 已修正。尚缺variant1/2、selection2–5、Right/wrap與secret chord E2。
