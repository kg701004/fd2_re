...
- [x] **`0x122dc` range call-table closure**：Docker Capstone 完整直讀 modes 1..5，新增 `fdother.NativeRangeOverlayPlacements` 保留原始 call order 的 1/1/5/13/21 個 `(x,y,descriptor)`；特別固定 mode3 centre=`#14`、mode5 的重複座標／不同 descriptor，禁止圖形化 normalize。mode6 不呼 `0x126f7`，而是算 `4*(x+y*raw53ac1)+7` 後清 `[0x53a51]` 指向資料的一個 byte；因此 drawable API 明確拒絕 mode6，僅保留 checked address arithmetic。仍待 `0x53a4d` descriptor bank、`0x4deda` resource binding、camera clip 與 indexed renderer adapter。
