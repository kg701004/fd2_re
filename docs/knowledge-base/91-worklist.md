...
- [~] **native foreground-terrain occlusion layer**：Docker trace 已把 `0x129ec` 定為 unit-sprite 後的前景補畫：每個可見 unit 走 `0x12ac6(x,y)`、`0x12ac6(x,y-1)`，移動中再按 pose 走一個鄰格；`0x12ac6` 僅在 resolved field-tile flag bit7 時寫 `0x53a49+0x8088+(y-cameraY)*24*456+(x-cameraX)*24`，raw/alternate path 由 field entry `+3` 分支。`0x4dd52` 已閉合為 24×24 four-mode RLE 的 caller-LUT indexed blitter（非未知 effect）；這關閉「0x129ec 是無關 maintenance」斷言，並證實 native 角色會被 map foreground 遮擋。tile descriptor/source/palette table selection、完整 layer order 與 GUI adapter 仍待，保持 fail-closed。
