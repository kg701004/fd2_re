...
- [~] **ch15 postbattle raw layout audit**：Docker Capstone 與合法 IDA Pro 9.4 固定 `0x23a0a→0x233c6` 的 slots `0..15`、special raw slot65=`(28,30,pose2)`、camera raw `(22,25)`，X=`[28,27,28,29,30,25,26,27,26,29,30,31,25,26,30,31]`、Y=`[28,27,27,27,27,28,28,28,27,28,28,28,29,29,29,29]`、constant pose0；acting resource49 只有 slot65 pose0/5 beats。IDA 另證實 `sub_3453E(index)` 讀 `runtime[index*0x50+5]&1`，故 handler 的 `0x42..0x49` 確為 slots66..73。仍缺一般玩家原版 runtime capture，因此 ch15_post 維持 unbound fail-closed。
...
- [~] 追 `[0x53a45]` producer、runtime slot 對應及 save/persistent boundary：constructor raw writer與 `sub_3453E` 的 `index*0x50+5` consumer 已閉合；74／78 只是假說。map14 group0 已有 `fig=15` ally，且準備介面最多選15名；須先證明原版如何表示已JOIN但未部署者、是否提升既有map record，再處理 save boundary。
...
- [x] **ch15 candidate editable CFG**：新增 `handlers/candidates/ch15_post_cfg.json`，保留 raw source addresses 與 nested OR/else branch。2026-08-02 IDA 重核修正 JOIN18 錯置：`0x23b1f` 跳到章節尾端，JOIN18 只屬於 `else word42>=0x140` arm。candidate binding 將74／78保存為待驗假說；production scenario 未接此建構，候選也不列入production handlers或campaign node。
- [ ] 執行 ch15 四條 branch trace，補 JOIN18 當下的 typed persistent record、`battle_ch15→postbattle→town_ch16` 與 save regression；在一般玩家原版 runtime capture 前保持 unbound fail-closed。
