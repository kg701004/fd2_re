...
155 - E - RE-BATTLE-AI-SPECIAL-TOPIC自述下一步需固定存檔trace驗證實際選中command/畫面順序，需DOSBox。 **2026-09-07 本輪未完成，附明確阻塞點**：已用一般玩家路徑進入 ch27 戰場並開出單位指令環，但要觀察敵方 AI 實際選中的 command，必須先結束我方回合；本輪未能定位「結束回合」控制（空地上 confirm 只顯示地形加成面板不開環，cancel 開的是該單位法術列表，tab 開的是單位指令環），而逐一讓 13 名單位「待機」的操作量在本輪不可行。下一輪應先靜態定位回合結束的輸入處理，再回來實測。
