...
**為何不把 ch03 判為「原版就是不會觸發」**：因為那個檢查可能是瞎的，本輪直接去驗「驗證有沒有跑到」——送出 DOSBox 主機熱鍵 **Ctrl+F5（存截圖）→ 工作目錄沒有任何新檔案**、**Ctrl+F9（終止模擬器）→ 模擬器存活**（`window` 與 `xtest` 兩種模式皆然）、**Alt+Pause（開 debugger）→ 連續兩次都沒開**。也就是說**主機端 mapper 收不到任何組合鍵**，與 `tools/dosbox_harness.sh` 自己在 2026-09-03 的註解自述完全一致（該註解也記載：當時已拿 ch06＝Ctrl+F5 gate 試過 `xtest` 模式，同樣沒有差別）。
