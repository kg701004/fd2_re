...
- [x] **FDSHAP transparency asset bridge**：four-mode decoder 現保留 opacity mask，`export_engine_assets.py` 以它輸出 RGBA tileset；實測 map0 sheet alpha 值域 `(0,255)`。透明完全依 mode3，不把 palette index0 猜成透明，因此 Ebiten 可保留 native tile asset；`0x129ec` 的前景重畫時機／alternate LUT selection 尚不自動接線。
