...
- [~] **REMAKE-FIELD-EVENT61-SELECTOR1**：event61 handler 已解碼並資料化；selector1 位於 AI `0x13A9F` 共用 mode 分派收尾及玩家 `0x18890→0x18D8C` 的三個完成操作成功臂，不可簡化成任意 walk completion。已更正 FDTXT_026 的未加引號可見訊息計數，63 句與 `ch26.json` 全量對齊；presentation 具型別保存 archive/resource/frame/base/stride/transparent/delay。`ch26.json` 改用 runtime append 且 initial 僅 group0。battle plan/commit 與 UI job 已在明示 materialized runtime bridge 完成 FDTXT2／3／4、59 幀兩 tick cadence、native compact remove D0、entry12、append group1 與 persistent JOIN31，缺 D0 不突變。正式路徑目前只從 wait 成功收尾進入；更大的 blocker 是 `battle_ch26` 尚無已證實的 dynamic `native_map_view/native_map_hud`，production 因此在 presentation source frame 失敗即關閉。尚需原版 runtime capture／caller state producer，及 attack/magic/item/AI 的其餘 selector1 成功臂；未達 E2。
