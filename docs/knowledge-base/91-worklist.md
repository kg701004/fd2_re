...
- [x] **RE-CHURCH-INVENTORY-TRANSFER-1BB8C**：官方 IDA 9.4 閉合 `0x1bb8c(destination,item)`：掃八格、第一個 flag byte `<0` 的 cell 寫 flag `0` 與 item byte，成功回 `1`，滿格回 `-1`；配合 `0x2f8ea` 的 `0x1b8e7(source)` 前置，raw topology 是來源→目的角色的物品轉移，目的格為未裝備。新增 atomic `battle.TransferNativeInventoryItem` 與 full-destination regression；未將 raw index 1 命名成教會服務或接入未證實 renderer。
