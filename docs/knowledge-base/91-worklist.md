...
- [~] **native command ID13 healing route**：IDA `0x21AD9→0x21B18` 已閉合 generic final target array、`0x1CA89(actor,13)` MP debit、`0x1C8ED→0x1C916` per-target HP restore 及 `+0x42` cap；其 amount 公式同 `9/10 + rand%100/1000`。專用 indexed animation、UI、SFX 和 engine resolver 尚未接入，禁止誤用 IDs0..9 damage executor。
