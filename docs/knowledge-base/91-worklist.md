...
- [~] **native command target flag exporter**：`0x14818→0x4e040` 的 raw target resolver 已有純資料層（camp predicates、cross、cardinal flood-fill，bit40 block／bit80 zero-cost）；但 current map `cost` 和 terrain-control byte0 都不是 runtime grid `+1` flags。須先追 composition→grid constructor writer，再輸出逐章 flags JSON；在此之前 native target UI 保持 fail-closed。
