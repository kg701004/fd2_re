...
- [~] 敵方 AI command-score bridge：`0x1C269→0x1598A→0x15B77` 直接使用 set-bit 索引，沒有 `command-0x10`；`NativeAvailableAIScoredCommandIDs` 與 `AIPlan.NativeScoredCommands` 已改為保留已知 `0..35`，不再錯誤排除 `0..15`。候選幾何與各命令群組分數已接；尚缺單位級最大值／平手狀態及動態原版 trace，因此仍不接正式 `NextAIPlan`。
