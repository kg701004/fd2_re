...
155 - E - RE-BATTLE-AI-SPECIAL-TOPIC自述下一步需固定存檔trace驗證實際選中command/畫面順序，需DOSBox。 **2026-09-07 本輪未完成，附明確阻塞點**：已用一般玩家路徑進入 ch27 戰場並開出單位指令環，但要觀察敵方 AI 實際選中的 command，必須先結束我方回合；本輪未能定位「結束回合」控制（空地上 confirm 只顯示地形加成面板不開環，cancel 開的是該單位法術列表，tab 開的是單位指令環），而逐一讓 13 名單位「待機」的操作量在本輪不可行。**同日稍後已解除此阻塞（純查既有文件，未做新 RE）**：doc11「`0x16F55`：手動強制結束回合」一節早已完整反組譯該入口——`0x117E7` 在**游標下沒有可行動己方單位**時呼叫 `0x16F55()`，那是一個 `[0x53C57]` 0..3 的戰場系統環（0=存檔、1=全軍前進、2=子選單、**3=結束回合**），選 3 會出 FDTXT `0x1A3`「要結束本回合的行動嗎？」，確認後 `0x1A4`「好的，就結束本回合的行動吧！」，接著直接進回合 orchestrator `0x1A30B`；另 doc12 續六記有實測過的驅動序列（取消×4→上→確認→下→確認×2→確認×6）。下一輪照此配方即可直達敵方 AI 回合，**不需要新的靜態工作**。（本輪的方法教訓：我在戰場上只按了一次 cancel 就認定找不到入口，應先 grep 既有文件——與 [[feedback_check_existing_evidence_before_disasm]] 同一類失誤。）
...
305 - E - 敵方AI雙預選bridge自述尚缺同一原版runtime動態trace，需DOSBox。 **2026-09-07 本輪未完成**：與 item 155 同一阻塞點——已進入 ch27 戰場，但敵方 AI 回合需先結束我方回合，本輪未定位該控制。見 item 155 的說明。
