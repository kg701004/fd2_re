...
- [~] **REMAKE-FIELD-EVENT61-SELECTOR1**：event61 handler 已解碼並資料化；selector1 位於 `0x13A9F` 共用 unit-action收尾及 `0x18890` 玩家操作多個成功臂，不可簡化成任意 walk completion。已更正 FDTXT_026 的未加引號可見訊息計數，63 句與 `ch26.json` 全量對齊，text2/3/4 精確指向 scene0 line10、line11 與 scene1 lines0–9；presentation 也已具型別保存 archive/resource/frame/base/stride/transparent/delay。`ch26.json` 改用 runtime append 且 initial 僅 group0，渥德 group1 不再提前進場。尚缺 selector1 成功收尾、59 幀畫面擁有權、原始 inventory 原子移除、entry12、append group1 與永久 JOIN31 的整合；完成前不得宣稱玩家路徑完成。
