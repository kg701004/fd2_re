...
- [~] 戰後 town/整備流程：campaign_full 的 postbattle→town、連戰 preparation 路線與 shop/rumor return 已盤點；`0x318ad` RE 已鎖定 30-byte 勾選表、一般 cap15／late cap19，remake 已接 `party_limit`、`partyDeploy`、save persistence 與可操作 preparation UI，永久 JOIN roster 不被改寫。church `0x3072f` 已證實四個 raw index→address dispatch（不是四個已命名服務）；`0x2d7bd` 只接受左右鍵並在四項循環。revive fee table、原子 `ReviveUnit`、church selector 與 class-change candidate/branch mutation 已接；尚待完整 GUI 轉職實機回歸與原版數值對照（無免費一般治療）。
...
- [~] ch14/ch15 handler：Docker Capstone 已證實 pre `0x334f3..0x334f7` 的 `roster_has(12)`→FDTXT_015「有 12：0/1/2；無：3/4/5」，以及 post `0x239d1..0x239d3` 的「有：12；無：13」。`postbattle_ch14_persist` 現已接回對應 `ch14_post` binding（JOIN15→set_chapter15→town_ch15）；`postbattle_ch15_persist` 撤回錯誤重用 `ch14_post` 的 binding，因 `ch15_post` 的 layout/acting/文字尚未達 gate，暫維持 unbound fail-closed。pre binding 含 map14/80-slot、pan、acting48；runtime 只讀 permanent party roster，缺此資料 fail-closed。
- [x] **ch14 postbattle campaign binding**：修正 `campaign_full.json` 原先錯誤的 synthetic `sync_party→set_chapter(14)`；`postbattle_ch14_persist` 現直接引用已驗證的 `ch14_post` binding（條件對話、sync、JOIN15、set_chapter15），並保留 `town_ch15` 戰間節點。generated compiler completion frontier 與 campaign regression 已更新。
- [~] **ch15 postbattle raw layout audit**：Docker Capstone 固定 `0x23a0a→0x233c6` 的 slots `0..15`、special raw slot65=`(28,30,pose2)`、camera raw `(22,25)`，X=`[28,27,28,29,30,25,26,27,26,29,30,31,25,26,30,31]`、Y=`[28,27,27,27,27,28,28,28,27,28,28,28,29,29,29,29]`、constant pose0；acting resource49 只有 slot65 pose0/5 beats。handler 另掃 slots66..73，inactive count `>4`、raw global `[0x53bef] >18`、record word `+0x42 >=0x140` 共同影響 branch，這不是現有 `any_unit_inactive`（any）條件可替代；因此 ch15_post 仍 unbound fail-closed，未把 raw predicates 猜成 roster/event flag。
...
- [x] **ch15 +0x42 constructor producer/export**：Docker Capstone 固定 constructor `0x10fe9` 將 caller-supplied 同值寫入 `+0x40/+0x42`，`+0x44/+0x46` 使用另一輸入；新增 raw-table fixture、`native_record_word42` constructor projection 與 33 張 editable map asset 的 provenance-only merge。normalized `hp` 不被覆寫；malformed／未覆蓋 selector 維持缺值與 fail-closed。
...
- [x] **ch15 candidate editable CFG**：新增 `handlers/candidates/ch15_post_cfg.json`，保留 raw source addresses 與 nested OR/else branch；固定 `runtime_context.slot_count=80` 後 compiler 可驗證 branch 內 acting，無固定 context 的 branch 仍 fail-closed；候選檔不列入 60 個 exported production handlers，也尚未接 campaign node。
...
- [x] ch19 post handler closure：Docker Capstone 釘住 handler 先寫 slots0..15、52..60，再以 `0x10b4e(group1)` 掃 FDFIELD raw group byte；map18 group1 確實一筆，且後續 resource60 使用 slot83，故入口 frontier83→spawn+1→84。resource59/60/61/62 已逐幀解碼並接 authored binding；runtime context 保留 exact slot_count83、spawn_groups[1]=1，不能改成 normalized map group 推論。
...
- [~] **主選單機制**:輸入迴圈/scancode dispatch(↑0x48/↓0x50/Enter/Space)/游標 wrap、return `0=新遊戲`、`1=0x30550` slot selector 已由 Docker Capstone 重跑；第三 return branch 直進 `0x10010`、持久資料來源未閉合。remake 現有四槽自有 JSON selector 是 fallback，不能稱原版 LOAD/CONTINUE 已還原 → `23`、`57`
...
**目前修法狀態**:9 個節點有 direct script、45 個由 handler binding 供應過場資料；另外 30 個 retreat、23 個 rumor 是刻意的短 authored node，10 個 unbound postbattle 與 4 個 generic story fallback 才是待 mapping/handler 的缺口。24 個 postbattle 有 generated binding skeleton（其中 14 個已由 authored binding 啟用），未經 override/compile gate 仍不算 active handler，不能把同一章 script 盲目灌入其餘節點。
下一步要依原版 handler／FDTXT scene label 逐節建立 mapping，完成後才可宣稱接通。
- [x] **story-script coverage audit tool**：`tools/audit_story_script_coverage.py` 以唯讀方式列出 story/cutscene、coverage role、script、scene、handler、generated skeleton 與 next，固定目前 121/9/45/30/23/10/4 + 24 skeleton（其中 10 個 unbound）分類；不依章號猜測映射。
...
- [x] **ch11 postbattle binding**：Docker Capstone 證實三個 14-byte arrays、slots0–13，special slot2 最終覆寫 `(10,4,pose0)`，camera raw `(14,0)`→`(336,0)`；map11 60-slot frontier。Docker acting exporter 解碼 resource45 的 slot8 special frames（0 與 6 beats），FDTXT_012 index3/4 對 ch12 scene3 lines0–2、scene3 lines3–9；新增 authored binding、compiler regression，並接 `postbattle_ch11_persist→town_ch12`，未宣稱 renderer parity。
- [x] **ch13 postbattle binding**：Docker Capstone 證實三個 16-byte arrays、slots0–15，special slot0 最終覆寫 `(0,0,pose0)`，camera raw `(12,10)`→`(288,240)`；map13 70-slot frontier、group1 僅一筆。Docker acting exporter 解碼 resource47 為 4 beats、slot67/pose2，FDTXT_014 index2/3 對 ch14 scene0 lines8–17、scene1 lines0–6；新增 authored binding、compiler regression，並接 `postbattle_ch13_persist→town_ch14`，未宣稱 renderer parity。
- [x] **ch24 postbattle binding**：Docker Capstone 證實 handler `0x24df2` 的 FDTXT_025 index6/7、PAN raw `(4,16)`→`(96,384)`、spawn group2=1、ACT resource75；Docker acting exporter 解碼為 4 beats、slot70/pose2，map24 raw frontier=70。FDTXT_025 index6/7 對應 scene2 lines0–17；新增 authored binding/compiler regression，並接 `postbattle_ch24_persist→town_ch25`，未宣稱 renderer parity。
- [x] **ch25 postbattle binding**：Docker Capstone 證實 `0x24e80` 的 `0x233c6` caller 以 16 slots、camera raw `(9,5)`→`(216,120)` 寫入 map25；Docker acting exporter 解碼 resource77(slot1/pose2)、78(slot2 pose2→1→special pose0)、79(slot0/special pose2)、80(slot0 pose3→2→slot2 pose2)。FDTXT_026 string5–11 已由 raw glyph/control stream 對到 ch26 scene2 lines0–14、scene3 lines0–17、scene4 lines0–7；新增 `dialogue_overrides` 的 address+text-index branch schema，避免把整體 61/63 utterance mismatch 偽造為 count-aligned，compiler/audit regression 通過並接 `postbattle_ch25_persist→town_ch26`，未宣稱 renderer parity。
- [~] **ch06 post conditional frontier**：Docker Capstone 固定 `[0x53ad5]+0x11==1` 後才檢查 `unit_inactive(43)`；inactive 走 FDTXT_007 index5，active 走 9-slot layout（X/Y/pose arrays、special slot43=`(12,7,pose2)`、camera raw `(6,2)`）→index4→JOIN12。map6 editable roster 只有 40 units，但 native caller 仍讀 96-slot buffer 的 slot43；未建立 explicit empty-record frontier 前禁止扁平化或啟用 `postbattle_ch06_persist`。
...
- [x] **post-resolution inventory occupied-prefix ABI**：Docker Capstone 固定 `0x1b8a6(unit)` 掃 `record+0x0a+2*i` 八個 cell；bit7 clear 才計入 occupied/prefix count，bit7 set 是 `0x1bb8c` 使用的 reserved 空格。更正並刪除錯誤的 free-slot 斷言，新增 `battle.NativeInventoryOccupiedCount` 與 item-byte-is-opaque regression，避免掉落流程以 normalized `len(Inventory)` 取代 raw cell flags。
...
- [~] **ch29 post dynamic palette loop**：`0x11df2(EBX,255,0)` 已依 direct 0x3e→0 loop materialize 成 63 組 palette/delay beats 並通過 regression；`0x1088d` 的舊文字-only 說法已由完整 `loadch` 取代。仍待的只有已列出的 `0x24618` indexed adapter 與專用 ending renderer。
...
- [~] **0x24618 indexed transition metadata/buffer/LUT/compositor boundary**：已保存 tile/radial-radius/frame/timing 與 32-step 全 palette brightness ramp（delta 0→62, step2）之 editable schema、binding resolver/compiler regression；`indexedmap.BuildNativeTerrainCells` 現嚴格接 exporter 的 raw tile/high-byte arrays，`loadNativeMapAssets` 亦要求 FDOTHER#3 LUT entries 1..9 才建立 all-or-nothing native bundle。`fdother.BuildNativeIndexedTransitionSchedule` 固定 LUT index `9..1`、radius progression、5ms/500ms/4ms cadence，`CopyNativeTransitionBuffer`／`CopyNativeTransitionViewport` 固定 `0x24618` full seed 與 `0x11eb0` `(320,456,312,192)` geometry，`NativeIndexedTransitionLUT` 固定 bank 的 raw LUT index 9..1，`BuildNativeIndexedTransitionPass` 固定 `0x22046` 的 scale16、第二 radial 起始 row 與 final-rectangle 的 raw `a2` alias；`indexedmap.ComposeNativeTransitionFrame` 已把 terrain→unit/foreground→pass→viewport 以 atomic indexed primitive 組合。campaign runtime asset binding、palette timing 與 Ebiten scene presentation 仍未接，故 handler 仍 fail-closed。
- [x] **RE-22046-INDEXED-PASS-SEQUENCE**：Docker Capstone 重讀 `0x22046`，新增 `fdother.ApplyIndexedTransitionPass` 保存第一 radial LUT→`0x127a9` middle redraw→第二 radial LUT→centered rectangle LUT 的不可省略順序；三段 geometry 先完整 preflight，缺 redraw/invalid second pass 不修改 buffer。LUT bank source、double-buffer presentation 與 Ebiten presentation 仍未接。
- [x] **ch29 final 0x24618 arguments**：依 layout→focus 的 native scroll-offset writes，`0x25848` dynamic args 已定案為 tile `(6,6)`、radial radius `(10,step8)`，並寫入 binding/compiler regression；真正 indexed LUT/compositor/double-buffer adapter 尚待，整支 handler 繼續 fail-closed。
...
- [x] **`[0x51a83]` full-domain correction**：合法 IDA 9.4 完整 data xrefs 已保存於 [`fd2_51a83_xrefs.txt`](../data/ida/fd2_51a83_xrefs.txt)。撤回把它限制為`0..5`或稱「戰鬥訊息索引」：`0x15140/0x153b1/0x1bd14/0x1d188` 都是 zero-extended record byte `+2`；原 spell table 的 range 5/7/9 會產生7/9/11。`0x122dc`對>6不畫圖，但`0x115b6`仍以selector-1進`0x14742` target legality。battle raw state現保留writer可達的`0..0x101`；campaign JSON只允許已證實bootstrap 0，避免靜態資料冒充互動writer。
...
- [~] **native terrain/unit map HUD (`0x1acf3`)**：它在 `0x11cac` 的 terrain/range/unit+foreground 後、viewport copy 前執行，且須 raw gates `0x51aab`、`0x51aac` 都非零。先以 `0x12e38(cursor)` 解 FDFIELD tile10/event-low5/FDSHAP control4，再以 `0x12c0d(cursor)` optional active-unit lookup；`NativeTerrainCursorInfoForCell` regression 固定前一 raw ABI。已釘住 control byte+1→`0x51a12/0x51a2a` 的 AP/DP 表，並由 `battle.Load` 以逐格 raw tile/control 接至戰鬥計算。layout 已收斂為 LMI1 #130（69×34）`buffer+stride*157+x`、terrain `+6`、AP `stride*8+0x2b`、DP `stride*19+0x2b`；`0x1aeb1` 依 table 值正負選 raw directory decimal #83/#84、取絕對值再走 native digits，兩 resource 的視覺語意仍不命名。`x` raw 初值1，已見條件改為0xf2或1。unit icon/HP resource、高階 global 名稱仍待。**撤回**把現行 map HUD 的 FDOTHER#5 full-screen battle frame 當 native equivalent 的說法；目前只保留可玩 approximation。
  ...
  - [x] chapter-1 editable vertical slice：`battle_ch01` campaign節點保存
    ...
    HUD `(gateA=1,gateB=1,anchor=1)`；loader要求view/HUD成對且raw
...
- [~] **UI-12 LOAD selector contract**：原版 DOSBox screenshot 與 Docker Capstone `0x30550` 已固定四槽、slots `0..3`、↑↓ bounded（不 wrap）、Enter/Space confirm、Esc cancel，以及 native save boundary=`0x59cb`、record=`+0x312b+i*0xa28`（metadata=`0x28`、roster=`0xa00`）、rolling-XOR/checksum。remake 現已提供 title→LOAD 四槽 selector、空槽顯示、取消與 bounded input regression；JSON 仍是自有格式，尚未實作 native `FD2.SAV` metadata/roster/checksum compatibility。
...
- [~] **UI-03 native target/effect state dispatch**：battle command grid confirm 現以 verified two-stage target contract 開啟原始 target cursor，並只白名單已有 state executor 的 IDs `0,13–16,20–22,24–29,31`；`main.go` 的 cursor highlight/confirm 共用 selected raw command ID 的 record `+3/+4/+6`，不再把 command 0 的 geometry 套到其他命令；未知 IDs、ID30 special cursor、缺 raw flags/record/resistance 仍 fail-closed。這接通 state/UI boundary，不宣稱 indexed effect renderer、SFX、完整 target visual 或所有 command semantics。
...
- [x] **RE-CHURCH-RAW-SERVICE-LISTS-2E6B8**：官方 IDA 9.4 閉合 `0x2e6b8/0x2df6b` 的 bounded two-column selector：左右 ±1、上下 ±2、六項可視窗以兩項為步長滾動、Enter/Space 回 `1`、Esc 回 `-1`。`0x2f8ea` 會掃 selected runtime record 的八格 inventory，保留 signed flag 非負的 raw cells（`0x40` equipped 與 `0x00` ordinary 都保留，只有 `0x80` reserved 排除），再進第二層 list/confirm、`0x2f4c6` feedback、`0x2d516` amount 與 remove/recompute topology；`0x2ffa5` 經 caller-owned list 與 `0x17aed`。不命名 service、`0x1bb8c` 或 amount 語意，remake 維持 raw index 0/1 fail-closed。
...
- [x] **RE-CHURCH-RAW-0-17AED**：官方 IDA 9.4 重讀 `0x2ffa5→0x17aed`：三個 64000-byte indexed buffer、`0x17e0b` staging、`0x16c57(0)` input wait、`0x1ceed` command/overlay render 與 restore/redraw loop；函式 body 未見 persistent roster/inventory/gold/HP/class writer。撤回「能力服務」命名，raw index 0 維持 unnamed information/presentation boundary，renderer/text contract 未閉合前不接 normalized gameplay。
