...
304 - D - 敵方AI回合完整權重/target selection仍缺，可續靜態分析（runtime execution另偏E）。**2026-09-06複核**：doc11(同日稍早段落，L1347-1353)已明確結論「物理/法術/道具三條評分公式與`0x14EF0`三分數tie-break早已個別關閉」，target selection的完整權重公式面**維持既有[x]狀態**——本行文字「仍缺」是過時敘述。真正剩下的只有一個**玩法設計層**問題(不是byte級RE缺口)：「為何需要預選與第二遍」(`0x1D8BA`/`0x1D80B`兩遍敵軍/友軍掃描為何分開，觸發時機本身doc11已釐清是`0x1A30B`裡先友後敵、緊接遞增回合數的固定順序，但"為何這樣設計"這個問題本身可能沒有byte級答案)。維持D，但範圍已從「完整權重/target selection仍缺」收斂到這一個設計層問題。
...
覆寫來源。詳見doc35 §9.29。
1018 - D - 同1017之montage解碼blocker。**本行已過期，同上；2026-09-06比照1017維持D，理由同1017新增段落。同1017續二/續三的`FUN_0002aa00`/`FUN_0004e7dd`/`FUN_0002fb2c`新發現同樣適用。**
