...
- [~] **UI-VIS-TOWN**：`0x2cd16/0x2cf71/0x11eb0`已閉合3個FDOTHER背景variant、#10 label、FDTXT `0x1ef+selection`、FDICON `0,1,2,1` pulse、6組variant座標與312×192→VGA `(4,4)`；23個town保存raw `native_town_variant`。ch02 variant0 [`selection0–5`](../figures/town-hub-six-selections-original-vs-remake.png) 都達原版／remake raw RGB 整幀相同；另以固定雜湊原版的修改 LOAD 副本取得 variant2，正常 selection0–4 與 `town_ch06` production 指定 pulse 逐幀 AE=0，見 [`native_town_variant2_e2.json`](../data/native_town_variant2_e2.json) 與 [`variant2 five-selection contact sheet`](../figures/town-variant2-five-selections-original-vs-remake.png)。Left/Right wrap、Shift+F1 reveal、Enter進variant5及Escape返回selection5亦有input E2；共用 glyph shadow 與誤 reset pulse 已修。仍缺variant1、variant2 selection5 的 BIOS 掃描碼／Enter，以及未修改一般玩家城鎮路徑 E2。
...
- [~] **魔法系統**（資料表與基礎 Cast 已接，native command/effect 尚未閉合）:magic.go(spells.json=EXE dump 36條+normalized spell names;InCastRange/Cast
      ...
      不得稱完整原版一致。
- [~] **商店+祕密商店**: 69個shop節點已用`native_hub_variant` 1/3/5啟用indexed production owner；四項service與23筆secret chord gate已接。`found_secret_ch*`／legacy `SecretIf`只保留editable擴充，不再當原版gate。sell高階adapter仍會canonicalize ignored stale tail，不宣稱FD2.SAV byte parity。後續E2已閉合ch02三種主選單、secret chord/return與weapon purchase list；variant2正常五項的修改 LOAD E2另已固定，剩餘為purchase後續、sell/equip/transfer、town variant1、variant2 selection5／未修改玩家路徑及其他章節。
