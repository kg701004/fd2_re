...
- [x] **RE-CHURCH-RAW-SERVICE-LISTS-2E6B8**：官方 IDA 9.4 閉合 `0x2e6b8/0x2df6b` 的 bounded two-column selector：左右 ±1、上下 ±2、六項可視窗以兩項為步長滾動、Enter/Space 回 `1`、Esc 回 `-1`。`0x2f8ea` 會掃 selected runtime record 的八格 inventory，保留 signed flag 非負的 raw cells，再進第二層 list/confirm、`0x2f4c6` feedback、`0x2d516` amount 與 remove/recompute topology；`0x2ffa5` 經 caller-owned list 與 `0x17aed`。不命名 service、`0x1bb8c` 或 amount 語意，remake 維持 raw index 0/1 fail-closed。
- [x] **RE-CHURCH-INVENTORY-TRANSFER-1BB8C**：官方 IDA 9.4 閉合 `0x1bb8c(destination,item)`：掃八格、第一個 flag byte `<0` 的 cell 寫 flag `0` 與 item byte，成功回 `1`，滿格回 `-1`；配合 `0x2f8ea` 的 `0x1b8e7(source)` 前置，raw topology 是來源→目的角色的物品轉移，目的格為未裝備。新增 atomic `battle.TransferNativeInventoryItem` 與 full-destination regression；church 已有明確 `transfer_source→transfer_item→transfer_dest` mechanics slice，但 source gate 使用 editable `Equipped` projection，未將 raw index 1 命名成高階服務或宣稱 native renderer parity。
