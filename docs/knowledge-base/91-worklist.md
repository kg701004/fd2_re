...
- [x] **RE-INVENTORY-ITEM-GATE-24B14**：Docker Capstone 閉合 `0x24b14(item)`→`0x31860(unit,item)`→`0x1b8a6/0x1b722`：只掃 runtime unit `0..15`；每 unit 先取 occupied/prefix count（flag bit7 clear），再只比對該 prefix 的 item bytes `record+0x0b+2*slot`，成功回 native `1`，缺失回 `-1`。新增 `battle.FindNativeInventoryItemInUnit`／`FindNativeInventoryItem` read-only regression；不消耗 item、不加 camp/activity filter，也不把 `0x64` 命名成天空之鑰以外的泛用效果。
