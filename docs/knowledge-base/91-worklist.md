...
- **2026-09-08 續三:`disasm_le.py` 的「已知正確位址也產出垃圾」找到根因並修好**。`lin2file()` 寫死 `data_off + (lin - CODE_BASE)`,那**只對 object 1 成立**(base 0x10000、first page 1)。object 2(base 0x50000、first page 64)偏 **+0x1000**、object 3(base 0x60000、first page 68)偏 **+0xD000**。實測:`0x51b91`(跳表)與 `0x5274e`(寶物表)讀出全 0 而非真實表格位元組;`0x620a1`(成長表)**直接讀到檔案結尾之外**。因為程式碼段(obj1)一直是對的,問題看起來時有時無——而既有記錄裡那些「已知正確位址」都是資料段的 0x5xxxx/0x6xxxx。已改成逐 object 換算(與 `callgraph_le.page_base_linear`/`le_xref` 同一條公式),並修正 docstring 裡那條現在已經錯誤的公式。新增 4 項 --selftest:三個資料段錨點的回歸(0x51b91→`54580200`、0x5274e→`1d2b333d`、0x620a1→`06080406`)、與 callgraph_le 的逐位址跨工具對照(3 object × 4 位址全一致)、越界位址必須丟錯、以及**非恆真控制**(三個 object 的換算基準必須互不相同,否則等於又退回單一公式——實測 0x26014/0x25014/0x19014)。突變 2/12 DISCRIMINATING。
- **2026-09-08 續四:更正上一則的過度宣稱**。上一則說 `lin2file` 的逐 object 修正是「`disasm_le` 即使在已知正確位址也產出垃圾」的根因——**那是錯的**。既有記錄(2026-08-14)的症狀位址是 `0x13a9f`,在 **object 1**,而新舊公式對 obj1 **完全等價**:實測整個 obj1 範圍內兩者差異位址數 = **0**,所以本次修正在數學上不可能改變它。那個症狀實際上是 2026-09-03 的 `55988977`(le_xref 的 page mapping,parse_le 直接回傳 LE 檔頭的 data-pages offset 而未換算)修好的。**本次修的是另一個、只影響資料段(obj2/obj3)的獨立 bug**,證據仍然成立(0x51b91/0x5274e 讀出全 0、0x620a1 讀到檔尾之外),只是它解釋不了那則舊記錄。教訓:兩個都是「位址換算錯」的 bug,又都出現在同一支工具,很容易把後找到的當成前一個的根因——**差異位址數這種可計算的量,才是判斷「這個修正能不能解釋那個症狀」的依據**。
- **2026-09-08 續五:同一個過期位址類別的第三個實例**。`event_handler_dump.PRIM` 11 筆中有 **3 筆在現行 EXE 是死的**(呼叫端 0):`0x36cd7`(__STK)、`0x3453e`(raw_record_byte5_bit0)、`0x2cad7`(戰後raw_gate)。前兩者正是 `dump_chapter_beats` 那輪已處理過的同兩個位址,對應明確(`0x3702f` 541 個呼叫端、`0x34894` 50 個),已加入表中。**第三筆不猜**——`0x2cad7` 已列於 `docs/data/known_address_errata.json`,與 `0x2ccb6`/`0x2fd93` 同類(改基準前的手動反組譯位址),該檔已載明 `+0x190` 只是局部有效的錨點、不是常數平移,現行位址至今未解。所以 selftest 的規則是**「解得開,或已列管於 KNOWN_STALE 並附理由」**:不要求解決已知未解的,但新的死位址不能靜默混進來;另加一題防列管腐爛(列管的位址必須真的還是死的,復活了就該移出)。第 (4) 題修正一個退化檢查:`VAR` 那 5 筆是**資料位址**不是 call 目標,用呼叫端數判斷兩種假設都得 0,改用 fixup map 驗證(5/5 都是 fixup target)。突變 2/12 DISCRIMINATING。
**2026-09-09:chapter_beats 遷移的前置條件(參數個數)實測是機械可導的,新建 `tools/derive_native_argcounts.py`**。本行先前把「逐一驗證尚未收錄原語的參數個數」當成需要人工反組譯的獨立 RE 工作;實測不是。**先更正一個數字**:本行寫「8 個尚未收錄原語」,而現行參考版下 `dump_chapter_beats all` 實際產出 **27 個相異的 unknown 呼叫目標**(101 個 unknown beats);前 8 名佔 64/101,「8 個」應該是指高頻的那批。**判準**:cdecl 的呼叫端同時做兩件互相獨立的事,兩者都編碼了參數個數——(A) `call` 之後的 `add esp, N` ÷ 4;(B) `call` 之前緊鄰的連續 `push` 個數。同一函式的所有呼叫端必須一致。兩個訊號同時用,是因為各有已知失效模式:A 會被**合併清理**騙(`call f; call g; add esp,8` 的清理涵蓋兩個呼叫,前一個被誤讀成 0 參數;全 image 只有 1.4% 的 call 後面緊接 call,但受影響的目標必須個別標出);B 會被**行內計算的參數**騙(參數邊算邊推時連續 push 數會低估)。**正向控制**:對照 `dump_chapter_beats.PRIM` 已記錄的值,24 個有呼叫端的項目**相符 19 個**。不符的 5 個全部可解釋,而且解釋方向支持本方法——`dialog`(記 2、導 9)PRIM 註解自己就寫「9 個 push,只取最近 2 個」是刻意的投影;`load_res`(0→3)與 `play_sfx`(1→3)的 PRIM 註解明寫「參數個數**未逐一核對**」;`layout_units`(0→11)記的 0 與「依 call-site 陣列佈置」的描述不合;`loadch`(0→1)有 3/3 呼叫端一致為 `add esp,4`。**雙訊號交叉驗證**:27 個未知目標中兩訊號一致 **26/27**。唯一例外 `0x24618`(A 得 4、B 得 2)由**第三條獨立證據**裁決——doc56 早已反編譯記載它「first two arguments feed tile geometry (arg1*24+12, arg2*24+16); the third starts a radial radius and the fourth increments that radius」= 4 個參數,與 A 相同、與 B 不同,確認 B 在此處因參數行內計算而低估。**結果**:27 個目標全部推得出參數個數,分為 CONFIRMED 19 / LIKELY 1(`0x24618`)/ WEAK 7(5 個只有單一呼叫端,「一致度 100%」在那裡恆真、不構成證據;`0x1f882` 與 `0x25089` 有合併清理風險的呼叫端)。**方法本身也被自己抓到一個 bug**:第一版多數決先濾掉「沒有清理」的呼叫端,於是 `palfade`(0x1f525)9 個呼叫端中 8 個沒清理、1 個 `add esp,48`,採信了那個離群值得出 12 個參數;已釘成第 (5) 題回歸。6 項 --selftest,突變 8/12 DISCRIMINATING。**範圍誠實**:這支只解參數**個數**,不解 op **名稱**與語意——本行說的「填錯比留 unknown 更糟」主要針對語意,那是另一件事,兩者不混為一談;要不要據此更新 PRIM 表也還沒做。
**同輪續:把「兩個訊號一致」升級成真正的第三方核對**。上一則的雙訊號交叉驗證有一個它自己回答不了的問題——兩個訊號一致,只證明它們**一起說同一件事**,而它們可能因為同一個錯誤假設(例如我對 cdecl 的認定)一起錯。這正是「退化驗證樣本」那條規則:若競爭假設會預測相同觀察,該檢查就是裝飾性的。補法是找**不是這兩個訊號**的來源:掃 repo 文件裡別人在別的時間用反編譯/反組譯寫下的呼叫簽名,共找到 **15 個目標有獨立記載**,與本工具推導的參數個數**逐一相符**,且跨 1~5 五種個數(不是全部同值,所以一個常數實作過不了):`0x22253(unitSlot,newX,newY,visualX,visualY)`=5、`0x1c2da(a1,subcommand,count,targetBytes)`=4、`0x24618`=4(doc56)、`0x11d40(0,0xff,esi*6)`=3、`0x11df2(0,255,delta)`=3、`0x35b78(5,7,0)`=3、`0x25052(start,delay_ms)`=2、`0x12cea(slot,x)`=2、`0x2aedb`=2、`0x24b4d(N)`/`0x24b14(0x64)`/`0x17aa9(1)`/`0x24d22(arg)`/`0x24bde(18)`=1、`0x4df4c`=1(verified_addresses 記載精確呼叫位址 `PUSH [0x53a51]; CALL 0x4df4c`)。**過程中反過來抓到一處文件不實**:doc58 L2291 把 `0x2aedb` 寫成 `(index)`(1 個參數),而 doc99 L4171 的 `FUN_0002aedb(char_idx, item_id)` 註明「decompile確認」是 2 個,doc58 L3158 的 thunk `0x31860(unit,item)` 也是 2 個;本工具推得 2(3/3 呼叫端一致為 `add esp,8`),與 decompile 簽名相同。L2291 的寫法是不精確的簡寫,已在工具的 DOC_SIGNATURES 註解裡標明,未逕行改動該文件。**另補做「多次」這一層**:先前每支工具的突變測試只跑一個 seed、12 個突變,而 `verify_selftest_discrimination` 本來就有 `--passes`(用途正是「多跑幾次會不會抽到別的突變」),一直沒用。本工具改跑 8 輪不同 seed 共 96 個突變:加第 (7) 題前 **69/96**,加之後 **80/96**(83%)——第三方核對不只是多一題,它實質提高了鑑別力。現為 7 項 --selftest。**仍未解的**:27 個目標中 `0x1b8e7` 只有 1 個呼叫端(工具已標 WEAK),而文件顯示它另有 `0x1bbdc`/`0x20c6f`/`0x1c9dd` 等呼叫端在 chapter handler 圖之外——擴大種子集可以把它升級,尚未做。
**同輪續二:工具改善,三項都是被量測逼出來的**。(1) **加了第二種呼叫端發現機制**——原本只從 chapter handler 跳表走 CFG,子系統外的呼叫端看不到。改成也可用全 image 位元組掃描找 `E8 rel32`(`--wide`):`0x1b8e7` 呼叫端 1 -> 13、`0x1c2da` 1 -> 11、`0x1f882` 11 -> 38,CONFIRMED 由 19 升到 **21/27**。**關鍵不變量寫成第 (7) 題**:27 個目標的參數個數在兩種機制下**沒有一個改變**——證據多了 3~6 倍(`0x3776e` 58 -> 205、`0x17aa9` 35 -> 96、`0x4df4c` 3 -> 32)而答案不動,這比「兩個訊號一致」強,因為連取樣母體都換了。同時誠實記錄一個**反向**案例:`0x22253` 呼叫端 3 -> 5 後一致度掉到 1.0 以下,判定由 CONFIRMED 降級成 LIKELY(參數個數仍是 5,與文件簽名相同)——證據變多不一定讓信心變高,這是對的。(2) **拿掉一個 22 秒的 Ghidra 相依**。量出瓶頸不在我的掃描迴圈(0.00 秒)而在 `verify_findings.segments()`:它是走 **Ghidra headless** 讀位元組的,實測 **22.3 秒**,而 `callgraph_le.CG` 早就把同一份 image 載在 `cg.code` 裡。改成自己掃:**0.004 秒,快 5500 倍**,且實測結果相同——函式入口 541 個逐一相同(`--verify-entries` 對活的兄弟工具完整比對確認),呼叫端索引在共同範圍內完全一致。**判準仍然是借來的**(Watcom `push <frame>; call 0x3702f`,入口 = 該 call 減 5),換掉的只是位元組來源;入口數 541 釘進第 (8) 題當回歸,完整比對移到可選的 `--verify-entries`。selftest 由 25.5 秒降到 **1.1 秒**——這很重要,因為它會被突變測試乘 96 次,也會被 discrim 軸每支乘 12 次。(3) 現為 **9 項 --selftest**,8 輪不同 seed 共 96 個突變累計 **74/96 DISCRIMINATING**。**同輪的一次自造事故**:先前為了改效能而中途砍掉突變測試,而 `verify_selftest_discrimination` 是**就地改寫原始碼再還原**的,被中斷就把檔案留在已突變狀態;更糟的是它用 `ast.unparse` 重寫,**所有註解與 shebang 永久遺失**,而該檔尚未提交、git 救不回,只能整份重寫還原。那個突變是 `if got != want` 被改成 `if got == want`,selftest 正確抓到——工具本身運作正常,但「被中斷會損毀原始碼」是它的真缺陷(應在突變前寫備份檔、啟動時偵測並還原),已列為下一批的獨立項目。教訓:**不要中途砍掉會改寫工作區的工具**。
**2026-09-09 續二:未收錄原語的 args 順序是反的,而參數個數推導剛好可以修它——不需要 op 名稱**。上一則解出了 27 個未收錄目標的參數個數。回頭看 `dump_chapter_beats` 怎麼用它們時發現一個具體 bug:**已收錄**的 op 會做 `pushes[-nargs:]` 再 `reversed()`(cdecl 由右到左 push,反轉後才是簽名順序,檔頭就寫著這件事),而**未收錄**的直接記 `list(pushes)`——沒有依參數個數切、也沒有反轉。後果是同一個檔案裡兩種 beat 的 args **順序相反**,而消費端沒有任何線索可以分辨。101 個 unknown beat 裡有 **59 個 args>=2**、順序有意義。**驗證錨點不是自己算的**:`0x11d40` 原本記成 `[64, 255, 0]`,而 doc 記載的簽名是 `0x11d40(0, 0xff, esi*6)`——**反轉後的 `[0, 255, 64]` 才對得上**;另一處 `[0, 255, 'esi']` 也一致。修法是讓未收錄的走同一套規則,參數個數取自 `derive_native_argcounts`,而且**只採信 CONFIRMED 的**(21/27)。WEAK/LIKELY 維持原本的「全部 push」行為並新增 `args_are_raw_pushes: true` 標記——猜錯個數比留著原樣更糟,但消費端要知道那一筆不是簽名順序。`0x1f882` 就是最好的例子:它被判 WEAK(有合併清理風險),而它記錄的「args」實測是 `['ebx','esi','edi']`——那是被呼叫端的**暫存器保存**,根本不是參數,與它推導的 argc=0 一致。延遲 import 是刻意的:`derive_native_argcounts` 會 import 本檔取 PRIM 與跳表位址,模組層互相 import 會成環;推導不可用時退回原本行為,不讓整批抽取失敗。新增第 (3b)(3c) 題:前者釘住 `0x11d40` 推得 3 個參數(與 doc 簽名的三個參數相符),後者釘住 `0x1f882`(WEAK)必須回 None、不被採信。突變 4/12 DISCRIMINATING。**這一步不需要 op 名稱**——本項原本的阻塞是「逐一驗證參數個數」,那已完成;op 名稱與語意是另一件事,仍未動,61 個檔也還沒重生。
**2026-09-09 續三:把參數個數的證據存成已登錄產物,以及一個我刻意不做的決定**。(A) `docs/data/native_argcounts.json`:27 個未收錄原語的參數個數(CONFIRMED 21 / LIKELY 2 / WEAK 4),連同呼叫端數、一致度、清理分布與失效模式旗標。`dump_chapter_beats` 用它決定 unknown beat 的 args 要切幾個、要不要反轉,所以這份證據不該只活在記憶體裡。已登錄進 `verify_generated_artifacts.REGISTRY`(產生器 `derive_native_argcounts.py --wide --json`),實跑 **IDENTICAL**;登錄項目 27 -> 28,而且**新產物一進來就是已登錄的**,沒有增加待處理數。(B) **op 名稱這一步我刻意沒做,理由值得記**:doc50 §3.1 有一張表對出現次數最高的幾個未知原語做了「淺層定性」,但用詞是「**疑是**」——例如 `0x24b4d` 寫「疑是該分支背後共用的『reveal/漸現』子程序」、`0x233c6` 寫「疑初始 pose」;而 doc31 對同一個 `0x24b4d` 說「2026-09-06 補完,原生邏輯**已完整反組譯**」,還附了 `0x24b4d(N)` 的完整呼叫序列與 15 個呼叫點(**與本工具推導出的呼叫點數相同**)。同一個位址,兩份文件的信心度不同。本 session 已經因為「看起來不一致就去統一」而差點改壞 `record[+5]` 的位元語意,所以這裡不用挑比較方便的那一份來解決矛盾。加 op 名稱是會被下游消費的語意主張,而本項自己就寫著「**填錯比留 unknown 更糟**」。**真正的下一步是先把 doc50 與 doc31 對這幾個位址的信心度差異收斂掉**(哪一份是後來的、有沒有把前一份的「疑」升級成定案),那是一輪獨立的文件核對工作,不該混在參數個數這一批裡順手做掉。
**2026-09-09 續四:op 名稱其實不需要人工裁決——上一則說的「信心度衝突」查證後不存在**。上一則把「先收斂 doc50 與 doc31 的信心度差異」列為獨立一輪的前置工作。實際查下去**沒有矛盾**:doc50 §3.1 是早期淺層定性(「疑是 reveal/漸現」),doc31 §9.6(2026-09-06)是後來的完整反組譯,而兩者**對可觀察量完全一致**——同樣的 `0x11eee`(背景重繪)+ `0x11cac`(present)+ present 迴圈 + delay、同樣 **15 個呼叫點**(與本工具從呼叫端獨立數出來的相同)。doc31 只是把「疑」拿掉,並且**修正了語意標籤**:它不是漸現,是「兩張相隔 0x1c8 bytes 的緩衝區依奇偶交替 blit、來回閃爍 N 次」。所以這是**後者取代前者**,不是二選一,不需要挑比較方便的那一份。**入場規則**:引文必須逐字存在,**而且**該位址要寫在引文的 ±3 行內。第二個條件不是裝飾——建表時先用純鄰近掃描得到「22/27 有完整反組譯」,查證後發現 **`91-worklist.md` L213 是本工具自己那條列了一堆位址的摘要行**,被算成六個位址各自的證據;`0x31529`(實際在講鄰近的 `FUN_00031266`)與 `0x25089`(一張排除假說的表格)也都是假陽性。錨點每次呼叫都會複驗,文件被改掉名稱就自動失效。**結果**:27 個目標中 **11 個**有可逐字定位的名稱——`memset`(doc58「`FUN_00037910(dest,byteVal,n)`…標準 memset wrapper」)、`find_item_slot`/`party_has_item`(doc99 L4166-4171 的完整偽碼:`FUN_00024b14(item_id)` 掃 16 格角色、對每格呼叫 `FUN_0002aedb(char_idx,item_id)` 找槽位)、`remove_inventory_slot`(doc56 官方 IDA 9.4 `sub_1B8E7(int unit, int slot)` 的 memmove 移槽)、`camera_step_to`、`unit_present`、`buffer_flicker`、`reveal_wipe`、`sprite_walk_on`、`palette_brightness_ramp`、`palette_delta_ramp`。**unknown 101 -> 49、相異目標 27 -> 16**,天花板 110 -> 55;11 個名稱全部有實際命中(第 (4b) 題擋「加了名稱卻一個都沒對上」,那代表位址認錯)。**刻意不給名稱的**:`0x33f78`/`0x35f10`/`0x361b0`/`0x13536` 全庫查無任何命名段落;`0x3776e` 的鄰居 `0x3771c` 才是被反組譯成 memmove 的那個,不是它本身。**關鍵設計:名稱與參數個數是兩種主張,不得互相帶動**。第 (4c) 題用 `0x22253` 釘住這件事——它有 doc31/doc35 記載的完整 5 參數 ABI(所以有名稱),但呼叫端母體擴大後判定 LIKELY,`args` 仍然保留原始 push 並標記 `args_are_raw_pushes`。若哪天名稱一上去就順手把 args 切了,這題會失敗。負向控制三個:引文對但位址換掉必須落空、不存在的引文必須落空,以及**真實假陽性回歸**(doc99 那句話出現 2 處,只有 1 處旁邊有 `0x2aedb`,純比對會多收一處)。`native_argcounts.json` 加 `doc_op_name` 欄位後重生仍 **IDENTICAL**。突變:derive 8 輪 74/96、dump_chapter_beats 4 輪 18/48,皆 DISCRIMINATING。**仍未動**:61 個已提交檔案還沒重生(它們是舊版產出且事後人工補過 op 名,要先確認人工名稱是不是本輪 doc 錨定名稱的超集)。
**2026-09-09 續五:6 個 ui-traces 不是「待重錄」,是已移除 remake 的產出**。它們一直掛在 regenerable 的待處理裡,先前被我當成「需要開實機重新錄製」。實際打開來看:`town-shop-ch02.json` 等 5 個的 `source_campaign` 指向 `remake/assets/scenarios/campaign_full.json`、`artifacts` 是 `*-remake.png`、`limitations` 自己寫「uses editable campaign data and remake inventory APIs」;`save-town-boundary-ch02.json` 的 `format` 是 `remake-json-v1`,`assertions` 甚至明寫「save is remake JSON and is **not** FD2.SAV byte-compatible」。worklist L2575 也記著它們出自 remake 的 Go 測試(`TestCampaignTownPreparationInputTrace`)。依 2026-09-02 的裁定,這類資料本身不可採信,而產生它們的測試已隨整個 `remake/` 移除——**既不能重生,內容也不算證據**,「重錄」更是另一件事(那會是原版側的新證據,不是重生這 6 個檔)。處理:比照既有的 `no_generator`,新增 `remake_derived` 永久豁免,證明分三層且可證偽——(1) 檔案必須**自述**為 remake 產出(只看字串**值**、不看 key 名,免得靠欄位名蒙混)、(2) repo 內不得存在 `remake/`(它一旦回來,這些就重新變成可重生的,宣稱立刻失敗)、(3) 沒有任何工具在寫入語境提到它。新增第 (3d) 題配對控制:兩個 trace 必須過、兩個原版側產物(`native_argcounts.json`/`glyph_map.json`)必須**不過**——只驗正例的話,一個恆真的 prover 也會通過。檔案保留不刪(它們是 worklist 既有記錄的一部分),只標明來源。**永久豁免 48 -> 54,實際待處理 67 -> 61,至此只剩 61 個 chapter_beats**。突變 4 輪 27/48 DISCRIMINATING。
**2026-09-09 續六:61 個 chapter_beats 終於重生了,而擋住它的理由查證後只剩 3 條真的**。本項寫「已提交的 61 個 chapter_beats 未一併重生:它們是舊版產出且事後人工補過 op 名,重生會弄丟那些補充」。逐檔比對後,這句話大部分不成立。**先更正我自己中途的一次誤判**:我打開 `ch06_post.json` 看到它是 IR schema(`schema_version`/`source:{addr,target}`/具名欄位/巢狀 `if`),就下結論說「已提交檔是另一種上層 schema,重生等於退步」——**那是錯的**,60 個檔裡只有 `ch06_post.json` 一個是 IR schema,其餘 **59 個是 `dump_chapter_beats` 的 raw schema**,我剛好挑到唯一的例外。教訓與「驗過的表、錯的推論」同一類:我先前那輪的比對是拿 **op 名稱計數**做的,計數本身沒錯,但計數看不到 schema,而我把結論建在它上面。**真正會失去的東西,逐欄位掃出來只有三類**:(1) **12 條人工註記**(ch22_post 10 條、ch26_post/pre 各 1 條,2026-09-06 寫的反組譯證據);(2) **`ch06_post.json` 的手工 IR 結構**(外層 `if native_event_state_eq` 沒有任何抽取器產得出來,內層 `any_unit_inactive` diamond 在現行 EXE 也沒被 `structure_control_flow` 的形狀比對命中——這是獨立待查項);(3) 沒了。其餘差異全部是**修正**:`0x375b2 -> 0x3790a`(delay)、`0x3453e -> 0x34894`(unit_inactive)、`0x31860 -> 0x2aedb` 這些舊版位址,以及 6 個檔各多出的 `memset` beat。**多出來的 beat 有查**:反組譯現行 EXE 的 `0x235ad` 確認是 `call 0x37910`(`memset(0xa0000, 0, N)`,清 VGA 畫面),是真實存在的呼叫,已提交檔完全沒有這條;合理推測舊管線把它當編譯器輔助函式放進 SKIP(舊 SKIP 有 `0x375c0`),但舊 EXE 已刪除、無法證實,所以只記推測不下斷言。**同樣方式查出一筆已提交檔是錯的**:`ch22_post` beat 6 記 `args:[18]`,重生記 `[13]`;反組譯現行 EXE 得 `0x24836: push 0xd; 0x24838: call 0x24bde`——**13 才對,18 是舊版的值**,重生是在修它。**處置**:12 條註記另存 `docs/data/chapter_beats_notes.json`(依 檔名+呼叫端位址 索引),`cmd_all` 匯出時貼回,**貼不上整批丟錯**(第 (4e) 題釘住這件事;沒有它,某次重生會把證據安靜刪掉);`ch06_post.json` 不動並記明原因;其餘 59 個重生,實際內容變動 37 個檔。**另外補了兩個名稱,而補的理由是我自己的判準不一致**:當初以「判定 WEAK/LIKELY」排除 `0x24bde`/`0x24618`,但 WEAK 講的是**參數個數**的信心,而我自己的設計明說名稱與參數個數互不帶動。`0x24bde` 的正確名稱就是 `roster_has`(doc25 明說它是 `0x33499` 同一原語的第二個獨立編譯實例,本體逐位元組相同),與 PRIM **同名不同址**——所以撞名規則改成「撞名時引文必須自己提到那個名字」,並加上兩極控制(真引文放行、換成沒提到的引文必須擋下)。unknown **49 -> 40**、命名 beats 52 -> 61、天花板 55 -> 45。**順手補一個一直沒人發現的缺口**:`export_handler_scripts.py`(把原始 beats 變成下游真正消費的 IR 的轉換層)**從來沒有 --selftest,因此從來沒被突變測試掃過**;它的 fall-through 分支會把新名稱轉成 `{op, args}` 而**丟掉 `op_name_source` 與 `args_are_raw_pushes`**,而且 `unknown` 分支把 args 一律叫 `raw_args`——在已切過並反轉的那一半那個標籤是錯的,消費端分不出兩種順序。都已修,並補 5 項 --selftest(含「不得憑空補上標記」的負向控制)與 2 個單元測試,納入 INVOKE(76 -> 77 支)。突變:export_handler_scripts 32/48、derive 70/96、dump_chapter_beats 18/48。**仍未做**:把 chapter_beats 登錄進 REGISTRY(需要目錄模式,且要先處理「輸出位元組隨作業系統換行而異」這件事),所以實際待處理仍是 61。
**2026-09-09 續七:artifacts 軸抓到我自己的漏做,而正確的修法是把那道檢查往前搬**。續六加了兩個 op 名稱後,`docs/data/native_argcounts.json` 的 `doc_op_name` 欄位跟著變,但我沒重生它——一直到 10 軸驗證的 `artifacts` 軸(整輪約 30 分鐘)才報出漂移。閘門運作正常,問題在**回饋時機**:改了餵給已登錄產物的表,沒有任何東西在當下提醒。修法是在 `derive_native_argcounts --selftest` 加第 (11) 題,直接比對已提交產物與本工具現在會產出的內容,**2.5 秒**就能知道,不必等整輪。這不是取代 `verify_generated_artifacts`,是把同一個判準提前到最靠近改動的地方(與「把驗證往前搬」那條記錄同一個道理)。**已驗證它不是恆真**:把產物的 `doc_op_name` 注入成 `WRONG` 後 selftest rc=1 且指名 `['0x24bde']`,還原後 rc=0。突變 8 輪 72/96 DISCRIMINATING。**順帶記一個踩到的操作錯誤**:這一則原本是用 shell 傳給 `worklist_status.py --append` 的,文字裡的反引號被 bash 當成命令替換,寫進檔案的那一行**所有反引號片段都被吃掉**,`audit_evidence_provenance --diff` 隨即報出 1 筆缺來源標記——閘門又抓到一次。含反引號的長文要走 heredoc 或檔案,不要直接當命令列參數。
**2026-09-09 續八:hygiene 棘輪的「實際待處理」歸零,而登錄過程本身挖出一個隱形漏洞**。(A) `docs/data/chapter_beats` 整個目錄登錄進 `verify_generated_artifacts.REGISTRY`(產生器 `dump_chapter_beats.py <EXE> all`,目錄模式本來就有),實跑 **IDENTICAL,61 個檔,0 個非此工具產出**。前置是把唯一手工結構化的 `ch06_post.json` 移到 `docs/data/chapter_beats_manual/`——留一個產不出來的檔在已登錄目錄裡,那個項目會永遠報漂移;移出去的檔加了 `_provenance` 欄位寫明來歷與為何不重生。**實際待處理 61 -> 0**:目前 56 筆違規全部是有可執行證明的永久豁免,不是被關掉的檢查。(B) **登錄時發現的真漏洞**:`coverage()` 對 `dir` 項目是 rglob 整個目錄,所以目錄裡每個 JSON 都拿到「已登錄」身分;但逐位元組比對只比對產生器**實際產出**的那些。兩者的差集**既不會被比對、也不會出現在待處理清單裡**——完全隱形。實測 `docs/data/exe_tables/` 有 7 個這種檔,其中 5 個既沒有各自的登錄項目、也沒有無產生器證明,而 **`characters.json` 就在裡面**——本 session 稍早正是在那個檔裡抓到三個錯的角色名。這與「乾淨的總計藏起缺席的列」同一個模式,只是這次藏的不是列,是**身分**。修法不是放寬:新增 `DIR_ORPHANS` 明列差集與理由,第 (9) 題兩邊夾——實際差集不能有沒列到的,列到的也必須真的沒有產生器(`discover()` 每次實掃)。**故障注入證明它不是恆真**:在已登錄目錄丟一個檔進去,rc=1 且指名該檔;移除後回到 0。(C) **一個我先量測後撤掉的最佳化**:新檢查讓 selftest 看起來從瞬間變成 25 秒,而 discrim 軸會把每支工具的 selftest 乘 12 個突變再乘 2 輪,我推估是 10 分鐘的退化,於是加了跨行程快取。**逐題計時後發現推估是錯的**:23.5 秒在第 (5) 題(它會實跑一輪完整驗證來證明沒動到已提交檔),那是既有成本;新檢查只佔 0.3 秒,兩個目錄產生器合計 1.3 秒。快取是在解一個不存在的問題,還多一個過期快取的失效面,已撤除。這是本 session 第三次遇到「完整性造成效能退化」的題型,而這次的教訓是反過來的:**先量,不然連該不該修都會判斷錯**。突變 4 輪 21/48 DISCRIMINATING。
**2026-09-09 續九:一則被自己的閘門擋下的記錄,以及它暴露的診斷訊息缺陷**。續八那則是純工具流程敘述,沒有任何原版側標記,`--diff` 依規則擋下——這部分是對的。問題出在補登錄的路徑上:`--mark-reviewed` 的 `file` 參數其實只吃**基本檔名**(掃描結果就是這樣存的),但沒有任何地方說明,而傳 repo 相對路徑時它一律回「行號可能已經漂移(檔案被改過),重新掃一次確認正確行號」。**我因此去查行號,查了三次才發現真正的原因是檔名格式**。指錯方向的診斷比沒有診斷更貴——我先前甚至因此在對話裡下過一個錯的結論(說登錄路徑對這類行走不通),那也是錯的。修法:`file` 正規化成基本檔名(路徑照樣接受),並把失敗分成兩種訊息——「這份文件在掃描結果裡沒有任何 NO_MARKER 主張」與「檔案在、但這個行號不是,同檔有 N 筆,最近的是第 M 行」。這與「讀取失敗不是負面結果」是同一個形狀:兩種不同的失敗被壓成同一句話,人就會往錯的方向查。selftest 加 4 題**成對**驗證(路徑要過、基本檔名要過、同檔錯行號的理由必須提到行號、檔案不存在的理由必須提到掃描範圍)——只驗「路徑能過」的話,一個把所有輸入都放行的實作也會通過。66/66 通過,突變 4 輪 24/48 DISCRIMINATING。
**2026-09-09 續十:兩支互相牽制的工具對同一批檔案給出不同答案,而原有的交叉驗證看不到**。續八把 hygiene 的實際待處理清到 0,但 `artifacts` 軸同時還在說「尚待處理 7 個」。查下去那 7 個正是 6 個 ui-traces(hygiene 已標 `remake_derived`)加 `docs/data/ida/fd2_xrefs.json`(已標 `lost_input`)——`verify_generated_artifacts.no_generator_set()` **只認 `no_generator` 一種永久豁免**,後來新增的另外兩種它看不到。三種說的其實是同一件事:重生比對這條路對這個產物不適用。(`ida_embedded` 不算,那是工具側的 correctness 規則,不是產物側的。)**更值得記的是為什麼沒被抓到**:既有的交叉驗證比的是「未涵蓋」的**原始集合**,而那個集合兩邊一直都一致(50/50)——分歧只出現在**扣掉永久豁免之後**的最終數字。乾淨的原始總計掩蓋了下游的不一致。修法兩件:(1) `no_generator_set()` 三種永久豁免都算,報表措辭同步改成「已證明重生比對不適用(無產生器/輸入已遺失/remake 產出)」——原本的「已證明沒有產生器」對其中 7 個是不準確的說法;(2) 新增一條交叉驗證,直接比**實際待處理**這個最終數字。**故障注入證明它有鑑別力**:把其中一種豁免從 `no_generator_set()` 拿掉,它立刻報「本工具 0 筆 / vg 7 個」並指名那 7 個檔;還原後回到一致。至此 hygiene 與 artifacts 兩個獨立計數**都是 0,而且兩邊對同一批檔案的說法一致**。突變:verify_tool_hygiene 27/48、verify_generated_artifacts 10/24。
**2026-09-09 續十一:diamond 辨識器的判準比它想測的性質嚴,而只放寬會製造假結構**。起因是上一則列為「待你決定」的 ch06_post——**那項其實完全離線可做,我又一次把範圍講窄了**。(A) **根因**:`structure_control_flow` 要求被跳往的那一臂含**顯式** `jmp merge`,而 ch06_post 的 taken 臂結尾 `0x233b7 add esp,0x24` 的下一個位址就是 merge `0x233ba`——編譯器沒有理由多發一條跳到下一條指令的 jmp。判準比性質嚴,後果不是報錯而是**靜默降級**成扁平。改成「落下去剛好結束於 merge 且最後一條不是無條件轉移」也算抵達。(B) **只放寬會出事,而且是我自己測出來的**:放寬後 ch06_post 產出 `if any_unit_inactive([43])`,那**丟掉了外層的事件旗標守衛**。真實條件是 `0x2332a jne 0x23393` 與 `0x23338 jne 0x23393` **跳到同一個位址**,即 `if (A && B)` 的短路編譯,兩個判準共用一個 else 臂。假的結構主張比扁平更糟——扁平至少看得出有損。所以同時加一道守衛:同一個 target 若已被更早的條件跳指向,就維持扁平。兩題成對釘進第 (2b) 題(合成案例:落下去且無共用 else 必須辨識出 `if`;同一段加上共用 else 必須維持扁平),真實資料 **0 個檔改變**——波及範圍實測只有 ch06_post 一個,而它現在維持扁平是正確的。(C) **外層旗標的語意查出來了,寫進 doc25 §12**:`[0x3ad5]` 就是既有記載的 `[0x53AD5]` = `State.NativeEventState`(doc11 L163)。**我先前說它全 repo 查無記載,那是錯的——我用 `0x3ad5` 去搜,搜不到 `0x53AD5`**;這是本 session 第三次「查無資料」的結論來自不完整的搜尋。位址關係用**已知對子**決定性確認:同一段 handler 的 `0x233ba inc dword ptr [0x3c03]` 就是 doc25 §1 的章節索引 `[0x53c03]`,反組譯輸出系統性省掉 obj2 的 `0x50000` 基底。(D) **順帶更正 doc25 §3.2.4 一個數字**:它記「12-byte 旗標區塊」,而 `0x20607..0x20611` 是 `memset(dest=[0x3ad5], 0, 0x20)` = **32 bytes**,緊接在章節 loader `0x1088d` 之後,即每章清零;ch06_post 讀的 index `0x11`=17 本身就超出 12,是不必靠 memset 的獨立佐證。原本那個「12」是當時live 探測的寶箱 slot 數,不是配置大小。寫入端全 image 35 處(`[+0x10]` 13、`[+0x11]` 5 等),形如 `0x34cec mov byte ptr [eax + 0x11], 1`,密集落在事件 handler 區 `0x348f4..0x363b5`;指標本身只有一個 writer `0x25d3d`。另外 `0x24f35`/`0x24fb6` 把 `[+0xc]` 當對白索引偏移用。(E) **一個誠實的覆蓋度觀察**:doc25 §12 的 33 條實質新增行,`audit_evidence_provenance --diff` 解析出 **0 筆主張**——因為 `VERIFY_LANG` 的觸發詞是「確認/驗證/實測/佐證/逐字」等,而我寫的是「核對」,不在表內(22/33 行本來就帶位址,所以不是誤放,是沒進入審查)。把「核對」加進觸發詞會重新分類既有 8017 筆主張,不是能順手做的改動,所以只記下實測數字,**不要事後假設這一節被閘門審過**。**剩下的**:要讓 ch06_post 可機械重生,缺的是「條件是連接詞」這個表達能力(下游消費端要認得),不是再放寬 diamond 判準——這一項現在有精確的形狀,不再是「不知道為什麼認不出來」。
**2026-09-09 續十二:「查無命名段落」又是搜尋方式的問題,unknown 40 -> 17**。上一則把 14 個未命名目標裡的多數列為「全庫查無命名段落」。**那個結論是錯的,而且錯在同一個地方**:我的搜尋要求位址與確認詞出現在**同一行**,而文件是在表格、標題、敘述裡命名的。這是本 session **第四次**「查無資料」來自不完整的搜尋(前三次:`0x3ad5` 用錯拼法搜不到 `0x53AD5`、`91-worklist` L213 的摘要行被當成六個位址各自的證據、`0x24bde`/`0x24618` 被我以 WEAK 為由排除而那是錯的判準)。**這次改成先列出所有含該位址的行再逐一讀段落**,結果 5 個立刻成立:`0x17aa9` = `wait_ticks`(doc23 L186「`0x17aa9` 是 tick 計數忙等(讀全域計數器 `[0x46c]`」,而且該段明寫是**修正** doc39 §10.7 原本猜 BGM/SFX 的說法);`0x25052` = `palette_ramp_down`(doc56 L1102 給出完整簽名 `0x25052(start,delay_ms)`,與推導的 2 相符);`0x13536` = `clear_acted_flags`(doc56 L1058「對全部 runtime records 執行 raw `+5 &= 0x7F`」——`record[+5]` bit7 正是已證實的 Acted 旗標,`&= 0x7F` 就是清掉它);`0x31529` = `scene_charcard_orchestrator`(doc35 §9.11.3 標題「其真正起點是 `0x31529`——一個「換場+角色卡」的 orchestrator」);`0x35b78` = `give_item_to_group`(doc25 §11 的 `func_0x35B78(group_id, item_id, count?)`,該段明寫是**訂正**先前 §10.2/§10.4.2/§10.4.3 的讀法)。**第 6 個是被規則擋下、回頭查證後才成立的**:doc31 §9.5 寫「`0x33f78` wrapper 的 5 引數」,而兩個訊號都得出 3,入場規則因 5≠3 擋下。反組譯本體發現那個 5 屬於它呼叫的 `0x22253`——`0x33f78` 收 3 個引數(9/10 個呼叫端 `add esp,12`),先用兩個呼叫 `0x12cea` 做鏡頭步進,再湊 5 個 push 交給 `0x22253`(`add esp,0x14`)。`NativeStagingPresent` 的五個欄位描述的是**下游那一層**的資料形狀,不是 wrapper 的呼叫慣例。訂正寫成 doc31 §10,名稱 `camera_step_then_present` 錨在那一節。**這是同一條規則第二次反過來抓到文件有誤**(第一次是 doc58 把 `0x2aedb` 簡寫成 `(index)`)。**兩個機制改動**:(1) 錨點的位址比對改成**不分大小寫**——doc25 同一節混用 `0x35B78` 與 `0x35b78`,分大小寫會讓錨點在正確段落上落空;引文仍逐字比對,不放寬。(2) `doc_argc` 允許 `None`,表示文件只給名稱沒給簽名,此時**參數個數那一半必須自己站得住**(推導須 CONFIRMED)——兩種主張各自有支撐,不互相借力。**結果**:命名 13 -> 19/27,chapter_beats unknown **40 -> 17**、相異目標 14 -> 8、命名 beats 61 -> 84,天花板 45 -> 20;19 個名稱全部有實際命中。**本 session 起點是 101 個 unknown**。仍維持 unknown 的 8 個各有理由(`0x3776e` doc13 明寫「推測,未證實」、`0x35f10` doc25 明寫「本體未展開」、`0x4df4c` 另有未收斂的行為矛盾、`0x1f882`/`0x25089` 合併清理風險、`0x24336` 單一呼叫端、`0x361b0`/`0x1c2da` 只被提到沒被命名)。突變:derive 76/96、dump_chapter_beats 14/48。
**2026-09-10:`--open` 報 0 而我把它當成「沒事了」轉述出去,那是假陰性**。盤點時 `worklist_status --open` 印「沒有任何項目的最新一段主張有待辦工作」,而同時有 6 個 D、38 個 C 標籤——兩者衝突,我卻直接引用了那句話。查根因:判準的**引文與否定處理是對的**(那兩層本來就是被真實假陽性逼出來的),問題在 `OPEN_WORDS` 只有 7 個詞,而實際文字寫的是「仍未完成的那一步」(155)、「剩餘只有 `+0x0b` vs `+0x0c`」(1118)、「仍未逐一展開」(857)——一個都不在表內。這是本 session 第五次「查無」來自查詢本身的缺陷。**修法刻意不是把詞加進 `OPEN_WORDS`**:先量再改——16 個候選裡只有 5~6 個是真的開放,其餘是**否定式**(1139「沒有剩餘缺口」)、**引文**(247/370/407 引用 doc32 舊結論的「未解決」)、或 **remake 已無對象**(447/510/555/1042)。直接加詞會讓 `--open` 產生約六成假陽性,**比現在的假陰性更糟**。所以分兩層:嚴格判準原樣不動、負責「可直接相信」;新增 `REVIEW_WORDS` 與 `--review` 負責「別讓 0 被讀成沒事」,列出候選項目與命中詞的前後文供一次讀完;`--summary`/`--open` **永遠附帶候選計數**,那句「0 項」再也不會單獨出現。順帶把否定詞從 不是/並非/非 補上 沒有/無(候選層常見「沒有剩餘」),並抽出共用的 `_scrubbed()`。實測:嚴格 0 項、候選 **11 項**。新增第 (8) 題四個**成對**案例釘住兩層的分工——嚴格漏掉的候選必須抓到、否定式兩層都不能抓、結案語要壓過候選用語、嚴格抓到的不得重複出現在候選——外加真實檔案上的非平凡性(0 < 候選 < 項目數/3)。只驗其中任何一個,一個「永遠回傳空」或「永遠回傳全部」的實作都會通過。突變 4 輪 26/48 DISCRIMINATING。
**2026-09-10:歸檔,以及歸檔動作本身踩到的閘門漏洞**。依 `Project_Continuity.md` 體例寫成 `SESSION-HANDOFF-2026-09-10.md`(涵蓋 `6d2ad9c5..192a0459`,59 個 commit、112 檔 +11336/−1109),並把 `00-index.md` 的「先讀這個」指標改指它、在前一份加上向後連結。**寫的過程中抓到 `audit_evidence_provenance --diff` 的一個真漏洞**:`git diff` **完全不含未追蹤檔案**,所以一份**全新的**知識庫文件會整份繞過這道閘門——而那正是風險最高的一種新增(一整份沒有任何來源標記的文字一次進庫)。當下的證據很直接:閘門對這份新文件報「新增 0 筆驗證主張」,而我手動跑 `classify()` 在同一份檔案上得到 **11 筆**。修法是把未追蹤的 `.md` 合成 unified diff 再交給既有的 `parse_diff_claims()`,而不是另寫一條掃描路徑——這樣審閱登錄表的跳過、長度門檻、`EXCLUSION_TAG` 等規則自動一致,不會兩邊漂移。**修好後它立刻在那份新文件裡抓到 9 筆缺來源標記的主張**,逐筆(不是一次放行)登錄為 `benign` 並各附理由——9 筆全是關於本 session 流程/統計/教訓的陳述,沒有一筆是關於遊戲的事實主張。新增成對 selftest:真的建一個未追蹤 `.md` 必須被看見,同時建的非 `.md` 必須被忽略(只驗前者的話,一個「把整個工作區都塞進來」的實作也會通過);檢查數 66 -> 68。**故障注入的過程本身也值得記**:第一次注入後 selftest 仍然全過,我沒有據此斷定「這道檢查沒有鑑別力」,而是先查注入有沒有生效——實際是我的替換字串在 shell 轉義下沒配上;改用 heredoc 並先斷言「注入點存在、內容有改變」之後,注入後 rc=1、67/68、並指名正確的失敗訊息。突變 4 輪 24/48 DISCRIMINATING。
**2026-09-11:58..89 全 32 格重驗完成,本項的「18 個具體行為 / 14 個 table artifact」統計整組作廢**。2026-09-07 那輪推翻 58/76/78 時誠實記下「其餘 11 格未逐一重驗,應視為存疑待重驗」;本輪驗完 32 格,結果比預期強——**一格 artifact 都沒有**,`CLEAN_PROLOGUE` 28 格 + `TAIL_MERGED_STUB` 4 格,`MID_BODY` 0 格,入口 `0x35854..0x3644e` 嚴格遞增。**根因是單一系統性錯誤**:跳表 `0x51b91` 每格存的是**相對 obj1 的位移**,真正入口是 `stored + 0x10000`,而 doc25 §10 的位址欄少了這一步——逐列比對文件裡的 25 列,**一律低 `0x356`,零例外**。少了這個偏移之後,有些格剛好重新同步到指令邊界(Ghidra `createFunction` 在任何位址都會硬生函式,看起來像乾淨 handler)、有些落在指令中段(被判 artifact)。**artifact 與否量的是那個偏移落在哪裡,不是 handler 有沒有語意。**新增 `tools/verify_event_dispatch_table.py`(已登錄產物重生表、突變測試表與 wsl 軸)產出 `docs/data/event_dispatch_table_58_89.json`;**刻意完全不做線性反組譯**——這個錯誤本來就是靠反組譯邊界推論造成的,改用 Watcom 兩種固定序頭的位元組樣式(`68 imm32 E8 rel32` → `__STK`;`68 imm32 EB rel8` → 共用尾段),call/jmp 目標都實際算出來核對。四項 selftest,兩項是要害:(i) 2026-09-07 那輪是「讀 raw 檔案 + 四錨點校準」、本工具是「讀已重定位 image + `+0x10000`」,兩條獨立路徑對 58/76/78 必須得到同樣的 `0x35854`/`0x360b6`/`0x36228`;(ii) **負向控制**:32 個入口各減 `0x356` 後重跑同一分類器,**32/32 全部變成 `MID_BODY`**,一格巧合重新同步都沒有——沒有這項,「32 格全是入口」會是平凡結論。**另外兩個副產物**:(a) 85..89 不是 artifact 而是**空 handler**(`__STK(4); return;`,86..89 以 tail-merge 共用 85 的尾段),「登記了但什麼都不做」是可用的事實,與「指標是巧合」完全不同;(b) §10.1/§10.2 的語意描述沒有白費,只是**掛錯 event_id**——偏移是常數,每條描述實際屬於「文件位址落在哪一格 handler 體內」的那一格,doc25 §19.4 給了機械換算表。這同時解釋了 §10 自己觀察到卻說不出原因的兩件事:「event 69 與 67 幾乎相同的尾段」是因為兩者**就是同一個函式**(event 61);§10 把 event 84 寫成精緻的 4 回合倒數、把 76 寫成近似 no-op,實際上**兩者對調**。§2241 對 event 62 `0x35898` 的舊疑慮也一併結案:它落在 event 58 的 handler 體內,62 真正的入口是 `0x35bee`。**誠實範圍**:本輪只證明「每格都是函式入口、舊位址錯在哪」,**沒有**以正確入口重新反編譯 32 個 handler 的語意;§10 的描述是從函式中段開始反編譯的,搬到正確 event_id 之後仍可能缺該 handler 的開頭。詳見 doc25 §19。
**2026-09-11 續:doc25 §15 與 doc35 §14 各留的「呼叫端未追」都補完了,而且挖出跳表的真實規模**。`call_scan` 窮舉:`0x25089` 由 `0x250cc`(2 次)與 `0x25757`(2 次)呼叫;`0x361b0` 由 `0x360b6`(3 次)、`0x2548c`(2 次)、`0x33e3c`(1 次)呼叫。**`0x360b6` 正是今天 §19.4 重新歸位的事件 76**,而 §10.1 那段(舊標 event 84)寫的就是「兩次 `0x361b0()`+等待;迴圈 3 次」——本輪用完全不同的方法(全 image `E8` 掃描 + Watcom 序頭回推)數出**恰好 3 個靜態呼叫點且全在 `0x360b6..0x36214`**,是對 §19.4 的獨立佐證。**一個我自己的假陰性**:那幾個父函式用 `refs` 掃絕對參照是零筆,但那是查法的問題——`build_fixups` 只走 obj1 的 fixup 頁,看不到資料段裡的跳表登記;用已知真值驗證:`refs 0x35854`(已知登記在 `0x51c79`)同樣回傳空。改成直接掃 obj2 的 dword(值 = 目標 − `0x10000`)後對照組立刻命中 `0x51c79`,方法才站得住。**結果是這一帶有兩張表**:`0x51b91` **90 格**(index 0..89,不只先前處理的 58..89)全部是入口;`0x51d01` **88 格**是**另一張未記載的表**,也全部是入口;中間的 `0x51cf9`/`0x51cfd` **不是表格**,是游標 x/y 備份變數(寫入端即今天道具路徑的 `0x1be07`/`0x1be11`,各 5 個程式端參照)。這個分界寫成 selftest 檢查 (6):若那兩格也能通過入口判定,「兩張表」就失去依據。第二張表是**指令/行動 dispatch**,`refs 0x51d01` 全 image 只有 2 個 dispatch 點——`0x1541f`(AI 路徑)與 `0x1d479`(玩家 command ring,在 `FUN_0001cff0` 內),兩處結構完全相同(都清三個引數、都緊接 `call 0x1d4f6`);對照之下 `0x51b91` 有 8 個 dispatch 點。**因此兩條殘留的答案**:`0x25089` 由第二張表 index 84/87 的 handler 呼叫,**不是**城鎮選單路徑,若真對應旅館/教會全體恢復也必定經由某個指令 id;`0x361b0` 則在事件 76 與第二張表 index 86/57 的下游。工具與產物已擴成涵蓋兩張表(突變 7/12 → 8/12)。**誠實範圍**:只窮舉了「誰呼叫」,沒有解那六個 handler 的語意,也沒有建立第二張表的 index → 指令名稱對照(需追兩個 dispatch 點的 `eax` 來源,本輪未做)。詳見 doc25 §20。
**2026-09-11 訂正同日前一段的一句話,並補上涵蓋率**:前一段把 `0x51d01` 寫成「另一張**未記載**的表」——**那是錯的**。`tools/verify_findings.py` 的 `FINDINGS` 早有一筆 `584-spell-table`(建於 2026-09-08)登記它。我查了知識庫的 `.md`,沒查工具裡已登記的結論,**與同一段在講的「refs 掃不到資料段」是同一種錯**:查法有盲區,而我把空結果當成事實。**真正的問題不是沒記載,是只驗了前 12 筆**(`probe=12` 是呼叫端設的上限),所以 `findings` 軸每輪報「相符」,卻從來沒有整張表被檢查過——**doc25 §10 那個 `0x356` 偏移能撐五輪,原因就在這裡:`findings` 的滿分是對已登記項目的滿分,不是對主張的涵蓋率**。處置(用既有機制,不新增機制):(1) `verify_findings` 新增 `is_tail_merged_stub()`/`is_handler_entry()` ——先前 `pointer_table_len` 遇到共用尾段殘樁就當「表結束」,`0x51b91` 的殘樁在 index 5,整張 90 格的表會被報成 5 格;兩者**刻意不合併**,因為 `is_function_entry` 的計數 541 是已登記結論 `584-entries`,預設判準維持嚴格,故 `0x524c6` 的 10 不變。(2) 新增 `212-event-table`=90 與 `212-command-table`=88 兩筆涵蓋**整張**表,由 `findings` 軸的路徑(Ghidra 已重定位映像)重新導出,與本輪用「原始檔 + `0x10000`」得到的數字一致——兩條位元組來源不同的路徑互相印證。`findings` 現為 **17/17 相符**。(3) `verify_event_dispatch_table.py` **沒有**改成 import `is_function_entry`:那條路徑走 Ghidra headless(要 JVM、WSL 下不可用),而本工具刻意免 capstone/免 JVM 才能待在 `wsl` 軸;改成 selftest 檢查 (7) **斷言兩套實作對 178 格逐格一致**(實測零分歧,WSL 下乾淨 SKIP)。這比只留一份更強,也是 `verify_findings` 自己對 `image_ref_scan.py` 用的做法。**誠實範圍**:只補上這兩張表的涵蓋率;「知識庫還有多少位址主張沒有任何 finding 列」仍無人測量,`findings` 報的 17/17 依舊是分母未知的滿分。詳見 doc25 §20.5。
**2026-09-11 待辦 30:用 §6.9 的判準往下挖一層,挖到三個「已經被訂正過、卻從未登記」的位址**。`claim_coverage` 報的 954 筆「無位元組證據」是**存量不是待辦**,所以新增 `--triage` 再分一層:**UNREVIEWED 954 → 合法指令邊界 769 / 不在指令邊界 184 / 無法判定 1**。769 那欄多半是文件在談某函式**內部**的位址(呼叫點、分支點),合理;**184 才是候選的位址誤記**。判準只從**所在函式入口單一起點**線性反組譯(與 doc27 §6.3 當年用 Ghidra `getInstructionContaining` 同源),兩個比率都釘進自檢:**誤報 0/150**(已知正確入口)、**召回 10/14**(已登記勘誤位址)。第 (9) 題刻意**不要求 14/14** —— 抓不到的 4 個(`0x27fc9`/`0x320fc`/`0x3453e`/`0x36d98`)是錯位址但**剛好落在合法邊界上**,那是判準的結構性上限,要求滿分只會逼出裝飾性補丁,要求「不得退步」才是對的形狀。**一個被自己的負向控制打掉的強化做法**:原想用**多起點收斂**(從目標前 k byte 各起一次、多數決),直覺上更穩健,實測 14 個已知錯誤位址裡 **5 個**被它判成「是邊界」—— 短視窗從任意偏移起算會湊出自洽但錯誤的解碼,是線性反組譯的經典偽陽性,與既有「線性反組譯會失去對齊」同一件事。已寫進`boundary_from_entry` 的 docstring,不要再試。**挖到的東西**:三個位址**早就被訂正過,只是訂正從未被登記**,所以 citations 棘輪蓋不到、錯誤形式仍在別處被引用 —— `0x354fe`(21 次引用;doc25 L1129「位址更正」;本輪另用 `range` 反組譯獨立確認它落在 `0x354fc push 0xcd` 這條 5-byte 指令的第 3 個 byte;但 doc11 L1229、doc25 L948 仍在用它立論)、`0x35898`(8 次;doc25 L2241 明記落在 `0x35895: FF 74 24 20` 的最後 1 byte)、`0x2a2e8`(7 次;doc32 L747 明記是「舊文件 `0x2a2e8` 的新版對應」→ `0x2ac7d`)。**這是本日第三次遇到同一個形狀**:訂正與被訂正的主張並存於知識庫,中間沒有機制連接。**刻意沒有做的事**:沒有批次登記那一批。「同一行有訂正字樣」這個啟發式有與 citations 工具相同的**行粒度**問題 —— 抽查 8 筆,`0x35ed2`/`0x35d60` 命中的是 `91-worklist.md` L205 那條超長的項目標題(根本不在訂正它們)、`0x35822` 那行的「已證實的勘誤」講的是 **PUSH 順序不是位址**;所以「59 筆已訂正未登記」是**上界**,抽樣約 3 真 / 2 假 / 3 待判。拿未逐筆驗證的前提去餵一個會擋人的機制,錯誤會帶著權威擴散 —— 這正是使用者本日稍早明確制止過的做法。**另外把分流做成工具模式而不是留一份一次性輸出**:184 與 59 與本日稍早那組「275/1044/954」是同一種數字,會隨文件編輯漂移而無人察覺,所以直接做進 `--triage` 並把判準的兩個比率釘進 selftest。**誠實範圍**:(1) 三筆的 `correct_address` 只有 `0x2a2e8`→`0x2ac7d` 是文件直接寫明的,另兩筆要讀完 doc25 §11.7 才能填,本輪未做;(2) 184 只涵蓋 **obj1 的入口主張**,資料段的結構主張仍無分母;(3) 三筆**尚未登記**,所以 citations 棘輪目前仍蓋不到它們。詳見 doc27 §6.10 與 doc98 2026-09-11 續四。
...
**2026-09-10:項目 248「97 組角色名仍未知」是過期數字,而真正開放的是定址不是名字**。本輪先做的是**獨立複驗**而不是相信既有結論:把 `docs/figures/fdtxt000-names-*.png` 三張字模渲染圖重新逐字轉錄一次,**先轉錄、後比對**——玩家名區塊 32/32 與 `character_names_by_id` 相同,單位/首領區塊 55/55 與 `unit_types_by_index` 相同,**合計 87/87 零不一致**。玩家名區塊本身又已對照過 2026-09-07 實機讀出的 13 個名字,所以複驗鏈是「實機名冊 → 玩家名區塊 → 我的判讀方法 → 未知區塊」,不是拿同一份 JSON 自我比對。**結論**:doc49 那句「97 組仍未知(D)、需等 FDFIELD roster 逐圖 RE」出自更早的**對話反推**路線,已被 2026-09-07 直接解 FDTXT.DAT resource 0 的做法取代;那 97 組裡 **36 組(id 32..67)在遊戲裡根本沒有名字條目**(字串索引 33..68 全空,是缺席不是未知),其餘落在已全數轉錄的單位類型/首領段。已在 doc49 附上更正表。**真正還開放的是定址**:`[0x53ad9]` 是共用的字串索引暫存器,全 image 有 11 個 `mov [0x53ad9],eax` 寫入點,而 `fdtxt000_name_table.json` 自己記著「本輪只逐一確認了 `0x2acca` 這一個的偏移」。本輪把 11 個全部解出來,發現**慣例有四種而不是兩種**:`+1` 六個(`0x28d23`/`0x2ab2f` 讀 record[+7] 職業,`0x28fb3`/`0x29193`/`0x2a553` 讀 record[+8] 角色 id,`0x2b496` 讀堆疊引數)、`+0x96` 一個(`0x2acca`,既有記載)、**`+0xb5` 三個(`0x191e3`/`0x1ac8e`/`0x35916`,先前未記為本暫存器的慣例)**、以及 `0x28a07` 的 `add eax, esi`——**偏移是變數**,esi 來源未追,是 11 個裡唯一沒有定值的。`+0xb5` 有**獨立佐證**:`docs/data/item_labels.json` 由另一條不經過 `[0x53ad9]` 的呼叫路徑記載同一基底(`0x184c0 -> 0x15f84([0x53a7d], 0xb5 + item_id, ...)`),兩份互不相干的產物一致。**對 `69..75` 該不該叫 races 這個既有存疑,本輪只給旁證不下定案**:走 `+1` 慣例時 id 68..74 就落在索引 69..75,與 76..127 落在 77..128 是同一個定址空間、同一組寫入點,所以它比較像單位類型表的前段;但沒有任何呼叫端被證明只產生 68..74,所以標籤不改。**同輪更正我自己的兩句話**:(a) 我先前說「項目 409 該標作廢」——不對,它 2026-09-08 就已複核並寫明「本項無殘留」,只是標籤還停在 `D→`;(b) 我盤點 6 個 D 項時是照**第一行**判斷的,而多數 D 項的最後一段才是現況,這與本 session 一再出現的「不完整搜尋」是同一個毛病,只是換成不完整閱讀。
**2026-09-10 續:`0x28a07` 的「變數偏移」被推翻,11 個寫入點全部有定值**。本行前段把 `0x28a07` 的 `add eax, esi` 記為「偏移是變數,esi 來源未追,是 11 個裡唯一沒有定值的」。本輪追完 esi:同一函式(`0x2872b`)內,`0x289bb mov esi, eax` 之後緊接 `0x289c8 cmp esi, 1` / `0x289cb jne 0x2882b` —— **只有 esi==1 才會往下走到 `0x28a05 add eax, esi`**,所以那個 `add` 就是 `+1`。**單一入口的證明分兩層**:(a) 函式內反組譯掃描,跳入保證區間 `[0x289cd, 0x28a08)` 的分支 0 個;(b) 再做**全影像位元組掃描**(rel8 的 `EB`/`70..7F`、`E9` rel32、`0F 8x` rel32),同樣 0 個 —— 用位元組掃是因為線性反組譯在整段 code 上會失去對齊(本日已被咬兩次),而這種掃描的偽陽性只會**增加**命中數,所以「0 個」是強結論。**歸屬**:`record[+7] + 1` 正是 `fdtxt000_name_table.json` 的 `_how_it_is_used.index_convention` 已記載的三個 `+1` 來源之一(該處寫「class change uses record[+7]+1」),與 `0x28d23`/`0x2ab2f` 同類,**不是第五種慣例**。慣例仍是四種,但 `+1` 由 6 個變成 **7 個**寫入點。**我自己的錯誤與教訓**:先前只看了 `add eax, esi` 這一條指令就判定「偏移是變數」——**看到暫存器就當變數,等於把「我沒追」寫成「它是變數」**。正確做法是先找該指令的支配條件,再證明保證區間單一入口。已同步更正 `fdtxt000_name_table.json` 的 writers 條目與 finding,並在該檔留下 `correction_2026_09_10` 說明。
...
**2026-09-11 措辭收尾**:本項 2026-09-07 的複核結論已經是終局——RE 面結論解出、所稱「剩餘」的工作對象(remake Go 函式)隨 `remake/` 於 2026-09-02 移除而不存在。本段把它改用本檔既有的正規措辭記一次:**範圍消失**,與 359/360 同類。無待辦工作;若未來重啟 remake 須連同該次重啟重新評估,不可直接沿用本段。
...
**2026-09-10 更正本項的一句描述**:本項先前記「SFX 交錯播放(`FUN_0002eb9f`,依 `id!=9` 門檻)」。本輪反組譯 `0x2eb9f` 本體(`0x2eb9f..0x2ebe0`,20 條指令)後確認**它不是 SFX 函式**:它做的是 `entry = container + index*4` -> `ptr = container + dword[entry+8]` -> 讀 `word[ptr]`/`word[ptr+2]` 當寬高 -> `0x4e98d(ptr+9, w, h, dest, stride, transparent)`,即**依索引取幀並貼圖**的 frame blit,整支函式沒有任何聲音相關呼叫。**本項觀察到的 `id!=9` 門檻不受影響**——那是 `0x2ff01` 那個**呼叫端**的條件,本輪沒有否證它;被否證的只是「`0x2eb9f` 是 SFX 播放」這個對**函式本身**的描述。同一輪也更正了 doc32 §935 的「疑似文字/數字渲染」(同樣否證),並把三方仲裁寫成 doc35 §13.3;doc25 §2168 的「逐幀演出迴圈的繪製呼叫」是三者中唯一正確的。**這個矛盾是怎麼被發現的**:命名 `0x24336`(全螢幕逐幀動畫演出)時,它的呼叫形狀 `(res, esi, [0x53a49], 0x140, -1)` 與「SFX 播放」不相容,於是先把三種記載並列釘出來、同輪再反組譯本體仲裁,而不是順手挑一個相信。
...
關閉慣例不影響此關閉判定。
**2026-09-10 本項殘留解決(靜態,全程離線)**:先前結論停在「AI 演出路徑 `FUN_00015055` 沒有玩家路徑那個 type-0x17 legality 閘門」,並自陳「AI 上游的道具選擇評分是否會選到這類道具是另一個問題,本輪未追」。本輪把上游追完。**(1) 選擇器定位**:全影像掃 E8 rel32 找 `0x15055` 的呼叫端,只有 `0x15006`/`0x15039`,兩者都在 `0x14ef0` 內。`0x14ef0` 是三選一的行動仲裁:開頭依序呼叫 `0x14237`/`0x1598a`/`0x1567e` 算出三個分數寫進 `[0x53c4f]`/`[0x53c23]`/`[0x53c33]`(反組譯輸出省略 obj2 的 0x50000 基底),三者皆 <6 就 `return 0` 不行動,否則取最大者;**`0x15055`(用道具)恰好在 `[0x53c33]` 勝出時被選中**,而 `[0x53c33]` 由 `0x1567e` 寫入。**(2) 評分器內容**:`0x1567e` 以 `0x1b8a6(unit)` 取得可用道具數後逐格掃 `unit[+0xb+slot*2]` 取道具 id,呼叫 `0x4e8bc(id)` 取效果列(逐指令確認就是 `0x602ad + id*0x17`,stride 23)。接著只做兩件事:`row[+0x10]` 若 `>0xf` 就**夾成 1**並以 dh 標記(不是排除),然後 `row[+0x0d] == 0` 則 `je` 跳去下一格。**整個評分器對道具 type 沒有任何比較**——與演出路徑一致,AI 側確實不存在 legality 閘門。**(3) 但實際選不到**:欄位對映先交叉驗證——`item.json` 的記錄起點 `0x792c0` 比效果列 `0x792c1` 早一個 byte,故 `effect_row = record + 1`,實測 `effect_row[0] == item.type` 在 **215/215 筆全數成立**。據此聯結兩表:type==23 的道具共 12 個(id 144~155),**其 `row[+0x0d]` 全部為 0**,因此全數被 `0x15744` 那道唯一過濾跳過。通過該過濾的 33 個道具分佈在 type {1,3,4,5,6,12,32,34},不含 23。**結論**:AI 選不到 type-23 道具,原因不是有閘門,而是這 12 個道具的 `+0x0d` 皆為 0。**誠實範圍**:這是靜態資料層結論,`+0x0d` 的完整語意尚未命名(只確定它是 AI 掃描的唯一過濾欄位);另外「敵方名冊是否曾持有 type-23 道具」是另一個問題,但因過濾結果已成定局而不影響本判定。順帶為 1354 補上兩個欄位的功能定位:`+0x0d`=AI 可選旗標、`+0x10`=效果型態(值域實測只有 {0,1,2,3,4,31})。
**2026-09-11 前一段的結論反了,原因是查錯欄位——訂正**:2026-09-10 那段把「type-0x17 legality 閘門」理解成 `item.type == 23`,據此查出 type==23 的 12 個道具(id 144~155)`row[+0x0d]` 全為 0、會被 AI 評分器唯一的過濾跳過,結論寫成「AI 選不到 type-23 道具」。**閘門比的不是 `item.type`,是 `row[+0x0d]`**。玩家路徑 `FUN_0001bbdc` 內逐指令為 `0x1bd1a movzx eax, byte ptr [eax+0xd]` / `0x1bd1e cmp eax, 0x17`(以及 `0x1bdad` 第二次同樣比較),`eax` 來自 `0x4e8bc(id)` 的回傳,即效果列本身;本項自己的舊文也寫著「玩家路徑 gate 用的三個欄位」包含 `item[+0xd]`,是我上一輪讀成了 type。**訂正後的事實**:全 215 列中 `row[+0x0d] == 0x17` 的**只有 id 101 傳送法杖**(它的 `item.type` 是 6,不是 23);它的 `+0x0d = 23 != 0`,因此**通過**評分器那道唯一過濾——結論與上一輪相反,**AI 側確實選得到玩家路徑特別設閘門的那個道具**。閘門內容也一併釘死:`0x1bdad` 之後檢查**使用者自己**(`[0x53a45] + actorIdx*0x50`)的 `byte[+8] == 0x18` 且 `word[+0x46](max MP) >= 0x14`,不滿足就把目標索引設 -1 中止(`+8` 的語意本輪未定,不命名;線索見 doc03 同日一節)。**誠實範圍**:「敵方名冊是否真的帶有 id 101」判定為**不可判定並就此結案**,不是待辦——`map*_units.json` 隨 `remake/` 一併移除,repo 內已無該資料來源,且不用已提交資料補弱證據;這一點上一段也已寫明不影響本判定。上一輪對兩個欄位的功能定位也一併訂正:`+0x0d` 不是「AI 可選旗標」而是**效果型態**(AI 只是測它 `!=0`;真正做 switch 分派的是套用端 `0x20cbd`),`+0x10` 不是「效果型態」而是**傳給 `0x14818` 的範圍形狀碼**——上一輪自己觀察到的「AI 側 `>0xf` 會被夾成 1」正是 `0x14818` 那套 `<0x10`/`>=0x10` 形狀編碼的另一半。完整論證見 doc27 §6.7 與 doc32 同日一節。
...
**2026-09-11 措辭收尾**:同 447。原版 dataflow 本身 doc56 已標「已驗」,所稱「剩餘」純粹是 remake wiring 表格,對象隨 `remake/` 移除而不存在。正規措辭:**範圍消失**。無待辦工作;若未來重啟 remake 須重新評估。
**2026-09-11:§6.3 的結論經獨立複驗,五項全部成立;並釐清它為何對而 doc25 §10 為何錯**。起因是本輪原本要拿 §6.3 的「`0x2a6bd`/`0x276ec` 不是有效指令邊界」當作「已證錯位址登記表」的種子,**使用者指出那個前提本身沒被複驗過**——doc25 §10/§11.7 的教訓正是一個寫得很有信心的舊結論可能整輪建立在錯的前提上,**把未複驗的結論拿去建阻擋機制等於替可能的錯誤加上權威**。複驗方法刻意與 §6.3 不同:只讀原始位元組(`le_xref` + `load_code` 自建全 image Watcom 序頭集合),不呼叫 Ghidra、不看 §6.3 的推論。**結果**:`0x2a6bb` 實測 `8b 44 24 24`(4 bytes,`0x2a6bd` 落在第 3 個 byte)、`0x276e9` 實測 `3b 05 57 3c 00 00`(6 bytes,`0x276ec` 落在第 4 個 byte)、`0x2a694` 是序頭且下一序頭 `0x2a857` → body 451 bytes、`0x2ff01` 是序頭且 3876 bytes、`0x2cf30` 是序頭且 2269 bytes ——**五項與 §6.3 逐字相符**。另補一項 §6.3 沒做、但 §19 之後必須做的檢查:`0x2a6bd`/`0x276ec` 會不會也是未重定位值?`0x3a6bd`/`0x376ec` 都不是序頭且落在無關函式深處,**重定位假說排除**。旁證:`0x2ff01` 的 3876 bytes 是全 image 最大的函式(第二名 2291),「大型 dispatcher」不是修辭。**方法論結論**:§6.3 與 §10 用同一個 Ghidra,差別在 §6.3 用的 `getInstructionAt`/`getFunctionContaining` **能回傳否定**,而 §10 用的 `createFunction` **在任何位址都會成功**——它的成功不帶資訊。§10 其實收到過訊號(自記「約半數出現 `unaff_EBX`/`halt_baddata`」),但把訊號歸因到「邊界猜測法不成立」而非「入口位址可能是錯的」,那正是它接下來五輪都沒回頭懷疑位址欄的原因。**可轉移判準:對「這個位址是不是函式」只採用能回傳否定的機制。** 另記一筆尚未處理的量測:以本節複驗過的 2 個 + doc25 §19 的 25 個共 27 個已證錯位址掃知識庫,**現存引用 275 次、分布 13 個檔案**(`0x2a6bd` 89、`0x276ec` 42、`0x354fe` 41);§6.3 當年「未回頭修改既有引用(超出本輪範圍)」的代價因此可量化。**依使用者指示本輪只記錄、不動那 275 次引用,也不建阻擋機制**;該 275 是一次性腳本輸出,尚未成為可重生的登記表,要動之前應先補上。詳見 doc27 §6.8。
**2026-09-11 待辦 25:把 `known_address_errata.json` 接上驗證軸 —— 它從 2026-08-20 就存在,但十條軸沒有一條讀它**。所以「已經證實錯的位址還有沒有人在拿它立論」這個問題,在今天以前**沒有任何機制在問**;其他每一條軸問的都是「文件現在主張的東西對不對」。最能說明問題的例子就在本檔自己身上:**第 1833 行**是位址勘誤總註記、明寫 `0x2a6bd` 無效,而**第 441 行**(本項555 的標題)仍寫著「原版 dataflow(`0x2A6BD`/…)本身無缺口」—— 同一份檔案、相距 1392 行,中間沒有任何東西把前者接到後者。新增 `tools/verify_address_citations.py`(第十一軸 `citations`,自檢 9 項全 PASS,突變 4/12 DISCRIMINATING,Windows 與 WSL python3 各跑一次)。**先修掉方法本身**:doc27 §6.8.4 那個「275 次引用」是一次性腳本的輸出,同日換 pattern 重量得到 1044、再換一次 954 —— 三個都是拿 regex 掃 `wrong_address` 這個**自由書寫的散文欄位**,實測兩類偽陽性:`0x2bce5` 掃出 628 筆(佔 66%),但該筆勘誤正文明講位址本身沒錯、錯的只是「0x2545d 直接 CALL 它」這條邊;`0x2ff01` 掃出 57 筆,它根本是**正確**位址,只是出現在另一筆勘誤的說明文字裡。所以每筆勘誤新增機器可讀的 `citation_check` 區塊(`mode`=address/edge/none+`flag`/`edges`/`reason`/`baseline`,附加欄位,`query_verified_address.py` 讀的欄位完全未動),**缺這個區塊會讓工具失敗,而且失敗時不印通過訊息**。豁免判準兩條都機械:同行帶正確位址、或同行有勘誤標記;`edge` 模式另要求兩位址相距 80 字元內(這個知識庫有大量 300~550 字元長行,把五六個位址當**各自獨立的事實**並列,doc31 L274 相距 30 字元是真的那條邊、doc91 L4373 相距123 字元是兩個並列事實 —— 兩行都以原文釘進 selftest,並附「把上限放到 10000 那行必須回來」的參數負向控制)。**真實數字:ARGUED 263 / EXEMPT 251**,雙向棘輪(超標與已還未更新基準線都失敗),基準線寫在 errata 檔內。三次真注入全部如預期(新增未訂正引用→EXIT=1、同行帶訂正→EXIT=0、拿掉 citation_check→EXIT=1)。**兩個順帶量出來的東西**:(a) errata #12(整備 UI 八位址)ARGUED=**0**,證明 2026-08-26 那輪是逐點就地加註的,豁免判準不是恆為 ARGUED;(b) **registry 自己的 `still_pending` 有一句話被量出來不準** —— #0 寫著 doc13/37/56「僅補了簡短訂正註記」,實測這三份共 74 次引用、其中 **60 次身上沒有任何訂正**(13: 10/14、37: 11/13、56: 39/58),連「已經補過註記」這句話本身在今天以前也沒有東西驗過。**寫文件時工具當場抓到我自己**:doc98 新節引用了 11 個死位址而未標註,閘門 EXIT=1,逐行補上標記後才過 —— 沒有調基準線繞過。**復發紀錄**:第一次跑注入時寫成 `python ... | tail -6`,拿到的 `$?` 是 `tail` 的 0,正是同一天待辦 20 才補正過的錯,當場改成輸出到檔案再取 `$?`。**誠實範圍**:(1) 判準是行粒度,`address` 模式分不出同一長行內的論據與訂正;(2) 263 是**存量不是待改清單**,本輪沒有改任何一個引用點,只是讓它們可量測且只能變少;(3) 本工具驗的是「引用有沒有帶訂正」,**不是「勘誤本身對不對」**;(4) 「知識庫還有多少位址主張完全沒有 finding 覆蓋」依舊沒人量過,`findings` 的17/17 仍是分母未知的滿分。詳見 doc98 2026-09-11 節。
**2026-09-11 待辦 31:位址訂正的源頭閘門 —— 回答「這是工具的問題還是機制的問題」**。上一輪抽查 59 筆「已訂正未登記」有 2 筆誤配,使用者問這是工具還是機制的問題。**主要是機制**,而且兩筆誤配是兩種不同的失敗:`0x35ed2`/`0x35d60` 命中的是一條 4000 字元的項目標題(**行粒度,工具可修**);`0x35822` 那行的訂正措辭**確實**指向它,但被訂正的是 handler export 的 **PUSH 順序**不是位址值(**再完美的切句也會命中,工具修不掉**)。59 筆裡 20 筆屬前者、39 筆是短行(中位數 96 字元)屬後者。再往上一層量:知識庫中「同時含訂正措辭與位址」的行有 **110 行 / 20 份文件 / 涉及 250 個相異位址**,登記表只有 **15 筆 / 22 個位址,約 8%** —— 九成以上的位址訂正從來只以散文存在,因為流程裡**沒有「發現位址錯了就登記」那一步**;登記表是專案開始兩個月後才補建的,補完只能對散文做考古,而那正是同日早上診斷出 `wrong_address` 不可機器消費的同一件事。**修法是機制不是掃描器**:`verify_address_citations.py --diff`,形狀沿用 `audit_evidence_provenance.py --diff` —— 新增一行「訂正措辭 + 位址」,該行的位址就必須已在登記表裡,否則用 `--mark-correction` 明確宣告被訂正的不是位址本身並寫理由(宣告存 `docs/data/correction_line_reviews.json`,key 為 `(file, excerpt_sha1)`)。**它不處理存量,只讓存量停止增長**。未追蹤檔案處理與摘要雜湊直接借 `audit_evidence_provenance` 的 `_untracked_as_diff()`/`_sha()`,不重寫(那支已踩過全新文件整份繞過閘門的洞)。**四條路真注入**:未登記位址→EXIT=1、已登記位址→0、PUSH 順序宣告前→1、宣告後→0;注入後已還原並移除宣告表的測試殘留。**突變測試抓到三個缺口**(可達 1/4 → 4/4):`elif spec.mode == "edge"` 改 `!=` 讓 edge 模式在真正的 `scan()` 裡整個失效而第 (5) 題走 `scan_text()` 感覺不到(新增第 14 題);距離算術的索引因地面真相離門檻太遠而逃掉(新增第 13 題,造門檻兩側案例);同一索引只有**反序**才分得出來(補反序案例)。第 13 題第一版是**我自己的夾具算錯**(反引號讓間距多 2),現在會先印出夾具實測 gap 並斷言。**已寫進 `AGENTS.md` 的提交政策**,與 `audit_evidence_provenance.py --diff` 並列 —— 沒寫進提交例行的閘門等於沒有閘門。**誠實範圍**:(1) 110 行存量**完全沒處理**;(2) `CORRECTION_WORDS` 是詞彙層,不含訂正措辭的訂正(例如只寫「實際位址是 X」而用了別的說法)會漏;(3) 行粒度問題今天已咬了三次(citations 的 edge 模式、claim_coverage 的散文掃描、這次),本輪**沒有**抽出共用的分句輔助函式。詳見 doc98 2026-09-11「位址訂正的源頭閘門」節。
...
**2026-09-11 待辦 29:量出 `findings` 軸從來沒有的分母,上線第一天就抓到一筆 `confidence: verified` 的錯**。`verify_findings.py` 每輪報 `17/17 相符`,但 **17 是「有人記得登記的結論」數,不是文件主張的數量** —— doc25 §10 那整欄 `0x356` 偏移能活五輪、中間還有一輪「仔細複驗」產出更有信心的錯答案,原因就是它從來沒被登記過。同日我已三次把這件事寫成「誠實範圍」卻沒動它,這是第四次。新增 `tools/verify_address_claim_coverage.py`(第十二軸 `claim_coverage`,自檢 7 項全 PASS)。**量測結果:知識庫宣稱為函式入口的相異 obj1 位址 1368 個,其中拿得出位元組證據的 405(29%)、已登記為勘誤 12、無訊號也沒人登記過的 951。**判準是三個互相獨立、只讀 EXE 位元組的訊號(Watcom 序頭 541 / 直接 E8 CALL 目標 849 / fixup 目標 634)。**一定要三個**:541 是「需要堆疊探測的函式」不是全部函式,實測有 491 個位址**只有**「直接 CALL」一個訊號,單用序頭判準會全部誤報。雙向控制都是實測的:errata 落在 obj1 的13 個位址 **13/13 全部無訊號**(正向);`0x4ebe3`(40 個呼叫端)必須有訊號、`0x4e893`(0 個呼叫端)必須無訊號(負向配對)。**刻意不把 951 當待辦**——「無訊號」不等於「錯」(談函式內部位址、改基準前的舊版位址、只透過 computed call 抵達的程式碼),本專案已量過一次天真判準會產生 2836 筆偽陽性;所以拆成 KNOWN_ERRATUM 與 UNREVIEWED,只對後者設雙向棘輪。**上線第一天的收穫**:`0x4e893` 三個訊號全無,而它在 `verified_addresses.json` 是 `confidence: verified`。逐位元組覆核 `0x1c7ed` 的 `E8` disp32=0x323f1,真正目標是 **`0x4ebe3`**(40 個呼叫端、起頭 `33 c0`),差 `0x350`,與 `0x2a6bd`/`0x276ec` 同屬 2026-08-19 那批手動反組譯誤記。順帶訂正兩處語意:`0x4ebe3` 是 **PRNG 本身**(`seed = rol(seed + 0x9014, 3)`,種子 word `[0x627b8]`),**不是**「%100<threshold 的檢查函式」——取模與比較**內聯在 `0x1c7f4..0x1c802`**(`idiv 100`;`cmp edx,[esp+0x70]`;`jge`,命中條件是餘數 < threshold,未命中回 `eax=0`)。**最值得記的是正確答案早就在知識庫裡**:doc13 L509/L511 寫的是「`rand()%100`(`CALL 0x4EBE3`)」—— 對的;而 doc27/doc56/`verified_addresses.json` 一路用 `0x4e893`,共 20 次、跨 7 份文件,兩種形式並存從未被接起來。這與同日 `91-worklist.md` L1833(勘誤)vs L441(仍以錯誤位址立論)是同一形狀,而這次是**機械掃出來的**。已訂正 `verified_addresses.json` 兩筆、登記第 15 筆勘誤、寫入 doc27 §6.9。**本輪自己踩到兩個坑,都被專案自己的閘門擋下**:(1) 又用 Python heredoc 改 .py 寫成 CRLF(上一個 commit 才把這教訓寫成記憶,下一個動作就重犯),hygiene 的 shebang 規則擋下;(2) 往 `hygiene_baseline.json` 加豁免時順手 `sort()`,audit 的「行尾整檔翻轉」檢查報出「git 看到 672 行變動、忽略空白後只有 74 行」,查出該檔用 `indent=1` 且排序本身就是不該做的改動,改成純附加後 diff 變成 6 insertions / 0 deletions。**誠實範圍**:(1) 951 是**存量不是待改清單**,本輪沒有改任何一個引用點;(2) `0x4ebe3` 的 40 個呼叫端**沒有逐一查過語意**,本輪只證明它是 `0x1c7ed` 的被呼叫者且是合法入口;(3) 種子 `[0x627b8]` 的初始化時機與其他寫入點未追;(4) 判準只涵蓋 obj1 的**入口**主張,資料段的結構主張仍無分母。詳見 doc27 §6.9 與 doc98 2026-09-11 節。
...
- **2026-09-08 續二十:把檔頭註解裡的宣稱變成每次重跑的對照**。`export_acting_resources.py` 的註解記載:舊常數 `0x565D8`/`0x53E00` 只對應**已遺失的 357074-byte 版本**,新常數是靠 resource 0 與 99 的位元組簽章重新解出來的,「106 筆全部乾淨解析」。selftest 把那個宣稱的**兩半都驗**:(1) 現行常數必須 **106/106 成功**(總 350 個 frame);(2) **舊常數在現行 EXE 上必須大量失敗**——實測只成功 20/106。少了第 (2) 題,第 (1) 題無法分辨「常數是對的」與「這段程式碼寬鬆到什麼常數都能過」,而那正是本輪在 `dump_chapter_beats` 上踩過的同一類版本漂移。另驗 read_u32 的三種越界與 parse_resource 的兩條拒絕路徑。突變 DISCRIMINATING。
- **2026-09-08 續二十一:兩份獨立維護、講同一件事的資料**。`export_engine_assets.MOVE_CODE_TO_WALK_COST`(硬編)與 `docs/data/exe_tables/terrain.json` 的 `move_code_meaning`(已提交資料)描述同一組地形 move code。它們一旦不同步不會報錯,只會讓地圖的可通行判定悄悄變成另一套規則。selftest 驗兩層:(1) 兩邊涵蓋的 code 集合必須完全相同(實測皆 0-5);(2) **語意一致**——被標「不可移動」的 code 必須對應 BLOCKED_COST 且反之亦然(實測皆 {1,5})。第 (3) 題針對另一個風險:`.get(code, 1)` 對未收錄的 code 會**靜默當成可通行**,所以掃全部 FDSHAP 地形控制表(1200B 的那些),要求實際出現的 code 全在表內——實測 {0:4308, 1:1393, 2:246, 5:53},未收錄 0 個。另有手算(flags 取 byte0、costs 查表)與非恆真控制。突變 DISCRIMINATING。
- **2026-09-08 續二十二:偵測器的盲點——`print(__doc__)` 這條路徑**。修 `disasm_le` 時發現它無參數執行會 cp950 崩潰(docstring 含 `↔`),但 hygiene 的 `console` 規則報 0 筆違規:`verify_docs_match_cli.console_encoding_risks()` **只看 print 的字面字串**,看不到 `print(__doc__)` 加 docstring 內容這條路徑。掃全 repo 找到 2 支中招(`disasm_le.py`/`le_xref.py`),都已修。新增 `docstring_console_risk()` 並接進 `console` 規則,配對控制四題:**用修正前的真實形狀**當正對照(不是合成案例)、已 reconfigure 不算、純 ASCII docstring 不算、沒有 print(__doc__) 不算。這是本 repo 已修過 15 支的同一個 cp950 類別,只是換了一條路徑進來。
- **2026-09-08 續二十三:釘住一個已經被拿來下結論的計數**。`export_story_index_map.count_logical_utterances` 不是普通函式——2026-08-31 正是用它判定 FDTXT_032/033 屬於 `ch00_meadow`/`ch00_palace`(**40/40、41/41 完全吻合**),而 ch32.json/ch33.json 原本宣稱的歸屬以 `utterance_count_mismatch` 被否決。那個結論至今生效,所以計數一旦飄掉、結論就失去依據。selftest 第 (1) 題讓它每次重跑,實測 40/41 相符。第 (2) 題驗 `parse_fdtxt_strings` docstring 的宣稱(「malformed offset table raises ValueError instead of silently manufacturing a different string layout」)——4 種壞 offset table 全部確實丟 ValueError。另有終止符不變量(前 20 個資源的字串都不含 0xFFFF)與非恆真控制(FDTXT_026/032/033 算出 63/40/41,三個互異且都 >0)。突變 DISCRIMINATING。
- **2026-09-08 續二十四:把 doc36 第 12 輪的家族結論變成回歸檢查**。`export_sfx.py` 註解記載:action_id 0-9 經 `FUN_0002ff01`(0x2ff01)用 linear `0x0526bc` 起的全域表動態載入,對應 FDOTHER.DAT 的 **#82-90**,「逐一解包驗證皆為合法巢狀 LLLLLL PCM 容器,sub0 逐位元組相同(共用揮擊音)」。實測完全屬實:**9/9 都是巢狀容器,sub0 相異雜湊數 = 1**(6561 bytes)。第 (2) 題釘住另一半結論——與 `--battle` 候選家族「相鄰但不同,僅 #88 重疊」,交集實測恰為 {88}。第 (5) 題是**非恆真控制**:必須在家族外找到 sub0 不同的資源(實測 #7),否則「全部相同」這個比對可能只是因為所有容器的 sub0 都一樣、根本沒有鑑別力。另釘住一個路徑地雷:`SRC` 指向 `extracted/raw/FDOTHER/FDOTHER_031.bin`(14 個子資源),而索引常數是對 **FDOTHER.DAT 頂層**(104 個)的——兩者容器不同,拿 SRC 去套索引會 IndexError,我寫這個 selftest 時就先踩了一次。突變 DISCRIMINATING。
- **2026-09-08 續二十五:位元順序錯了不會崩,只會讓整張字模翻轉**。`font_grid.render_glyph` 把每列 2 bytes 當 16 個位元、高位在左展開。這種順序錯了不報錯,只會讓字模左右翻轉或錯位——而字模正是拿來人工判讀建 glyph→Unicode 對照表的,錯了會一路錯進對照表。所以第 (1) 題用**手算點陣**驗:餵 `0x8001`/`0x0180` 兩列,必須畫出 `#..............#` 與 `.......##.......`,實測相符。第 (3) 題釘住兩個已記錄的結論:字型檔(FDOTHER #4)**1824 個字模**;以及 **glyph 64(名稱表的「塞」)與 glyph 904(對白裡的「賽」)點陣在 32 bytes 中差 25 bytes**——證明 glyph_map 沒錯,是原版真的用了兩個字。兩個數字都完全吻合。另有越界字模須回全黑(不崩、不讀到鄰格)與非恆真控制(自比差 0、兩者都非空)。突變 DISCRIMINATING。
- **2026-09-08 續二十六:公式改壞不會報錯,只會讓 1818 個單位一起偏掉**。`patch_units_ap_dp_mv` 的寫入側(remake/assets 的 mapN_units.json)已隨 remake/ 移除,但它依賴的 `export_units.native_*_for_raw_unit_key()` 是 2026-08-31 從 constructor `FUN_00010c50`(0x10d7f..0x10e23)完整反組譯推導的公式,而 doc03 用 ch24 那隻 **LV14 惡魔**(`record=[5,26,40,0,5,30,18,6,6,180]`)手算驗過:**AP=30×14=420、DP=18×14=252、MV=6(flat)、DX=6×14=84**。那組數字是這些公式唯一的獨立錨點——公式一旦被改壞不會有任何錯誤訊息,只會讓全部單位的數值一起偏掉(當初就是這樣:舊 flat 值與正確值的不同比例高達 98%/98%/87%)。selftest 4 項:錨點 record 必須唯一命中(實測 high_class index 41)、**四值精確回歸**(全部相符)、公式形狀(AP/DP/DX 在 LV 1/7/14/30 都是 growth×level,MV 為 flat)、非恆真控制(前 8 個單位的 LV14 AP 有 5 種相異值,證明真的有查表)。第 (5) 項刻意印出「寫入側沒有目標可寫、不在檢查範圍」而非默默略過。突變 DISCRIMINATING。
- **2026-09-08 續二十七:把當初的 bug 本身釘成對照**。`patch_units_hit_ev` 的 selftest 釘住同一隻 ch24 LV14 惡魔的 HIT/EV 錨點:DX=record[7]×14=84,出場 inventory 前兩格 [81,183] 依 `spawn_equipped_item_ids()` 判定已裝備(item81 hit=110/ev=0、item183 hit=0/ev=0,與 item.json 逐項相符),故 **hit=84+110=194、ev=84+0=84** —— 實測精確吻合。**第 (3) 題最有價值**:它把修正前的錯誤值也釘進來——用舊的 flat `dx=4` 重算必須得到 `(114, 4)`,證明那個舊值確實來自 `base_stats()` 那個 race/cls 對錯列、又沒乘等級的 dx,而不是別的原因。少了這一題,第 (2) 題無法分辨「公式對」與「湊巧算出 194」。另有線性形狀(5 個 base_dx 都符合 (dx+110, dx))與空裝備控制(無裝備時加成必須為 0,得 (84,84))。當初這個修正讓跨 30 張圖 1826 個單位有 **98.7%** 的 hit/ev 改變。突變 DISCRIMINATING。
- **2026-09-08 續二十八:一支「偵測解包過期」的工具,在最該起作用的情況下會給錯答案**。`verify_dat_extraction_freshness.py` 原本用 `sorted(dir.glob('*.bin'))` 再 `existing[idx]`——**按排序位置取檔**,等於假設「第 idx 個檔就是編號 idx」。目前 FDOTHER 恰好 0..103 連續所以看不出來,但只要**少一個檔**,它後面每一個都會往前錯一格。而「解包過期」最常見的形式正是缺檔或多檔——也就是說,這支工具在它唯一的用途上會失效。已改成用檔名編號查表(`index_extracted()`),並把比對邏輯抽成可測的 `compare()`。selftest 5 項,重點是**兩種負向**:把某個已解包檔案改大小(idx 7:live 23377 vs extracted 23393)、把某個移走(idx 9),兩者判定都必須翻成不符——一個永遠回「相符」的檢查器會通過任何只驗正向的測試,而它的用途正是在過期時發出警告。第 (4) 題是錯位 bug 的回歸:移走 idx 9 之後,idx 10/11/50/103 仍必須各自對到自己的檔案。突變 DISCRIMINATING。
- **2026-09-08 續二十九:分錯的後果是憑空生出一個 sfx index**。`trace_item_sfx_dispatch._classify_operand` 決定一個 push 是「常數 sfx index」還是「執行期才知道的值」——分錯不會崩,只會把暫存器當成常數(憑空生出索引)或把立即數當成暫存器(漏掉已知常數)。selftest 逐一釘住 capstone 實際會產生的兩類 operand(`0x11`/`1`/`0x0` vs `eax`/`dword ptr [eax + 0x10]`/`word ptr [ebp - 8]`)、空白處理、以及不完整輸入(`0x`/空字串/`ptr`)必須落到 register_or_memory 而非丟例外。第 (4) 題**刻意記錄一個已知的寬鬆處**而不是假裝它不存在:`int(operand, 0)` 會接受 Python 的底線分隔數字(`'1_0'` → 10)與二/八進位字面(`'0b101'` → 5)。capstone 不產生這些寫法所以實務上碰不到,但這代表分類器接受的比它該接受的寬——寫進測試,以免日後有人把它當成嚴格的 x86 立即數解析器來用,也讓行為若改變會被看見。突變 DISCRIMINATING。
- **2026-09-08 續三十:live→native 換算錯了會讓整批位址一起偏掉**。`dosbox_exec_trace_analyze.parse_trace` 把 live 追蹤的 `CS:EIP` 換算回 native 位址(`native = live_eip - delta`),而那些位址正是拿去比對 Ghidra 反組譯、判斷「哪段程式碼真的被執行到」的依據——換算錯不會報錯,只會讓整批一起偏掉。selftest 第 (1) 題用**手算**驗:`0170:001AC000` 減 `0x19C000` 必須得到 `0x10000`。第 (2)(3) 題釘住兩個容易被忽略的靜默丟棄:CS 不符的行(但仍要計入 CS 統計,那是判斷 segment 分布用的)、以及換算後為負/無冒號/EIP 非十六進位的行。那些跳過是對的行為,但必須是**刻意**的。第 (4) 題把預設常數釘住(`delta=0x19C000`、`CS=0170`,與 live harness 的既有記錄一致——資料選擇器 0178、程式碼 0170)。第 (5) 題非恆真控制:換一個 delta 必須算出不同的 native,證明 delta 真的有參與運算。順帶把 `trace_file`/`--out-dir` 改成可選,否則 `--selftest` 連 argparse 都過不了。突變 DISCRIMINATING。
- **2026-09-08 續三十一:把 frame size 當成參數會得出相反結論**。`audit_global_writers._leading_pushes` 的輸出是拿來判斷「這個寫入者函式會不會讀棧上引數」的原料——也就是「這個全域是不是每次都寫同樣的值」這個問題的答案。它有一個容易誤讀的地方,已明確釘住:Watcom 函式的真入口是 `push <frame>; call 0x3702f`,所以**從真入口起收集,第一個 push 是 stack-check 的 frame size 而不是參數**(緊接的 call 會中斷收集,只會拿到那一個);要看暫存器保存序列必須從 call 之後起算(得 `['ebx','esi']`,被 sub 中斷)。搞錯這個區別會把 frame size 當成形式參數,進而對該工具唯一的用途得出相反結論。selftest 6 項,另含 max_pushes 封頂、輸入亂序不變性(函式自己會排序)、兩種邊界回空、以及**非恆真控制**(不同起點必須給出不同結果,否則 entry 參數根本沒作用)。順帶把 `--address` 改成可選,否則 --selftest 過不了 argparse。突變 DISCRIMINATING。
**2026-09-09:兩支「靜默產出錯東西」的工具**。`extract_maps.py` 的 docstring 寫的配對規則是「地圖 N 用 FDSHAP 解包序 2N」,程式碼實際用的卻是 `big = [f for f in shp if size > 2000]` 再取 `big[m]`——兩個不同判準。目前碰巧一致(33 個 >2000 的檔剛好就是偶數序 0..64,而且門檻很寬:最小 tileset 43564 bytes vs 最大非 tileset 1600),但**只要 FDSHAP 少解包出一個資源,後面每張圖都會整體位移**。實測拿掉 FDSHAP_020:map10 拿 FDSHAP_022 的 tileset 靜默畫完,**tile 索引越界格數 0/1575**、一個錯都不報,圖整張是錯的卻看起來成功;map32 則因 `m >= len(big)` 直接被跳過。而且越界檢查本身**只能抓到約 1/3 的錯配對**(相鄰偏移實測:2N 失敗 0/33、2N+2 失敗 11/33、2N-2 失敗 12/33),靠它兜底不夠。已改成依檔名序號查表(`index_resources`/`tileset_path`),缺哪個資源就只讓那一張圖拋錯。同類 bug:verify_dat_extraction_freshness(2026-09-08)。6 項 --selftest:4 個各自獨立記錄過且承重的尺寸錨點(map0 24×24 doc25 §7.6 反組譯戰場重繪鏈、map1 27×21 doc50 PAN 像素推導、map9 31×45 doc05 §1 ch10 資產、map32 18×51 doc23 逐像素對齊實機錄影)、33 張的檔長不變量 `len == 4 + w*h*4`、2N 配對交叉驗證加非平凡性對照、tile 幾何(全部 24×24,故 map32 = 432×1224,與 doc23 的渲染圖尺寸相同)、缺檔位移回歸(同時釘住舊寫法會位移到 FDSHAP_022)、表頭負向控制。突變 6/12 DISCRIMINATING。`char_summary.py` 的硬編 `NAMES` 只到 21,而 `docs/data/exe_tables/characters.json` 有 0–31——**22–31(卡里斯/羅蘭/希爾法/謝多/聖寇拉斯/巴拿羅西亞/達克賽/亞奇梅吉/蓋亞/渥德)在總覽表上全部無名**,而該檔自己的註解就已經指向那個 JSON。改成讀 JSON 並與 characters.json 沒有的通用組(68/76/96/97/103)合併。同一輪順帶發現 `render_story.py` 有一份**從未被任何程式碼引用**的完整肖像名表,反而比在用的這份齊全(另列)。6 項 --selftest:22–31 缺口回歸、0–21 換來源後逐一與舊硬編表相同(釘住「改資料來源沒動到既有標籤」)、doc31 統一角色編號恆等(face_portrait == index,32/32)、通用組合併保全且與 JSON 無重疊、sprite 索引公式(doc31 反組譯 0x1291e/0x12928 的 組×12+方向×3+幀,6 例 + 140×12=1680)、非平凡性對照(新表給「30 蓋亞」、舊表同一組只給「30」)。圖片組裝需要不入庫的 extracted/fdicon 與 extracted/portraits,本機沒有,已在 docstring 標明為刻意的範圍限制而非「跑過了」。突變 5/12 DISCRIMINATING。基準線 134 -> 132。
**同輪:閘門抓到我自己引入的回歸**。上面那批的 `verify_everything --rounds 2` 第 2 輪 hygiene **FAIL**——不是既有問題,是我改寫 `extract_maps.py` docstring 時寫了 `map9↔FDSHAP_018`,而 `↔`(U+2194)不在 cp950 裡,該檔又有 `print(__doc__)`,所以無參數執行會 UnicodeEncodeError。抓到它的正是 2026-09-08 那輪為了同一類問題加進 `verify_tool_hygiene` 的 `docstring_console_risk()` 偵測器(當時是為了 `disasm_le.py`/`le_xref.py` 兩支真的會崩的工具而加)。值得記的有兩點:一是這條規則在**新寫的程式碼**上第一次發揮作用,證明它不只是把既有存量登錄起來;二是這個 bug 在 `--selftest` 全過、突變 6/12 DISCRIMINATING 的情況下照樣存在——selftest 測的是 `lay_out`/配對那些核心邏輯,而崩的是「無參數執行印說明」這條路徑,兩者不重疊。已改成「map9 配 FDSHAP_018」,無參數執行 rc=0,基準線回到 132 筆、無新增違規,selftest 6/6 與突變 6/12 重跑後不變。
**2026-09-09 續:一份從未被引用的死表,配一句不實的功能宣稱**。`render_story.py` 有一份 32 筆的 `PORTRAIT` 肖像 ID→角色名表,**定義後從未被任何程式碼引用**,而 docstring 把「說話者肖像 ID → 角色名(>0x41 為 NPC/敵,顯示原 glyph)」寫成本工具的功能——實際上那個 operand 只是被跳過,從來沒畫出來。逐筆比對確認該表與 `docs/data/exe_tables/characters.json` 完全相同(32/32),是重複的死碼;諷刺的是它比當時 `char_summary.py` 真正在用的那份還齊全(後者只到 21,見上一則)。**修法選擇**:沒有把說話者名字接上去讓 docstring 成真,而是刪表並改正 docstring——因為 (a) 原版渲染器 `0x15F84` 本來就是把該 operand 當二進位參數消耗、從不畫出(doc40),接上去等於偏離原版;(b) 帶說話者的可讀輸出 `decode_story_text.py` 早就有了,再實作一份是製造重複而非修 bug。5 項 --selftest:(1) **歷史 bug 的機制回歸**——把 operand 跳過關掉,必須精確重現當初的洩漏量與值分布(FDTXT_032 多出恰好 40 個字模,值分布 {0:17,1:12,3:3,4:8},正好是說話者 id 直方圖,而 40 同時是該資源已記錄的 utterance 數);這與 `decode_story_text.py` 檔頭記載的 2026-08-30「說話者 ID 數字字面滲進內文」是同一件事的另一半。(2) **跨工具判準交叉驗證**——0xFFEC..0xFFEF 的 operand 數 vs `export_story_index_map.count_logical_utterances`,34 個 FDTXT 中 **29 個完全相同**,5 個不同各有來由,其中 FDTXT_026 恰好差 2 且那 2 個就是該函式 docstring 點名的無引號字串 index [2,3]。(3) 字型邊界:字模數 1824 = 0x720(docstring 宣稱的上界),全部 52605 個索引越界 0 個,越界索引渲染回空圖不崩潰。(4) 折行手算。(5) 非平凡性(三章 row 數 193/85/118 互異)加負向控制(純控制碼字串產生 0 個字模)。突變 **10/12** DISCRIMINATING。**同輪修掉一個判準寫錯數字的 docstring**:`export_story_index_map.count_logical_utterances` 的 docstring 寫「opening quote glyph (557)」,而常數 `OPEN_GLYPH` 其實是 **558**。這不是無關痛癢的錯字——這段 docstring 正是 2026-08-31 判定 FDTXT_032/033 屬於 ch00 的判準說明,而用 557 重算得到 12/11/6,與已記錄的 63/40/41 全不同,亦即那個數字描述的是一條會推翻該歸屬的判準。來源已查明:557/560 是 `decode_story_text.py` 檔頭記載、2026-08-30 已修正的舊常數,此處是漏改的遺留。已改正 docstring 並新增第 (5) 題把它釘成**承重對照**:`OPEN_GLYPH` 必須是 558,且改用 557 必須得到 12/11/6——單邊斷言不算數,要證明這個常數換掉會壞。突變 6/12 DISCRIMINATING。基準線 132 -> 131。
**2026-09-09 續二:一個偵測器的假陽性,擋住了正當的豁免,也讓真的產生器沒被看見**。追 `artifacts` 軸那 70 個「尚待處理」時,對 `docs/data/glyph_map.json` 試 `no_generator` 永久豁免,`verify_tool_hygiene._proves_no_generator` 拒絕,說 `decode_story_text.py` 看起來會產生它。查下去:被觸發的是該檔第 410 行 `f.write(\"> 由 FDTXT.DAT + glyph_map.json 自動解碼...\")`——**一句寫進 markdown 輸出的出處註記**,檔名在被寫出去的內容裡,不是寫入目標;`discover()` 只看「檔名前後兩行有沒有寫入呼叫」,而上面兩行剛好是 `open(out,\"w\")`。修掉這一種之後又冒出第二、第三種:`export_command_labels.py:72` `output = export(..., \".../glyph_map.json\")`,檔名是 `export()` 的**讀取輸入**、真正的寫入目標是 `Path(argv[2])`,被同一行的 `output =` 命中;以及 `decode_story_text.py:104` `d = os.path.join(..., \"glyph_map.json\")`,是路徑沒錯但那是 `json.load` 的讀取路徑。三種都是同一個毛病:**「檔名離寫入很近」被當成「檔名是寫入目標」**。改成 AST 角色判定——從字串字面值往上走,先跳過純路徑組裝(`Path(...)`/`os.path.join`/`/` 運算),再由第一個非組裝節點定角色:`.write()`/`print()` 的引數是內容、`open`/`write_text`/`json.dump` 的引數是寫入目標、指派敘述是候選、**其他任何呼叫的引數都是別人的輸入**;再與原本的 ±2 行寫入語境取合取(`decode_story_text` 對 `glyph_map.json` 正好各只滿足一個條件)。**同一次調查反過來找到一個一直沒被登錄的真產生器**:`encode_text.py revtable` 就是 `docs/data/unicode_to_glyph.json` 的產生器(把 glyph_map 反轉、重複字取最小索引,規則就寫在產物自己的 `_comment` 裡),實測**逐位元組相同**(19064 bytes)。它先前掛在 70 個待處理裡,而舊判準的雜訊反而蓋掉了這條真線索。為了能登錄,`revtable` 加了一個選用的輸出路徑參數(預設不變)——寫死輸出路徑的產生器沒辦法被重生比對。結果:`glyph_map.json` 改判 `no_generator` 永久豁免(理由寫明它是人工轉錄的 1824 筆字模→字元對照,需人眼辨字),`unicode_to_glyph.json` 登錄進 REGISTRY 並實跑 IDENTICAL,登錄項目 26 -> 27、待處理 70 -> 69。新增第 (7) 題把判準本身釘住:5 個**真實原始碼**案例(前四個是三種誤判來源、第五個是真產生器),外加非平凡性檢查——五題必須正反例俱在,否則一個「永遠回 False」的實作也會全過。突變 4/12 與 2/12,皆 DISCRIMINATING。基準線 131 -> 130,實際待處理 85 -> 83。
**同輪:我自己造成的效能回歸,以及一個依賴執行環境的正對照**。上一則的判準修正第一次跑 `verify_everything` 就在第 1 輪掛掉——`audit` FAIL、`verify_tool_hygiene.py` **timed out(1220s,平時 320s)**。原因不是判準錯,是**代價下在錯的地方**:角色判定第一版寫成 `f(src, basename)`,於是 `discover()` 的 109 個產物 × 119 支工具會各自重新 `ast.parse` 一次整份原始碼(約一萬三千次),單次 `discover()` 實測 **32.2 秒**;而 `_proves_no_generator` 每個永久豁免項目呼叫一次(47 項),乘起來就是逾時。角色與寫入語境**都只跟原始碼有關、跟要找哪個檔名無關**,所以改成整份原始碼算一次並 `lru_cache`:`discover()` **32.2s -> 0.3s**(107 倍),5 個判準案例維持全對。新增第 (8) 題防呆,用**解析次數**而不是耗時斷言(耗時會隨機器飄,次數不會):同一份原始碼查 61 個不同檔名,`ast.parse` 只准被呼叫 1 次。**另外修掉一個依賴執行環境的正對照**:`verify_tool_hygiene` 第 (2d) 題的 `docstring_console_risk` 正對照用的是本機 locale,於是在 `python -X utf8` 之下 `↔` 編得出來、偵測器正確地什麼都不報,而斷言「必須抓到」失敗,訊息卻寫成「偵測失衡」——看起來像偵測邏輯壞了,實際上是機器設定不同。判準本身(「這份 docstring 在編碼 E 下印得出來嗎」)跟跑測試的機器無關,已把 `encoding` 拉成參數、正對照寫死 `cp950`,並補一題非平凡性:**同一份原始碼在 utf-8 下必須不被報**,否則「抓到」可能只是因為它永遠都報。兩種 locale 下都全過。突變 5/12 與 7/12。**第三件**:用 Python 腳本加 `import functools` 時整份檔案被寫成 CRLF,`shebang` 規則立刻報「shebang 行是 CRLF,WSL 下無法執行」——已正規化回 LF。這三件都是同一輪裡由不同的軸各自抓到的,沒有一件是 selftest 自己會發現的。
**2026-09-09 續三:第三種永久豁免——輸入本身已經不存在**。`docs/data/ida/fd2_xrefs.json` 先前被歸在「需要 IDA」而長期擱著,查證後那個歸因是錯的:該產物的檔頭自己記著 `input_size: 357074` / `input_md5: b97caf2239a27a896069d03549d96e1e`,而現行基準是 509158 bytes / `33464c81e6a364fd0660141139aa8e6e`——**兩者是不同的二進位檔**,使用者於 2026-09-09 明確確認舊版已永久刪除(USB 備份也是同一份)。所以裝上 IDA 也解不了:缺的是輸入不是工具,「重跑產生器再比對」在定義上做不到。既有的兩種永久證明都不適用(`ida_embedded` 是給「在 IDA 外跑不起來的**工具**」,`no_generator` 是給「沒有產生器的產物」),因此新增第三種 `lost_input`,並維持可證偽——三層都要成立:(1) 產物必須自己記錄 `input_md5`,沒記錄就無從主張;(2) 該 md5 必須正好是 `docs/data/fd2-reference-files.json` 裡登錄的 `previous_edition`(已退役版本)而非現行基準,若日後基準改回去或那筆記錄被移除就立刻失敗;(3) repo 內若出現任何 md5 相符的檔案,宣稱立刻失敗——輸入回來了就該回頭重生比對。新增第 (3c) 題做配對控制,三個案例全來自 repo 裡真實存在的產物:`fd2_xrefs.json`(輸入是已退役版本 -> True)、`native_unit_tables.json`(輸入是現行基準 -> False,訊息直接說「請直接重生比對」)、`glyph_map.json`(沒有記錄 input_md5 -> False),並檢查正反例俱在——只有正例的話,一個永遠回 True 的實作也會通過第 (3b) 題。**沿用上一批剛學到的教訓**:第一版用 `ROOT.rglob(\"*\")` 全樹掃描找相同 md5,實測 4.6 秒,而 `_proves_lost_input` 是每個永久豁免項目呼叫一次的——正是上一批那種效能回歸的形狀。已改成「檔案大小 -> 路徑」索引整棵樹只走一次並快取:首次 1.3 秒、後續 20 次合計 0.01 秒。**範圍誠實**:要建立一份**現行版本**的等價 xref 產物是另一個獨立工作項,不是這一項的重生——與 chapter_beats 那 61 個的區分方式相同(那些的遷移路徑存在,所以留在待處理;這一項的輸入不存在,所以是永久)。突變 8/12 DISCRIMINATING。永久豁免 47 -> 48,實際待處理 83 -> 82。
**2026-09-09 續四:突變工具被中斷會損毀原始碼,已修**。`verify_selftest_discrimination` 是**就地改寫原始碼再還原**的,還原寫在 `finally` 裡——正常結束與例外都救得到,但**行程被 kill 時 `finally` 根本不會執行**,檔案就停在突變狀態。更糟的是突變體由 `ast.unparse` 產生:docstring 是 AST 節點所以留著,而**註解與 shebang 全部消失**,十六進位常數也被轉成十進位(`0x3702f` -> `225327`)。這不是理論風險——同一輪我為了改效能而中途砍掉它,`derive_native_argcounts.py`(當時尚未提交,git 救不回)被留在已突變狀態且註解全失,只能整份重寫。**修法**:在第一次改寫**之前**先寫 sidecar 備份 `<name>.py.premutation`,還原成功才刪(還原失敗時備份是最後退路,不能丟);每次 `test_tool` 啟動先掃殘留備份,有就還原並大聲報告。`finally` 擋不住 SIGKILL,但落地的檔案擋得住。**對照必須是真的中斷,不是只呼叫還原函式**:第 (3b) 題 spawn 一個子行程、等它把備份與突變都落地、`proc.kill()` 真的殺掉,再驗證四件事——被砍當下檔案確實損毀、備份存在、還原後**逐位元組**相同(探針檔刻意含註解與 shebang,因為那正是 `ast.unparse` 會吃掉的東西)、備份已清除。第 (3c) 題是負向控制:沒有備份的檔案不得被還原邏輯動到。**同輪補上一個一直沒說明的排除**:本檔自己不在 INVOKE 表內,先前只是沒登錄、沒寫理由。現在明確記下——突變自己會造成**實際損害**:突變體會成為那個執行突變的行程,而第 (3b) 題會 spawn 再殺子行程,被突變過的殺行程邏輯可能留下孤兒行程、被突變過的還原邏輯可能動到別的工具的備份。代替方案不是「不驗」:本檔 selftest 的第 (1)(2) 題本來就是拿**探針檔**做正負向控制,那就是這支工具的鑑別力測試,只是施加在探針而非自己身上。端到端確認:突變期間備份存在、正常結束後 `tools/*.premutation` 無殘留。
**2026-09-09 續五:兩支「exit 0 但什麼都沒做」的工具,離線 correctness 至此清空**。改成一批多支(先前每批 1~2 支,而瓶頸是每批 25~28 分鐘的完整驗證,不是工具本身)。`export_sprites.py`:越界的角色組號原本只是 `continue`,於是 `export_sprites out 200` 印「導出 0 幀」然後 **rc=0**——與 char_summary 2026-09-03 那筆同一類別,只是換個位置。已改成明確報錯並回 1(FDICON 只有 140 組)。5 項 --selftest:FDICON 結構(1680 幀 = 140 組 × 12、24×24,與 doc31 記載相同)、**跨工具索引式對照**(doc31 反組譯定案的 `組×12 + 方向×3 + 幀` 有兩個使用端,與 `char_summary.sprite_frame_index` 逐例比對,最後一組最後一幀 = 1679)、內容結構(4 組各自 12 幀互異、4 組首幀互異)、越界報錯回歸(越界與負數皆 rc=1 且產出 0 檔、正常 rc=0 產出 12 幀)、以及一題**誠實記錄的負面結果**:實測 4 組的跨方向平均像素差異只有同方向的 1.4~1.6 倍且分布重疊(每組都出現「同向最大 ≥ 跨向最小」),**用像素差異分不開方向**——所以方向配置只能來自 doc31 的反組譯,這也正是第 (2) 題為什麼是必要的。這一題刻意寫成「必須 4/4 重疊」而不是反過來,避免日後有人誤以為相似度可以當方向判準。突變 4/12。`extract_all.py`:8 個階段跑完直接 `return 0`,**即使每個計數都是 0**,只會寫出一份寫著「0 個」的 INDEX.md;而 portraits/ 與 maps/ 兩個階段的例外只變成一行「略過(...)」寫進 INDEX.md,整體照樣成功。抽出 `stage_verdict(counts, skipped)` 把「有沒有真的產出東西」變成可測的判斷,失敗回 rc=3 並把問題印到 stderr(清單仍寫出供對照)。5 項 --selftest:全 0 必須不可接受(4 個問題)而正常計數可接受、**單一階段掛掉也要擋下**(不是只有全掛才算失敗)、被例外略過的階段必須進入判斷、容器數宣稱(11 個 .DAT 對 11 個解包目錄)、非平凡性(不同計數必須給出不同判斷)加負向控制。**範圍限制寫進 docstring**:完整抽取要解 409 個 FIGANI 動畫並寫 GIF,不適合放進每次都跑的 selftest,這裡驗的是編排層的判斷;解包新鮮度由 verify_dat_extraction_freshness 擁有,不在這裡重做。突變 2/12。基準線 130 -> 128,實際待處理 82 -> **80**;correctness 裡**純離線的部分至此全部清空**,剩下的 13 支是 6 支純實機、5 支有純核心的實機工具、2 支 Real-ESRGAN。
**2026-09-09 續六:一個真的判準漂移,以及造成資產誤入庫的副作用防護**。**(A) 突變測試的副作用**:上一批推送時夾帶了 12 個 FDICON sprite 到 repo 根目錄的 `0/`(著作權資產,已於 `da6a91a5` 移除)。成因不是手滑而是機制:突變測試以 `cwd=ROOT` 執行工具(不能改——很多工具用 `extracted/`、`org_game/` 這類相對路徑),而**突變過的工具會產生真實副作用**;`export_sprites` 被突變後參數索引偏移,輸出目錄變成字面值 `\"0\"`。`.gitignore` 只排除 `extracted/`,根目錄的 `0/` 不在任何忽略規則裡。`verify_all_tools` 的 invoke 層早就為了同一件事在空的暫存 cwd 執行工具,突變測試沒有同樣的保護。已加 `quarantine_side_effects()`:比對根目錄項目,新出現的一律**搬到 repo 之外**並大聲報告——搬而不刪(那可能是別人正在進行的工作),搬出 repo 而不是加進 .gitignore(忽略只會讓問題安靜下來)。新增第 (3d)(3e) 題,並且**用真實情境驗證過**:同一支工具、同一個 seed 重跑,`0/` 再次被產生、當場被攔下,`git status` 乾淨。**(B) 判準漂移**:同一條「單位記錄是否可信」的判準有兩個使用端,而它們各走各的——2026-09-04 發現 `maxHP <= 9999` 與本專案自己的 `fd2_stat_override` 工作流衝突(HP 覆寫 9999 後單位一升級變 10016),`fd2_game_state.classify_units` 把界線放寬到 u16 並寫下理由,**`fd2_in_battle_check.why` 沒有跟著改**;而 `fd2_game_state` 的 docstring 還寫著 NOT_IN_BATTLE 的判準「見 fd2_in_battle_check.py」,也就是它引用的正是沒修的那一支。這正是 `fd2_game_state` 檔頭自己在抱怨的「每支工具各自實作半套判斷」再次發生。**根因是結構性的**:`fd2_in_battle_check.py` 是腳本不是模組,`INST = sys.argv[1]` 與實機呼叫都在模組層執行,所以 `why()` 根本無法被 import 測試——沒有人能對它寫測試,它才會漂。更糟的是檔案裡有**第二份 `why()`** 定義在模組層、覆蓋掉前一個,帶著舊界線;兩份同名邏輯,改了一份不會有任何東西報錯。已把純邏輯抽成 `why`/`parse_rows`/`verdict`、界線統一為模組層的 `MAX_HP_CEILING`、實機層全部收進 `main()` 並加 `__name__` 守衛。**過程中自己撞到同一個坑**:第一版把 `--selftest` 判斷留在模組層,於是 `fd2_game_state --selftest` 一 import 本檔就觸發它並 `sys.exit`,把另一支的 selftest 整個蓋掉——這反而是最直接的證據,說明模組層副作用為什麼不可接受。兩支各 5 項 --selftest,**互相驗證對方**:`fd2_in_battle_check` 第 (1) 題釘住 maxHP=10016 兩邊都不得判 NOT_IN_BATTLE、第 (2) 題證明舊界線**確實會判錯**(否則第 (1) 題可能恆真)、第 (3) 題對 5 組記錄要求兩支方向一致;`fd2_game_state` 第 (1) 題釘住 2026-09-04 的轉換窗口事件(12 槽全同陣營必須回 TRANSITION_UNREADABLE,不得硬給數字)、第 (4) 題反向對照並檢查共用界線 = 0xffff。突變 5/12 與 3/12。基準線 128 -> 126,實際待處理 80 -> **78**。
**2026-09-09 續七:一個跨三處傳播的資料錯誤,而我這輪的測試把它鎖住了**。做 `fd2_speaker_capture` 的 selftest 時比對它用的 `decode_story_text.PORT` 與 `docs/data/exe_tables/characters.json`,發現**三個名字不一致**:id15 塞可邦勒/賽可邦勒、id28 達克塞/達克賽、id29 亞齊梅吉/亞奇梅吉。裁決不靠判斷:`docs/data/fdtxt000_name_table.json` 是把 FDTXT 資源 0 的字模**渲染成圖後人工轉錄**的,證據圖已入庫,且其中 13 個名字與 2026-09-07 實機讀出的 ch27 名冊逐一吻合——它寫的是**塞可邦勒 / 達克塞 / 亞齊梅吉**,與 PORT 相同。`verify_generated_artifacts` 的 docstring 早就記載 PORT 是「corrected against the game's own name table」(2026-08-30),`characters.json` 沒跟著改。**傳播範圍包含我自己**:`char_summary` 讀 characters.json,所以總覽表顯示三個錯名;更糟的是**我在 batch 28 把這三個錯名釘進了它的 selftest**——第 (2) 題要求「換來源後必須與舊硬編表逐一相同」,而舊硬編表是從 characters.json 抄的,於是那道檢查驗的是**兩個副本互相一致**,而兩個副本一起錯,檢查就把錯誤鎖住了。這正是「已驗證的表,錯誤的推論」那一類:表可以通過所有檢查,建立在它上面的宣稱仍是錯的。**修法把裁決權放對地方**:`characters.json` 三筆改成權威表的寫法(該檔無產生器,已用 write_target_lines 確認,可直接改);`char_summary` 第 (2) 題改成**對權威表逐一核對**而非比對副本,並新增第 (2b) 題釘住**差異本身**(三個舊名不得再出現在任何一處)——沒有 (2b) 的話,萬一權威表也被改回舊值,第 (2) 題仍會通過。`GAP_22_31` 裡的 id28/29 第一版也是從 characters.json 抄來的錯字,被第 (1) 題自己抓到並改正。`fd2_chapter_sweep.py` 三處註解裡的舊寫法一併更正(資料 id 未動)。**三方一致性(PORT / characters.json / 權威表)現為 32 筆全部相同**。同批另加兩支的 selftest:`fd2_sfx_screen_map.describe_hit` 是**判讀層**——2026-09-04 實測 probe 命中 0x32307 時它照靜態表印出「index 11」而活體值是 9,**給了一個看起來正常的錯數字**;5 項檢查釘住三條分支(已知錯歸屬必須標明並給活體值、已活體確認才可給裸 index、未確認必須明說不可當結論),外加三分支輸出必須可區分的非平凡性(否則一個永遠回同一句話的實作也會通過關鍵字檢查)、以及靜態表 11 個 index / 65 個呼叫點與 docstring 記載吻合。`fd2_speaker_capture.resolve_dato` 5 項,含一題**誠實記錄**:PORT 完整覆蓋 0x00-0x1F 而第二條分支吃掉 >0x1F,所以第三條 return 在 byte 輸入下**不可達**,記下來免得日後有人以為它在運作。突變 4/12、6/12,char_summary 重測 3/12。基準線 126 -> 124,實際待處理 78 -> **76**。
**2026-09-09 續八:同一條判準寫兩次,兩次不一樣(fd2_battle_autoplay)**。這支的目標篩選有兩個使用端,而它們的條件**不同**:`nearest_foe` 是「敵方陣營 AND 未行動 AND **hp > 0**」,`adjacent_foe` 少了最後一項。後果不是理論上的——`main()` 第 464-467 行用 `adjacent_foe` 把守「原地攻擊」那一支,所以只要旁邊躺著一具**屍體**(hp=0 但 acted 未設,killed 的單位會留在陣列裡),就會走進原地攻擊,而遊戲的 `enableFlags[0]` 正確判定射程內無活候選、把攻擊 disable 掉。**那正是 doc13 §12/§13 記載、`fd2_crash_capture.py` 多輪「環選擇是 2/3(期望 0)」的症狀形狀**,當時被歸因為輸入不可靠。更糟的是真正活著的敵人在遠處時,單位會一直原地打屍體而不去接近——重現 2026-09-04 那個「四回合都四人全動但敵方數字不動」的失敗形狀。修法與同輪 `fd2_in_battle_check` 那筆相同:把判準抽成**一個** `is_attackable_foe()`,兩個使用端共用,要改只有一個地方可以改;順手把重複的曼哈頓距離也抽成 `manhattan()`。5 項 --selftest:第 (1) 題釘住 bug 本身——現行判準對相鄰屍體回 False,而**舊判準回 True**(證明這個 bug 真的存在過,不是我事後想像的);第 (2) 題釘住**兩者的關係**而不是各自的值(判準寫兩次才是成因),用一個 5 筆的混合池要求可攻擊集合與相鄰可攻擊集合都精確相符;第 (3) 題驗**分派後果**——屍體相鄰加活敵在遠處時,必須選「接近」而不是「原地攻擊」,沒有這一題,前兩題只證明了函式本身、沒證明它改變了工具實際會做的事;第 (4) 題曼哈頓距離手算 4 例加三候選取最近;第 (5) 題空輸入與非平凡性。突變 3/12 DISCRIMINATING。基準線 124 -> 123,實際待處理 76 -> **75**。至此 correctness 裡**有純邏輯核心的 5 支實機工具全部完成**(fd2_game_state / fd2_in_battle_check / fd2_sfx_screen_map / fd2_speaker_capture / fd2_battle_autoplay),剩下 6 支純實機編排與 2 支 Real-ESRGAN。
**2026-09-09 續九:Real-ESRGAN 兩支——分塊幾何可以在沒有權重的情況下驗死**。先更正一個歸類:這兩支先前被我列成「需要 GPU/模型,被擋住」,實際上 `torch 2.11.0+cu128` 與 `tools/models/RealESRGAN_x4plus.pth` **本來就都在**,從來沒被擋。**關鍵手法**:用一個位移不變的假模型(nearest-neighbour ×4)取代 RRDBNet,那樣「分塊再拼接」的結果**必須逐位元組等於整張一次做**。這把 padding/crop/placement 的幾何變成不需要 64MB 權重、不需要 GPU 就能驗死的東西,而且驗的正是 `TILE_OVERLAP` 存在的理由——doc「HD map pipeline」記載的 tile seam 問題。6 項 --selftest:輸出幾何恰為 SCALE 倍;**四種不同的分塊(1 塊、2×2、4×3、46×30,overlap 各異)輸出逐位元組相同**——crop 偏移、目的地切片、邊緣 padding 的 off-by-one 任一出錯都會破壞它,而管線裡沒有別的東西會抓到;負向控制(1 像素位移必須測得出來,否則第二題對一個回傳常數的實作也會通過);overlap 0/8/31 結果相同(釘住「overlap 只是上下文、絕不進入輸出」這個理由,而不只是症狀);**alpha 規則**(三通道權重不能吃 alpha,必須是 LANCZOS 重採樣;RGBA→RGBA、RGB→RGB,且兩條路徑的 RGB 完全相同);常數與文件記載的理由相符。突變 6/12。**同輪修掉今天第三個「同一條邏輯寫兩次」**:`realesrgan_batch` 把 `realesrgan_upscale.main()` 的 alpha 六行**整段複製**了一份,兩份都沒有測試。抽成共用的 `upscale_image()`,兩邊都改用它。`realesrgan_batch` 的第 (1) 題釘的是**單一擁有者**而不是輸出值——值相同時複製一份也會通過,所以它檢查 `main()` 的程式碼裡不得再出現自己的 `Image.merge`/`LANCZOS`/`split()`。**這一題我自己寫錯兩次,過程本身值得記**:第一版掃全檔,被**本 selftest 第 (2) 題**裡的 `Image.LANCZOS` 命中;第二版改掃 `main()` 原始碼,又被 `main()` 裡那條解釋這次修正的**註解**命中(註解本來就會提到 LANCZOS)。判準要看的是**程式碼**,所以最後用 `ast.unparse(ast.parse(...))` 剝掉註解再比對。4 項 --selftest,突變 6/12。基準線 123 -> 121,實際待處理 75 -> **73**。correctness 只剩 6 支純實機編排工具。
**2026-09-09 續十:第六支被我歸成「純實機」的工具,其實核心是離線可判的**。`fd2_stat_override` 先前列在「6 支純實機編排」裡。實際看程式碼後:**決定要寫什麼、寫到哪裡**完全是離線可判的,而 2026-09-04 出事的正是那一層。抽出三個純函式:`u16_le()`(u16 拆 little-endian)、`field_addr()`(base + idx*STRIDE + offset)、`build_plan()`(依陣營與旗標產生寫入計畫),`main()` 改用它們,原本 22 行的行內邏輯移除。6 項 --selftest,兩題是配對的:第 (1) 題釘住 **2026-09-04 的事故**——三項的預設值是 9999 而當時只有 MV 有「0 = 不動」的逃生口,於是想做「只改敵方」的對照組時我方也被一起灌大,**正好毀掉要隔離的變因**;第 (2) 題是它的反向對照——不給逃生口(沿用 9999 預設)時我方**確實**會被寫,否則第 (1) 題對一個「永遠什麼都不寫」的實作也會通過。第 (3) 題空槽必須跳過(超出 `[0x53beb]` 計數的殘留值 hp_max 常常非零,會通過「非空槽」檢查而被誤寫,與 fd2_in_battle_check 第二版同一個坑)。第 (4) 題 LE 5 例加位址算術——位元組序寫反不會報錯只會安靜寫出錯的數字,STRIDE/offset 算錯會寫到別人身上。第 (5) 題 cur/max **兩個都要寫**且 cur=0x40 在前(2026-09-04 用兩個不同值加強制重繪才實測更正過來,先前標反是因為原始驗證用滿血單位、cur == max 無法分辨)。第 (6) 題非平凡性:四種旗標組合必須得出 4 種不同的計畫大小。突變 **9/12** DISCRIMINATING。基準線 121 -> 120,實際待處理 73 -> **72**。correctness 剩 5 支(fd2_chapter_sweep / fd2_crash_capture / fd2_crash_ladder / fd2_floodfill_stack_probe / fd2_zero_read_capture)。
**2026-09-09 續十一:一個「看起來不一致、其實兩邊都對」的陷阱,我差點踩下去**。看 `fd2_crash_capture.run_one_round` 時發現它篩單位用 `not (u[\"acted\"] & 0x80)`,而我 batch 37 剛統一過的 `fd2_battle_autoplay.is_attackable_foe` 用的是 `& 0x01`;更可疑的是**同一支檔案**的攻擊驗證(第 360 行)也用 `& 0x80`。三處寫法不同、名字都叫 `acted`,看起來就是今天已經修過三次的「同一條判準寫兩次」。**但逐一查證後兩邊都是對的**,因為它們問的不是同一個問題——`record[+5]` 這個 byte 至少承載兩個語意不同的位元:**bit0 (0x01) = 死亡／隱藏**,證據是 `dump_chapter_beats.PRIM` 的 `0x3453e unit_inactive`「查 [0x53a45]+idx*0x50+5 bit0;1=死亡／隱藏,0=有效存活」;**bit7 (0x80) = 已行動**,證據是 doc13 §523 的逐指令反組譯「`AND record[+5], 0x7f` 清除該 bit(**這正是「acted」旗標**,`SetNativeRecordBit7`/`ClearNativeRecordBit7All` 的同一個 bit)」。所以 `is_attackable_foe` 的 `& 0x01` 問的是「這個槽是不是還在場上的敵人」,不是「它行動過沒有」——敵方有沒有行動過與我方能不能打它無關。**真正的缺陷是命名**:`fd2_dosbox_live_helper.py:898` 把整個原始 byte 取名叫 `acted`,名字只講了其中一個位元,於是兩個正確的用法看起來像矛盾,而下一個人(這次是我)很可能會據此去「修」一個沒壞的東西。已把兩個位元命名為 `BIT_INACTIVE`/`BIT_ACTED` 並在常數處寫明各自的證據與 doc13:287「end-turn 的組合條件仍待 evidence」(所以也不要把兩者合併成一條規則)。新增第 (5) 題把區別釘住:bit0 置起的敵人必須被排除、**bit7 置起的敵人相鄰時仍必須可打**。**這一題我自己第一版寫錯**——把後半寫成 `is False`,等於把 bit7 當 bit0 用,斷言本身違背了它要證的論點,被 selftest 當場抓到。突變 3/12 -> **5/12**。**本輪沒有改變任何行為**,只改了名字、註解與測試;這是刻意的,因為 doc13:287 明說組合條件還沒有證據,而在開放問題上「統一」就是猜。
**2026-09-09 續十二:第七支「純實機」工具,可測的是「選錯單位」那一層**。`fd2_crash_capture` 的 docstring 自己記載了第一版的失敗:當時為了排除 debugger 干擾而全程盲送同一個算好一次的方向、不重讀單位,實跑 43 輪(3 個 instance、mv 4/6/8)**全部存活**,遠高於 C.16 已知的死亡率——因為重複的方向多半打在**已經行動過**的單位或不存在的落點上,根本沒有真的重跑一次完整的移動+確認流程。也就是說「選對單位」是這支工具能不能成立的前提,而那一層完全離線可判。把選取邏輯抽成 `pick_unit(cursor, units)`(離游標最近、還沒行動過、我方、HP>0),`run_one_round` 改用它;位元改用上一批命名的 `AP.BIT_ACTED` 而不是裸的 `0x80`。5 項 --selftest,前兩題是配對的:第 (1) 題釘住第一版的根因——較近但**已行動**的 idx0 不得被選,必須選較遠的 idx1;第 (2) 題是它的對照——同一組單位若 idx0 未行動就必須選它,否則第 (1) 題證明的只是距離而不是行動旗標。第 (3) 題敵方與 HP=0 的我方都不得被選。第 (4) 題釘住上一批查清楚的兩個位元不得混用:**bit0(死亡／隱藏)置起仍可選、bit7(已行動)置起不可選**——這裡問的是「還能不能動」,存活由 hp 欄位負責,不要拿 bit0 重複做一次還做錯。第 (5) 題非平凡性(三組輸入必須選出三個不同單位)。突變 4/12 DISCRIMINATING。基準線 120 -> 119,實際待處理 72 -> **71**。correctness 剩 4 支(fd2_chapter_sweep / fd2_crash_ladder / fd2_floodfill_stack_probe / fd2_zero_read_capture)。
**2026-09-09 續十三:階梯測試在結構上就不是單變因——一個與取樣無關、可離線驗死的第二個問題**。`fd2_crash_ladder` 的 docstring 已經誠實記載過它的**統計**前提被推翻(退回 DOS 是機率性的,所以「A-G 八輪全部存活」只能讀成「該次取樣下未觸發」,不構成排除)。本輪發現**另一個獨立的問題,而且是結構性的**:設計說「一次只加一個動作,死在哪一階就鎖定哪個動作」,但把六個階段的動作序列從 AST 靜態抽出來逐一比對後——`B->C` 新增 3 移除 2、`C->D` 新增 3 移除 1(`cancel` 被換成 ring+confirm×2)、`D->E` 新增 7 移除 1(前置游標移動 7 步,且移動選格內的 right 由 2 步減為 1 步)——**沒有任何一對相鄰階段只差一個動作**。所以即使崩潰是確定性的,某一階死了也指不出是新增的哪一個動作造成的:階梯式歸因在結構上就不成立。這與取樣無關,完全離線可驗,而且一直沒人查。**沒有重新設計階段**——那需要新的實驗設計工作,不在本次範圍;做的是把限制寫進 docstring 並用 selftest 釘住,免得下一個人再把結果讀成排除。5 項 --selftest:六階序列長度、**單變因設計不成立的實測值**(釘的是問題本身,要把它們改成 (1,0) 就必須重新設計階段,那時這些數字本來就該一起改)、兩個已被推翻的前提警告都必須還在 docstring 裡(任一段被刪,下一個讀的人就會重新把八輪存活讀成排除)、六階互異且 B 仍是走待機的正對照、以及抽取器本身的順序與迴圈展開。**最後一題是必要的**:抽取器第一版用 `ast.walk`(廣度優先、不是原始碼順序)且沒有展開 `for _ in range(n)`,兩個錯疊在一起會讓 `D->E` 看起來只差 1 個動作,**正好得出「設計沒問題」的相反結論**。另外 selftest 當場抓到我第二個 bug:新加的 `stage_actions`/`stage_deltas` 兩個輔助函式名字也以 `stage_` 開頭,被自己的抽取器當成階段(長度表多出兩個 0 的項目),判準已收緊成 `stage_<單一字母>`。突變 5/12 DISCRIMINATING。基準線 119 -> 118,實際待處理 71 -> **70**。correctness 剩 3 支。
**2026-09-09 續十四:「沒寫檔」與「寫了全 0 的檔」必須分得開,那正是這支工具存在的理由**。`fd2_zero_read_capture` 要查的是一個還沒解釋的失敗模式:debugger 停住了、`MEMDUMPBIN` 回報成功、`error` 是 None、遊戲活著,而讀回來的單位陣列**整段全 0**。它的 docstring 明確分出兩種模式——模式 1(未停住就 dump,根本不寫檔,已由 `wait_halted()` 擋掉)與模式 2(停住、回報成功、內容全 0,**仍未解釋**)。而判斷一次讀取算不算成功的那三個條件原本行內寫在迴圈裡:`err is None and len(data) == n and any(data)`,失敗時只記一個 `all_zero` 布林。問題是這樣**分不出四種失敗**:例外、沒有檔案、短讀、全 0——而其中「沒有檔案」與「寫了全 0 的檔」正好就是模式 1 與模式 2,混成一類的話這兩個模式在資料裡就再也分不開,**而分開它們正是待辦事項本身**。抽成 `classify_read(err, data, expected_len)` 回傳 `(是否成功, 失敗種類)`,`main()` 改用它並把 `failure_kind` 一起寫進事件記錄。5 項 --selftest:四種失敗各自回傳自己的種類;**「0 bytes」與「960 個 0」必須得到不同答案**(這一題就是工具的存在理由);**短讀不得被算成全 0**——長度 959 的全 0 檔是 `short_read` 而不是 `all_zero`,否則模式 2 的計數會虛高;正向(長度正確且有內容才算成功,長度過長也不算);非平凡性(五種輸入必須得出五種不同結果)。突變 3/12 DISCRIMINATING。基準線 118 -> 117,實際待處理 70 -> **69**。correctness 剩 2 支(`fd2_chapter_sweep` 3483 行的實機編排、`fd2_floodfill_stack_probe`)。
**2026-09-09 續十五:量測工具的兩層判斷——「對映不成立」與「兩次相同」都不是負面結果**。`fd2_floodfill_stack_probe` 要量的是移動範圍 flood-fill 的軟堆疊(`0x60079`,每層 7 bytes)實際長多高——doc13 記載那個位址「往上接什麼、有沒有上限,全專案沒有任何文件記載」,而本專案會用 `--ours-mv` 放大 MV,可達格數隨面積成長。它 `main()` 裡有兩層判斷,都行內寫著:**位址自我檢查**(`4 <= w <= 64`)與**差異分析**(最高改動 offset -> bytes -> 層數)。兩層都是 2026-09-04 出錯的地方。抽成 `addr_check(w, h)` 與 `analyse_diff(before, after, want)`。5 項 --selftest:第 (1) 題釘住那次事故——`[0x60068]/[0x60069]` 讀到 0/0(真實地圖是 24×24)時必須判成「**0x60xxx 的位址對映不成立**」而不是「軟堆疊沒被用到」,而且訊息裡兩句話都要在;當時整段 dump 全 0,「兩次 dump 完全相同」看起來就像後者。第 (2) 題界線雙向:6 個**真實地圖尺寸**(24×24、27×21、18×26、50×50、31×45、18×51,取自 extract_maps 的錨點)必須全過,6 個殘留值形狀必須全擋。第 (3) 題「兩次相同」必須是**自己的一種狀態**且**不提供 bytes_used**——回 0 會被讀成「量到了,答案是 0」,那正是把讀取失敗當成負面結果。第 (4) 題高度換算(high=20 -> 21 bytes -> 3 層)加邊緣偵測。第 (5) 題短讀獨立成一類(任一側短讀都算),並釘住 `NEIGHBOURS` 全部位於軟堆疊**下方**——那是 docstring 的結構性宣稱(「真正未知的是它上方接什麼」),若日後有人加了一個上方的鄰居,這個宣稱就該重新檢視。突變 3/12 DISCRIMINATING。基準線 117 -> 116,實際待處理 69 -> **68**。**correctness 只剩 `fd2_chapter_sweep` 一支**(3483 行、在 verify_all_tools 的 NO_EXEC 清單裡,是真正會啟動模擬器的編排工具)。
**2026-09-09 續十六:correctness 全部清空**。最後一支 `fd2_chapter_sweep`(3483 行,在 `verify_all_tools` 的 `NO_EXEC` 清單裡,是真正會啟動模擬器的編排工具)——即使如此,**「這一章開打前隊上有誰」**完全由已提交的 chapter_beats 決定,離線可判,而那正是 2026-08-29「fullroster21」那輪最容易搞錯的地方。5 項 --selftest:第 (1) 題釘住當時為了確定索引慣例而做的**兩個獨立交叉核對**——`ch17_post.json` 的 join ids `[21, 7]`(對照反組譯已證的 ch21 guard id 21/約拿在 raw17=遊戲 ch18 加入)與 `ch20_post.json` 的 `[24, 23]`(羅蘭/希爾法,已由「map-native guard」那輪獨立證明是 ch21 自己的 own-camp 單位,不是 ch21 之前的招募)。第 (2)(3) 題是配對的,而且釘的是**一個刻意的不一致**:`natural_join_order` 用 `range(0, n-1)`、`estimate_roster_size` 用 `range(1, n)`,docstring 明寫「不要為了一致而『修』,除非先重新推導哪一個才是錯的」。第 (2) 題驗它的可觀察後果(`natural_join_order(21)` 不得含 23/24),第 (3) 題把範圍換成另一種寫法、證明 23/24 **確實會混進來**——沒有第 (3) 題,第 (2) 題對一個「永遠不含 23/24」的實作也會通過。這與本輪稍早 `record[+5]` 兩個位元那件事是同一個模式:**看起來不一致的兩處可能各自正確,而測試該釘的是差異本身,不是把它們統一掉**。第 (4) 題時序去重、第 (5) 題非平凡性(五個章節得出五種名冊大小 [0, 2, 6, 18, 24])。突變 2/12 DISCRIMINATING。基準線 116 -> 115。**實際待處理 68 -> 67,而且 correctness 這一類至此歸零**——115 筆基準線裡 48 筆永久豁免、67 筆全部是 regenerable(61 個 chapter_beats + 6 個 ui-traces)。**本 session 的 correctness 起點是 18 支**,其中我一開始判定「需要 live DOSBox / GPU、你不介入就做不了」的有 13 支;實際做下來,**13 支全部都有離線可驗的核心**,一支都沒有真的被擋住。
**2026-09-10:`item_sfx_known_types.json` 的兩個 blindspot 呼叫端已定性**。本行殘留寫的是「池內 sub-sample 的個別觸發時機(需逐一反組譯其餘 9 個函式;17 個 sub-entry 中已對應 7 個)」。那 7 筆裡其實有 **2 筆不是對應,是掛著 `unidentified_caller_*_blindspot`**(音效索引 9 與 25 有呼叫端但不知道是什麼動作)。本輪把這 2 支反組譯完:**`0x214ad`(音效 9)= 單一目標傷害動作**:`[0x53ec4]=0` 後 `0x1c4cc(...,9,...)` / `0x1ca89(...,9)` 做演出,再 `eax = 0x1c75e(byte[ebx], 9)`;`eax==0` 走 `0x1e1dc`(未造成傷害),否則 `0x1e0db(eax, 0x5e, byte[ebx])` 顯示傷害;收尾 `0x11cac(0)` + `0x1df58`。**`0x22c04`(音效 25)= 多目標、依等級加權的經驗值授予**:對 esi 陣列中 edi 個單位逐一處理,**只對 `unit[+5] & 0x80` 已設(Acted)者生效**,清掉該 bit 後取 `level = byte[+0x21]`、`class = byte[+0x20]`;若 `8 < class < 0x19` 則 `level += 0x1e`,再 `<<3` 累加進 `[0x53ec8]`。**三個欄位與兩個函式的身分都不是本輪新命名,而是引用既有記載**:`0x1c75e` 是傷害函式(doc03 表列 + doc13 §110 的完整 dataflow);`[0x53ec8]` 是 EXP pipeline(doc13 §622 `NativeExperience`);`unit[+0x20]`=class ID(doc03 表列)、`unit[+0x21]`=level(doc25 §13 的窮舉)、`unit[+5]` bit7=Acted(doc13 battle input dispatch)。**仍未關閉**:其餘 sub-entry 的觸發時機仍缺;另外 `0x1c2da` 在 `0x22c04` 內以 `(arg, 0x19, edi, esi)` 四參數被呼叫、與 `0x1c4cc` 同簽名並列於演出路徑,但它仍是 8 個未命名原語之一,本輪只確認其呼叫位置,未定語意。維持 D。
**2026-09-11:最後一個子缺口關閉——10 個演出 handler 的 sfx 觸發條件全部解出**。本行的殘留寫的是「池內 sub-sample 的個別觸發時機(需逐一反組譯其餘 9 個函式)」。**先把數字對清楚**:那個「9」指的是 **sfx 呼叫點**不是函式——`attack_spell_presentation_dispatch.json` 的 10 個 handler(即 command id 0-9 的元素攻擊法術)共 20 個呼叫點,2026-09-07 已解 4 個 handler / 11 個呼叫點(該檔 finding 原文寫「確認 **2/3**」),剩 6 個 handler / **9 個呼叫點**。**本輪逐一反組譯完**:火炎術 `0x2ba63` 由 `[ebx*4 + 0x53f76] == 3` 觸發;烈炎術 `0x2bd62` 由 `[ebx*4 + 0x53f92] == 5`;電擊術 `0x2c395` 由 `[eax*4 + 0x54018] == 0`;落雷術兩點分別是階段 `eax == 3` 與 `test eax,eax` 後落下來(`eax == 0`);咒殺兩點由**動畫影格計數** `[0x5412e]` 的 `== 6` 與 `== 0x24` 分流成 idx 1 / idx 2(迴圈跑到 0x2c 結束)。**神雷術 `0x2cc05` 那個先前記為 `index: None` 的呼叫點也解了**:index 是暫存器 `ebx`,而 `0x2cbf8 cmp ebx,1` / `jne` 這道支配條件保證它到達時 `ebx == 1`,故 **index = 1**。記成 None 是因為抽取器看到 push 暫存器而非立即值——與本日稍早 `0x28a07` 的「變數偏移其實被 `cmp esi,1` 釘死成 +1」是同一個形狀、同一個解法。**結論**:2026-09-07 在 4 個 handler 上得到的「觸發條件一律是動畫階段或逐目標槽位狀態,沒有任何一個是戰鬥結果(命中/未中/連擊)」,現在在 **10/10** 上都成立——不是取樣巧合。**誠實範圍**:`play_sfx_a`(`0x25a96`)與 `play_sfx_b`(`0x25b45`)的差別(音量/聲道)仍未查;各條件所讀的全域(`0x53f76`/`0x53f92`/`0x54018`/`0x5412e`)之欄位語意本輪未展開,只確認它們是動畫/槽位狀態而非戰鬥結果。資料寫回 `docs/data/attack_spell_presentation_dispatch.json` 的 `_sfx_index_trigger_conditions.completed_2026_09_11`,並把該檔 `0x2cc05` 的 `index` 由 null 補為 1。
**2026-09-11:`play_sfx_a` / `play_sfx_b` 的差別解出——是兩個獨立的播放 voice,不是音量**。本行的誠實範圍一直記著「兩者差別(音量/聲道)仍未查」。對比兩支本體後發現**指令序列逐條相同**,唯一差別是讀 `[0x53ee4]` 還是 `[0x53ee8]`;守衛(`[0x53ef1]`/`[0x51e62]`/`[0x54133]`)、呼叫序列(`0x39805`→`0x39521`→`0x39694`)、取樣表偏移(`+6`/`+0xa`)全部一樣。初始化處 `0x25c30` 給出答案:取得驅動 handle 後,**對同一個驅動呼叫 `0x392d0` 兩次**,把兩個回傳值存進那兩個相鄰 dword——即配置兩個 sample handle。**用途是讓兩個音效重疊播放**,這正好解釋本表已解出的交錯模式:咒殺在影格 6 用 `_a`、`0x24` 用 `_b`;落雷術先 `_b` 後 `_a`。共用一個 handle 的話,後一個會截斷前一個。順帶確認兩個守衛全域:`[0x53ef1]` = 音效已初始化(僅 3 個引用);`[0x54133]` = 靜音/暫停旗標(19 個引用,`!= 0` 就跳過播放)。寫成 doc36 該節。**誠實範圍**:`0x392d0` 等驅動層函式本體未展開,「Miles AIL sample handle」是由呼叫形狀 + 兩次配置 + 驅動層位址群推得;兩個 handle 是否有優先權差異未查。
**2026-09-11:doc36 對 `play_sfx_a`/`_b` 那節留下的兩個「誠實範圍」都關掉了,而且是用 binary 自帶的資料關的**。該節寫著 `0x392d0`/`0x39805`/`0x39521`/`0x39694` 本體未展開、「Miles AIL sample handle」只是由呼叫形狀推得,另問「兩個 handle 是否有優先權差異」。**不需要展開本體**:這份 binary 連進來的 Miles AIL 保留了它的 `AIL_DEBUG` 追蹤設施——每個 API 進入點在共用前導之後都會把**自己的名字**當格式字串傳給 `0x3f46b`(例:`0x3931b push 0x5078d` → `"AIL_allocate_sample_handle(0x%X)"`)。obj2 內這種追蹤字串共 **105 條**,涵蓋整個 AIL API 表面。新增 `tools/derive_ail_entry_points.py`(已登錄產物重生表與突變測試表,突變 9/12 DISCRIMINATING),以「字串端 + LE fixup 反查 + 共用前導錨點 `0x54178`」機械導出 `docs/data/ail_entry_points.json`,**105/105 全解出**。判準是四個先以手工反組譯獨立確認過的位址必須被純機械重現,另有「換掉前導錨點就必須對不上真值」的負向控制。**這條真值檢查在開發中真的抓到一個錯**:前導對 `[0x54178]` 有讀、寫兩次引用(相隔 7 byte),第一版取「最近的一次」抓到寫,105 個進入點全部固定偏後 9~10 byte、名字卻全對——只核對名字驗不出來。**三處訂正**:(1) doc36「位址勘誤」表寫 `play_sfx_a` 內的 `AIL_init_sample` 呼叫是 `call 0x391d1`,實際 `0x25afd` 是 `push [0x53ee4]`、`call` 在 `0x25b03` 且目標是 `0x39521`,`0x391d1` 根本不是進入點(落在 `AIL_install_DIG_driver_file` 中段);(2) `play_sfx_a(table, index, priority)` 的第三個參數**不是優先權,是 loop count**——`[esp+0x18]` 由 `0x25b20` 直接餵給 `AIL_set_sample_loop_count(h, n)`,呼叫端傳的 `1` 是「播一次」;(3)「優先權差異」這個問題不成立:`AIL_set_sample_volume`/`_pan`/`_playback_rate` 的呼叫端數皆為 **0**,而同一支掃描對 `AIL_init_sample`/`AIL_start_sample` 各回 2 筆(正向對照,所以那個 0 不是掃描失效)。這獨立佐證了本項上一段的結論「差別是兩個獨立 voice,不是音量」。完整序列現在是字面上的 AIL 呼叫:`AIL_stop_sample` → `AIL_init_sample` → `AIL_set_sample_address` → `AIL_set_sample_loop_count` → `AIL_start_sample`。詳見 doc36 §19。
...
**2026-09-10:兩個剩餘子項都收斂了,而且都不需要新的 RE 基礎建設**。(a) **逐章 join_id/handler 對應表**——本行把它定性為「隨 doc57 UI-07 逐章稽核持續累積的副產品,不是獨立可一次關閉的任務」。**實測不必累積**:`docs/data/chapter_beats/` 已是全 30 章 pre/post handler 的機械抽取結果,每個 `join` beat 都帶呼叫端位址與 `char_id`,整張表一次導出,共 **28 筆**,已寫成 doc26 §8(含角色名)。**兩個獨立對照**:`fd2_chapter_sweep` 的 selftest 早就釘住 `CH17_POST_JOINS=[21,7]` 與 `CH20_POST_JOINS=[24,23]`,本表逐字相符;序章 `0/9/4/30` 與 doc47 §7 記載的四連呼相同。**一個容易誤讀的地方也記下來了**:`0x2327d`/`0x237c8` 在不同章出現卻有不同 char_id——那是共用尾段(`call 0x112a5`),各章的 `push <id>` 在跳進去之前(例 `0x235b5 push 5; jmp 0x2327d`),所以識別一筆 JOIN 要用 (handler, char_id)。範圍誠實:只涵蓋走 `0x112a5` 的 JOIN。(b) **等級上限第三層**——doc25 §10.4 寫「在 `0x1e292` 目前只看到兩種分支,未證實存在第三分支」。本輪把「目前只看到」升級成**窮舉**,寫成 doc25 §13:完整反組譯 `0x1e292`(職業 30/31 -> `cmp eax,0x63` = 99,其餘 -> `cmp eax,0x28` = 40),再做兩個查詢,兩個都空——全 image 找「讀等級 `byte[reg+0x21]` 後 6 條指令內對同暫存器 `cmp`」的立即值只有 `8`/`0x63`/`0x14`(轉職 Lv>=20 閘),沒有 `0x50`;放寬到不看暫存器,4126 條 `cmp` 裡立即值為 `0x50` 的 16 條,逐一往回 48 bytes,**0 條**讀過等級 byte。社群說的「80」在指令層面沒有依據。**範圍**:這是對「等級與立即值比較」的窮舉,不排除某處用表格查詢或暫存器來實施第三種上限,那種形狀本輪沒查。順帶佐證 `0x1e2d6` 的 `test byte [esi+5],1` 與已證實的 `record[+5]` bit0(死亡/隱藏)一致。
**2026-09-11 措辭收尾**:上一段已寫「兩個剩餘子項都收斂了」並逐項給出依據(逐章 join_id/handler 對應表已一次導出 28 筆寫進 doc26 §8)。本段補一句本檔的正規措辭以免「剩餘」二字被機械判準一再撈出來人工複讀:**本項閉合**,無待辦工作(依據見 doc26 §8:全 30 章 pre/post handler 的 `join` beat 由 `docs/data/chapter_beats/` 機械導出,每筆都帶原版側 `0x112a5`(JOIN constructor)的呼叫端位址,例如序章 `0x3231b`、`0x327f7`/`0x32801`)。
...
**2026-09-10 byte-pair 各欄位語意解出(靜態)**。前一段已把 `0x1366a` 收斂為「依索引取表、播放 byte-pair 序列的演出 driver」,並自陳「byte-pair 各欄位的精確語意仍未逐一展開」。本輪展開。**容器格式(兩層,不是平坦的 pair 陣列)**:`0x4eb48(index)` 回傳 `ptr`;`ptr[0]` = **步數 N**(存進 `[esp+0x48]`,外層迴圈上界,見 `0x13832..0x1383e`);自 `ptr+1` 起,每一步先讀 **2 bytes 的步頭**:`header[0]` -> `[esp+0x54]`、`header[1]` -> `[esp+0x44]` = **本步的 pair 數 M**(`0x13844..0x13857`);接著 M 組 2-byte pair 被拆進兩個區域陣列 —— 第一 byte 進 `[esp+0x20+i]`(陣列 A)、第二 byte 進 `[esp+i]`(陣列 B)(`0x1369a..0x136a4`)。**pair 的兩個欄位**:兩條互相獨立的消費路徑都證明同一件事 —— (a) `0x13882..0x1389a`:`ebx = A[i]`,以 `shl 2 / add / shl 4` 算出 `ebx*0x50`,加上 `[0x53a45]`(單位陣列基底,stride 0x50)後 `mov byte [ebx+eax+3], B[i]`;(b) `0x13727..0x1373f`:先以 `cmp esi, A[i]` 拿 A[i] 與目前單位索引比對,命中才 `mov byte [eax+3], B[i]`。**兩條路徑都把 A 當單位索引、把 B 寫進 `unit[+3]`**。**`unit[+3]` = pose,有獨立佐證**:PRIM 表早已把 `0x134e4` 命名為 `reset_pose`(註解「所有 materialized units pose=down」),而其本體逐指令就是 `for 每個單位: byte [unit+3] = 0` 後 `delay(0x14)`(`0x134ee..0x13509`)—— 同一個 offset,同一個語意,由一個先前已命名的原語佐證,不是本輪自己推的。**結論**:每組 byte-pair = **(單位索引, pose)**。**步頭 `header[0]` 的 bit7 是分支選擇器**(`0x136bc test byte [esp+0x54], 0x80`):bit7=1 -> 去掉 bit7 後若餘值非 0 走 `0x137c7`,否則 `0x17aa9(1)` 等一個 tick、`0x11eee` 貼圖、再逐單位套用 pose 並對未失效單位(`unit[+5] & 1 == 0`)呼叫 `0x127e0` 重繪;bit7=0 -> 低 7 位當**重複次數**,重複套用該步的 pose,並額外把 `[esp+0x50]` 寫進 `unit[+4]`(子影格計數)。**誠實範圍**:`0x137c7` 這條第三分支本輪未追;`unit[+4]` 只確認在 bit7=0 分支被寫入 1 起算的計數,其完整語意未命名。ch26 post 以 `0x52` 為索引呼叫本函式這件事沿用前一段結論,本輪未重驗。
**2026-09-11:本項留下的兩個未解都補完了(靜態)**。前一段寫「`0x137c7` 這條第三分支本輪未追;`unit[+4]` 只確認在 bit7=0 分支被寫入,完整語意未命名」。**第三分支 `0x137c7`**:先逐單位套用 pose,再以步頭 byte A 的**低 7 位當 tick 數**重複「`0x11cac(0)` 重繪 + `0x17aa9(1)` 等待」。因此三條分支的分工是:bit7=1 且低 7 位為 0 → 等 1 tick + `0x11eee` 貼圖 + 逐單位套 pose + 重繪存活單位;bit7=1 且低 7 位非 0 → 套 pose 後保持該 tick 數;bit7=0 → 重複套用並寫 `unit[+4]`。**即 bit7 = 保持模式、低 7 位 = 時長**。**`unit[+4]` = 該姿勢內的子影格索引,值域 1..6**:`[esp+0x50]` 由 1 起算,每個 tick `inc` 一次並寫進 `unit[+4]`,`>= 7` 即結束迴圈。故 `unit[+3]` 選姿勢、`unit[+4]` 選該姿勢內的第幾張圖——與 `reset_pose`(`0x134e4` 只把 `+3` 寫 0、不動 `+4`)一致。同一條路徑還會在 `0 < [0x53afb] < 0x40` 時遞增該變數並餵給 `palette_brightness_ramp`,做與動畫同步的亮度淡變。**一個刻意寫明的區分**:doc27 §278/§279 談的 `record[+3]`/`record[+4]` 是**法術/道具記錄**(傳給 `0x14818` 的那一個),語意是選取範圍/生效範圍,**與單位陣列記錄的 `+3`/`+4` 完全無關**。同一個偏移數字、不同記錄型別——本專案已在 `itemRow +0x0b`/`+0x0c`(本表 1118)上被這種混淆咬過一次,所以在 doc35 §16.1 明寫出來。**誠實範圍**:`0x32230`(每圈結束時以 `A[0]` 呼叫)未展開;`[0x53afb]` 的完整生命週期(誰初始化、誰清零)未追。
**2026-09-11 續:兩個殘留都關掉了(靜態)**。(a) `0x32230` 已在同日 doc35 §16.2 展開(以全域影格計數 mod 6 驅動的每單位動畫步進),前一段寫「未展開」時該節尚未寫完。(b) **`[0x53afb]` 的完整生命週期**:`tools/disasm_le.py refs 53afb` 對全 image 窮舉,共 12 筆 fixup——讀取端 4 筆全在 `FUN_0001366a` 內(`0x138b8..0x138ec`),寫入端 8 筆是四組成對的「設 1 → 呼叫 `0x1366a(actId)` → 清 0」:`0x23568`(act 0x22)、`0x323e9`(act 0x64,即 doc47 §3 序章第 7 beat)、`0x32506`(act 0x69)、`0x327cd`(act 0x62)。因此**沒有全域初始化這回事**,它的生命週期完全由呼叫端包住一次 `act` 決定。語意則由消費端定死:`0x138e7` 把它當作`0x11d40(0, 0xff, n)` 的 `n`,而 `0x11d40` 這個呼叫形狀在 doc35 §220 已定案是 DAC 亮度斜坡——所以 `[0x53afb]` 是**亮度淡變的步進位置**(0=關閉、1..0x3f=進行中、0x40=飽和後停止),兼作開關。**順帶否證 doc47** 那張表把它猜成「疑=嘴型/不可跳過,待證」,已改掉;這是「只看呼叫端會把它看成旗標,消費端才說出它是什麼」的又一例。詳見 doc35 §16.3。
**2026-09-11 再訂正(同日第三段的 (a) 說錯了)**:上一段寫「`0x32230` 已在 doc35 §16.2 展開(以全域影格計數 mod 6 驅動的每單位動畫步進)」——那個展開只讀到分支點就停,結論是錯的。把 `0x32230..0x3231a` 整個反組譯完之後,**三條分支的終點全部是 `0x32301` 的同一個 `play_sfx_a([0x53eec], index, 1)`**,所以它是**腳步音效的節奏驅動,不是動畫步進**。模數也不只有 6:`0x32287` 那條先取 `unit[+0x20]`(職業 ID,doc03 已由 constructor 定案)去查 `0x52725` 那 29 bytes 的**職業→步態表**(索引 = 職業 ID − 1),步態 0 → `% 6` + 音效 9、步態 1 → `% 4` + 音效 9、步態 2/3 → `% 9` + 音效 `0xb`;`0x1f183(unit)` 為真的那條則是 `% 6` + 音效 `0xa`。**因此 §16.2 那句「`% 6` 與 `unit[+4]` 的 1..6 相符,是同一套動畫時脈的兩個面向,不是巧合」已否證**——一個 6 步動畫時脈容納不了 4 與 9;當時是看到兩個數字都等於 6 就把它們接起來,而**兩個數字相等不是機制,共同的消費端才是**。§16.2 留下的另兩個殘留也一併補完:`[0x54132]` 由 `0x3230f` 自己遞增,且 `refs 54132` 全 image 只有 5 筆、全在 `0x32230` 體內,所以它是本函式私有的節奏相位、不是別處驅動的全域影格計數;`0x52725` 的內容語意即上述步態表。§16.2 保留原文並標註勘誤,不刪除。詳見 doc35 §16.4。
...
**2026-09-11 措辭收尾**:同 447。doc53 已結論原版資料流(raw byte writer/handler/FDFIELD 座標)全數完成,所稱「剩餘」的兩個 partial 項目都是 remake 場景整合/視覺比對層級,對象隨 `remake/` 移除而不存在。正規措辭:**範圍消失**。無待辦工作;若未來重啟 remake 須重新評估。
...
**2026-09-10 `+0x0b` vs `+0x0c` 的殘留有直接證據了(靜態)**。doc32 在 2026-09-06 已完整decompile `0x14818` 本體,仲裁出 `param_5`(radius)是真正的曼哈頓距離 threshold、`param_4`(mode)是離散開關,並明確保留「**這只確認了 `0x18d8c`(Attack ring)這一個 caller 的語意**」。本輪把「其他 caller 是否同樣把 itemRow `+0xb`/`+0xc` 餵進這兩個槽」這件事直接證掉。**方法**:全影像掃 `E8 rel32` 找 `0x14818` 的呼叫端,共 **17 個**,逐一讀 call 後的 `add esp,N`,**17 個全部是 `add esp,0x18` = 6 個參數**,ABI 一致。再逐一讀各呼叫端之前的 push 序(cdecl 由右至左,故最後 push 的是 param_1)。**三個呼叫端,同樣兩個槽,三種完全不同的來源**:(a) `0x18e25`(Attack ring,doc32 分析的那一個):`param_4 = itemRow[+0xc]`(`0x18e0b movzx eax, byte ptr [eax + 0xc]`)、`param_5 = ebx`(itemRow `+0xb`)—— 與 doc32 相符;(b) `0x1575a`(**AI 道具評分器 `0x1567e`**,本日 541 一併解出的那條路徑):`param_4 = dl` = `itemRow[+0x10]`(且 `>0xf` 時被夾成 1)、`param_5 = dh` = 「是否被夾過」的**布林旗標**(0 或 1)—— **兩個槽都不是 `+0xb`/`+0xc`**;(c) `0x1bc3c`:`param_4 = 1`、`param_5 = 1`,兩個都是**字面常數**,完全不讀 item row。**結論**:`param_4`/`param_5` 是 `0x14818` 自己的幾何參數槽,**不是「道具射程欄位」**。itemRow `+0xb`/`+0xc` 只有在 Attack ring 這條路徑上才被放進這兩個槽;doc32 拒絕給它們通用命名是對的,現在有跨 caller 的直接反證,而不只是「未證實」。順帶一個可證偽的推論:在 (b) 這條路徑上,`itemRow[+0x10] <= 0xf` 時 `param_5 = 0`,依 doc32 的 decompile「radius==0 時整段跳過,不畫 marker」,故該呼叫**完全不標記任何格**;`>0xf` 時 radius=1,只標記 `d < 1` 即中心格。也就是 AI 評分器呼叫 `0x14818` 不是為了求射程範圍。**方法上的坑**:讀 (a) 的 push 序時,從 `0x18df0` 起線性反組譯會得到明顯錯亂的指令(`sub dword ptr cs:[eax], eax` 之類)。用 6 個不同起點反組譯做收斂檢查,其中 5 個(`0x18dd0`/`0x18de0`/`0x18df8`/`0x18e00`/`0x18e0b`)都在 `0x18e0b` 收斂到同一條 `movzx eax, byte ptr [eax + 0xc]`,只有刻意選在指令中間的 `0x18e05` 不同——確認該讀取為真。**誠實範圍**:17 個呼叫端本輪只逐一核對了 3 個的參數來源(其餘 14 個只確認了參數個數)。但要否證「`+0xb`/`+0xc` 是通用道具射程欄位」這個主張,一個反例就夠,這裡有兩個。
**2026-09-11:17 個呼叫端的參數來源全部取出,通用射程解讀再多兩個量化反證**。前一段只逐一核對了 3 個呼叫端的參數來源,其餘 14 個僅確認參數個數。本輪補完——作法是**允許中間夾雜指令的回溯掃描**(自包住呼叫端的函式入口線性反組譯,取最後 6 個 push),而不是只認緊鄰的連續 push;後者在 11 個呼叫端上取不到,正是 `derive_native_argcounts` 記載的失效模式 B(參數行內計算)。**量化結論一:`p5`(radius 槽)在 17 個呼叫端裡有 9 個傳的是字面 `0`**(`0x15114`/`0x15381`/`0x157d4`/`0x15aaa`/`0x1bd93`/`0x1d211`/`0x1d2bf`/`0x1d32a`/`0x1d38e`),另有 3 個傳字面 `1`(`0x1bc3c`/`0x1becf`/`0x1d1c9`)。依 doc32 已完整 decompile 的本體,`param_5 == 0` 時整段標記迴圈被跳過。**超過半數的呼叫端根本沒有在標記任何範圍**——一個「計算道具射程」的函式不會有這種呼叫分佈。**量化結論二:`p1`/`p2`(中心座標)來自四種不同來源**:`[0x53ab1]`/`[0x53ab5]`(11 個,Attack ring `0x18d8c`、`0x1bbdc`、`0x1cff0` 三個函式共用)、`[0x53c37]`/`[0x53c3b]`(`0x15055`,AI 道具使用演出)、`[0x53c27]`/`[0x53c2b]`(`0x15311`)、以及堆疊區域變數(`0x14237`/`0x1567e`/`0x1598a`)。中心點由呼叫端自己決定,不是道具資料的一部分。**誠實範圍(重要)**:本輪取到的是**運算元形式**,不是語意。9 個字面 `0`、3 個字面 `1`、以及 `[0x53ab1]` 這類全域是可直接判讀的;但多數 `p4`/`p5` 是 `eax`/`ebx`(行內計算),本輪**沒有回溯它們的來源**——「解出參數來源」在這些位置只到「知道它是計算出來的」為止。已逐一核對到欄位層級的仍只有三個:`0x18e25`(`itemRow[+0xc]`/`[+0xb]`)、`0x1575a`(AI 評分器的 `itemRow[+0x10]` 與夾取旗標)、`0x1bc3c`(兩個字面 `1`)。
**2026-09-11:`0x51f15` 起四個樣板的內容欄位語意解完(doc35 §11.2)**——這句誠實範圍在 doc35 §11.1 與 §16 各出現一次,現在兩處都關掉。做法與同日那張 `0x52725` 職業→步態表相同:**先找誰讀它、讀去做什麼**。**(1) 第一個樣板 `0x51f15` 是死的**,而且鏈路可以完全列舉:`refs 51f15` 全 image 只有 1 筆(`0x1c2f7` 的 30-byte 複製);`0x1c2da` 內 `ebp`(第 2 參數)只在 `0x1c34b` 一處被用到;那個 byte 的唯一去向是 `0x4e127` 的第 4 個引數——而 **`0x4e127` 只讀三個引數**(逐位元組核對 `0x4e127..0x4e1a5` 沒有任何一處讀 `[ebp+0x14]`;Ghidra 獨立推得的簽名也是三參數;兩個呼叫端 `0x1c357`/`0x1db58` 都 `add esp,0x10` 推四個,是穩定的呼叫慣例落差不是單一寫錯)。**所以那 30 個 byte 執行期沒有任何效果,`0x1c2da` 的第 2 參數也是惰性的**;§11.1 把它從 `sfx_index` 更名為 `attr_index` 機制上仍對,但不應再期待它有可觀察行為,重製可直接忽略。順帶釘死實際填色值:`0x4e127` 的迴圈是 `lodsb → mov al,ah → stosb`(丟掉來源像素)即**剪影填色**,`ah` 來自 `param_3 & 0xff`,而 `param_3` 同時被當列距;`0x1c2da` 傳 `0x1c8`(456),故填色恆為 `0xc8`。**(2) 另外三個樣板全部解出**,由 `0x1c4cc` 讀,四個讀取點各有明確消費端:`0x51f33`=**起始資源索引**(`+幀序` 後索引 `[0x53ad1]` 的資源位移表)、`0x51f54`=**幀數**(迴圈上界)、`0x51f75`=**音效閘**(非 0 才播)與**音效索引**。**決定性的結構佐證**:兩張表 33 個 attr 的 `(base, count)` 去重後得 14 個相異區間,依 base 排序**互相重疊 0 個**,涵蓋 49..229 只有兩個小缺口,多數首尾相接;`count==0` 的兩個 attr 其 `base` 也同為 0。14 個隨機區間互不重疊的機率極低——不是「看起來像」,是資料自己撐住了解讀。**一個如實記下不解釋的觀察**:音效索引是從同一張表**往後 4 個 byte** 讀的(`t4[attr+4]`),使某 attr 的音效索引恰等於下一組的閘值,且 `attr>=29` 時會越過表尾落進相鄰複本;是刻意設計或原版缺陷需要知道 attr 的實際值域才能分辨,本輪未追呼叫端。
...
**2026-09-10:本行想要的東西已經是既有產物,不需要新反編譯**。上一段說 ch17/22/29 的戰後演出「變成可直接反編譯的問題」並給了三個 handler 位址。查證:那三個 handler 早就在已提交的 `docs/data/chapter_beats/` 裡被完整抽取——`ch16_post`(=遊戲 ch17,handler `0x23b5f`)**16 beats、0 個 unknown**,acting id `[50,51,53]`;`ch21_post`(=ch22,`0x244b6`)15 beats、1 個 unknown(`0x1f882`,已記錄為合併清理風險);`ch28_post`(=ch29,`0x2548c`)41 beats、4 個 unknown(`0x35f10`×1、`0x361b0`×3,兩者都已在 `derive_native_argcounts` 的 docstring 記明維持 unknown 的理由)。合計 72 條 beat 裡 67 條有具名 op,剩下 5 條全是**已記錄理由**的那一組。本行原本要餵給 remake binding 的「live act 資料」,其 acting id 就在 `act` beat 的引數裡。**所以本行的 RE 側沒有剩餘缺口**,要再往前只剩那 5 個 unknown 原語的語意,而那是 item 212 的範圍不是本行的。**同時更正我自己的一個做法**:我先前盤點 D 項時是照**第一行**判斷的,而這類項目是追加式記錄,最後一段才是現況——本行就是被我照第一行誤報成「待 handler-offset 層級靜態稽核」的。
...
**2026-09-10 `0x602ad` table 真正邊界解出(靜態,兩個獨立方向互證)**。項目自陳殘留為「table 真正邊界與未命名欄位語意」;產生器 `dump_exe_tables.dump_native_item_effect_rows` 自己也註明 `count=0xd7` 只是「目前可追蹤的已知 item ID 前綴,不宣稱 native table 在此結束」,所以邊界一直是假設而非結論。**(A) 二進位側上界**:用位元組指紋掃全影像找 table 定址器(`BA <imm32>` + `F7 E2`(mul edx)+ `8D 15 <disp32>`,再用 LE fixup 還原 lea 的真實 linear),共找到 **9 個**,以 `0x4e8c2 -> 0x602ad stride 0x17` 這個已知項當錨點驗證還原正確。依 base 排序後,**`0x602ad` 的下一張表是 `0x61646`(stride 0x14,定址器 `0x4e8ab`)**,相距 `0x1399` = 5017 bytes,故本表**最多 218 列**。完整定址器表:`0x602ad`/0x17、`0x61646`/0x14、`0x6188a`/0x07、`0x619fd`/0x07、`0x61af9`/0x0a、`0x61da1`/0x18、`0x620a1`/0x0b、`0x6238d`/0x1f、`0x626b3`/0x0c。**注意**:先用 capstone 從 code 起點線性反組譯掃同一指紋得到 **0 個**——那是線性反組譯在整段 code 上失去對齊,不是「不存在」;窄範圍掃得到 7 個才暴露這一點。改用位元組指紋後與窄掃一致並多出 2 個。**(B) 文字側獨立定界**:道具名走的是另一條路徑(`0x184c0 -> 0x15f84([0x53a7d], 0xb5 + item_id, ...)`,見 `docs/data/item_labels.json`)。實際解析 FDTXT_000 的字串表(共 661 筆):道具名區塊佔索引 **181..395**(= `0xb5 + 0..214`),**索引 396..409 全空**,而 **410 起又有 239 筆非空**——所以 396 不是資料結尾,是**真正的區塊邊界**(空隙後換類別)。區塊內 215 個索引有 200 個非空、15 個缺號(如 289..296),與 doc49 已確立的「缺席不等於未知」一致。**結論**:本表為 **215 列(id 0..214)**。二進位側給出「不超過 218」,文字側給出「恰為 215」,兩者相容且由不同資料來源得出。列 214 結束於 linear `0x615fe`,其後到 `0x61646` 的 **72 bytes 不是道具列**(內容呈 2-byte 對結構,且若是道具則會是 3 個無名道具,與文字側矛盾),屬另一個沒有 fixup 定址器的結構,未命名。**欄位語意進展**(接續本日 541 的發現):`row[+0x0d]` = AI 道具評分掃描的**唯一過濾欄位**(為 0 即跳過該格),`row[+0x10]` = 效果型態(`>0xf` 會在 AI 側被夾成 1 並以 dh 標記);`row[0]` = `item.type`,此對映以 `effect_row[0] == item.type` 在 **215/215 筆全數成立**釘死。**仍未命名**:`+0x01..+0x0c`、`+0x0e`、`+0x0f`、`+0x11..+0x16` 各欄位,以及上述 72 bytes 的結構。
**2026-09-11:欄位語意這一半也補完了,整列 23 個 byte 全部有歸屬**。本項殘留是「table 真正邊界與未命名欄位語意」;邊界已於 2026-09-10 由兩個獨立方向定出(215 列),欄位這半先前只知道 5 個。新增 `tools/derive_item_row_fields.py`(已登錄產物重生表與突變測試表,突變 4/12 DISCRIMINATING),用兩條互相獨立的機制導出:**(A) 消費端掃描**——全 image 找 `0x4e8bc` 的 23 個呼叫端,追蹤回傳值流向的暫存器,收集 stride 內的欄位讀取,回答「哪些偏移真的被讀」。**(B) 跨產物全列對映**——把 `item.json`(獨立的 normalized 視圖,與 effect_row 相差一個 byte)的每個純量欄位,對 effect_row 的每個 u8/u16 偏移做**全 215 列相符**的窮舉比對。**判準是 215/215,不是「大致吻合」——這一點是關鍵**:`ap` 一度得到 **191/215**,沒有採信,而是去看那 24 筆,發現全是 `ap > 255` 的道具、高位元組落在 `+0x02`;改判 u16 後 215/215,並順帶解釋了 `+0x02` 的值分佈 `0×191, 1×24`(191+24=215)。這條門檻連同**負向控制**寫進 selftest:把門檻放寬到 85%(u8 的實際符合率 88.8%)時,`ap` 必須被誤配到 `u8 @+0x01`——第一版控制設 90% 不會觸發,那是裝飾性檢查,已校準。**結果**:`+0x00` type、`+0x01` ap(u16)、`+0x03` hit、`+0x05` dp、`+0x07` ev、`+0x09` atk_attr、**`+0x0a` atk_rate(新解)**、`+0x0b`/`+0x0c`(見下)、`+0x0d..+0x12` = `K[0..5]`、**`+0x13` price(u16,新解)**、`+0x15` 與 `+0x11` 在 **215/215 列完全相等**(唯一的實質鏡像;另三組鏡像都是全零欄位的平凡相等,已分開列出)。u16 欄位的高位元組也一併標出,免得 `+0x02`/`+0x06`/`+0x14` 被誤讀成未知。**一個刻意保留的矛盾**:`item.json` 把 `+0x0b`/`+0x0c` 標為 `range_min`/`range_max`,而本表 1118 已用跨 caller 反證推翻「通用射程欄位」的解讀(它們在 Attack ring 路徑是傳給 `0x14818` 的 radius/mode 槽)。兩種標籤並存於 repo,產出裡明寫,不擇一相信。**仍未命名的是語意而非位置**:`K[1]`/`K[2]`/`K[4]`/`K[5]` 的意義(`item.json` 自己也只叫它們 K),以及 `+0x15` 為何鏡像 `+0x11`。注意 `K[0]`/`K[3]` 已由消費端各自解出語意,**所以 K 不是同質陣列**——「K」只是萃取器的佔位名稱。
**2026-09-11 殘留的 `K[1]`/`K[2]`/`K[4]`/`K[5]` 與「`+0x15` 為何鏡像 `+0x11`」全部解出,本項可完全關閉**:一律由消費端定名,不看欄位形狀。`+0x0e` = **效果強度(u16)**——套用端 `0x20cb5` 是 `movzx edx, word ptr [eax+0xe]`,`word ptr` 是機器層證據,不是把資料湊成 u16;`+0x0f`(K[2])因此是它的高位元組(資料側:非零的恰好 5 筆,全部對應 >255 的值)。`+0x0d`(K[0])是套用端 `0x20cbd` 之後的 **switch 選擇器 = 效果型態**(型態 5 與 13 共用 `0x211a4` 並吃強度值),不是舊記的「AI 可選旗標」(見 541 訂正)。`+0x10`(K[3])與 `+0x12`(K[5])是**同一個 `0x14818` 的 param_4 範圍形狀碼**,分別用在選取階段與生效階段。`+0x11`(K[4])與 `+0x15` 是**敵我關係選擇器**(`0`=對敵方、`1`=對我方),鏡像的原因終於有解:**兩條路徑各自從不同偏移讀同一個語意欄位**——`+0x11` 只被 `0x15091`(AI 用道具)讀、`+0x15` 只被 `0x1bd24`(玩家用道具)讀,且 AI 端 `0x150a2` 讀完**取反**才當 param_6、玩家端直接傳,於是攻擊道具在兩邊都指向「對方」;資料必須讓兩欄同步,鏡像是結果不是巧合。這個機制已寫成 `derive_item_row_fields.py` 的檢查 (6):可達的 33 列(`+0x0d != 0`)兩欄都必須落在 `0x14818` 認得的 0..3,不可達的 182 列則必須落在範圍外(實測值是 5)——負向控制使這條斷言非平凡。另外把消費端掃描的**涵蓋性**也證了:`tools/disasm_le.py refs 602ad` 對全 image 只有 `0x4e8cb` 一筆 fixup,故 `0x4e8bc` 是唯一取址閘門、那 23 個呼叫端就是全部消費端,沒有 inline 算位址的漏網路徑。順帶撤回 doc27/doc32 兩處「`0x4e56c` 是同一張表的第二個編譯副本」——`0x4e56c` 實際是 `格子緩衝區 + 7 + (x + y*width)*4` 的地圖格位址計算,且 `call_scan` 對它的呼叫端數是 0(它根本不是 function entry)。
