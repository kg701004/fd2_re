...
- [~] **② 複查 `+0x40`／lunge offset**：**`+0x40=當前HP`**（`0x18c98` 血條 `word[+0x40]×101/word[+0x42]`=HP%）與 `+0x44/+0x46=current/max MP` 已閉合，舊「戰場格 X」已清。先前稱 `0x29f72` lunge 讀 `+0x48/+0x4a` 螢幕投影的 assertion 已撤回：command17/18 direct writers 證實兩欄是 derived AP/DP，lunge stack-local provenance 必須重判；doc35 已同步降為 partial。
...
- [x] **commands 0..8 direct compositor + numeric route**：`0x1cff0` 對 IDs0..8 直入 `0x2a6bd`，確實不是 handler table 的 `0x21227/0x213b7` wrappers；但 direct `0x2a6bd` 的 `sub_2b659` MP event 和 final-target loop `1C75E(targetSlot, commandID)` 已重新逐行確認。故 ID0 executor／target UI 恢復為 bounded state slice，renderer/post-resolution 仍未宣稱完成。
  - [x] generic BG selector boundary：`0x2a6bd→0x2b5e1(finalCount, finalTargetArray)` 倒序 target scan，經 `0x12e38`／`0x1f183` 以 raw output byte+6 選 selector，再載 `BG.DAT`；command ID 只先選 generic/special presentation branch，不直接選 BG resource。selector semantic 保持 raw。
...
- [~] **native commands32..35 `0x27FC9` compound family**：ID32 `→0x2111A→0x1C75E` numeric、ID33 先清 final target `+0x25..+0x27` 再以固定 `0x320` 走 `0x211A4→0x1C916` restore、ID34 依序 `0x22721/0x22866/0x22997`、ID35 依序用 IDs26/22/27 呼 `0x22D1B` 寫 `+0x25/+0x27/+0x26`。`0x27FC9`／上述 helpers 的 direct trace 未見 `0x1CA89`，雖 selector 有 record `+5` MP gate，不能猜為免費或擅自沿用 SpendNativeCommandMP；transaction writer、multi-effect ordering/rollback、UI/SFX 待動態／額外 evidence，engine 保持 fail-closed。
...
- [~] **shared object redraw compositor**：`0x127a9` 的 `0x127e0` 不是單純 loop bookkeeping：active roster entry 以 camera-relative placement 選 24×24 descriptor，走 `0x4deda` raw indexed-RLE 或 `0x4de56` palette-band-RLE 寫 `0x53a49`；尾端 `0x129ec` 又在同 buffer 疊 map/object layer。`+5 bit7` clear→raw、set→band 已由 direct branch 關閉，`BlitForNativeFlags` 有 regression。`0x53a61` 不是未知 archive descriptor；`0x11019(group)` 建立每 group 12 個 pointer，索引為 `group×12 + pose×3 + cycle`，與既有 FDICON map-sprite evidence 一致。cycle 由全域 idle/moving counter 選出，unit+4 僅是 movement offset。仍待實作／驗證 roster-state 與 layer ordering；在此之前不得把 `0x22046` passes 或 `unit_present` 接成 native UI。
...
- [x] **FDICON native selector primitive**：`Bank.SpriteFor(group,pose,cycle)` 嚴格表達 `0x127e0` 的 `group×12 + pose×3 + cycle`（pose 0..3、cycle 0..2）並 regression。`NativeFrameIndex` 依 +4 movement offset 選 `0x3C0B/0x3C07`，將 global cycle 3 正規化為 1，`+0x26` 則強制 0；撤回「runtime +4 frame」說法，故沒有把它隱式接入 GUI。
...
- [~] **native foreground-terrain occlusion layer**：Docker trace 已把 `0x129ec` 定為 unit-sprite 後的前景補畫，但修正「每個可見 unit」的簡化說法：它先跳過 `0x1f183(slot)` true 的 raw gate（`unit+7==0x1c` 放行；其他 `unit+7` 的 class `0x13` 或 race `4/5` 跳過），再跳過 `0x3453e` inactive slot。**撤回**剛才將 `unit+7` 稱為 group 的錯誤：map sprite group 是 `unit+2`。`fdicon.NativeForegroundRedrawEligible` 以 regression 固定兩 gate，`NativeForegroundRedrawCells` 則保留 eligible slot 的精確 `(x,y)`、`(x,y-1)`、移動 pose-neighbour 順序，且故意不提前剪掉 off-map cell（native bounds gate 是 `0x12ac6` 的責任）。Official IDA 9.4 再證明 `0x1366a` 的 scripted-step redraw 也做 `0x11eee` terrain→per-slot `0x127e0`→`0x129ec`，並在 `0x129ec` 後才進 `0x11eb0`／present；故不可只把 occlusion 掛在 steady `0x127a9`。`0x12ac6` 僅在 resolved field-tile flag bit7 時寫 `0x53a49+0x8088+(y-cameraY)*24*456+(x-cameraX)*24`，bit8 加 `2*flip`，FDSHAP descriptor 正確取 offset-table `index+1`。`NativeForegroundFrameIndex` 已 regression。raw/alternate path 由 field entry `+3` 分支；`0x4dd52` 已閉合為 24×24 four-mode RLE 的 caller-LUT indexed blitter（非未知 effect）。`0x10937..0x1096f` 再關閉 source provenance：`0x111ba` 以 selected FDSHAP even/odd pair 填 `0x53a5d` image descriptors／`0x53a69` flags；`0x12ac6` 的 LUT selector 亦已閉合為同一 `0x51A97[0x53C1F]`→FDOTHER#3 table。這證實 native 角色會被 map foreground 遮擋；runtime roster bridge、complete layer order 與 GUI adapter 仍待，保持 fail-closed。
...
- [~] **native terrain/unit map HUD (`0x1acf3`)**：它在 `0x11cac` 的 terrain/range/unit+foreground 後、viewport copy 前執行，且須 raw gates `0x51aab`、`0x51aac` 都非零。先以 `0x12e38(cursor)` 解 FDFIELD tile10/event-low5/FDSHAP control4，再以 `0x12c0d(cursor)` optional active-unit lookup；`NativeTerrainCursorInfoForCell` regression 固定前一 raw ABI。已釘住 control byte+1→`0x51a12/0x51a2a` 的 AP/DP 表，並由 `battle.Load` 以逐格 raw tile/control 接至戰鬥計算。layout 已收斂為 LMI1 #130（69×34）`buffer+stride*157+x`、terrain `+6`、AP `stride*8+0x2b`、DP `stride*19+0x2b`；`0x1aeb1` 依 table 值正負選 raw #0x83/#0x84、取絕對值再走 native digits，兩 resource 的視覺語意仍不命名。`x` raw 初值1，已見條件改為0xf2或1。unit icon/HP resource、高階 global 名稱仍待。**撤回**把現行 map HUD 的 FDOTHER#5 full-screen battle frame 當 native equivalent 的說法；目前只保留可玩 approximation。
  - [x] codec boundary：#130／#0x83／#0x84 不走 `ParseLMI1` 的 `0x4e916` cell codec；native caller 明確走 four-mode `0x4e63d`。新增 `ParseLMI1FrameEntry`／`DecodeLMI1FrameResource`，player-archive regression 驗證 69×34、44×12、45×12 及 transparent decode；不把同容器其他 entry 的 codec 猜成一致。
