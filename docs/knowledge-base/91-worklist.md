...
1150 - E - 項目自陳仍需DOSBox E2視聽diff，doc57同章節row確認此為唯一阻擋點。 **2026-09-07 live**：ch27 測試存檔全員存活，復活服務無從觸發，費用公式本輪**無法驗證**——需要一份有陣亡成員的存檔（可用 `fd2save.py` 把某成員 raw record `+5 bit0` 改成陣亡後再測）。這是明確的前置條件，不是驗證失敗。
