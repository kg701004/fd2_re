...
- 2026-07-26 ID24 player/AI dispatcher split（official IDA 9.4 ASM）：撤回「`funcs_1541f[24]=0x22153` 因此玩家 ID24 是 ID16 heal alias」的錯誤捷徑。該 table 只在 `0x15311` AI／自動 dispatcher 使用，確實會把 ID16 傳入共同 heal tail；玩家 `0x1cff0` 則對 `0x18` 直入 `0x2a6bd→0x276ec`，不經此 table。後者已讀到 `trunc(actor.+0x48*15/10)-target.+0x4a` 後呼 `0x1c81f` 的 HP writer；MP event、multi-hit/presentation/UI 尚未關閉，remake 保持 fail-closed。同步刪除 doc37 將 `0x2a6bd` 說成武器專用、與 command 無關的舊斷言。
