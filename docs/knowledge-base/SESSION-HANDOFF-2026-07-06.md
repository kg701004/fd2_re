...
- 2026-07-26 FDICON selector correction：`Bank.SpriteFor(group,pose,cycle)` 實作 `0x127e0` 的 `group×12 + pose×3 + cycle` 並拒絕越界 pose/cycle；`NativeFrameIndex` 實作已證實的 global idle/moving cycle selector。撤回先前「runtime +4 frame」錯誤，+4 是 placement offset；現有 battle `Fig`/`Dir` 不因此自動構成完整 native renderer state。
...
- 2026-07-26 generic command BG selector closure（Docker Capstone）：`0x2a6bd` 以 final target count/array 呼 `0x2b5e1`；後者倒序掃 target unit、`0x12e38` decode 與 `0x1f183` gate 後，回傳 raw output byte+6 selector，才交 `0x111ba("BG.DAT", selector)`。撤回 generic command ID 直接選 BG resource 的暗示；raw selector 的高階場景／地形語意仍待。
