...
- 2026-07-26 native damage route correction for IDs10..12（official IDA 9.4 ASM）：撤回「`0x21548` 只知 compositor、尚未定位 gameplay writer」：它開頭 `0x1CA89(actor,id)` 扣 MP，尾端 final target loop 直接 `0x1C75E(target,id)`。ID10 `0x21527`、ID11 `0x2185F`、ID12 `0x21A9E` 因此與 IDs0..9 共用 verified numeric/MP/acted state core；`ExecuteNativeCommandDamage` bounded range 擴為0..12，UI仍ID0-only，presentation/SFX不推論相同。
