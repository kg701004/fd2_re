...
- 2026-07-26 native commands26/27 engine slice：`State.ExecuteNativeCommandApplication` 只接受 IDs26/27，依各 record generic two-stage targets/MP debit；final target 需 raw `+0x25/+0x26==0`、class 非 `0x19/0x1a`、`rand()%100<50` 才 fixed-10 HP damage 並寫 `rand()%4+2` raw duration。target gate 失敗時不 mutation，但已成功 handler 仍耗 MP/actor completion；不映射 legacy Poisoned/Paralyzed，UI/renderer未接。
