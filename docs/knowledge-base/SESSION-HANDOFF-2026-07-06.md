...
- 2026-07-25 UI action wrapper recheck（Docker-only Capstone）：`0x18d8c` 是 action dispatch wrapper，先清 output/global、由 `0x1b83d` 的 `-1` 走取消，非取消才建立前序 target state，並以 `0x1b8a6`／`0x1c269`／unit `+0x27` 寫 caller output `+8/+4` raw flags。`0x177fc` 回 `-1` 也是取消；僅其後依 `[0x53c57]` 分派 attack／`0x1cff0` command／`0x1bbdc` item／wait-field。flags 的 UI/玩法名稱未經 callee 或實機閉合，保持 raw，沒有接 renderer。
