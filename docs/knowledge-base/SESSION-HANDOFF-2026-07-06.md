...
- 2026-07-26 native commands 17..19 modifier-writer closure（official IDA 9.4 ASM）：ID17 `0x226EA→0x22721` 以 `+0x22` gate，ID18 `0x2282F→0x22866` 以 `+0x23` gate，兩者在零值時設 `rand()%4+2` duration，對 `+0x48/+0x4a` 加 `ceil(value*0.15+1)`；ID19 `0x22960→0x22997` 以 `+0x24` gate，設 duration 後對 `+0x4c/+0x4e` 各加 15。這與 `0x1b750` 的 derived AP/DP/HIT/EV writers 一致，故撤回 doc35 將 `+0x48..+0x4e` 誤稱螢幕座標／bounding box 的 assertion；status names、tick/decrement、UI、remake integration 未閉合。
