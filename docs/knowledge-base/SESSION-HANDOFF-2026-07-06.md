...
- 2026-07-26 FDSHAP alpha bridge：codec 改回傳 pixels 與獨立 opacity mask；`export_engine_assets.py` 依 mask 輸出 RGBA `tileset.png`，實測 map0 alpha extrema=`(0,255)`。opaque palette index0 仍保留 alpha255，禁止用 colour value 猜透明；這只保住 Ebiten asset data，沒有把 `0x129ec` layer scheduler 推論成已完成。
