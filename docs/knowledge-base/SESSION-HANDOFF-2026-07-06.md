...
- 2026-07-26 retraction — command0 damage closure：重查 `0x1cff0` 到 `0x2a6bd` 的可達性和 callee 後，沒有保留 `0x2a6bd→0x1c75e` 的可靠 direct dataflow 證據。先前把 ID0 的 record/hit/HP 公式綁至 `0x2a6bd` 的結論與 remake executor 已刪除；`+0x40/+0x42` 的 HP 意義仍由其他已閉合 handlers 支持，但不得再歸因於 ID0。
...
- 2026-07-26 command0 target-array closure（official IDA 9.4 ASM）：一般 command record 的 `+3/+6` 進 `0x14818` 產生 candidate unit-index stack array；`0x115b6(mode=+6,count,array)` 做 cursor/confirm，之後第二階段 effect array 才傳 `0x2a6bd(unit,id,count,array)`。這只閉合 selector pipeline；IDs0..8 不再宣稱逐 candidate `0x1c75e`。撤回把 native command0 當單格 legacy cast 的暗示；尚未命名 target-code/geometry 值域。
...
- 2026-07-26 retraction — ID0 UI vertical slice：先前啟用的 raw ID0 target/highlight/executor 已撤除。`0x1cff0` 對 IDs0..8 直接到 `0x2a6bd`，而非 table wrapper；未完成 `0x2a6bd` state dataflow 前，UI 必須 fail-closed。
- 2026-07-26 retraction — shared damage IDs0..8：`sub_2134B/21364/2137D`、`21449/462/47B`、`21494` 的 wrapper 存在不等於玩家 dispatch 可達。原先 IDs0..9 共用 numeric/MP/acted core 的結論刪除；ID9 另有 direct `1CA89→1C75E` 證據。
...
- 2026-07-26 native damage route correction for IDs9..12（official IDA 9.4 ASM）：`0x21548` 開頭 `0x1CA89(actor,id)` 扣 MP，尾端 final target loop 直接 `0x1C75E(target,id)`；ID10 `0x21527`、ID11 `0x2185F`、ID12 `0x21A9E` 皆可閉合 numeric core，ID9 亦 direct `1CA89→1C75E`。修正後 `ExecuteNativeCommandDamage` bounded range 僅 9..12；IDs0..8 走 direct `0x2a6bd`，不可混入。
