...
- 2026-07-26 battle-animation assertion audit：修正 doc06 舊摘要。`0x29164` 不是狀態欄而是 figure+TAI 台座淡入；狀態欄是 `0x2a289→0x18c6d`。撤回「全螢幕繪圖已完整反組譯」與 `figure X=unit+0x40`；後者是 current HP。保留已證實 anchors/codec，command presentation、layer schedule、lunge coordinate source 等維持未閉合。
