...
- **歷史、已撤回（2026-07-27）** ch24 postbattle slice：當時曾把 raw `ch24_post`（實際屬玩家第25戰）誤接到 `postbattle_ch24_persist→town_ch25`。直接指令、主表索引與玩家戰次零起算關係後續已推翻這個 owner；目前 raw `ch24_post` 的正式 owner 是玩家第25戰 `postbattle_ch25_persist→town_ch26`，而 `postbattle_ch24_persist` 仍失敗即關閉。原始 handler 的 dialog/PAN/spawn/ACT/JOIN 證據仍保留，不能把這筆歷史當成現況。
...
- 2026-08-09 ch23 post handler recheck：合法 IDA Pro 9.4 與 Docker Capstone 固定 `0x24c1e..0x24d22` 的兩段 loop：第一段每 stage 30 次 `0x11cac(1)→0x17aa9(1)`，第二段每 stage 12 次 raw PUSH `[ESI,255,0]→0x11d40→0x11cac(0)→0x17aa9(1)`，`ESI=0..11`；形式參數是 `0x11d40(0,255,ESI)`。`0x17aa9` 是 BIOS tick wait；`0x11d40` 是全 256-entry DAC 減法／夾零；`0x24d22` 在此 handler 只走非零 setter，寫 raw `0x51a10`，`arg==0` copy branch 未被呼叫。IDA 又固定 `0x11cac(1/0)` 共用 `0x11eee→0x122dc→0x127a9→0x1acf3→0x11eb0` 的 456-stride staging copy，只有 `a1==0` 額外呼叫 `0x4dfcc`；它沒有直接讀 `0x51a10`。故不命名或猜 renderer。證據見 [`fd2_ch23_post_ida.txt`](../data/ida/fd2_ch23_post_ida.txt)；12 組 palette／redraw／tick／setter 的 indexed 消費端仍未閉合，`postbattle_ch23_persist` 繼續失敗即關閉。
