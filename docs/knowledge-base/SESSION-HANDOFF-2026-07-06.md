...
- 2026-07-28 church raw service0 roster bridge：instruction decode 固定 `0x17aed` 只有 actor 一個參數，並由 `0x17e0b(actor)` item/status staging、`0x1c269(actor,0)` gate、`0x1ceed(actor,-1,...)` 將 raw index0 窄化為角色資訊／狀態 presentation，無 persistent writer。remake 已把 caller `0x2e6b8/0x2ea90` 接成原版 indexed roster：FDOTHER#14 entry16、兩欄最多六人、FDICON/name exact coordinates、201/205＋shadow76、bounded ±1/±2、6-open/5-close＋source restore。選定 actor 後仍停在未閉合的 `0x1ceed` command/MP overlay，沒有猜接 authored UI 或 gameplay mutation。
