...
- 2026-07-26 command0 damage closure（official IDA 9.4 DB/ASM）：`0x2a6bd` 對 target 呼叫 `0x1c75e(target,id)`，ID0 讀 record `u16+0`、`byte+2`，以 target `+0x20` multiplier 得 base，`rand()%100 < byte+2` 才命中；`0x1c81f` 以 base 的 90% 加 0..9.9% 隨機量扣 target `+0x40`、clamp0。直接 writer/read cap 證實 `+0x40=current HP`、`+0x42=max HP`；撤回舊 doc03 把 `+0x40`/`+0x42` 寫成 DF/MV、`+0x48` 寫成 HP 的斷言。multiplier identity/target family 尚未證實，未接猜測性 remake effect。
...
- 2026-07-26 command0 resistance closure（official IDA 9.4 DB/ASM + EXE bytes）：`word_51f96` 的 loaded-data file mapping 是 `0x51d96`，即既有職業魔抗 table 的 4-byte rows；其 low byte=`resist_raw`（法師 7→30% resistance）。故 command0 base 精確為 `record.dmg*resist_raw[target class]/10`，撤回「multiplier identity 未知」斷言。新增獨立 `NativeCommand0Damage` resolver（hit draw、damage draw、HP clamp）及 fail-closed editable-table loader；未把它錯接到尚未證實 target geometry/effect UI。
- 2026-07-26 command0 target-array closure（official IDA 9.4 ASM）：一般 command record 的 `+3/+6` 進 `0x14818` 產生 candidate unit-index stack array；`0x115b6(mode=+6,count,array)` 做 cursor/confirm，之後 `0x2a6bd(unit,id,count,array)` 逐 candidate call `0x1c75e`。撤回把 native command0 當單格 legacy cast 的暗示；尚未命名 target-code/geometry 值域。
...
- 2026-07-26 command completion flag closure（official IDA 9.4 ASM）：`0x18d8c` 的 native command branch 僅在 `0x1cff0` success（非 0/non-cancel）後呼叫 `0x13512(unitSlot)`；後者唯一寫 `runtime unit+5 |= 0x80`。這直接證實 native command 成功耗用行動、失敗/取消不耗用。`ExecuteNativeCommand0` 因此只在所有 verified core mutation 成功後設 `Unit.Acted`，不以舊 gameplay 習慣猜測。
- 2026-07-26 ID0 UI vertical slice：raw native grid 的 command 0 現啟用 actor `+3` candidate highlight、confirmed cursor `+4` final effect、state-bound record/resistance/flags core 與 ESC 回 grid；其餘 command ID 保持 fail-closed。此只是 E0 gameplay slice，不包括原版 compositor、post-resolution、SFX 或 screenshot oracle。
- 2026-07-26 shared damage route closure：official ASM 的 `sub_2134B/21364/2137D` 分別 push ID1/2/3 後跳同一 `sub_2121A→sub_21227`，其 MP debit、target iteration、`0x1c75e` writer 與 success acted 都與 ID0 同路。engine 因此 bounded 支援 IDs0..3；UI 保持 ID0-only，不能以此推論 renderer/effect 全同。
- 2026-07-26 shared damage route extension：official ASM 的 IDs5/6/7 wrappers `0x21449/21462/2147B` 都跳 `0x213AA→0x213B7`，ID8 回 `0x2121A`，ID9 direct `0x1CA89→0x1C75E`；所以 IDs0..9 都有同一 verified MP/numeric/acted core。engine bounded range 擴為0..9；UI仍只開ID0，renderer/effect不推論。
- 2026-07-26 native command ID13 healing closure（official IDA 9.4 ASM）：jump-table `0x21AD9→0x21B18` 對 generic final target array 先跑專用 indexed presentation `0x1C4CC/0x1C2DA`，再 `0x1CA89(actor,13)` 扣 MP；每個 target 的 `0x1C8ED→0x1C916` 以 record `u16+0` 算 `floor(amount*0.9)+floor(rand()%100*amount/1000)`，將 runtime `+0x40` 加至最多 `+0x42`，並以 `0x1E0DB(...,0x69,target)` 顯示數字。ID13 因此是 per-target HP restore，不是 IDs0..9 numeric damage route；renderer/UI/SFX 及 remake resolver 未接，維持 fail-closed。
