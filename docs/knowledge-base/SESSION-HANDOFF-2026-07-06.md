...
- 2026-08-09 ch23 post handler recheck 勘誤：合法 IDA Pro 9.4 與 Docker Capstone 固定 `0x24c1e..0x24d22` 的兩段 loop：第一段每 stage 30 次 `0x11cac(1)→0x17aa9(1)`，第二段每 stage 12 次 raw PUSH `[ESI,255,0]→0x11d40→0x11cac(0)→0x17aa9(1)`，`ESI=0..11`；形式參數是 `0x11d40(0,255,ESI)`。`0x17aa9` 是 BIOS tick wait；`0x11d40` 是全 256-entry DAC 減法／夾零；`0x24d22` 在此 handler 只走非零 setter，寫 raw `0x51a10`。沿 `0x11cac→0x11eee` 追查後確認 case 23 在 BIOS tick 變化時會間接呼叫 `0x24d22(0)`，其 `0x138` bytes row copy 是 312-byte staging 列旋轉消費端；因此舊的「沒有消費端」說法僅適用直接資料交叉參照，已不再作為現況結論。重製端已加入 `RotateNativeCh23Rows`／`ApplyNativeCh23PaletteCycle` 原語，並以兩個 `native_ch23_loop` 候選 beats 保存完整 call-site、30／12 次數與 stage 2..14；runner 在 indexed／latch adapter 未完成時失敗即關閉。初始 latch、`dword_53C03` 生命週期、tick gate、完整 indexed 消費與一般玩家 E2 仍未閉合，故不命名泛用 renderer，也不接 `postbattle_ch23_persist`。證據見 [`fd2_ch23_post_ida.txt`](../data/ida/fd2_ch23_post_ida.txt)。
