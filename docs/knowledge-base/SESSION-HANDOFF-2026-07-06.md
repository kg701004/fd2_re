...
- 2026-07-26 target path-cost closure（official IDA 9.4 ASM）：修正 `0x4e555` 是 20-byte terrain-cost-row address helper，mask producer 是 `0x4e040`。它以 record dist 作 budget 的 cardinal flood-fill，每步按 tile type 扣 cost；grid bit40 block、bit80 cost=0。撤回任何「native spell range 為 Manhattan」暗示；需完整 tile costs 才能啟用 remake native target resolver。
