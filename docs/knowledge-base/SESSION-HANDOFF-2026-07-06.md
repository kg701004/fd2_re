...
- 2026-07-26 foreground roster-gate correction（Docker Capstone）：撤回「每個 visible unit 都進 `0x129ec`」的過度簡化。loop 先以 `0x1f183(slot)` 過濾，後以 `0x3453e(slot)` inactive 過濾；`0x1f183` 的 raw predicate 是 `group==0x1c` 放行，否則 class `0x13` 或 race `4/5` 回 true 而跳過。`NativeForegroundRedrawEligible` 已 regression 固定兩 gate；數值暫不命名，runtime roster／GUI adapter 繼續 fail-closed。
