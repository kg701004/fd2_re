...
- 2026-07-25 learning runtime asset binding：generated `command_learn.json` 已加入 `remake/assets/data`，Game 在 bootstrap 讀它並在 `resetBattle` 的每個新 State bind 同一 table；缺檔不回退 legacy Spells。Docker internal regression 全綠。UI package compile 已到 container 缺 ALSA `pkg-config` development metadata 的環境 gate（modules 正常下載），非 source/test failure。
