...
- 2026-07-27 church/class-change vertical trace：新增 `Game.applyChurchClassChange` 與 `TestCampaignTownChurchClassChangeReturnTrace`，驗證 editable branch 的 portrait09→portrait34/class21、MV 5→7、Exp reset、item `0x5a` 消耗與 Escape 回 `town_ch02`；新增 `town-church-class-change-ch02.json`。原版 class-target renderer、native callee 與 FD2.SAV 相容性維持 fail-closed。
...
- 2026-07-27 ch24 postbattle slice：Docker Capstone 證實 handler `0x24df2` 只有 dialog idx6、PAN raw `(4,16)`、spawn group2、ACT75、dialog idx7、JOIN26/sync/JOIN29/set_chapter25；map24 raw frontier=70、group2 僅一筆，PAN materialize 為 `(96,384)`。Docker acting exporter 解碼 resource75 為 4 beats、slot70/pose2；FDTXT_025 index6/7 count-aligned 為 scene2 lines0–17。新增 authored binding 與 compiler/campaign regression，`postbattle_ch24_persist` 接回 `town_ch25`；ch06/ch07/ch14/ch16/ch17/ch20/ch22/ch23/ch25/ch26/ch28 仍 fail-closed，未猜測 handler 或 renderer 語意。
