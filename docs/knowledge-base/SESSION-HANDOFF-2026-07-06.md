...
- 2026-07-26 FDICON pointer-table / cycle correction（Docker Capstone）：撤回把 `0x53a61` 留為未知 descriptor 格式的待辦，也撤回「unit+4 直接是 frame」。`0x11019(group)` 配置/填入每 group 12 pointers；`0x127e0` 的 index 為 `group×12 + pose×3 + cycle`。unit+4 僅乘 pose direction offset 進 screen placement；+4=0 選 global idle `0x3c0b`，非零選 moving `0x3c07`，cycle3→1，`+0x26` 強制0。這正是既有 FDICON 24×24 map sprite path；仍待 palette-remap selector 與 `0x129ec` layer order，不可誤連到 FIGANI battle archive。
- 2026-07-26 FDICON codec slice：新增 `internal/fdicon`，以 24×24 B24 offset table + four-mode RLE decode 保存 indexed pixels/transparent mask；`BlitAt` 可選 raw 或 strict 256-entry LUT，對應 `0x4deda/0x4de56`。Docker regression 同時覆蓋 fixture 的 run/dither/transparent/LUT 與原始 `FDICON.B24` 1680 entries。這是 asset/compositor primitive，未主張 native roster scheduling 或 UI 已完成。
