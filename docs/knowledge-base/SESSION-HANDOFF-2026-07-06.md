...
- 2026-07-26 scripted foreground caller closure（official IDA 9.4 ASM）：`0x129ec` 還有 `0x1366a` caller，故撤回它只屬於 `0x127a9` steady redraw 的隱含說法。`0x1366a` 在 step loop 的 base `0x11eee`、逐 slot `0x127e0` 後呼 `0x129ec`，之後才 `0x11eb0` 和 present/redraw；loop 會由 script-step bytes 改寫 runtime `unit+3`。完整 step input/presentation ABI 未接，GUI 仍 fail-closed。
