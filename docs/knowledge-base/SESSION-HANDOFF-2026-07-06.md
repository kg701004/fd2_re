...
- 2026-07-26 FDICON pointer-table correction（Docker Capstone）：撤回把 `0x53a61` 留為未知 descriptor 格式的待辦。`0x11019(group)` 配置/填入每 group 12 pointers；`0x127e0` 的 index 為 `group×12 + pose×3 + frame`，正是既有 FDICON 24×24 map sprite path。仍待的是 roster-driven pose/frame、palette-remap selector 與 `0x129ec` layer order；不可誤連到 FIGANI battle archive。
...
- 2026-07-26 FDICON selector slice：`Bank.SpriteFor(group,pose,frame)` 實作 `0x127e0` 的 `group×12 + pose×3 + frame` 並拒絕越界 pose/frame。現有 battle `Fig`/`Dir` 僅能供應 group/部分 pose；runtime +4 frame 未證實前維持 explicit，避免把 legacy movement offset 猜成 native animation state。
