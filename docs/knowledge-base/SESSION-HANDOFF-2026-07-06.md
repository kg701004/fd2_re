...
- 2026-07-26 0x276EC family expansion（official IDA 9.4 ASM）：同一 player handler 對 ID28 選倍率20、ID29 選12、ID31（default）選18，均沿一般 two-stage selector 與 `0x2b659→0x1ca89(actor,id)`；`ExecuteNativeCommandDerivedStrike` 因此將 24/28/29/31 限定接成 state-only final delta。ID30 在 `0x1cff0` 先改走 `0x149f8` special selector，再由 `0x2a6bd→0x276ec` 使用 default倍率18；因其 target array ABI 未重製，仍未泛化接線。IDs32..35 走 `0x27fc9`。
