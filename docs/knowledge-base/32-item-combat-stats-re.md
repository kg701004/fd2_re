...
### 6.7 原版側獨立實機驗證(2026-09-06,非 remake,回應 worklist L1565)

> 本節是**本輪新增的原版 live 證據**,不是重新反組譯——§6.1-§6.3 的機制早由官方 IDA/Capstone
> 在舊版 EXE 位址上閉合。本輪先在**本地 Ghidra 新版 EXE**(`FD2Analysis3`)上獨立重新定位這條
> 呼叫鏈(用 `xref_to`/`call_scan` 沿呼叫圖找,不假設位址平移):
> `FUN_00029daa`(教會主選單 dispatcher,`DAT_00053c57`==0..3 分派)→ index 3 呼叫
> `FUN_0002aa00`(轉職候選迴圈)→ `FUN_0002ae0e`(候選 filter,逐位元組對應 §6.1 的
> `Lv>=20 且 portrait<0x12 且 portrait!=7`)→ confirm 後 `FUN_0002ac7d`(轉職重算,內含
> 5 次 `FUN_0001e529` 呼叫,對應 §6.3 的五組 growth pair)。新版位址與舊版位址不同,但邏輯
> 與 §6.1-§6.3 逐條吻合,是**同一機制在新版 EXE 上的獨立再確認**。

**Live 驗證(DOSBox-X,`.wsl_build/chapter_sweep_v2/ch15/final.SAV`,原始存檔未經任何位元組
patch)**:該存檔 slot0 roster 13 人中,凱麗(portrait 0x0c,武者,Lv40)與洛鄉(portrait 0x05,
騎士,Lv40)兩人自然滿足候選條件(存檔本身就有 Lv>=20 的合格單位,不需要靠 save-file patch)。
教會 4 個 icon 依序對應 `DAT_00053c57`==0/1/2/3:①查看角色(狀態瀏覽)②誰的東西呢(item 領取)
③復活④**轉職**(此前 2026-09-05 的 session 誤把第 4 個 icon 看成武器/聖物「商店清單」——
那其實就是轉職候選清單,只是清單裡的「目標職業」欄位被誤讀成商品欄位;本輪已用真實遊玩畫面
文字「誰要轉職呢？」逐字確認,舊誤讀作廢)。

三筆真實成長樣本(凱麗,武者→武聖,皆為同一存檔在**完全獨立、重新 launch 的 DOSBox-X instance**
上重跑,非同一 process 內連續操作):

| 樣本 | 前置動作 | STR | DEF | SPD | MHP | MOVE |
|---|---|---|---|---|---|---|
| #1 | 直接進教會 | +13 | +7 | +5 | +16 | +1 |
| #2 | 直接進教會(與#1**逐鍵重播完全相同**的輸入序列,全新 fresh boot) | +13 | +7 | +5 | +16 | +1 |
| #3 | 先到道具店買一瓶回復劑(真實交易對話)才進教會 | +14 | +8 | +4 | +16 | +1(觀察) |

**結論一(決定性重播)**:#1 與 #2 逐項 bit-for-bit 相同,即使是完全獨立的 fresh WSL/DOSBox-X
boot、逐鍵手動重播相同輸入序列。這比 worklist L1565 原本「同一 boot 內按 0~45 個城鎮方向鍵不
變值」的觀察更強——**連重新開機都不變值,只要輸入序列本身相同**。

**結論二(worklist L1565 的解法驗證成功)**:#3 在完全相同的存檔與角色下,只因為進教會前多做了
一次真實商店交易(買回復劑→分寶物給角色→取消退出商店),STR/DEF/SPD 三項全部改變(13→14、
7→8、5→4),MHP 不變(16→16,可能是該職業轉換的 MHP growth range 很窄剛好都落在同一整數格,
不代表方法無效)。**這證實了 doc58「2026-09-03 續五」提出的假說是對的**:讓 RNG 消費者狀態
往前推進的是「走過不同的前置交易/對白」,不是城鎮方向鍵移動或開機次數。**worklist L1565
的「大樣本 range 驗證仍未做」現在有一個已驗證可行的取樣方法**:每次取樣前,插入一段長度/內容
不同的商店交易或對話,再進教會轉職。

**誠實範圍**:①只驗證了武者→武聖這一組轉換,STR/DEF/SPD/MHP/MOVE 五項也只各有 2-3 個樣本,
遠不到「大樣本」門檻,doc27/doc58 既有的 growth range JSON 邊界仍未被本輪數字觸及或推翻。
②MOVE 三次都是 +1,樣本太少無法判斷是否為該轉換固定值或純粹巧合。③洛鄉(騎士→聖騎士)只有
1 個樣本(STR+9/DEF+8/SPD+3/MHP+12/MOVE+1),未做重播或變異對照。④item-gated 的「+0x32」
...
- **L245**(doc27§5 剩餘清單:經驗值攻守等級因子/`0x2f7b6` cVar4 分支/6 種傳送魔刃經驗公式/法術命中率
  ...
  **2026-08-30 更新**:此項已由 `doc27 §6.6` 收口——36 筆逐 byte 核對 0 誤差,並確認 `magic.go rollsHit` 的
  `hit=0→恆中`特例與原生 `FUN_0001c75e` 的 `record[+2]=0→恆 miss` 語意方向相反,已記錄為未修正 bug,詳見該節。
...
> **同輪自我訂正（範圍誠實化）**：上面第 2、3 點裡「`FUN_0002ac7d` 是新版轉職合成函式」與
> 「`FUN_0002aa00` → `FUN_0002ae0e` 這條 caller 鏈」**並不是本輪首次發現**——本檔 §6.3.1 與
> L748/L949 早已用 `xref_to 0x2ac7d` 記載過同一條鏈（含「舊文件 `0x31385`/`0x31571`/`0x3151a`
> 鏈的新版對應」這個對照）。本輪是獨立重推一次並確認結果一致（可當交叉驗證），但**真正新的
> 只有兩件事**：(a) 道具表在新版 EXE 的位址 `0x523d5`（先前全庫查無此值，也正是 §6.5 輪轉懸念
> 一直卡住的原因——鏈路雖已定位，但沒人往下抓到這張表）、(b) 用它對 JSON 做的逐 byte 核對與
> 3 筆修正。敘述若讓人以為整條鏈都是本輪新解，會高估本輪貢獻，故在此訂正。

## 2026-09-11:`0x602ad` 效果列 `+0x0d..+0x12`/`+0x15` 的語意全部由消費端解出(K[0..5] 收尾)

worklist 1354 收斂時把 `+0x0d..+0x12` 記為 `K[0..5]`,並明說「仍未命名的是**語意**而非位置」,
殘留 `K[1]`/`K[2]`/`K[4]`/`K[5]` 與「`+0x15` 為何鏡像 `+0x11`」。本輪由**消費端**逐一解出——
方法一律是「先找誰讀它、讀它做什麼」,不從欄位形狀猜(見
`[[feedback_name_a_field_from_its_consumer]]` 的教訓)。全程靜態離線。

### 先確立:消費端掃描是窮舉的,不是抽樣

`tools/disasm_le.py refs 602ad` 對整個 code image 找被 fixup 成 `0x602ad` 的位置,**只有一筆**:
`0x4e8cb`(在 `0x4e8bc` 內)。因此 `0x4e8bc` 是這張表**唯一**的取址閘門,
`tools/derive_item_row_fields.py` 對它的 23 個呼叫端所做的欄位讀取掃描,涵蓋的是全部消費端,
沒有「另一條 inline 算位址的路徑」漏在外面。

> **順帶勘誤(doc27 §「`FUN_0004e8bc`」段、本文 §B)**:兩處都寫「與已知 item pointer helper
> `0x4e56c`(table base `0x602ad`、stride `0x17`)是同一張表的第二個編譯副本」。逐指令反組譯
> `0x4e56c` 實際是
> `mov al,[0x60068] / mov ah,[0x6006f] / mul ah / add eax,[0x6006e] / shl 2 / add 7 / add [0x60064]`,
> 即 `格子緩衝區 + 7 + (x + y*width)*4` 的**地圖格位址計算**,與 `0x602ad` 無關;`call_scan`
> 對 `0x4e56c` 的呼叫端數是 **0**(它根本不是 function entry,落在 `FUN_0004e4f6` 中段)。
> 「第二個編譯副本」這個說法應撤回。
...
| 偏移 | 舊名 | 定案語意 | 釘死它的消費端 |
|---|---|---|---|
| `+0x0d` | K[0] | **效果型態**(值域 `0` 與 `5..24`;`0` = 無效果,182/215 筆) | `0x20cad` 的 switch 分派 |
| `+0x0e` | K[1] | **效果強度(u16)** | `0x20cb5` `movzx edx, word ptr [eax+0xe]` |
| `+0x0f` | K[2] | 效果強度的高位元組 | 同上(整字讀取) |
| `+0x10` | K[3] | **範圍形狀碼(選取階段)** | `0x1bd2d` → `0x14818` param_4;`0x150c2` → `0x149f8` steps |
| `+0x11` | K[4] | **敵我關係選擇器**(`0`=對敵方,`1`=對我方) | `0x150a2`(AI 路徑),取反後當 `0x14818` param_6 |
| `+0x12` | K[5] | **範圍形狀碼(生效階段)** | `0x1bd7d`/`0x150fe` → `0x14818` param_4 |
| `+0x15` | K[4] 鏡像 | 同 `+0x11`,玩家路徑專用的第二份 | `0x1bd24`(玩家路徑),直接當 `0x14818` param_6 |

`+0x10`/`+0x12`/`+0x11`/`+0x15` 如何流進 `0x14818`、以及「同一個選擇器在玩家/AI 兩條路徑上
極性相反正是它的語意」,完整論證在 doc27 §6.7,此處不重複。

**`+0x0e` 是 u16 這件事是機器層證據,不是資料擬合。** `0x20cb5` 用 `word ptr` 整字讀。
資料側一致:`+0x0f` 非零的恰好是 5 筆(力之杖 300、光之杖 500、修理套件 500、再生藥 300、
神聖之水 999),其餘 210 筆 `+0x0f == 0` 且 u16 值等於 `+0x0e`。

**`+0x0d` 是 switch 選擇器,不是旗標。** 套用端:

```
0x020cbd  mov    bl, byte ptr [eax + 0xd]
0x020cc3  cmp    eax, 5
0x020cc6  je     0x20ccd
0x020cc8  cmp    eax, 0xd
0x020ccb  jne    0x20cf6
0x020ccd  push   dword ptr [esp + 0x68]     ; ← +0x0e 的 u16 強度
0x020ce0  call   0x211a4
```

型態 `5` 與 `13` 共用 `0x211a4` 並吃強度值,道具名也一致(型態 5:藥草 40 / 回復劑 120 /
再生藥 300 / 修理套件 500 / 神聖之水 999;型態 13:光之斧 100 / 光之眼 200 / 力之杖 300 /
光之杖 500),兩族都落在「對我方施放」那 23 個道具裡。

**一個查過但不成立、記下以免後人重做的交叉檢查**:曾想用「強度應與售價(`+0x13` u16)同向」
再驗一次,**不成立**——修理套件(強度 500 / 售價 1000)vs 神聖之水(強度 999 / 售價 800)就反了,
型態 13 更亂(光之眼、光之杖售價都是 2,顯然是劇情道具)。只有把範圍縮到 4 個純回復消耗品
(藥草 40/10、回復劑 120/80、再生藥 300/300、神聖之水 999/800)才單調,那是事後挑樣本,
不能當證據。命名依據是 `word ptr` 那條指令,不是這個相關性。
