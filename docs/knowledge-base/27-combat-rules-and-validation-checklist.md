...
**2026-09-06補充(回應worklist L848「等級上限是否有社群傳聞的第三層級80」)**：重新完整
decompile `0x1e292`(不只讀第14/15項摘要的既有轉述)發現迴圈內的重置條件比表格摘要的
「達職業等級上限('c'=99或'('=40)則local_18=0」多一個**先前轉述漏掉的OR分支**：
```
if ( ((cVar1=='\x1e'||cVar1=='\x1f') && (unit+0x21=='c'/*99*/))
   || (unit+0x21=='\x1e'/*30,不是40!*/) )
{ local_18 = 0; }
```
`cVar1`(portrait)跟`unit+0x21`(等級)兩個各自比對`0x1e`是巧合的同一個byte值用在不同欄位，
容易在手動轉述時被誤判成同一條件而漏掉——**這第二個OR分支是無條件的：不管portrait是誰，
只要等級升到30就會把當次exp累加歸零**，跟蓋亞/渥德的99/40上限判定是獨立的機制。**誠實
範圍**：這證實了**等級30是一個全角色通用的exp處理硬停止點**，但本輪沒有找到、也不宣稱
證實這代表「轉職(worklist #247)發生在30級」——只是一個新發現的、先前任何文件轉述都
沒提到的具體byte-exact行為，兩者是否有因果關係待下一輪對照轉職觸發條件核實。「社群傳聞
的80上限」在`0x1e292`全函式範圍內找不到任何`0x50`(80)常數，此假說依然沒有反組譯支持，
維持第5項既有的「兩分支、無第三分支」結論不變，只是新增了這個之前被轉述遺漏的30級細節。
...
## 6.7 `0x14818` 的第二、第三個呼叫端家族(道具路徑)——並訂正 §6.4/§6.2 的四處(2026-09-11)

> 方法:純靜態。`tools/ghidra_batch_probe.py`(`FD2Analysis3`,`-readOnly -noanalysis`)取
> `0x14818`/`0x1bbdc` 的 decompile 與 function_bounds,再用 `tools/disasm_le.py` 對每個
> `CALL` 前的 `PUSH` 序列逐指令核對實參;資料側用 `docs/data/exe_tables/native_item_effect_rows.json`
> 全 215 列比對。全程離線,未執行遊戲。

§6.4 把 `0x14818` 追到「唯一上游是 `FUN_0001cff0` 的法術/command ring 路徑,record 來自
`0x4e866`,range 欄位是 `record[+3]`/`record[+4]`」。這個結論本身沒錯,但它只覆蓋了一個呼叫端家族。
本輪從**道具表** `0x602ad`(`0x4e8bc`,stride `0x17`)的消費端反向追,找到另外兩個:

| 呼叫端 | 誰在行動 | record 取得 | param_4(形狀) | param_5(排除半徑) | param_6(陣營選擇器) |
|---|---|---|---|---|---|
| `0x1d2bf` / `0x1d32a`(§6.4) | 玩家 command ring | `0x4e866` | `record[+3]` / `record[+4]` | 固定 `0` | `record[+6]` |
| **`0x1bd43` / `0x1bd93`(新)** | **玩家用道具** | `0x4e8bc` | `row[+0x10]` / `row[+0x12]` | **`row[+0x0d]==0x17`** | `row[+0x15]` |
| **`0x15114`(新)** | **敵方 AI 用道具** | `0x4e8bc` | `row[+0x12]` | 固定 `0` | **`row[+0x11]` 取反** |

玩家用道具的呼叫端是 `FUN_0001bbdc(actorIdx)` 的 `DAT_00053c57 == 0` 分支(`0x1bd03..0x1be60`);
AI 用道具的是 `0x15055`(worklist 541 已釘死:`0x14ef0` 三選一仲裁中 `[0x53c33]` 勝出時走的那支)。

**訂正一:§6.4 的「`extraThreshold` 在兩次呼叫裡都固定傳 `0`,永遠不執行」只對法術路徑成立。**
道具路徑第一次呼叫傳的是 `sete dl` 的結果:

```
0x01bd1a  movzx  eax, byte ptr [eax + 0xd]     ; 道具效果列 +0x0d
0x01bd1e  cmp    eax, 0x17
0x01bd21  sete   dl                            ; → param_5
```

全 215 列中 `row[+0x0d] == 0x17` 的**恰好只有 1 筆:id 101 傳送法杖**。也就是說 §6.4 判為死碼的
那段曼哈頓距離迴圈,在全遊戲裡會為這一個道具執行一次,`extraThreshold = 1` →
把 `距離 < 1`(即施術原點自己那一格)標成 `0xff`。由於末段收集迴圈的條件是
「該格 `!= 0xff`」,效果是**把站在原點的單位本身排除在候選之外**。措辭已由「永遠不執行」
改為「法術路徑永遠不執行;道具路徑僅 id 101 會執行,且作用是排除原點」。

**訂正二:§6.4 的陣營選擇器標籤與 doc03/doc10/doc13 相反。** §6.4 寫「0=己方 `==0`」,
但 `unit[+6]` 的定案語意(doc03 表、doc10 §、doc13:1617,皆由 `0x10c50` constructor trace)是
**`00=敵、01=友、02=己方`**(出處是 `0x10c50` constructor 的 direct trace),所以 `==0` 是**敵方**。道具資料獨立佐證這一點:33 個
`+0x0d != 0` 的道具中,選擇器為 `0` 的 10 個全是攻擊道具(炎龍劍/戰神戟/閃電鎚/風神弓/魔法杖/
封咒杖/龍之杖/光束炮/雷神鞭/暗之眼),為 `1`(`!= 0`,同時涵蓋 camp 1 與 2)的 23 個全是
回復/強化/傳送類。攻擊道具指向 `camp==0`,只有在 `camp==0` 是敵方時才說得通。

**訂正三:§6.4 說 `rangeByte >= 0x10` 是「直線/十字類形狀」,現在有具名實例可釘。**
該分支以 `rangeByte-0x10` 為門檻,分別把**通過原點的整列與整欄**寫 `0`(= 在範圍內;
`0xff` 才是範圍外)。全 215 列裡 `+0x10 >= 0x10` 的只有 **id 79 光束炮,值 `0x1f`**,
arm 長度 `0x1f-0x10 = 15`,即一條貫穿全圖的十字。名稱與形狀互相印證。

**訂正四(§6.2 附記):`0x149f8` 的步進次數不是 spellId,那個「可疑」可以結案。**
§6.2 附記記下一個未排除的疑點:「傳入的步進次數在此路徑上等於 spellId 本身
(`0x150d6: PUSH EAX` 緊接 `0x150d3: SUB EAX,0x10` 的結果),不是任何距離/半徑欄位——
若 spellId=0 該迴圈完全不執行……尚未排除是筆者參數對應錯誤」。逐指令追 `EAX` 的來源:

```
0x015091  call   0x4e8bc          ; 道具效果列
0x015099  mov    esi, eax
0x0150c2  mov    al, byte ptr [esi + 0x10]   ; ← EAX 來自 row[+0x10],不是 spellId
0x0150cc  cmp    eax, 0xf
0x0150cf  jle    0x150fb                     ; <=0xf 走 0x14818
0x0150d3  sub    eax, 0x10
0x0150d6  push   eax                         ; → 0x149f8 的 steps
```

`EAX` 是道具效果列的 `+0x10`,與玩家路徑 `0x14818` 的 `>= 0x10` 十字編碼是同一套。
且 `jle 0x150fb` 保證只有 `row[+0x10] > 0xf` 才走這條,故 `steps >= 1` 恆成立——
「spellId=0 則迴圈不執行」這個懷疑的前提不存在。附帶:§6.2 附記把
`0x14ef0`/`0x15055` 這一叢稱為「AI 法術執行」,就 `0x15055` 這一支而言應正名為
**AI 道具使用**(它取的 record 是 `0x4e8bc` 道具表);`0x14ef0` 仲裁的另外兩支不在本輪範圍。

**AI 與玩家對同一個欄位的極性是相反的,而這正是它的意思。**
玩家路徑把 `row[+0x15]` 原封不動當 param_6;AI 路徑則:

```
0x0150a2  movzx  eax, byte ptr [eax + 0x11]
0x0150a6  test   eax, eax
0x0150a8  jne    0x150b1
0x0150aa  mov    ebx, 1            ; +0x11 == 0 → param_6 = 1
0x0150b1  xor    ebx, ebx          ; +0x11 != 0 → param_6 = 0
```

攻擊道具 `+0x11 = 0`:玩家用 → `camp==0`(敵方);敵方 AI 用 → `camp!=0`(玩家/友軍)。
兩邊都指向「對方」。所以 `+0x11`/`+0x15` 的語意是**敵我關係選擇器**
(`0`=對敵方施放,`1`=對我方施放),由各自的行動方在執行期解析成實際 camp 碼,
而不是一個絕對的 camp 編號。這也解釋了 worklist 1354 記下的「唯一的實質鏡像」:
`+0x11` 與 `+0x15` 全 215 列相等,是因為**兩條路徑各自從不同偏移讀同一個語意欄位**
(`+0x11` 只被 `0x15091` 讀、`+0x15` 只被 `0x1bd03` 讀,消費端掃描結果見
`docs/data/item_row_field_consumers.json`),資料側必須讓兩欄保持同步。

## 6.8 §6.3 的獨立複驗:全部成立 —— 並釐清它為何對、而 §10 為何錯(2026-09-11)

### 6.8.1 為什麼要複驗一個「已經有結論」的章節

本輪原本要拿 §6.3 的結論(`0x2a6bd`/`0x276ec` 不是有效指令邊界)當作「已證錯位址登記表」的
種子資料。**使用者指出這個前提本身沒有被複驗過**——doc25 §10/§11.7 的教訓正是:一個寫得
很有信心的舊結論,可能整輪都建立在錯的前提上。**把未複驗的結論拿去建阻擋機制,等於替一個
可能的錯誤加上權威。** 所以先複驗。

方法刻意與 §6.3 不同:§6.3 用 Ghidra 的 `getInstructionContaining`/`getFunctionContaining`;
本輪只讀原始位元組(`le_xref` + `load_code`),自行建出全 image 的 Watcom 序頭集合,
不呼叫 Ghidra、不看 §6.3 的推論。

### 6.8.2 逐項核對:五項全部相符

| §6.3 的主張 | 原始位元組複驗 |
|---|---|
| `0x2a6bb` 是 4-byte `8B 44 24 24` | 實測 `8b 44 24 24` **相符**;`0x2a6bd` 落在該指令第 3 個 byte |
| `0x276e9` 是 6-byte `CMP EAX,[0x53c57]` | 實測 `3b 05 57 3c 00 00` **相符**;`0x276ec` 落在第 4 個 byte |
| `FUN_0002a694` body 451 bytes | `0x2a694` 是序頭,下一個序頭 `0x2a857` → **451** 相符 |
| 正解是 `0x2ff01`,3876 bytes | `0x2ff01` 是乾淨序頭,實測 **3876** 相符 |
| id=0x18 的正解 `0x2cf30`,2269 bytes | 序頭 ✓,實測 **2269** 相符 |

**另補一項 §6.3 沒做、但 §19 之後必須做的檢查**:`0x2a6bd` 會不會也是「未重定位值」
(即真正的位址是 `+0x10000`)?`0x3a6bd` 與 `0x376ec` **都不是序頭**,且分別落在
`0x3642e`+`0x428f` 與 `0x3642e`+`0x12be` 這種毫無關係的深處——**重定位假說排除**。

順帶一個獨立的旁證:`0x2ff01` 的 3876 bytes 是**全 image 最大的函式**(第二名 `0x2ebe1`
是 2291),所以 §6.3「規模與『大型 presentation/state dispatcher』的描述相符」不是修辭。

### 6.8.3 §6.3 對、§10 錯,差別不在仔細程度,在**選了會不會說「不」的 API**

兩輪用的是同一個 Ghidra、同一份 project,結論品質卻相反:

| | §6.3(2026-08-19,**對**) | doc25 §10(2026-08-20,**錯**) |
|---|---|---|
| 用的 API | `getInstructionAt` / `getInstructionContaining` / `getFunctionContaining` | `createFunction` + `DecompInterface` |
| 該 API 能不能回答「不是」 | **能**——`getInstructionAt` 回 `null`,`getFunctionContaining` 回別的函式 | **不能**——在任何位址都會成功生出一個函式 |
| 結果 | 當場指出位址落在指令中段 | 從垃圾入口生出看似合理的偽代碼 |

§10 其實**有**收到訊號:它自己記著「約半數出現 `unaff_EBX`/`halt_baddata` 等 Ghidra 邊界錯誤
訊號」,並把它解釋成「本次用的邊界猜測法在那個位置不成立」。**訊號被歸因到邊界猜測法,
而不是入口位址本身**——那正是它接下來五輪都沒回頭懷疑位址欄的原因。

**可轉移的判準**:對「這個位址是不是函式」這類問題,只採用**能回傳否定**的機制。
`createFunction` 這類「必定成功」的 API 不能拿來當證據,它的成功不帶資訊。

### 6.8.4 一筆尚未處理的量測:已證錯位址(`0x2a6bd`/`0x276ec`/`0x354fe` 等 27 個)仍有 275 次現存引用

以本節複驗過的 `0x2a6bd`/`0x276ec`,加上 doc25 §19 今天證實的 25 個未重定位值(`0x354fe`..`0x360f8`,共 27 個),
掃描 `docs/knowledge-base/*.md` 的 `0x[0-9a-f]{4,6}` 字面值:

| 位址 | 現存引用 | 出處 |
|---|---:|---|
| `0x2a6bd` | 89 | §6.3(本節複驗) |
| `0x276ec` | 42 | §6.3(本節複驗) |
| `0x354fe`(event 58) | 41 | doc25 §19 |
| 其餘 24 個 §10 值 | 103 | doc25 §19 |
| **合計** | **275**,分布於 **13 個檔案** | |

檔案分布前四名:`25-battle-event-system.md` 115、`91-worklist.md` 50、`56-fd2-remake-sdd.md` 21、
`SESSION-HANDOFF-2026-07-06.md` 21。

§6.3 當年寫「本文件僅記錄此勘誤……**未回頭修改 doc13/37/56 的既有引用**(超出本輪任務範圍)」。
**23 天後那個決定的代價是可量化的**:任何人今天打開 doc13/doc37 讀到的仍是 `0x2a6bd` 這個落在指令中段的位址。
這不是「沒人檢查」,是**修正不會傳播**。

**同一個檔案內就看得到這個斷裂**:`91-worklist.md` 第 1833 行有一段「⚠ 位址勘誤總註記(2026-08-19/20,兩個獨立批次交叉印證)」明確記著 `0x2a6bd`/`0x276ec` 不是有效指令邊界;而同一檔第 441 行,項目 555 的**標題**仍寫著「原版 dataflow(`0x2A6BD`/`0x1C75E`/`0x21AD9`)本身無缺口」——**勘誤與被勘誤的主張並存於同一份文件,相距 1392 行,沒有任何機制讓前者作用到後者**。

**誠實範圍**:上表的 275 是**一次性腳本**的輸出,尚未寫成工具、也還沒登記成 finding,
...
## 6.10 用 §6.9 的同一套判準往下挖:三個「已經被訂正過、卻從未登記」的位址(2026-09-11)

### 6.10.1 分流的漏斗,以及每一層的控制

`tools/verify_address_claim_coverage.py --triage`:

| 層 | 數量 | 判準與控制 |
|---|---|---|
| 知識庫宣稱為入口的 obj1 位址 | 1368 | |
| 三個位元組訊號全無 | 954 | 正向控制:errata 的 obj1 位址 13/13 全部落在此 |
| **不落在合法指令邊界** | **184** | **誤報 0/150** 已知正確入口;對已知錯誤召回 **10/14** |

判準只從**所在函式的入口**單一起點線性反組譯 —— 與 doc27 §6.3 當年用 Ghidra
`getInstructionContaining` 同源。

### 6.10.2 一個被自己的控制打掉的做法(留下來避免重做)

原本想用**多起點收斂**強化:從目標位址之前 k 個 byte 各起一次反組譯,多數同意才算數。
直覺上更穩健,**負向控制直接否決**:14 個已登記勘誤的位址裡有 **5 個**
(已勘誤的 `0x276ec`/`0x27fc9`/`0x320fc`/`0x3453e`/`0x36d98`)被多起點判成「是邊界」。

原因是短視窗從任意偏移起算會湊出**自洽但錯誤**的解碼 —— 線性反組譯的經典偽陽性,
與本專案既有的「線性反組譯會失去對齊」是同一件事的另一面。單一起點反而乾淨。
**不要再試這條路。**

### 6.10.3 判準的結構性上限

抓不到的 4 個(已勘誤的 `0x27fc9`/`0x320fc`/`0x3453e`/`0x36d98`)是**錯位址但剛好落在合法邊界上**:它們指向函式**中段**。
**「是邊界」不等於「是入口」**——這是上限,不是 bug,已釘進 selftest 第 (9) 題
(要求不得低於 10/14,而不是要求 14/14)。

### 6.10.4 三個確認為真、且從未登記進 `known_address_errata.json` 的位址

| 位址 | 現存引用 | 已有的訂正出處 | 本輪的獨立確認 |
|---|---|---|---|
| `0x354fe` | 21 | doc25 L1129「**位址更正**:本節原標「handler `0x354FE`」」 | `range` 反組譯:落在 `0x354fc push 0xcd` 這條 5-byte 指令的第 3 個 byte |
| `0x35898` | 8 | doc25 L2241 明記它落在 `0x35895: FF 74 24 20` 的**最後 1 byte** | 邊界判準一致 |
| `0x2a2e8` | 7 | doc32 L747「`FUN_0002ac7d`(轉職重算——**舊文件 `0x2a2e8` 的新版對應**)」 | 邊界判準一致;屬舊版 EXE 位址類 |

**三個都已經被訂正過了 —— 只是訂正沒有被登記,所以 citations 棘輪蓋不到它們,
而錯誤形式仍在別處被引用**(例如 `0x354fe` 仍出現在 doc11 L1229、doc25 L948)。
這是本日第三次遇到**同一個形狀**:訂正與被訂正的主張並存於知識庫,中間沒有機制連接。

### 6.10.5 誠實範圍 —— 特別是我**沒有**做的事

* **沒有批次登記那一批。** 「同一行有訂正字樣」這個啟發式的**行粒度**問題與 citations
  工具相同:抽查 8 筆,`0x35ed2`/`0x35d60` 命中的是 `91-worklist.md` L205 那條超長的項目
  標題(根本不是在訂正它們)、`0x35822` 那行的「已證實的勘誤」講的是 **PUSH 順序不是位址**。
  所以「59 筆已訂正未登記」是**上界**,抽樣顯示約 3 真 / 2 假 / 3 待判。
* **拿未逐筆驗證的前提去餵一個會擋人的機制,錯誤會帶著權威擴散** —— 這是使用者在本日稍早
  明確制止過的做法(「doc27 §6.3 當年說的先不使用,另外重新驗證」),同樣適用於此。
* 三筆的 `correct_address` 只有 `0x2a2e8`→`0x2ac7d` 是文件直接寫明的;另兩筆要讀完
  doc25 §11.7 才能填,本輪未做。
* 184 這個數字**只涵蓋 obj1 的入口主張**;資料段的結構主張仍然沒有分母。
