...
**誠實範圍**:`0x3d670` / `0x3d5c0` 本體本輪未展開,所以「這是 C runtime 的 free/malloc
實作」是由**呼叫形狀 + 共用 heap 描述子 + 對稱性**推得,不是逐指令證實到 allocator 內部;
`0x5419c` 這個旗標的用途也未追。doc13 的「未證實」註記本輪不刪除,改為指向本節。
