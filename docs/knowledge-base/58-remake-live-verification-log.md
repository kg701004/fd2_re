...
### 2026-09-03 五續：把上一節剛發現「沒有有效截圖」的「售出→欄位清空」用原版重測補回

上一節指出`shop-sell-slot-cleared-original-dosbox.png`與transfer那張逐位元組相同、且內容
（列表裡`淬毒刀`仍在）並不呈現欄位清空，所以該結論**當時沒有有效佐證**。本輪直接在原版把
整條售出流程重走一次補上證據。

**流程（ch02羅德鎮武器店，全原版、無任何hook）**：進武器店→Escape回到「還要什麼嗎？」四項
服務選單→Enter→**角色名冊**（索爾/悠妮/亞雷斯/蓋亞/哈諾/希莉亞＋向下箭頭）→選索爾→
出現他的**可售出品項與售價**列表。
...
| 階段 | 畫面內容 | 持有金錢 |
|---|---|---|
| 售出前 | 索爾持有`巨神戟 +AP320 $18000`、`龍神鎧甲 +DP300 $00000` | `$10070183` |
| 確認框 | 「這個巨神戟，18000元，賣不賣啊？」YES/NO | `$10070183` |
| 售出後 | 回到角色名冊 | **`$10088183`** |
| 重開索爾欄位 | **只剩`龍神鎧甲 +DP300 $00000`，`巨神戟`已消失** | `$10088183` |

`10088183 - 10070183 = 18000`，與確認框報價**完全相符**；欄位清空也直接目視確認。三態MD5
各異(`d98f73ab`/`b26a4dd8`/`c73c534e`)。新圖：
[`shop-sell-ch02-slot-cleared-original-dosbox.png`](../figures/shop-sell-ch02-slot-cleared-original-dosbox.png)

**結論**：「售出後該欄位清空、金錢正確增加售價」這條結論**本身是對的**，只是先前配的截圖
掛錯；現在有純原版、含前後對照與金額算式的有效證據。順帶記錄兩個原版行為細節：
(a)售價直接顯示在角色欄位列表上（不是另開報價畫面）；(b)`龍神鎧甲`售價為`$00000`，
即存在「售價為0」的品項，售出流程對這類品項的行為本輪未測。

**誠實限制**：只測了索爾一人、一件商品、一次售出；`$0`品項是否可售、售出後是否可取消/還原、
以及裝備中品項能否售出都沒測。名冊第二頁同樣未捲動。

**清理**：`sell`instance已`teardown`、workdir已刪除，`status`確認清空。

### 2026-09-03 六續：★ 原版商店「服務選單」沒有任何可見的選取標示——這解釋了舊shop文件為何
容易把截圖歸錯服務

**發現經過**：想補裝備/轉移兩個子流程，先要知道怎麼在「還要什麼嗎？」的4個服務圖示之間切換。
實測`Left`/`Up`/`Down`/`Right`四個方向鍵，**畫面變化像素數全部是0**——不是變化很小，是逐位元組
完全相同。乍看像「方向鍵在這個畫面無效」。

**但方向鍵其實是有效的，只是完全沒有視覺回饋**：同一個服務選單下

| 操作 | 實際進入的服務 | 判別依據（畫面內容） |
|---|---|---|
| 直接`Enter` | **購買** | 商品列表→「這個布衣，50元，**要不要啊**？」YES/NO |
| `Right`→`Enter` | **售出** | 角色名冊→索爾欄位列出`巨神戟 +AP320 $18000`／`龍神鎧甲 +DP300 $00000`（**售價**）→「**賣不賣啊**？」 |

兩者是同一個instance、同一個服務選單、只差一個`Right`，卻進到不同服務，證明**隱形游標確實
會移動**，只是遊戲不繪製任何highlight/游標框在那4個圖示上。`Right`按更多次（×2、×3）本輪
沒能穩定分辨出裝備/轉移（都停在名冊畫面，未進一步確認），故**4個圖示與index的完整對應本輪
未完成**，只確認了index0=購買、index1=售出這兩格。

**對本專案的方法論意涵（這才是重點）**：因為選取狀態在畫面上**完全不可見**，任何
「我們驗證了商店的第N項服務」的記錄，都**不能**用「我按了N次方向鍵」當依據，只能用
**進入後畫面本身的特徵**判定：購買=「要不要啊？」、售出=角色欄位帶售價＋「賣不賣啊？」、
轉移=「誰的東西呢？」（doc58既有shop段落記載的prompt）。這正好也是一個合理的解釋，說明
上一節發現的「`shop-sell-slot-cleared`與`shop-transfer-result-verified`兩張圖搞混」這類
歸錯服務的錯誤，在這個UI下**特別容易發生**且不易自我察覺。

新圖（3態並排：服務選單無highlight／Enter→購買／Right+Enter→售出）：
[`shop-service-selector-invisible-original-dosbox.png`](../figures/shop-service-selector-invisible-original-dosbox.png)

**誠實限制**：裝備、轉移兩個子流程本輪仍未真正走到（只確認名冊會出現）；4圖示與index的
完整對應未定；隱形游標的初始index、是否wrap、Escape後是否重置都未測。

**清理**：`svc`instance已`teardown`、workdir已刪除，`status`確認清空。

### 2026-09-03 七續：教會名冊完整捲動（13人全員）＋`record+0x08` character id ↔ 姓名對照表
用原版UI獨立確認

上一節留下的「名冊第二頁未捲動」缺口，本輪補完。教會→Enter→名冊，用`Down`逐步捲動，
三頁截到全部13名成員（每頁6格、右側箭頭指示可上/下捲）：

| 顯示順序 | 姓名 | 該slot存檔`record+0x08` id |
|---|---|---|
| 1 | 索爾 | 0 |
| 2 | 悠妮 | 9 |
| 3 | 亞雷斯 | 4 |
...
**交叉驗證**：同一份存檔用`tools/fd2save.py`解出
`roster_count=0x0d`(13)、`roster_char_ids(record+0x08)=[0,9,4,30,1,8,2,10,13,12,5,11,6]`
——**人數與順序都與原版UI逐格顯示的名單完全一致**。這等於用「原版UI實際顯示」這條完全獨立
的管道，再次確認了`fd2save.py` docstring裡`UNIT_CHARACTER_ID_OFFSET`(record+0x08)
「PROVEN」等級的欄位語意與其記載的招募順序；先前那次確認是靠live patch＋三份存檔靜態
比對，本輪是第三條獨立證據（原版畫面直接把名字列出來）。

每頁MD5各異(`0d47a4f8`/`7d398cae`/`89cd01a6`)。新圖：
[`church-roster-scroll-ch02-original-dosbox.png`](../figures/church-roster-scroll-ch02-original-dosbox.png)

**附帶**：名冊每格左側的地圖小圖示與姓名一一對應，可作為「character id → FDICON圖示」
對照的原版基準（本輪未逐格解出FDICON group編號，只留基準圖）。

**教會選到成員後的畫面**：顯示該成員的裝備／持有品，本例為
`先鋒矛 +AP085`、`魔法皮甲 +DP048`、`飛龍卵 ???`。兩個UI細節值得記錄：
(a)**圖示外框顏色有區別**——前兩項（裝備中）是**紅框**，第三項是**藍框**，
與武器店購買清單裡未持有品項一律藍框、以及索爾售出清單裡兩件都紅框的呈現一致，
看起來紅框＝已裝備／裝備類、藍框＝道具類，本輪未進一步驗證這個歸納；
(b)`飛龍卵`的效果值顯示為**`???`**，與doc58既有shop段落記載悠妮持有的`領悟之書 ???`
同樣型態，代表原版對某類特殊道具刻意不顯示數值，不是缺資料或解析失敗。
新圖：[`church-member-items-ch02-original-dosbox.png`](../figures/church-member-items-ch02-original-dosbox.png)

**誠實限制**：教會的**復活**與**轉職**兩個實際服務本輪仍未走到（只到成員資訊畫面）；
依上一節發現的「服務選取無視覺回饋」特性，教會很可能也是同一套隱形游標機制，但本輪未驗證。

**清理**：`ch`instance已`teardown`、workdir已刪除，`status`確認清空。

### 2026-09-03 八續：★ 用新工具把「隱形服務選單」的完整對應表跑出來——商店/教會共8個服務全部
識別，剩餘的裝備/轉移/復活/轉職四項一次補齊

**方法**：用`tools/fd2_original_verify.py`（本日新建，見doc98）平行驅動。因為服務選單**沒有
任何可見選取標示**（本文件2026-09-03六續），唯一誠實的識別方式是「按Right N次→確認→看
出現的畫面自己寫了什麼」。

**第一次嘗試失敗，而且是工具幫忙擋下來的**：原本想在同一個instance裡連續探測4個index，
每次探完用`Escape`退回服務選單。新加的`assert_same_as`斷言（動畫容忍的「是否回到同一畫面」）
直接判定**沒有回到服務選單**——`Escape`×3會**整個退出商店回到城鎮地圖**（97%像素不同），
於是第一次之後的每次探測都在漂移中的畫面上進行，實測也確實出現兩個index拍到相同frame。
**如果沒有這道gate，這輪就會產出4張自信滿滿但標錯的截圖**——正是本文件同日稽核出的那13張
自我複製對照圖的同一種失敗模式。改成**每個index獨立一個instance、各自從頭導航**（平行跑，
成本只是wall-clock），8個scenario全部PASS。

**完整對應表（每一格都由畫面自身文字判定，不是按鍵次數推論）**：

| Right次數 | 武器店 | 教會 |
|---|---|---|
| ×0 | **購買**（商品列表＋購買價：布衣+DP002/$50、皮甲+DP008/$300、旅行裝+DP010/$500、法師袍+DP012/$750） | **狀態/持有品**（角色名冊） |
| ×1 | **售出**（角色持有品＋**售價**：巨神戟$18000、龍神鎧甲$00000） | **轉移**（「誰的東西呢？」） |
| ×2 | **裝備**（完整能力卡＋持有品，**不顯示價格**） | **復活**（「隊伍中沒有須要復活的！」） |
| ×3 | **轉移**（「誰的東西呢？」） | **轉職**（「誰要轉職呢？」） |

**★ 同一個服務在不同場所的index不同**：轉移在武器店是`×3`、在教會是`×1`。這比六續的結論更
強——**不只不能用按鍵次數推論服務，連跨場所的index都不能互相套用**。（教會與商店共用同一段
轉移程式碼這件事本身與doc58既有記錄`0x2f8ea`由shop service3與church raw1共同呼叫吻合，
連「service3/raw1」這組不同編號都對得上。）

**武器店×1 vs ×2的辨識**：兩者第一個畫面都是角色名冊、**RGB逐位元組完全相同**，無法區分；
是靠選到索爾後的**第二個畫面**分開的——×1顯示帶售價的持有品（售出），×2顯示完整能力卡
（索爾 魔族 劍聖 LV.06／HP823/823／MP805/805／DX192 MV30 HIT292 AP938 EV212 DP724）
與**不帶價格**的持有品（裝備）。這也是為什麼scenario要連拍兩張畫面而不是一張。

**因此本輪一次補齊了先前列為未完成的四項**：商店裝備、商店轉移、教會復活、教會轉職——
四者都已到達並由畫面文字確認。教會復活顯示「隊伍中沒有須要復活的！」是因為這份存檔全員
存活，屬**正常拒絕**而非流程走不到。

**誠實限制**：只做到「識別服務並到達其第一/第二個畫面」，**沒有實際執行**裝備變更、轉移、
復活、轉職的完整交易流程（會改動存檔狀態）。另外本輪用的是ch26進度存檔patch章節byte到ch02，
所以名冊數值是ch26當時的（索爾已是LV.06魔族劍聖），**不是ch02的真實數值**——要ch02真實
數值請看本文件2026-09-02「純DOSBox-X原版重新驗證ch01隊伍MV」那節用新遊戲取得的LV.01數據。

新圖（2×4，上排商店、下排教會）：
[`shop-church-service-map-original-dosbox.png`](../figures/shop-church-service-map-original-dosbox.png)

**清理**：8個instance全部自動teardown，`status`無殘留、`ps aux`零程序、工作目錄0個。

### 2026-09-03 續：轉職 trial3／trial4 差異圖逐位元組相同——查明，**不是檔案覆寫錯誤**

前一節的212張圖重複稽核把 `church-classchange-trial3-deltas.png` ＝
`church-classchange-trial4-deltas.png` 標為「可能是真的擲出相同成長值、也可能是存檔時
覆蓋錯誤，待查」。本輪查明：**是前者**，而且答案本來就已經寫在同一份文件裡。

**第一步：本文件自己的紀錄早就給了答案**（照 memory「先查既有證據再開新調查」的教訓，先讀不先跑）。
本節上方那張5次trial的原始數值表：
...
**六個欄位完全相同**。成長差異畫面顯示的就是這六個數字，數字相同、圖自然相同。

**第二步：確認那個畫面除了數字之外沒有任何會變的東西**（否則「數值相同」也未必推得出
「逐位元組相同」，例如有待機動畫sprite就一定會差）。對5張圖做兩兩像素差分：

| 配對 | 是否有差異 | 差異bounding box | 差異像素% |
|---|---|---|---|
| 1v2 | 有 | (574,515)-(602,545) | 0.051 |
| 1v3 / 1v4 | 有 | (574,477)-(602,545) | 0.119 |
| 1v5 | 有 | (574,477)-(602,507) | 0.064 |
| 2v3 / 2v4 | 有 | (574,477)-(602,545) | 0.126 |
| 2v5 | 有 | (574,477)-(602,545) | 0.115 |
| **3v4** | **無（bbox=None）** | — | **0.000** |
| 3v5 / 4v5 | 有 | (578,477)-(602,545) | 0.079 |

**10組配對中，所有差異都落在同一個 28×68px 的數字欄位內**（x∈[574,602]、y∈[477,545]），
沒有任何一組出現在畫面別處。也就是說這個畫面**完全確定性**：五次獨立instance、五次獨立
launch，除了數字以外零像素變動，沒有動畫sprite、沒有游標、沒有時序雜訊。

差異範圍也與數值表逐項吻合：1v2 只差 MMP（21 vs 24）→ 只有下面那一行變；1v3 差 MHP＋MMP
→ 兩行都變；3v5 差 MHP(16/15)＋MMP(23/21) → 兩行都變。

**結論**：畫面既然是「數值的確定性函數」，數值相同就**必然**逐位元組相同。trial3／trial4
圖相同是資料事實的直接後果，不需要用檔案覆寫來解釋。本項自 `91-worklist`／交接文件 §4 的
「待查」移除。

**但同時要修正一個被高估的說法（比上面那題重要）**：本節原本把range驗證記為
「**PASS(5/5)**」。實際上5次trial不是5個獨立樣本——trial1／trial2 六欄中有四欄相同、
trial3／trial4 **六欄全部相同**。本節自己也已經診斷出原因：這份原生RNG是跟**已執行的按鍵／
tick數**掛鉤而非牆鐘時間，所以「不同instance」不等於「獨立抽樣」。trial3起加入的不規則等待
只是把結果推到**另一個固定點**（10/8/4/16/23/2），並沒有真的解除相關性——trial4與trial3
仍然一模一樣就是最直接的證據。

因此有效樣本大致只有 **3 組相異結果**（{9,8,4,14,21}、{9,8,4,14,24}、{10,8,4,16,23}、
{10,8,4,15,21} 其實是4組相異，但由5次trial產生），而 DP 5次全是8、DX 5次全是4 這種
「全部取到同一值」的觀察，**幾乎沒有證據力**——它極可能只是同一個RNG狀態被重複取樣，
而不是「該欄真的偏好上界」。range本身沒有被違反（沒有任何一次超出宣稱區間）這點仍然成立，
但**不應該把它當成「已用5個獨立樣本驗證過range」來引用**。要真正驗range需要系統性打亂
每次launch後的按鍵數，本輪未做。

### 2026-09-03 續二：秘密商店的 `Shift+F1` **只在 ch02 有效**，不是城鎮通則

交接文件 §9 的待辦之一：ch02 剛解出的「在 selection0 酒店按 `Shift+F1`」是否也適用於
另外兩個城鎮 variant。答案是**否**。

**工具面先做的一件事（重要）**：這個問題的兩個答案都是正常結果，所以**不能用斷言去問**——
斷言任一方都會把發現變成「工具失敗」。因此在 `fd2_original_verify.py` 新增
`measure_change` op：**記錄但不判定**。

而且**不能用既有的 `assert_distinct` 來問這題**：它比的是 MD5 相等。任何含待機動畫 sprite
的畫面本來就會 run-to-run 不同（本專案實測 0.54~0.57% 像素、≤48×48 框），所以「什麼都沒
發生」會被 MD5 判成「兩張圖不同」——**這個工具根本回答不了這個問題**。`measure_change`
改用 `classify_instability`，把 STRUCTURAL（畫面真的變了）與 ANIMATION（只有待機動畫）分開。

**結果**（ch02 同時作為陽性對照，若它報 unchanged 就代表探針壞了、另兩章的結果無效）：

| 章節 | 城鎮 | `Shift+F1` @ 酒店 | 判定 |
|---|---|---|---|
| ch02（raw 1） | 羅德鎮 | **changed**，1.98% 像素、576×348 框 | 秘密商店出現 |
| ch12（raw 11） | 北山道 | **unchanged** | 無反應 |
| ch03（raw 2） | 往塞拉村途中 | **unchanged** | 無反應 |

**畫面自身的文字才是真正的證據**（不是差異百分比）：ch02 的招牌由「酒店」變成
「**？？？**」，ch12／ch03 的招牌**始終是「酒店」**。
[`secret-shop-chapter-scope-original-dosbox.png`](../figures/secret-shop-chapter-scope-original-dosbox.png)
（2×3，左＝按之前、右＝按之後）

**排除了三個會讓 null 結果不成立的可能**，每一個都實測而非推理：

1. **「其實沒站在酒店」**——把三個 probe 的落地畫面與**另一次獨立 run**（不同 instance）
   的 `town_variantN/sel0` 基準比對，三者都是**零像素差異**。同時附兩個對照證明這個比對有
   鑑別力：ch12 sel0 vs ch03 sel0 差 89.19%、ch02 sel0 vs sel1 差 1.92%，都判為 STRUCTURAL。
   （招牌在三張「按之前」圖裡都寫著「酒店」，也獨立佐證同一件事。）
2. **「1.5 秒等太短」**——另跑一輪把等待拉長到 **6 秒與 12 秒**，兩章仍然 unchanged。
3. **「emulator 當掉了所以什麼都沒變」**——這輪反而給出了最漂亮的反證：6s／12s 的量測
   差異是 **0.52%／0.54% 像素、48×48 框**，正是本專案已知的隊長待機動畫特徵。
   **畫面一直在動**，只是招牌沒變——所以不是凍結，是真的沒反應。

**因此**：秘密商店不是「所有城鎮 hub 都有的隱藏指令」，而是**繫在 ch02 羅德鎮這個 variant
上**。`91-worklist` 的 UI-VIS-TOWN 記述應理解為 ch02 專屬。

**尚未回答**：是否有另一組按鍵在 variant1／variant2 觸發各自的隱藏內容（本輪只測了 ch02
那一組按鍵，沒有做按鍵空間搜尋），以及觸發條件是否綁在章節 byte 還是綁在城鎮 variant
（兩者在本專案的存檔 patch 手法下無法分離——patch 章節 byte 就同時換掉了 variant）。

### 2026-09-03 續三：商店「裝備」**實際執行**（會改動狀態），並解出裝備替換的完整計算式

交接文件 §9「服務的實際執行」的第一項。先前只做到「識別服務並到達畫面」，而 2026-08-31
續八十五雖然走過裝備 UI，但**只碰到兩個 no-op 情形**（選已裝備品、選不相容的消耗品），
從未真的換裝過。本輪補上，並且順帶把整個購買流程與計算式一起解出來。

#### 方法：先偵察、再斷言（不先寫死按鍵序列）

購買流程在原版側從未被驅動過，所以第一步是**每按一次鍵就截一張圖、且除了 title 之外
不做任何斷言**的偵察。這是本專案反覆踩過的坑的反面做法——先假設一組序列、再把落到的畫面
寫成結論。

**流程（由畫面自身文字確認，非按鍵次數推論）**：
`武器店 → Enter(店主) → Enter(商品格) → 選品 → Enter →「這個<品名>，<價>元，要不要啊？」
→ Enter → 收受者清單(含預覽) → 選人 → Enter →「要裝備上去嗎？」→ Enter → 回商品格，金錢已扣`

**2×2 商品格的方向鍵對應**（4 個 instance 各驗一組，**由確認框自己念出的品名**判定）：
...
與商品格版面（布衣左上／旅行裝右上／皮甲左下／法師袍右下）一致。
**與服務選單不同，商品格「有」可見游標**（方向鍵造成 0.36~0.51% 像素變化）。

#### 收受者清單會顯示遊戲自己算好的 before→after

這是本輪最有價值的發現：選品之後的收受者清單**直接列出每個候選人的 AP/DP/HIT/EV 變化**。
拿 4 件商品 × 各自清單共 12 筆觀測回歸，得到：

> **after = base − (目前所穿護甲的加成) + (新品的加成)** ——**替換**，不是疊加。

**這個式子的證據力來自「同一個人、不同商品」的交叉一致性**：索爾／悠妮／亞雷斯各自被
**3 件不同商品**分別驗過，反推出的「被脫下的護甲」都精確等於 DP300／EV20。
而索菲亞反推出的是 DP60／EV5——**不是同一個常數**，這排除了「300 是寫死的」這種解釋。

#### 實際執行並與獨立對照比對

買 `皮甲 +DP008 $300` 給亞雷斯（遊戲自己預覽：DP 642→350、EV 186→166），
另開一顆**完全不購買**的 instance 從同一份存檔走到同一個畫面當 BEFORE
（不在同一顆 instance 內來回，是因為本專案已實測服務之間 Escape 來回會漂移，
漂掉的 BEFORE 會讓比較靜默失效）。

| | DP | EV | 物品欄 |
|---|---|---|---|
| BEFORE | **642** | **186** | 巨神戟 +AP320（紅＝已裝備）、龍神鎧甲 +DP300（**紅**） |
| AFTER | **350** | **166** | 巨神戟（紅）、龍神鎧甲（**橘＝已卸下**）、皮甲 +DP008（**紅**） |

**與預覽逐位數字相同**。642 − 300 + 8 = 350 ✓、186 − 20 + 0 = 166 ✓。
[`shop-equip-executed-ares-original-dosbox.png`](../figures/shop-equip-executed-ares-original-dosbox.png)

三件事同時被證實：**(1)** 裝備真的執行了、狀態真的變了（不是只顯示訊息）；
**(2)** 語意是替換，被換下的護甲**留在物品欄**且框色由紅轉橘；
**(3)** 預覽畫面說的數字是真的。

而且索爾的狀態卡獨立佐證了上面反推的數值：他的護甲就是**龍神鎧甲 +DP 300**，
與 12 筆觀測回歸出的「被脫下 300」完全吻合——**兩條互相獨立的路徑得到同一個數字**。

#### 附帶發現：**物品標籤沒有列出它的全部效果**

龍神鎧甲的標籤只寫 `+DP 300`，但卸下它同時掉了 **20 點 EV**。
歸因是乾淨的：4 件不同商品換到同一個人身上時，EV 變化**都是 −20**（與新品無關），
所以那 20 點必然來自被卸下的那件，而不是新穿上的那件。
**因此不能把物品標籤當成該物品效果的完整清單。**

#### 收受者清單是按職業過濾的（新發現）

同一個位置的清單內容隨商品而變：布衣／旅行裝＝索爾/悠妮/亞雷斯…；
**皮甲的清單裡沒有悠妮**（法師）；**法師袍的清單只有悠妮/瑪琳/索菲亞**（施法者）。
所以「誰可以穿」在收受者清單階段就已經先過濾掉了。

**誠實限制**：
- 每份清單都有向下捲動箭頭，本輪**只讀了各清單的前 3 列**，沒有捲完，
  所以「哪些職業可穿哪件」只是由這 3 列推得的方向性結論，不是完整職業對照表。
- 只執行了**裝備**一項。轉移／復活／轉職的實際執行仍未做（復活與轉職在更早的輪次
  有各自的執行紀錄，但那是另外的流程，不在本輪範圍）。
- 全程在隔離 instance 內，`~/fd2-run` 本體未被寫入；teardown 後變更即消失。

#### 工具面：本輪新增的兩個斷言原語，以及一個「自比對」的自我糾正

- **`measure_change`（記錄但不判定）**：用於「兩個答案都合法」的開放問題（例如秘密商店
  那題）。斷言任一方都會把發現變成工具失敗。
- **`assert_ref_differs`（必須**不**等於某個參考狀態）**：`assert_ref` 只能斷言「等於預期的
  after」，那只是半個論證——**如果兩張參考圖哪天變成同一個檔案，只有等式的檢查會繼續通過卻
  什麼都沒證明**（這正是本專案在 18 張對照圖中抓到 13 張的那個缺陷）。把「等於 after」與
  「不等於 before」成對使用，那個不可證偽的組合就不可能成立。
- `--selftest` 另加一項靜態檢查：**同一個 label 上成對使用的兩張參考圖必須真的是不同的影像**
  （直接比 MD5）。等於把那個歷史缺陷寫成一條會自動失敗的規則。
- **自我糾正**：`equip_execute` 的第一版最後一步寫成「把 `ares_panel` 與 `ares_panel`
  自己比對」——那永遠回報 unchanged，是零資訊的自比對，正是 `feedback_verify_evidence_is_
  not_self_comparison` 記的那個錯誤。發現後改成上面的 `assert_ref_differs`。

偵察用的一次性腳本刻意沒有進版：流程確定之後才寫成**會斷言的 scenario**
（`equip_control` 為不購買的對照、`equip_execute` 為實際執行），這樣下次任何人重跑就會
自動比對到同一組畫面，而不是再靠人眼看一次。

### 2026-09-03 續四：M5-C —— 13 張被證偽圖的**原版側命題**逐項重測（使用者指示）

使用者指示：**「那 13 張被證偽圖對應的結論要逐項用原版重測」**。

**要重測的到底是什麼**：這 13 張圖的原始命題是「remake 與原版一致」，parity 那一半隨
`remake/` 移除已失去意義。但每個對應的 worklist 條目裡都還包含一個**純原版側的可驗事實**
（例如「原版 service0 Enter 後 Right `0→1`、Down `1→3`、Left `3→2`」）——重測的是那一半。

#### 結果總表

| # | 原版側命題 | 結果 |
|---|---|---|
| 1 | ch02 存在秘密商店揭示 | ✅ 成立（並補測適用範圍只有 ch02） |
| 2 | 秘密商店 4 服務 `Right 0→1→2→3→0`／`Left 0→3`；Escape 回 town 隱藏 selection5 | ✅ **全部成立** |
| 3 | 收受者清單三列 window | ✅ 成立，並解出捲動規則 |
| 4 | good index 1 的收受者清單 | ✅ 成立（旅行裝清單） |
| 5 | 購買格 `Right 0→1`／`Down 1→3`／`Left 3→2` | ✅ 成立（與本次獨立測得的網格對應一致） |
| 6 | 「布衣／50元／要不要啊？」；`Right`→No、`Left`→Yes | ✅ 全部成立 |
| 7 | 購買扣款 | ✅ 成立（精確 −300） |
| 8 | `gold<price` 顯示「錢不夠！」＋等待標記＋**不扣金** | ✅ **全部成立** |
| 9 | 購買成功 | ✅ 成立 |
| 10 | selection1→武器店 variant1、selection3→道具店 variant3、variant5 | ✅ 成立 |
| 11 | ch02 城鎮 hub | ✅ 成立 |
| 12 | selection1 | ✅ 成立 |
| 13 | selection 0–5（含隱藏 5） | ✅ 成立（0–4 ＋ #2 的隱藏 5） |

**13/13 原版側命題全部成立**。也就是說：**那批圖是壞的，但它們所支撐的原版側事實本身沒有錯**
——與「售出→欄位清空」那條的情形一致（圖錯、結論對）。**證據被撤回 ≠ 結論被推翻**。

#### #2 秘密商店服務對應（新解出）

服務選單一樣**沒有可見選取標示**，所以每個 index 各用一顆全新 instance，
由 Enter 後**畫面自身的內容**判定：

| index | 畫面證據 | 服務 |
|---|---|---|
| 0 | 商品清單（闇劍 $250／長戟 $1200／釘頭錘 $180／藥草 $10／回復劑 $80／皮甲 $300） | 購買 |
| 1 | 第二畫面帶**售價**（巨神戟 $18000／龍神鎧甲 $00000） | 售出 |
| 2 | 第二畫面是完整能力卡、**無價格** | 裝備 |
| 3 | 「誰的東西呢？」 | 轉移 |

**與武器店同一組對應**。
- `Left`×1 從 0 出發 → 與 svc3 同一畫面（差異 0.11%／26×20px，只是閃爍的 ▼ 標記）→ **`0→3` 回繞成立**
- `Right`×4 → 與 svc0 **零像素差** → **`3→0` 回繞成立**
- Escape 離開秘密商店 → 回到城鎮且招牌仍是「？？？」→ **回到隱藏 selection5 成立**

[`secret-shop-service-map-original-dosbox.png`](../figures/secret-shop-service-map-original-dosbox.png)
**注意**：該圖 svc0 與「Right×4」兩格的 RGB MD5 **刻意相同**（`d81e668cd243`）——
那正是回繞成立的證據本身，**不是**自我複製缺陷。未來做重複圖稽核時請勿誤報。

#### #8 錢不夠（需要先製造買不起的狀態）

本機唯一存檔有 $10,070,183，而最貴的商品是 $1200，**這個分支用玩的到不了**。
因此在 `tools/fd2save.py` 補上 `--set-currency SLOT:VALUE`（寫的是與 `--set-chapter`
同一個、文件已標記 PROVEN 的 per-slot metadata 區塊的 u32）。

**順帶獨立佐證了欄位對映**：未修改的存檔解出 `currency=0x0099a8a7` ＝ 10,070,183，
與畫面上的 `$10070183` 逐位相同；patch 成 30 之後畫面顯示 `$00000030`。

結果：YES → 「**錢不夠！**」＋▼ 等待標記 → 確認後回到商品清單，金錢**仍是 $30**（未扣款）。
[`shop-insufficient-funds-original-dosbox.png`](../figures/shop-insufficient-funds-original-dosbox.png)

#### #3 收受者清單的三列 window（捲動規則）

window **固定 3 列**。前 3 次 `Down` 是游標在窗內移動（列 0→1→2），
**第 4 次起窗格每次捲一列**；捲動箭頭同步變化（頂端只有 ▼，捲動後 ▲＋▼ 並存）。
逐列讀出的 8 個人也全部符合 `after = base − 300 + 2`（布衣），把裝備計算式的觀測數
從 12 筆再往上擴充。

#### 附帶解決 M5-D 的「`$0` 售價品項能否售出」

在 #2 的 svc1 畫面上直接看到答案的**對象**：索爾的龍神鎧甲售價就是 `$00000`。
實測：「這個龍神鎧甲，**0元**，賣不賣啊？」→ YES → 品項從清單消失、金錢完全不變。
**$0 品項可以賣，只是拿不到錢。**

**而且賣掉的是他身上「裝備中」的那件**，所以順帶驗證了裝備中品項可被售出：
索爾 **EV·212→192、DP·724→424**（−20／−300），與龍神鎧甲的加成完全吻合。
[`shop-sell-equipped-zero-price-original-dosbox.png`](../figures/shop-sell-equipped-zero-price-original-dosbox.png)

**這條同時用完全不同的路徑（售出，而非續三的替換）再次證實**「龍神鎧甲標籤只寫 `+DP300`、
實際還附帶 +20 EV」——兩條獨立路徑得到同一個隱藏數值。

#### 兩次導航漂移（誠實記錄，都是被自己的檢查抓到的）

「賣掉裝備中品項會不會真的掉 DP」這題**前兩次的結果無效**，兩次都是導航漂掉：
1. 售出後用 `Escape`×2 想回服務選單，**實際上直接離開了場所**，之後的按鍵把 instance
   帶進了**教會**（畫面 70% 不同）。實測正確深度是 **`Escape`×1**。
2. 修正深度後仍然漂：**服務選單的游標不會重置回 index 0**（doc58 更早就記過這個坑），
   剛用完 service1 的 instance 再按 `Right`×2 會跑到 service3（轉移），不是 service2（裝備）。
   對照組從 0 出發要 ×2、售出組從 1 出發只要 ×1。

兩次都是因為**拿對照組去比**才立刻看出不對（70%／57% 的差異對「只掉了 DP」明顯過大）。
若當初只截了「售出組」那一張、再照著預期去解讀，就會寫出一個看起來很有說服力的錯誤結論。

### 2026-09-03 續五：M5-D 四項缺口 —— 轉移實際執行、職業過濾完整表、每章秘密商店組合鍵

#### #1 轉移（transfer）**實際執行**——四個服務全部補齊

流程（每按一鍵截一張圖偵察出來，非假設）：
`誰的東西呢？ → 來源名冊 → 該員物品清單（帶售價） → 要給誰呢？ → 目的名冊 → 回到「誰的東西呢？」`

**實測**：把索爾**裝備中**的 `巨神戟 +AP320` 轉給悠妮。
悠妮原本 3 件（光之杖 +AP400／龍神鎧甲 +DP300／領悟之書），轉移後**多出巨神戟**成為 4 件，
差異 2.00% 像素、集中在單一 272×40px 的新增列。對照組（同一存檔、不轉移）維持 3 件。
[`shop-transfer-executed-original-dosbox.png`](../figures/shop-transfer-executed-original-dosbox.png)

至此 `裝備／轉移／復活／轉職` 四個服務都有實際執行紀錄（後兩者在更早輪次）。

#### #2 收受者清單的職業過濾——完整對照表

把 4 件商品的清單各自捲到底（window 3 列、每次 `Down` 捲一列），逐列讀出成員：

| 商品 | 可穿人數 | 名單 |
|---|---|---|
| 布衣 `+DP2`／旅行裝 `+DP10` | **12** | 索爾・悠妮・亞雷斯・哈諾・希莉亞・鐵諾・瑪琳・貝克威・凱麗・洛娜・索菲亞・萊汀 |
| 皮甲 `+DP8` | **8** | 索爾・亞雷斯・哈諾・希莉亞・鐵諾・貝克威・洛娜・萊汀 |
| 法師袍 `+DP12` | **3** | 悠妮・瑪琳・索菲亞 |

**名冊共 13 人**（`roster_count=0x0d`），推出三件事：

1. **蓋亞完全不在任何清單裡**——13 人中唯一一個，四件防具都不能穿。
2. **皮甲(8) ＋ 法師袍(3) ＝ 11，比布衣的 12 少一個，差的是凱麗**——
   她可以穿布衣／旅行裝，但皮甲與法師袍都不行。所以**至少存在三種可穿性群組**
   （近戰／施法者／凱麗自成一類），不是單純的「戰士 vs 法師」二分。
3. 布衣與旅行裝的名單**完全相同**——兩者是同一個可穿性類別。

[`shop-recipient-class-filter-original-dosbox.png`](../figures/shop-recipient-class-filter-original-dosbox.png)
（4 欄＝4 件商品，每欄由上而下是同一份清單捲動的 5 個窗格，覆蓋整份名單無斷點）

**誠實限制**：這個畫面只顯示姓名與 AP/DP/HIT/EV，**不顯示職業**，所以上表是
「哪些**角色**可穿」的完整對照，不是「哪些**職業**可穿」。要後者需另外把這 13 人對到職業。

#### #3 秘密商店的組合鍵是**每章不同**——先查表、再實測，不做按鍵空間搜尋

原本這項寫成「variant1／variant2 是否有各自的隱藏觸發鍵」，聽起來像要做按鍵空間搜尋。
**先查既有靜態資料**（memory「先查既有證據」的教訓）發現根本不需要：
`tools/gen_campaign.py:145` 的 `NATIVE_SECRET_GATES` 已經是從 EXE 的 0x1f-byte town record
（`+1`＝所需 selection、`+2`＝所需 BIOS scan code）抽出來的**23 章完整表**。

BIOS：`Shift+F1..F10 = 0x54..0x5D`、`Ctrl+F1..F10 = 0x5E..0x67`、`Alt+F1..F10 = 0x68..0x71`。

| 章節 | 表中 (selection, scan) | 對應按鍵 | 實測 |
|---|---|---|---|
| ch02 | (0, `0x54`) | Shift+F1 | ✅ **與本輪先前獨立實測完全一致** |
| ch04 | (2, `0x6A`) | Alt+F3 | ✅ |
| ch05 | (3, `0x57`) | Shift+F4 | ✅ |
| ch12 | (0, `0x5E`) | Ctrl+F1 | ✅ 招牌變「？？？」，Enter 進去是灰色秘密商店店主 |
| ch03 | (1, `0x5F`) | Ctrl+F2 | ❌ **仍不觸發** |

**這也回頭解釋了本輪稍早的結果**：ch12／ch03 對 `Shift+F1` 無反應是正確的，因為那是 ch02 的鍵。

**負對照**：ch02 按 `Ctrl+F1` 完全無反應——所以不是「任何組合鍵都會揭示」。

**ch03 這個特例被大幅收窄**（doc58 2026-08-26 曾把它列為未解，並猜測可能是
「scan code 換算錯誤或 xdotool 合成鍵不被 BIOS 層接受」）：

- **不是修飾鍵送不進去**：Shift／Ctrl／Alt 三族在 ch02／ch12／ch04／ch05 都成功觸發。
- **不是 selection 索引對不上**：ch03 的 `Ctrl+F2` 在**全部 5 個 selection** 逐一測過，都無反應。

剩下的可能是「ch03 那一筆表格值本身有誤」或「另有未知前置條件」，本輪**不臆測**，誠實留開。
[`town-secret-gate-per-chapter-original-dosbox.png`](../figures/town-secret-gate-per-chapter-original-dosbox.png)

#### #4 轉職成長的獨立取樣——**隨機性確認，但原本設想的去相關手法實測無效**

M5-D 的第四項是「原 5 次 trial 因 RNG 綁按鍵/tick 而非獨立，需重新取樣」。
本輪照那個診斷去設計：**每個 instance 在進教會前多按不同數量的城鎮方向鍵**
（0/5/15/25/35/45，都是 5 的倍數所以最後仍落在同一個 selection），並讓按鍵間隔各不相同。

**第一次跑出來的結果是錯的，而且錯得很有說服力**：8 個樣本的成長畫面 hash **完全相同**，
差一點就寫成「成長值是決定性的、根本不隨機」。把圖拉出來才發現
**八張全是「誰要轉職呢？」**——對白多按了一次 Enter，全部掉出成長畫面回到提示。
**它們相同是因為同時停在同一個錯畫面**，不是因為數值相同。
（這直接促成了工具端的 `assert_not_signature`，見 doc98 同日段落。）

修正 Enter 次數並加上簽章閘門後重跑 6 個樣本，得到真正的答案：

| 執行條件 | 耐力 | 速度 | MHP | 移動力 |
|---|---|---|---|---|
| 偵察輪（另一組按鍵時序） | 8 | 4 | **14** | 1 |
| padding 0 鍵 | 8 | 4 | **15** | 1 |
| padding 5／15／25／35／45 鍵 | 8 | 4 | **15** | 1 |
| 先做另一筆轉職再轉凱麗 | 7 | 3 | **16** | 1 |

**兩個結論**（同一角色凱麗、同一轉職 武者→武聖）：

1. **成長確實是隨機的**——MHP 觀測到 14／15／16 三個值。原文件的 range 檢查有真實基礎。
2. **但「多按城鎮方向鍵」這個去相關手法實測無效**——0 到 45 鍵之間**六個樣本值完全相同**。
   也就是說這個 RNG **不是**被 hub 方向鍵推進的。原本的診斷（「綁按鍵/tick」）
   在方向上對，但**具體推進來源不是任意按鍵**。

**真正會改變擲值的是「進行不同的前置交易／對白」**：偵察輪與 gated 輪的差別在對白等待與
按鍵時序，就換到了另一個值；先做一筆別的轉職再做凱麗，三個欄位一起變。

**所以下一輪要做大樣本 range 驗證的正確做法，不是加按鍵數，而是讓每個 instance 在擲值前
走過不同的前置流程**（例如先完成一筆購買、一次轉移、或另一名角色的轉職）。
本輪把這條路徑指出來但**沒有做成大樣本**，range 仍維持「未以獨立樣本驗證」。

[`church-classchange-rng-independence-original-dosbox.png`](../figures/church-classchange-rng-independence-original-dosbox.png)

**誠實限制**：最後一列（先轉洛娜）那次的畫面**無法百分之百確認是凱麗的成長**
（對白位置與前面幾輪不同，可能仍在洛娜的段落內），所以它只作為「不同前置流程會換到不同值」
的佐證，不列為凱麗的第三個確定樣本。14 vs 15 那組則是明確的同角色同轉職對比，
**隨機性的結論靠的是這一組**。

### 2026-09-03 續六：秘密商店組合鍵擴大實測 —— 8/8 成立、2 個例外，三個假說全被推翻

續五記錄 ch03（`Ctrl+F2`）不觸發，並把它列為孤立未解。本輪擴大取樣：

| 章節 | town variant | 組合鍵（由 `NATIVE_SECRET_GATES` 換算） | 位置 | 結果 |
|---|---|---|---|---|
| ch02 | 0 | `Shift+F1` | sel0 | ✅ |
| ch04 | 0 | `Alt+F3` | sel2 | ✅ |
| ch05 | 0 | `Shift+F4` | sel3 | ✅ |
| ch12 | 1 | `Ctrl+F1` | sel0 | ✅ |
| ch13 | 1 | `Alt+F2` | sel1 | ✅ |
| ch20 | 1 | `Shift+F9` | sel3 | ✅ |
| ch07 | 2 | `Alt+F6` | sel0 | ✅ |
| ch19 | 2 | `Alt+F8` | sel2 | ✅ |
| **ch03** | 2 | `Ctrl+F2` | sel1 | ❌ |
| **ch06** | 2 | `Ctrl+F5` | sel4 | ❌ |

**`gen_campaign.py` 的 `NATIVE_SECRET_GATES` 表因此獲得 8 章的獨立實測支持**，
涵蓋三種修飾鍵族（Shift／Ctrl／Alt）、三種 town variant、五個 selection 位置。

#### 被推翻的三個假說（逐一實測，不是推理）

1. **~~修飾鍵送不進去~~** —— Shift／Ctrl／Alt 三族都有成功案例，`Ctrl+F1`(ch12) 就成功。
2. **~~selection 索引對不上~~** —— ch03 的 `Ctrl+F2` 在**全部 5 個 selection** 逐一測過，全無反應。
3. **~~是 town variant 2 的問題~~** —— 一度看起來很像（ch03/ch06 都是 variant 2），
   但 **ch07 與 ch19 同樣是 variant 2 且都成功**，8/8 那組裡有 2 章是 variant 2。
4. **~~被 DOSBox-X 宿主熱鍵攔截~~** —— `Ctrl+F5` 是 DOSBox 標準的「存截圖」綁定，
   看起來是很好的解釋。實測反證：對同一顆 instance 依序送 `Ctrl+F1`／`F2`／`F5`／`F6`，
   用**工作目錄檔案清單前後差集**檢查，**四者都沒有產生任何檔案**——
   mapper 根本沒收到它們（與同日 `Ctrl+F6` 錄不到音訊的結果一致）。

**因此 ch03／ch06 仍未解**，但排除範圍已大幅收窄：
剩下的可能是**那兩筆表格值本身有誤**，或那兩章另有未知前置條件。
本輪不臆測，誠實留開。

#### 附帶更正一個我自己上一輪下錯的技術結論

續五曾把擴大量測失敗歸因於「MEMDUMPBIN 走暫停中 debugger 這條通道很慢，7 分鐘/次」。
**實測後這個結論是錯的**：
...
是**固定成本**，與大小幾乎無關。三條校準路徑（2MB 完整程式碼簽章掃描／384KB 窄窗資料簽章／
16 bytes 候選驗證）實測**都只要約 1 秒**。

**所以慢不是慢，是「卡住」**——真正的成因是批次中 `enter-debugger` 在大量導航之後不穩定。
處置方式因此從「調小掃描範圍」改成 **`read_bgm_track` 預設 90 秒逾時並回報 `STUCK`**：
既然正常成本約 1 秒，跑了幾分鐘就一定是卡住，長逾時只會讓一次卡住癱瘓整批。
改完後 24 個場景的批次順利跑完。
