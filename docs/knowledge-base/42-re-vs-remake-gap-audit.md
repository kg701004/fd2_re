...
| 敵方 AI:目標評分(dmg、擊殺加成×2) | doc11（2026-07-27 recheck） | 🟡 | remake 的 `aiActUnit/NextAIPlan` 仍是 normalized approximation；canonical Docker scan 找不到 `0x15140`/`0x15356` direct callers，且 `0x15140` bytes 不像 AI entry，故 `dmg≤2`、擊殺×2、`0x1529E/0x152AB` 不能再標為已證實原版公式。唯一保留的 raw lead 是 `0x15AD8→0x15B77`，待重建 caller context | 高 |
