...
| 敵方 AI：物理目標評分 | doc11（2026-07-29 recheck） | 🟡 | 舊 `0x15140/0x15356` 位址仍作廢；真正的 `0x14237` 已閉合候選格×目標列舉、雙方地形修正、差值`<=2`拒絕、嚴格超過target `+0x40`時分數×2/priority 18、raw helper/byte8調整及priority→score→先出現者同分規則。新增raw-only `ScoreNativePhysicalAttackCandidate`；`0x146D1`候選順序、`0x1DEBE/+8`語意、turn/camp與runtime execution仍未閉合，`aiActUnit/NextAIPlan`仍是近似 | 高 |
