...
| 商店(一般商品) | `0x2e341→0x2f0b0/0x2f642/0x2f883/0x2f8ea`、doc56 UI-09 | 🟡 | 69個原版shop節點以`native_hub_variant`選擇1/3/5 production owner；完整purchase縱切已到insert→optional equip→10/9/14-tick success→debit→product loop。equipment preview嚴格要求base AP/DP與raw inventory provenance，raw equipped flags同步。no-eligible、其他三service與DOSBox E2未閉合；custom variant 0保留generic fallback | 高；接sell/equip/transfer owner與no-eligible，再做E2 |
