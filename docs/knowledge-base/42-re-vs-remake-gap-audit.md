...
| 敵方 AI:**施法決策**(法師/僧侶主動用攻擊術/補血術) | direct disasm 證實兩條不同 producer：`0x1567E` 枚舉 inventory slot→item row command，候選交 `0x15880`；`0x1598A` 枚舉 unit command mask，候選交 `0x15B77`。兩條靜態 producer 均已有具型別實作；前者 command>0x0F 才做 `command-0x10`，兩套 ranking 不可合併 | 🟡 | `ScoreNativeAI1567E` 與 `ScoreNativeAIScoredCommandGroups` 已保存各自候選順序、分數與嚴格平手規則；但同一原版執行期狀態的動態 trace、phase callback、MP／交易與呈現仍缺，因此未接 `NextAIPlan` | 高；補動態原版 trace、phase／執行交易與 runtime Cast |
