...
| 敵方 AI：物理落點與目標評分 | doc11（2026-07-29 recheck） | 🟡 | 舊 `0x15140/0x15356` 位址仍作廢；`0x145CD→0x4E040→0x146D1→0x14B16` 已閉合阻擋標記、原版地形成本、佔用排除及 row-major 落點順序，`0x14237` 已閉合逐落點／目標評分、地形修正、差值`<=2`拒絕、priority→score→先出現者同分規則。新增 raw-only destination／score adapters；`0x1DEBE/+8` 語意、無攻擊方案 fallback、turn/camp 與 runtime execution仍未閉合，`aiActUnit/NextAIPlan`仍是近似 | 高 |
