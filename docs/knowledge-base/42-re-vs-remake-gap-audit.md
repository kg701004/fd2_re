...
| 敵方 AI:目標評分(dmg、擊殺加成×2) | doc11（2026-07-27 recheck） | 🟡 | remake 的 `aiActUnit/NextAIPlan` 仍是 normalized approximation；canonical Docker scan 找不到 `0x15140`/`0x15356` direct callers，且 `0x15140` bytes 不像 AI entry，故舊公式不能再標為已證實。已補證 `0x1A4EB/0x1A58F` phase-specific callsites、`0x1D80B/0x1D8BA/0x1D988` unit scans、`0x13A9F` dispatcher、`0x15A1E..0x15B76→0x15B77` score 邊界與 `0x14EF0` candidate dispatch；raw selectors/table semantics、`0x14237/0x1567E` 契約與完整 AI runtime 仍未閉合 | 高 |
...
| 商店(一般商品) | `0x2e341→0x2f0b0/0x2f642/0x2f883/0x2f8ea`、doc56 UI-09 | 🟡 | purchase、sell、standalone equip與transfer四條production縱切已閉合。transfer保存512/511/510/506訊息、source/item/destination/full loop、raw remove→append→source recalc與self-transfer；strict adapter拒絕raw/compact divergence。sell仍只宣稱gameplay parity，不宣稱save byte parity。DOSBox E2未閉合；custom variant 0保留generic fallback | 高；做同save/tick E2與secret/variant驗收 |
| 城鎮hub畫面／selector | `0x2cd16/0x2cf71/0x11eb0`、FDOTHER#10/#11/#61/#62、FDTXT `0x1ef+selection`、FDICON0..2 | 🟡 production、E2 pending | 23個town保存raw variant0/1/2；production已接原版背景、label、六組座標、`0,1,2,1` pulse與312×192→VGA `(4,4)`，並取代generic半透明清單。仍缺同章／同selection／同BIOS tick DOSBox pixel diff | 高；補E2 pair/diff與逐variant capture |
| 祕密商店進入gate | `0x2cde0..0x2cef7`、town table `0x6238d`、`0x2d28c` selection5→`0x2e341` | 🟡 production/E2 pending | 撤回舊✅、persistent-flag等價及「chord立即進店」說：原版以每章0x1f-byte record的`+1`目前選項、`+2` BIOS Shift/Ctrl/Alt-F1..F10 scan同時命中，當次只把selection改為5並重畫；後續Enter才dispatch。23筆gate已進editable `native_secret_gate`；legacy `SecretIf`／`found_secret_ch*`只保留擴充相容，不再當native證據 | 高；DOSBox逐章chord/reveal/confirm與town→variant5 E2 |
