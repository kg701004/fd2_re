...
| 敵方 AI：物理落點、尋路與目標評分 | doc11（2026-07-29 recheck） | 🟡 | `0x145CD→0x4E040→0x146D1→0x14B16` 已閉合可達落點；`0x14237` 已閉合逐落點／目標評分；`0x14B78→0x4E1A6→0x13488` 已閉合方向碼、地形成本、路徑與實際落點排序。無 action 的一般 mode 0 會先走 `0x14121` mode-2 blocked-cell 搜索，再以 `0x13E9C` Manhattan 最近 raw opposite group 備援；舊 `0x15192` 說法撤回。新增 raw-only destination／score／path adapters；`0x1DEBE/+8`、完整 mode/selector/turn 語意與 production runtime 接線仍未閉合。現行 `aiTargets` 是「最近 hostile 作移動目標」、`aiApproachPath` 是「可達格再最小化 Manhattan」的重製近似，不等於上述原版鏈 | 高 |
