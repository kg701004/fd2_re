...
| 敵方 AI:目標評分(dmg、擊殺加成×2) | doc11（2026-07-27 recheck） | 🟡 | remake 的 `aiActUnit/NextAIPlan` 仍是 normalized approximation；canonical Docker scan 找不到 `0x15140`/`0x15356` direct callers，且 `0x15140` bytes 不像 AI entry，故舊公式不能再標為已證實。已補證 `0x1D80B/0x1D8BA/0x1D988` unit-scan callers、`0x13A9F` record command dispatcher、`0x15A1E..0x15B76→0x15B77` score 邊界與 `0x14EF0` candidate dispatch；raw `+6`/table/loop semantics、`0x14237/0x1567E` 契約與完整 AI runtime 仍未閉合 | 高 |
