...
| 祕密商店進入gate | `0x2cde0..0x2cef7`、town table `0x6238d`、`0x2d28c` selection5→`0x2e341` | 🟡 production/E2 pending | 撤回舊✅與persistent-flag等價說：原版以每章0x1f-byte record的`+1`目前選項、`+2` BIOS Shift/Ctrl/Alt-F1..F10 scan同時命中，當次把selection改為5；不先解鎖第六個可見選項。23筆gate已進editable `native_secret_gate`，runtime chord可直接進variant5；legacy `SecretIf`／`found_secret_ch*`只保留擴充相容，不再當native證據 | 高；DOSBox逐章chord/input與town→variant5 E2 |
