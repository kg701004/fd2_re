...
1. **次要,仍值得做**:逐位元組核對 `0x18890` 函式體內 ESP 位移鏈,坐實(或推翻)§5
   「`outBuf` 就是 `FUN_0003706e()` 回傳的 `local_20`」這個目前只有推論、沒有逐位元組
   核對的環節——即使 §6 已經推翻了「表有 0 點」,`outBuf` 到底是不是 `malloc(MV)` 這件事
   本身仍是一個先前完全沒人記載過的新事實,值得單獨坐實或撤回。
2. **開放候選(本節未查)**:`FUN_0004e42c`/`FUN_0004e4be`(移動選單開啟時的第一套
   flood-fill)有沒有類似的緩衝區/深度不匹配問題?本節只查了目的地確認(`e5cc` 家族),
   C.16 的按鍵序列(移動候選格→確認)理論上兩套都會被觸發,`e42c` 那套本節完全沒有
   重新用「有沒有溢位候選」這個角度查過。
3. 如果 §7.2 也沒查到東西,C.16 的機制搜索需要換一個完全不同的角度(例如:不是這條
