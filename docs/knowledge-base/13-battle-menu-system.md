...
**這是本節能給出的最具體候選,但需要一項本節反組譯範圍答不了的資料**:成本等級表的
**實際內容**(不是查表邏輯,是表裡的數字)是 runtime 從 `FUN_000111ba()` 載入的
per-地圖資源,不是編譯期常數,靜態讀 EXE 讀不到——**需要活體 dump `DAT_0006006a`
指向的表(`FUN_0004e8a5()` 回傳值,每單位版本)或直接讀 `DAT_00053a69` 指向的
地形類別表,兩者其一即可確認有沒有 0 點 tier**。這是本節唯一建議回到活體驗證的
一步,其餘都已用純靜態方法查完。

### 6. 給下一輪的具體建議

1. **最高優先(唯一需要活體的一步)**:在戰鬥中對 `DAT_0006006a`(每單位地形成本表指標)
   或 `DAT_00053a69`(地形類別→成本等級表指標)做一次 `MEMDUMPBIN`,逐值檢查有沒有 0。
   如果有,§5 的候選機制成立,下一步是找出哪種地形觸發(河流?道路?)、能否用
   `fd2_battle_autoplay.py`/`fd2_stat_override.py` 刻意誘導單位走上那種地形來重現 DOS-exit,
   取代現有「隨機 `--attack` 期望撞到」的盲測方式。
2. **次要**:逐位元組核對 `0x18890` 函式體內 ESP 位移鏈,坐實(或推翻)§5「`outBuf` 就是
   `local_20`」這個目前只有推論、沒有逐位元組核對的環節。
3. 如果 §5 被推翻(表裡沒有 0 點 tier),回到 C.16 交接清單原本的「純軟堆疊深度」角度——
   但本節 §3 已經算過,mode=0 下深度上限遠低於撞到 `DAT_00061646` 的門檻,那條路目前
   仍然缺一個「深度為何能失控」的具體機制,不能只靠「反正是遞迴」帶過。
