...
### 28. 2026-09-06(續五):補§27留下的小bug——acted位元設起來的時機比傷害生效晚,`main()`外層改成也比對敵方HP有沒有下降,不再對已經打中的單位誤判重試

`main()`的事後驗證(`todo`迴圈裡)原本只看`done["acted"] & 0x80`決定這一步
有沒有生效,沒設起來就當失敗、對同一個單位再補一次`rest_unit()`。§27親眼
看到這個判定誤判了一次真正成功的攻擊(idx1真的把敵方HP從18打到13,但acted
讀到的還是0x00)。改成:acted沒設起來時,先比對這個單位行動前後、敵方陣營
有沒有任何單位HP下降——有的話視為已經生效,不重試、不對它補按`rest_unit()`。
避免對一個其實已經行動完的單位亂送按鍵(這正是本輪反覆撞到的「對錯誤畫面
狀態送確認鍵」問題的又一個潛在來源,雖然這輪沒有直接證據說這個特定誤判
導致過崩潰)。語法與既有90項unittest全數通過;**這輪沒有另開instance重新
live驗證**這個特定修法,留給下一輪。
