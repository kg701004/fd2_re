...
1. **次要,仍值得做**:逐位元組核對 `0x18890` 函式體內 ESP 位移鏈,坐實(或推翻)§5
   ...
   本身仍是一個先前完全沒人記載過的新事實,值得單獨坐實或撤回。**本節嘗試過手動追蹤
   這條 ESP 位移鏈,中途放棄**:`push ARG;...;call F;add esp,N` 這種「呼叫後立刻清理」的
   pattern 在這段函式裡反覆出現,手動疊加位移非常容易算錯(doc13 §2 自己也提過同樣的
   風險)——寧可誠實留白,不要硬推一個沒把握的位移數字出來。
