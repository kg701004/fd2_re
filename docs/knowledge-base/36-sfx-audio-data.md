...
1. action_id 0-9 對應的玩家可讀招式名稱(劍/斧/拳/弓/…)未解 ── ~~同一函式另外用 `unit.byte[7]`
   ...
   **這徹底推翻本節開頭把`FUN_0002ff01`own body分支稱為「物理攻擊動畫」的假設——這9個
   ...
   表)——**這兩個是本輪唯一還說不通的地方,誠實保留,不強行套進movement敘事**。9=咒殺術，
   剛好卡在doc36引述的「bVar1<9才進own body」邊界外一格，可能是doc36對dispatch條件的
   文字摘要本身有微小誤差(例如實際是`<=9`或`<10`)，或`DAT_000524c6`的第10個entry(index9)
   是透過別的路徑被使用，這個邊界問題本輪未查清，留給下一輪。**item 584「招式中文名稱」
   這個子缺口，對9/10個action_id而言已經解開(引用既有command_labels.json,非本輪新解碼，
   是本輪首次把它跟`DAT_000524c6`結構分類接上)，只剩action_id9邊界與2/8兩個語意不吻合處
   待查**。
