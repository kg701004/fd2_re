...
1. action_id 0-9 對應的玩家可讀招式名稱(劍/斧/拳/弓/…)未解 ── ~~同一函式另外用 `unit.byte[7]`
   ...
   **2026-09-06查證,這條路本身在專案既有紀律下也已知會走到瓶頸,誠實記錄而非再猜一個新假說**：
   ...
   立下的紀律。**item 584語意半仍維持D,誠實标记為「已達純靜態方法的自然瓶頸」，需要下一輪
   換方法(不是換猜測)**。
...
| action_id | 位址 | 資料形狀 | 分類 |
|---|---|---|---|
| 0 | `0x2b996` | 7-byte flag + 7-dword X + 7-dword Y (各自獨立表) | template-A(movement,雙軸) |
| 1 | `0x2bb33` | 8-dword X + 8-dword Y (各自獨立表) | template-A(movement,雙軸) |
| 2 | `0x2bd6c` | 無關鍵幀表,依第5參數分流寫`[0x53fb2..4]`狀態旗標 | template-B(狀態效果) |
| 3 | `0x2bfd9` | 12-dword + 3-dword + 3-dword(三段,比0/1更大) | template-A變體(movement,規模更大) |
| 4 | `0x2c217` | 單一10-dword陣列(無獨立Y陣列) | template-A變體(movement,單軸) |
| 5 | `0x2c441` | 與action_id4逐指令同構,10-dword陣列(不同位址) | template-A變體(movement,單軸,與4同型) |
| 6 | `0x2c67d` | 寫入一個literal常數`0x3c8efa2d`(若視為float≈0.0174,接近1度弧度制,疑似角度) + 5-dword + 5-byte | template-A變體(movement,含角度常數,疑似拋物線/投擲類) |
| 7 | `0x2cafc` | 與action_id4/5逐指令同構,10-dword陣列(第三個同型) | template-A變體(movement,單軸,與4/5同型) |
| 8 | `0x2ccf4` | 只有4-dword(比其他都小) + 依`[ESP+0x34]`參數分流 | template-B候選(資料量小、有flag分流，形狀更接近action_id2/9而非0/1) |
| 9 | `0x2ce1a` | 無關鍵幀表,依第1參數分流寫`[0x5412e]`/`[0x5412f]`狀態旗標,回傳0x14(20) | template-B(狀態效果,已於更早輪次讀出但未寫入本文件) |
