...
旗標而非移動座標**，與action_id0的「7-step X/Y攻擊撲擊動畫」是完全不同類型的機制。這證實
10個action函式彼此異質(不是同一套movement模板套 10 次)，招式名稱交叉對照需要逐一分辨
「近戰位移類」vs「狀態效果類」等不同機制群組，不能假設全部走同一套資料格式。`0x53fb2`
...
action函式本體去找有沒有其他函式也碰這三個位址。其餘8個action函式(action_id 1,3-9)仍未
反組譯。
