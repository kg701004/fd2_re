...
1. `local_20`在Ghidra decompile裡宣告成`byte local_20[12]`(12 bytes,對照
   ...
   但**填值來源不在這個函式本體內部**。
2. **排除了兩個最直接的候選**:函式一開始有個`for(iVar3=0xb; -1<iVar3; iVar3=iVar3-1)
   ...
   任何記憶體回傳,是**純UI繪製**(畫12個選單格icon),不是填值迴圈。`DAT_00053c57=0;
   do{FUN_0001d51d()}while(...)`(選單導覽主迴圈)本身也複查過:`FUN_0001d51d`
   只操作`DAT_00053c57`(依方向鍵±1/±4做2D網格導覽,`FUN_0001c269()`的回傳值
   當作已知法術/道具數量做邊界判斷)跟按確認鍵時的MP足夠判定,**同樣沒有直接寫
   `local_20`**。
3. **最可能的候選,留給下一輪**:`FUN_0001c269()`本身。本文件第11輪已經證實過
   這個函式在**另一個呼叫點**(`0x1d150 lea eax,[esp+0xc8]; push eax; ...call
   0x1c269`)是一個「掃單位的能力bitfield、把設定位元轉成index、寫進呼叫端傳入的
   緩衝區、回傳計數」的通用函式——`FUN_0001cff0`跟`FUN_0001d51d`都各自呼叫過
   `FUN_0001c269()`(decompile因為呼叫點參數解析失敗顯示成空括號),很可能其中一次
   (或兩次都)把`local_20`的位址當緩衝區傳入,填出這個commandId清單。**這輪沒有
   逐指令核對這兩個呼叫點實際傳的參數位址**(需要對`0x1cff0`跟`0x1d51d`各自呼叫
   `0x1c269`的組合語言PUSH序列個別核對,才能證實/推翻),是下一輪的具體切入點,比
   本輪已經排除掉的兩個候選更有希望。

**誠實現況**:填值來源比之前(完全不知道往哪查)收斂了不少——確定不是`0x18409`
繪圖迴圈,也不是`0x1d51d`導覽迴圈自己直接寫,最可能是`0x1c269`透過某個呼叫點的
隱藏指標參數填的,但這一輪沒有把最後一步逐指令釘死,不宣稱完全解決worklist
L604/L622。
