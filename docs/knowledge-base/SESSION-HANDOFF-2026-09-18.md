...
- 只看 Ghidra 的 750 個:機器登錄 139、文件記載入口 187、文件只在本體內提及 141、完全無 283。
  完全無的 283 個有 **245 個在 `0x3702f`(`__STK`)之後的函式庫區**(Watcom CRT、AIL),共 26373 bytes;
  遊戲邏輯區 `0x10000`–`0x3702f` 只有 **38 個**(5461 bytes)。283 個全部 ≤ 1024 bytes。
...
- 路 4(函式庫簽名比對)的目標就是函式庫區那 245 個小函式,範圍比原估的 250–350 更具體。遊戲邏輯區完全沒人碰過的只剩 38 個。
- 洩漏名稱那半天不用做了。
