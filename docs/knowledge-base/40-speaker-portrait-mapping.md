...
**下一輪建議**:
1. 如果要繼續找 032,**改用更密的截圖間隔**(每 2-3 個確認鍵就截一次圖,不要
   一次送 10 個),重新從 033 結尾的「陪我去後山」那句開始追,原本 10 個一批的
   密度可能真的漏看了中間一段。
2. 或者接受 032 不在這條路徑上,改查 `docs/knowledge-base/91-worklist.md`/
   `campaign_full.json` 之類的章節腳本綁定資料,看 032 實際掛在哪個章節的哪個
   時間點觸發,直接跳過去,不要再靠「跟 033 相鄰」這個已被推翻的猜測。
4. `docs/data/runtime_speaker_resolved.json` 目前只有一個 FDTXT 的 key,格式
   已經預留給後續 FDTXT 累加,不需要改結構。
5. idx0/1 目前只是「依對話內容推測」是國王/王后,不是靠 DATO 值本身查出真名
