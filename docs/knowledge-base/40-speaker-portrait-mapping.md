...
## 2026-09-06 續:採用上面「優先解box_index 4-13登場台詞」的建議,FDTXT_001 5/7真正解出

新開一局(從標題START新遊戲,非讀檔——避開chapter_sweep存檔的LV/HP異常疑慮)、快轉
ch00王宮/練劍場開場劇情,直到畫面出現「乖乖的把身上的錢財和那個漂亮小妞交出來」
(ch01海盜遭遇戰的登場台詞)。這段文字逐字對到`runtime_speaker_todo.json`裡
FDTXT_001 box_index=13的`text_snippet`,用`--confirm-text`核對通過後立刻對活體
單位陣列跑`--resolve-todo`:

- box5/6/13/17/18(operand=idx 9/10/11)全部解出**DATO=0x60(泛用盜賊)**——跟同一天
  稍早針對FDTXT_002 roster組成的獨立觀察(`idx4-11皆為DATO=0x60`)完全吻合,是同一
  批盜賊單位,兩次獨立操作互相印證。這是誠實、正確的答案:這幾句本來就是無名雜兵
  喊話,不是命名角色。
- box31/32(operand=idx 15/16,「弟兄們好像陷入苦戰」援軍台詞)這個時間點讀到
  **DATO=0xff(空格)**——援軍還沒出現,不能勉強套用,老實記錄為`still_open`,
  留給下一輪推進到戰鬥後段再讀一次。

已寫入`docs/data/runtime_speaker_resolved.json`的新`FDTXT_001`區塊(5筆entries+
2筆still_open),`resolved_count`從72→77。**273筆待解清單目前累計:FDTXT_032(40)+
FDTXT_033(32)+FDTXT_001(5)=77筆已解**,FDTXT_001剩餘2筆與其餘FDTXT_002-029全部
仍待後續輪次。
