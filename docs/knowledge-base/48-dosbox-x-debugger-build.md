...
## 9. 2026-09-04:「確認鍵送不進戰鬥輸入迴圈」——**這道牆不是輸入問題,是量測工具問題**

這一項在 `SESSION-HANDOFF-2026-09-03.md` §9 被列為最高槓桿、擋住 7 件事。本輪用原版
DOSBox-X 從開機一路驅動到 ch01 戰場,結論是**原本的診斷本身站不住**。

### 9.1 確認鍵一直是好的

從標題畫面按 START 開始,連續約 **120 次** `Return` 一路推完 ch01 全部開場演出
(王宮對話 → 草原 → 戰場地圖),**沒有觀察到任何一次掉鍵**。

### 9.2 量測方法在「提問的那個畫面上」本身就是壞的,而且壞了兩次

**(a) `wait-settle` 會停在「轉場開始之前」的那一幀。**
settle 的規則是「連續兩幀 md5 相同就算靜止」。當一個鍵啟動的是**轉場**(對白翻頁、
場景切換、演出),畫面在最初幾次輪詢時**還是舊的**,於是 settle 回報
`SETTLED tries=2` —— 拍到的是舊畫面。本輪直接觀察到:在標題畫面按 Enter 後
settle 回報 tries=2 且畫面仍是標題,**但遊戲其實已經接受了 START**,下一次操作就出現
開場對白。

這對**確認鍵的傷害遠大於方向鍵**:方向鍵的效果(游標移動)是**立即**的,確認鍵的效果
是**延遲的轉場**。**這正好就是原始診斷「方向鍵會到、確認鍵不會到」的形狀。**

**(b) 戰場畫面永遠不會靜止。**
單位有待機動畫循環,畫面持續重繪。實測:**同一秒內連拍三張,三個不同的 md5**。
因此在戰場上:

- `wait-settle` **不可能成功**——實測 80/80 次全部 TIMEOUT,而 TIMEOUT 讀起來像
  「工具卡住」,不像「這個方法在這裡不適用」;
- `--flag-no-response` 的「前後畫面像素相同」判定**恆為假**——沒按鍵畫面也在變。

也就是說,在戰場上「有沒有靜止」和「畫面有沒有變」**兩個訊號都不帶任何關於鍵是否送達
的資訊**。判斷鍵有沒有送到的儀器,在被提問的那個畫面上根本測不到那件事。

### 9.3 順帶否證了既有的「gate 沒過」假說

用活體記憶體讀單位陣列(delta 由 gate-check byte signature 實測校準 = **`0x19c000`**,
單一命中):我方四名單位(idx 0-3)全部 `camp(+6)=0x02`、`acted(+5 bit7)=0`、
`gate3(+0x26)=0` —— **doc13 記載的三個指令環 gate 全部通過**。所以「gate 沒過 →
`0x17aed` 非互動卡片」不是這個現象的解釋。
(idx0 讀到 HP42/42、MP0/0、AP16、DP12、HIT97、DX2,與畫面上索爾的狀態卡逐欄吻合,
證明讀到的確實是當前戰局的資料,不是殘留。)

### 9.4 斷點證據(機制先驗過才採信)

> ⚠ **本節與 9.5 的推論已於同一輪被自己推翻,見 9.7。** 保留原文供追溯:
> 「沒打到斷點」只能否證「這一次執行經過該處」,不能否證「程式從未經過該處」——
> 當時我是在子畫面裡按 Enter,不是在瀏覽游標狀態。重做後 `0x18890` 當場命中。


在**驗證過的**斷點機制下,`0x117e7`(頂層戰場游標 handler)、`0x18890`(指令環入口)、
`0x17aed`(非互動卡片)**一個都沒有被執行**,而同一時間遊戲對 Enter/Escape 有明顯反應
(在單位狀態卡與一個四圖示環之間切換)。

機制怎麼驗的(兩個對照先被自己否決,值得記):

1. 第一個對照選了 `0x17fc0`(狀態面板渲染器)——它**未必**會被執行,
   **「可能被執行」的對照不是對照**,作廢。
2. 第二個對照選了 debugger 停下時 Code Overview 顯示的位址 `0x1EAA03` ——
   那正是 CPU **原本就停住**的地方,在當前 EIP 設斷點是套套邏輯,作廢。
3. 採用的對照:同一個**觀察到正在執行**的迴圈裡、但**不同**的位址 `0x1EAA0C`。
   設下後 EIP 由 `0x1EAA03` 前進到 `0x1EAA0C`、EBX 由 6 變 4 —— 機制確實有效,
   於是上面那三個「沒被打到」才有意義。

### 9.5 現在還開放的是什麼(不要寫成已解)

> ⚠ **本節已作廢,見 9.7**:這裡說的「開放問題」是錯誤推論的產物,
> 該介面就是 `0x18890`／`0x18d8c`,已當場斷點命中確認。


ch01 開場演出跑完後到達的那個介面(單位狀態卡 ⇄ 四圖示環,Escape 互切)**不是**由戰鬥
指令環那條程式路徑驅動的。它由**哪一段**驅動仍未定:EIP 取樣三次全落在 `0x4E2xx`/
`0x4EC6C` 的解壓/blit 區(代表畫面在持續重繪,不是輸入 handler),而在文件記載的整備
選人 handler `0x318ad`/`0x31e80` 下斷點也沒有命中。

所以這道牆的形狀從「確認鍵送不進戰鬥輸入迴圈」改成:**遊戲根本還沒進到戰鬥輸入迴圈,
而原本用來判斷『鍵有沒有送到』的方法在那個畫面上測不出東西。** 後半已經修掉(見 9.6),
前半仍是 worklist 自己列的那個開放問題「如何穩定進到戰鬥」。

### 9.6 工具已修(兩項,皆 live 雙向驗證)

- `wait_for_settle(..., require_change=True)`(**預設開啟**):給了 baseline 時,若 settle
  的結果仍與 baseline 相同,會再多看最多 4 輪,讓慢轉場有機會開始;真正的 no-op 鍵仍會
  回報 NO_RESPONSE,只是多看幾眼。`--no-require-change` 可退回舊行為。
- `screen_is_animated()` + `ANIMATED` 警告:settle 失敗時自動連拍 3 張無輸入樣本,若三張
  全不同就明講「這個畫面會自己重繪,settle 與 flag-no-response 在這裡都不帶資訊,改用固定
  等待 + 語意檢查(讀活體記憶體,或用畫面自身內容辨識)」。
  雙向驗證:戰場畫面 → `True`;debugger 暫停(畫面必定凍結)→ `False`。

### 9.7 **同輪更正:9.4／9.5 的推論寫過頭了,這道牆是完全否證,不是「還沒進到戰鬥」**

9.4 記錄「`0x117e7`／`0x18890`／`0x17aed` 一個都沒被執行」,9.5 據此推論「遊戲根本還沒進到
戰鬥輸入迴圈」。**繼續探測後發現那個推論是錯的**,原因如下,一併記錄以免重犯:

**錯在哪:** 那幾次按 Enter 我人在**子畫面裡**(指令環已經開著、或停在狀態卡上),不是在
瀏覽游標狀態。指令環是在我**設斷點之前**就已經進入的,所以「沒打到 `0x18890`」只說明
「這次按鍵不是從瀏覽游標按的」,完全不能推論戰鬥迴圈沒在跑。**斷點沒命中,只能否證
「這一次執行經過該處」,不能否證「程式從未經過該處」。**

**狀態全域先打臉了那個推論**(讀 live memory,delta 0x19c000):

| 全域 | 值 | 意義 |
|---|---|---|
| `[0x53c03]` | 0 | 章節 = ch01 |
| `[0x53bef]` | **1** | 回合/進度 counter = **第 1 回合,戰鬥正在進行** |
| `[0x53beb]` | **12** | 場上 12 個單位(我方 4 + 敵方 8) |
| `[0x53ab1]`／`[0x53ab5]` | 10 / 15 | 游標格 (10,15) |

單位陣列解出的座標證實游標 (10,15) 正好落在 idx1(`char_id=9`=悠妮,camp 0x02,acted 0)。
戰鬥不但在跑,游標還正停在一名可行動的我方單位上。

**重做的乾淨測試(這才是一開始就該做的):** 先按 Escape 退回**瀏覽游標**狀態(截圖確認
有選取框、沒有指令環),再武裝 `BP 0170:001B4890`／`001B3AED`,然後按 Enter ——
**EIP 命中 `0x001B4890`**,即 `0x18890`。

也就是說:**從瀏覽游標按確認鍵,進的是真正的指令環路徑,不是非互動卡片 `0x17aed`。**
Resume 後截圖看到的是**移動範圍顯示**(索爾 LV-01/HP042/MP000 標頭卡 + 周圍亮起的可移動
格),與 doc13 §0 記載的鏈路逐段吻合:

```
0x117e7(Enter,三 gate 全過)→ 0x18890(移動範圍/選格)
   → 0x115b6 移動確認 → do{ 0x18d8c }while(...)  ← 四方向指令環
```

先前看到的「四圖示環 + 左下迷你狀態卡」就是移動確認之後的 `0x18d8c` 指令環本身。

**結論改寫:** 這道牆**不存在**。原版的戰鬥輸入迴圈、三個 gate、移動選格與指令環全部正常;
擋住 7 件事的不是遊戲,是 9.2 那兩個量測缺陷。9.5 說的「仍然開放:那個介面由哪一段驅動」
也隨之取消——它就是 `0x18890`／`0x18d8c`,已當場命中確認。

**留下的真正限制**(不要當成牆):按上(攻擊)在當時沒有反應,而那是 doc13 §1 記載的正常行為
——武器射程內沒有敵方候選時攻擊被 disable,「按上不會有任何提示,就是沒有反應」。當時
索爾周圍確實沒有相鄰敵人。要驗攻擊路徑得先把單位移到敵人旁邊。

---

## 10. 2026-09-04:`REAL-UI-MOVE-CONFIRM-ENTER-SPACE-INTERMITTENT-INPUT-DROP` 量化重測

這一項的既有調查(doc58 續四十五~五十五)品質很高:用斷點證實 CPU 停在 `0x12dac`
(`0x115b6` 專用的阻塞式讀鍵輪詢器)合法等鍵,送 `Enter`/`Space` 時 `0x11719`
(confirm 驗證段)與 `0x117e6`(RET)**從未命中**,同一刻送方向鍵游標卻立即反應,
並跑過一輪零 `Alt+Pause` 的乾淨對照。**所以 §9 那個「settle 假象」的解釋不適用於它**
——不能因為別的結論被否證就順手推翻這一個。它需要自己的量化重測。

### 判定方式

沿用它自己的判準:`BP 0170:001AD719`(= `0x11719`,前輪明確記錄「送 Enter 從未命中」
的那個位址)是否命中。載入 delta 由 gate-check byte signature 實測校準(`0x19c000`)。
每次試驗:瀏覽游標 → `Enter`(進 `0x18890` 移動選格,即 `0x115b6` 等鍵的狀態)
→ 確認鍵。斷點只武裝一次,之後靠 BP 自身中斷 + `RUN` 續跑,**全程不再送 Alt+Pause**。

### 結果

| 組態 | 鍵 | 命中/試驗 |
|---|---|---|
| ch01(全新遊戲) | Enter | 19/25 —— 失敗 6 次**全部集中在最前面**,之後連續 19 次全中 |
| ch01(全新遊戲) | Space | **20/20** |
| ch01(全新遊戲,重現對照) | Enter | **20/20** |
| **ch27(前輪那份存檔,`FD2.SAV` md5 `e6d9a35756cddfc2519969b10f039181`,已逐位元組確認相符)** | Enter | **20/20** |
| **ch27(同上)** | Space | **15/15** |

**狀態與量測都正確之後合計 94/94,零失敗。** 原始宣稱的核心是
「`Enter`/`Space` 這兩個 scancode 選擇性地沒有被讀鍵迴圈觀察到」——在現行 WSL2-native
DOSBox-X build 上、**在它自己那份存檔的同一章**,重測不到。

### 三個「長得跟輸入丟失一模一樣」的非輸入成因(本輪全部親自踩過)

這是本節真正的價值:在畫面上,以下三者與「鍵被丟掉」**完全無法區分**。

1. **游標停在空地。** ch27 第一輪拿到 **0/25**,我差點寫成「失效重現」。實際是游標在
   `(15,54)`,而單位在 `(14,54)`／`(16,54)`——**中間那格沒有單位**,按 `Enter` 本來就
   不該有反應。只有把游標格對照單位陣列的 `+0/+1` 才分得出來。
2. **命中判定過嚴。** 斷點命中後 CPU 可能已步過該指令,`EIP` 顯示 `001AD71C`(+3)而非
   `001AD719`。用嚴格等值比對會把真命中記成 MISS。
3. **失敗分支忘了 resume。** 判為 MISS 時沒有 `RUN`,遊戲就一直停在 debugger,之後
   每一次按鍵都無效,**失真連鎖擴散到後續所有試驗**——單看結果就是「一開始好好的,
   後來全部掉鍵」,正是「間歇性」的長相。

ch01 那 6 次開頭失敗屬於第 1 類(UI 尚未進入瀏覽游標狀態);同一組態重跑,從第 1 次
就 20/20。**階梯狀而非散佈的失敗分佈,本身就是狀態問題的指紋,不是隨機掉鍵的指紋。**

### 誠實的界線

- **不宣稱前輪的觀察是假的。** 環境(DOSBox-X build、Xvfb、當時的腳本)與本輪不同,
  前輪的斷點證據也確實顯示 CPU 在 `0x12dac` 等鍵。可以確定的是:**在現行環境、同一份
  存檔的同一章,用它自己的判準重測 94/94 全中**,而且本輪找到了三個會產生同樣表象的
  非輸入成因。
- 前輪的證據**與第 1 類成因並不衝突**:`0x12dac` 在該狀態下本來就會合法地一直等鍵,
  它證明「程式在等鍵」,不證明「當下有可確認的目標」。
- 建議把此項由「未解的間歇性輸入丟失」改列為「**現行環境不可重現**;若再出現,先依序
  排除上列三個成因再談輸入層」。

---

## 11. 2026-09-04:活體數值覆寫加速驗證(`tools/fd2_stat_override.py`)

把我方 HP/MP/AP 拉高、敵方 HP 壓到 1,讓一場戰鬥能在幾回合內結束,用來驗證
**由控制流決定**的東西(勝利曲、結局演出、章節轉場、存檔行為、字模截圖)。
**改過數值之後,任何依賴數值的結論都不算數**(AI 評分、傷害公式、命中迴避)。

### 欄位與陣營(只用實測過的那組)

全部 u16 LE:`+0x40` HPmax、`+0x42` HPcur、`+0x44` MPmax、`+0x46` MPcur、
`+0x48` AP、`+0x4a` DP、`+0x4c` HIT、`+0x4e` DX。陣營在 `+0x06`(`0x02` 我方 / `0x00` 敵方)。

**MV 不在這組裡,工具不碰。** 計畫檔記 MV=`+0x3b`,但同一份來源把 AP 記成 `+0x37`,
而實測 AP 在 `+0x48`——兩套偏移互相矛盾,在 MV 被獨立定位之前寫 `+0x3b` 有風險。
(就算定位到也不該設上萬:可移動格是 flood fill,地圖才 ~20×60。)

### 什麼時候套用:**進戰鬥之後,或戰前演出階段都可以**

單位記錄由 constructor(`0x10c50`,依等級×成長表算)在戰鬥開始時建好,所以**先於它套用會被蓋掉**。
本輪實測:在 ch01 **戰前演出**階段套用(`[0x53beb]` 已是 4 → 12),
再推進 20 次確認鍵進入可操作戰鬥後重讀,**數值完整存活**
(我方 999/999/999、敵方 HP 1、`+5` bit0 仍設著),`[0x51a11]` 也回到戰鬥曲 19。

### ⚠ 寫入後畫面**不會**自動重繪

寫完之後狀態卡仍顯示舊內容。本輪先看到 `AP·???`/`HP ???/???`,
差一點寫成「4 位數超出 3 位數欄寬會顯示 `???`」——**那個觀察被 stale render 汙染了**。
按 Escape 再開一次卡片(強制重繪)後,同一份記憶體讀出 `HP 999/999`、`AP·999`,顯示完全正常。
**因此 9999 的顯示行為仍屬未知,不宣稱。** 要從畫面讀值一律先強制重繪。

### 建議參數:**我方 999 / 敵方 HP 1**

999 已實測正常顯示;對 ch01 仍是無敵(敵人約 20 傷害)。這與專案記憶裡記載的
`party=999/enemy=1` 一致。

### 已知仍未驗證的三項風險

1. **增援波次**:中途生成的是全新記錄,帶原本數值,每波後要重寫。
2. **升級重算**:用高 AP 殺敵給大量 EXP,升級會依成長表重算 AP/DP/HP/MP,可能蓋掉覆寫。
3. **戰後同步**:場上陣列 `[0x53a45]` 的結果會同步回持久名冊 `[0x53bf7]`,
   覆寫值有可能進到存檔。用可丟棄的存檔跑。

### 一條不成立的捷徑

把八個敵人的 `+5` bit0(`0x3453e` 讀的那個 raw 位元)全部設起來,**沒有**觸發勝利判定,
戰鬥照常進行。要贏還是得真的把敵人打掉。
