...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、`0x17eef/0x17fc0/0x184c0`完整 indexed compositor與12-frame三區clip schedule已有 Ebiten adapter；bounded oracle通過，但正常campaign主角缺constructor provenance而fallback。`0x19953` 是另一 selector | partial | 主角constructor/class-change lifecycle、item Enter effect/target transaction、end-turn entry、DOSBox visual diff與缺archive UX |
