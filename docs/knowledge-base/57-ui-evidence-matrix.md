...
| UI-06 HUD | native map HUD `0x1acf3` 的 panel→terrain→AP→DP→optional unit icon→HP 已由 `BlitNativeMapHUD→ComposeNativeFrame` 接入 ch01 production full frame；display gates、persistent anchor、LMI1 #130／hex #0x83/#0x84、digit banks與FDICON selector均有 regression。`0x11cfa`證實HUD base是`work+0x8088`，修正後原版錄影與remake截圖都在下緣顯示panel。full-screen #22只在native admission失敗或尚未materialize章節作fallback | partial | ch02+逐章 view/gates/anchor provenance、`0x12c0d` exact raw lookup predicate/order、同一state的DOSBox/remake pixel diff；raw globals高階名稱仍不猜 |
