...
| UI-07 postbattle | `campInput` battle result 約 2394；campaign node 可表達 post node；`campaign_full` 30 戰 transition matrix 已逐列展開。主迴圈直接指令、scenario `chNN→map(N-1)` 與 handler `chNN_post→set_chapter(N+1)` 共同證實玩家戰鬥 N 使用 raw `ch(N-1)_post`。13個既有同號錯接已全數清除；目前稽核為12 active／12 blocked。新增的 raw ch05→玩家ch06 由 IDA 固定入口 `0x23296` 與 `0x232e3→0x231df` 共享尾段，順序為 JOIN13／spawn3／pan／ACT27／dialog／sync／chapter6，現接 `postbattle_ch06→town_ch07`。raw ch25／ch27 則分別屬玩家ch26／ch28；第27戰天空之鑰成功分支不重用 raw ch27。七個未綁定 postbattle 原有的泛用 inline beats已移除。raw `ch15_post` 實屬玩家第16戰，仍缺一般玩家原版 runtime capture、JOIN-time persistent record及town17/save回歸，故 `postbattle_ch16` 維持 unbound。IDA 另固定 `0x2CAD7` 子流程 raw 0=內部重複；目前只保存 raw outcome，不命名退出原因。位址證據見 [`fd2_ch05_post_dispatch_ida.txt`](../data/fd2_ch05_post_dispatch_ida.txt) 與 [`fd2_post26_28_dispatch_ida.txt`](../data/fd2_post26_28_dispatch_ida.txt) | partial | 以原版 handler offset／DOSBox input 差分核對每章是否進 town/shop/rest/preparation/ending；ch07/ch16 等仍 fail-closed |
