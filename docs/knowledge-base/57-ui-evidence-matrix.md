...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、geometry/icon/label ABI與12-frame三區clip opening/closing schedule已釘；official IDA另閉合 `0x17eef/0x17fc0` 的 FDOTHER#5 entries20/21、DATO selector與全部固定 destination/record-offset schedule。remake ring與item shell仍是partial approximation；`0x19953` 是另一 selector | partial | 將已資料化的 indexed source/data plan接成 Ebiten adapter、item Enter transaction、end-turn entry、DOSBox visual diff |
| UI-04 target/range | `0x1cff0` + `0x149f8` 證實 command record `+3/+4/+6` 參與 target-candidate geometry；`0x1bbdc` item case 0 的 two-stage targets、observed type5–24 effect dispatch 已閉合。type23 mode6 落點另以 other-unit raw `+5 bit0` occupancy gate、target class/race selector、29×20 `0x4e555` cost row與 terrain entry==20 判定，已有 pure adapter；remake movement/attack/spell selection 已有，但 item action Enter 仍 fail-closed | partial/missing | 將已證實 item target/effect transaction接入 UI、native argument↔weapon min/max mapping、AOE/LOS、不可用目標灰化、indexed relocation presentation |
...
| UI-06 HUD | native map HUD `0x1acf3` 已釘 display gates、LMI1 #130(69×34) at row157、terrain/AP/DP offsets（AP row8 col43；DP row19 col43）及依正負選 directory entry decimal #83/#84、取絕對值後 decimal-digit 的 raw path；remake 尚畫 full-screen #22 approximation | partial | #83/#84 artwork semantics、optional unit icon/HP resource、raw anchor globals 的高階語意、native asset/runtime adapter 與 320×200 ground truth |
...
| UI-10 church | `0x2d7bd` 左右四項循環、Enter/Space confirm、Esc cancel；`0x3072f` raw dispatch `0→0x2ffa5`、`1→0x2f8ea`、`2→0x30dc3`、`3→0x31385`；`0x2e6b8/0x2df6b` 的 caller-owned list selector 亦已閉合；remake selector 已對齊左右 ABI，index 1 有 transfer mechanics slice | partial/fail-closed | `0x2d669` indexed menu transition adapter、raw index 0 的 `0x17aed` renderer/text、raw inventory flag export 與未命名 service renderer |
