...
| UI-02 field | map/camera/cursor/unit/HUD Draw 約 3441–3568、4571、4595；camera、absolute/visible cursor、HUD anchor/gates 與 FDOTHER #130 panel 已有直接原版資料流；ch26 event61 所需 view/HUD 已達 E1。ch00 的 `0x32999` 已以 FDOTHER #9 接12次索引呈現、pass6/7/8 snapshot 重建及 pass1 #95，兩次各12幀後能沿正式 handler 進入戰鬥、戰後、城鎮與整備；目前是 E1 決定性路徑，不是同狀態 DOSBox 畫面 E2 | partial | 除 ch26／ch00 E1 切片外，ch02+ 的逐章 dynamic view/gates/anchor producer、同 roster/event/tick 的 DOSBox 像素差分，以及 `0x12c0d` 的 exact raw lookup predicate/order；另補 ch00 同 camera/roster/pass 的原版逐幀比較 |
