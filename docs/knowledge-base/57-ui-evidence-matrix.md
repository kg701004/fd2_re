...
| UI-07 postbattle | `campInput` battle result 約 2394；campaign node 可表達 post node；`campaign_full` 30 戰 transition matrix 已逐列展開。主迴圈直接指令、scenario `chNN→map(N-1)` 與 handler `chNN_post→set_chapter(N+1)` 共同證實玩家戰鬥 N 使用 raw `ch(N-1)_post`。13個既有同號錯接已全數清除；目前稽核為14 active／10 blocked。raw ch06→玩家ch07 已閉合 map6 六格 selector0 event26 的 raw `+6` gate、slots9..27 mode寫入與 state16 producer；enemy turn10 event25 只有在 state16==1 時才建立34→44 runtime、寫state17，戰後再經slot43 raw gate、唯一JOIN12 persistent record進 `town_ch08`。未踏格反例維持34 slots；先前「第10回合必定增援」與96-slot空白 frontier斷言均已撤回。raw ch12→玩家ch13 由 table bytes 固定 interior entry `0x2389f`；FDTXT_013 index9 依序跨 scene3／4各6句，再走 sync／JOIN3／chapter13，現接 `postbattle_ch13→town_ch14`。raw ch05／ch25／ch27則分別屬玩家ch06／ch26／ch28；第27戰天空之鑰成功分支不重用 raw ch27。七個未綁定 postbattle 原有的泛用 inline beats已移除。raw `ch15_post` 實屬玩家第16戰，仍缺一般玩家原版 runtime capture、JOIN-time persistent record及town17/save回歸，故 `postbattle_ch16` 維持 unbound。IDA 另固定 `0x2CAD7` 子流程 raw 0=內部重複；目前只保存 raw outcome，不命名退出原因。位址證據見 [`fd2_ch06_post_event25_ida.txt`](../data/ida/fd2_ch06_post_event25_ida.txt)、[`fd2_ch12_post_dispatch_ida.txt`](../data/fd2_ch12_post_dispatch_ida.txt)、[`fd2_ch05_post_dispatch_ida.txt`](../data/fd2_ch05_post_dispatch_ida.txt) 與 [`fd2_post26_28_dispatch_ida.txt`](../data/fd2_post26_28_dispatch_ida.txt) | partial | 以原版 handler offset／DOSBox input 差分核對每章是否進 town/shop/rest/preparation/ending；ch16 等仍 fail-closed，第7戰尚缺一般玩家DOSBox E2 |
