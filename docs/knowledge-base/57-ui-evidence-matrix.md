...
| UI-04 target/range | `0x1cff0` + `0x149f8` 證實 command record `+3/+4/+6` 參與 target-candidate geometry；`0x1bbdc` item case 0 的 two-stage targets、observed type5–24 effect dispatch 已閉合。remake已接tracked type5–24 target transaction；type23另有mode6 destination cursor、occupancy/class/race/29×20 cost/terrain gate。unsupported branches與indexed effect presentation仍fail-closed | partial | native argument↔weapon min/max mapping、AOE/LOS、不可用目標灰化、indexed item/relocation presentation |
