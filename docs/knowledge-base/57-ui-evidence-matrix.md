...
| preparation | 35–45% | 舊「兩欄文字核取方塊」斷言已失效。`0x318ad/0x31e80` 的原版三區背景、二位數計數、10 欄角色格、游標、彩色／灰色繪法、四向輸入與游標角色 `0x17fc0` 完整狀態已接生產路徑；[`preparation-roster-compositor-partial.png`](../figures/preparation-roster-compositor-partial.png) 是原版資源合成證據，不是 DOSBox 實機。仍缺待機／進退動畫、完整確認介面、晚期存檔與同狀態實機差分 |
