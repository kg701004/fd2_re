...
| town hub | 75–85% | ch02 variant0 的 selection0/pulse2 與 Left 後 selection1/pulse2 均取得原版 DOSBox E2；修正共用 `0x4ea2a` glyph shadow 位址後，兩組 production remake 與原版的 320×200 raw RGB MD5 分別整幀相同。variant1/2、selection2–5、Right/wrap 與 secret chord 仍待 E2 |
| weapon/item/secret shop | 80–85% | 69個shop節點已用variant 1/3/5啟用indexed owner；四項service production owner均已接。secret entry已由town record `selection+BIOS scan` exact gate接runtime，撤回persistent flag等價說。此比例仍非pixel diff；缺variant1/3/5同狀態DOSBox E2與secret chord實機驗收 |
...
| UI-08 town | `0x2cd16/0x2cf71/0x11eb0`；FDOTHER#11/#61/#62背景、#10 label、FDTXT `0x1ef+selection`、FDICON pulse、三variant×六selection座標；23筆raw variant已接production。ch02 postbattle 以 `/tmp` sandbox route patch 走完原版20頁確認，第21次確認取得 town；[`selection0 pair`](../figures/town-hub-original-vs-remake.png) 與 [`Left→selection1 pair`](../figures/town-hub-selection1-original-vs-remake.png) 在 pulse2 都是 raw RGB 整幀相同。`0x2ce7a/0x2ceac/0x2cef7` 不寫 pulse counter，已刪除方向鍵／secret reveal reset | partial（E1 + ch02 two-state E2） | 補variant1/2、selection2–5、Right/wrap、hidden selection5 reveal/confirm E2 |
| UI-09 shop | purchase、sell、standalone equip與transfer均有original-resource regression及production owner；strict adapters在raw projection不完整時fail-closed。secret gate保存23筆normal selection與Shift/Ctrl/Alt-F1..F10 BIOS scan；chord只揭露selection5，後續confirm才進variant5，不是永久第六項 | partial | variant1/3/5同狀態DOSBox E2；secret chord reveal/confirm與town restore實機diff |
