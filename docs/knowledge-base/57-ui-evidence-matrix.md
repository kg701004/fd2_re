...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；remake `ringInput` 約 2407 已改同序 | partial | `0x1cff0` spell-vs-attack command table、`0x1bbdc` item effects、完整 enable gate、end-turn entry |
| UI-04 target/range | `0x1cff0` + `0x149f8` 證實 command record `+3/+4/+6` 參與 target-candidate geometry；`0x149f8` 會沿格步進、篩 unit index；remake movement/attack/spell selection 已有，item action 約 2475 仍提示未實裝 | partial/missing | selector↔spell family 對照、weapon structure reach、AOE/LOS、不可用目標灰化 |
