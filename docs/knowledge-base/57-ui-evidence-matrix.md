...
| UI-09 shop | purchase、sell、standalone equip與transfer均有original-resource regression及production owner；strict adapters在raw projection不完整時fail-closed。secret gate保存23筆normal selection與Shift/Ctrl/Alt-F1..F10 BIOS scan；chord只揭露selection5，後續confirm才進variant5。ch02 variant1/3/5 service0 selected phase均與同gold remake整幀AE=0；variant5四service、wrap及Escape→town selection5亦閉合。weapon purchase list四個selection、其後Yes/No、gold0不足金，以及gold1000裝備收件者selection0/cycle1也各自全幀AE=0；recipient使用screenshot-only LOADCH typed-party bootstrap，DX 2/2/1/2是visible HIT/EV與known equipment rows交叉約束的projection、不是直接raw dump，並修正AP/DP相對HIT/EV三像素layout | partial（E1 + ch02 shop menu/purchase-list/confirmation/insufficient/equipment-recipient stable E2） | 正常campaign/save party persistence、recipient input/scroll、no-recipient/full/success、sell/equip/transfer child panel E2；其他章節route/state |
