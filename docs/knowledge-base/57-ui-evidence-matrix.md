...
| UI-07 postbattle | `campInput` battle result 約 2394；campaign node 可表達 post node；`campaign_full` 30 戰 transition matrix 已逐列展開。主迴圈直接指令、scenario `chNN→map(N-1)` 與 handler `chNN_post→set_chapter(N+1)` 共同證實玩家戰鬥 N 使用 raw `ch(N-1)_post`。13個既有同號錯接已全數清除；連同較早修正的 ch14/15，以及本輪經 IDA 直接指令確認後啟用的 raw ch25→玩家ch26、raw ch27→玩家ch28，目前稽核為11 active／13 blocked／0 mapping complete。`0x25464` 的 raw ch27 handler 會跳到 `0x231df` 共用尾段（dialog→sync→chapter increment），不是天空之鑰成功分支；該成功分支只保留 raw ch26 已證實的 `sync_party→set_chapter(27)`。七個未綁定 postbattle 原有的泛用 inline beats 已移除，避免繞過 runtime guard。raw `ch15_post` 實屬玩家第16戰，IDA 證實 JOIN18 只在 `word42>=0x140` 巢狀分支，constructor 對 map15 形成76-slot入口；仍缺一般玩家原版 runtime capture、JOIN-time persistent record及town17/save回歸，故 `postbattle_ch16` 維持 unbound。IDA 另固定 `0x2CAD7` 子流程 raw 0=內部重複；直接整備／option2 的非零結果使 gate 回傳0，其餘 option 的非零結果使 gate 回傳1；目前只保存 raw outcome，不命名退出原因。位址證據見 [`fd2_post26_28_dispatch_ida.txt`](../data/fd2_post26_28_dispatch_ida.txt) | partial | 以原版 handler offset／DOSBox input 差分核對每章是否進 town/shop/rest/preparation/ending；ch06/ch07/ch16 等仍 fail-closed |
