...
| weapon/item shop | 70–75% | 69個原版shop節點已用variant 1/3/5啟用indexed owner；purchase從stable/menu/list一路到confirm、兩種recipient、滿欄、insert、optional equip、variant success與最後debit均接production。success總長10/9/14 BIOS ticks且portrait restore有別；raw equipped flags與compact view同步。剩餘為no-eligible production、sell/equip/transfer三service及DOSBox E2 |
...
| UI-09 shop | stable/menu、purchase rows/confirm/insufficient、兩種recipient、滿欄及完整purchase transaction均有original-resource regression及production owner；缺raw identity/inventory/equipment-base provenance fail-closed。`0x2f4c6` variants1/3/5按10/9/14 BIOS ticks播放，optional-equip後success、演出完成才debit並回product loop | partial | no-eligible message、sell/equip/transfer production owner與DOSBox E2 |
