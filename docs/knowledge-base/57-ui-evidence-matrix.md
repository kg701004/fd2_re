...
| weapon/item shop | 45–50% | 主選單、商品child panel、購買確認／不足金、兩種recipient及滿欄回饋均有原版資源indexed fixture。`0x2f4c6`三種shop success variant的entry/座標/wait/portrait restore亦已閉合，且debit固定在演出後。production仍是現代文字清單；成功timeline尚未接runtime owner，DOSBox E2 未接 |
...
| UI-09 shop | stable/menu、purchase rows/confirm/insufficient、兩種recipient與滿欄均有original-resource regression。`0x2f4c6` shop variants1/3/5分別為5/1/7幀、不同anchor/wait/portrait restore；optional-equip→success→debit→product-loop順序已保存 | partial | production timeline owner、secret shop flag、variant cancel semantics與DOSBox E2 |
