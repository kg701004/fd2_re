...
| UI-10 church | `0x2d7bd` 左右四項循環、Enter/Space confirm、Esc cancel；`0x3072f` raw dispatch `0→0x2ffa5`、`1→0x2f8ea`、`2→0x30dc3`、`3→0x31385`。class path `0x31385→0x31793→0x311DC→0x19953` 已閉合 state/input。`0x31019` 已接 FDICON、name/current class/FDTXT593/target class、palette201/205；FDOTHER#14 entry16=310×86，`0x1974c` opening 6 幀、`0x2d31b` closing 5 幀＋source restore均由 Draw acknowledgement 推進。`0x19953` FFFC=name substitution、FDOTHER#2 choice 4-open/4-close與BIOS pulse已接；其後 dialogue 5-close＋source restore才執行 mutation／返回。raw selector0 已接 `0x2e6b8/0x2ea90` 兩欄六人名冊及完整唯讀 `0x17aed`：status/items 12-open→key wait→有 command 時 bottom 7-close/7-open切 `0x1ceed` FDTXT/MP overlay→key wait→12-close/restore→重開名冊。`0x3072f` stable scene及transition/pulse已接runtime | partial/fail-closed | command effect/target、inventory flag export、FD2.SAV與DOSBox E2 visual diff |
