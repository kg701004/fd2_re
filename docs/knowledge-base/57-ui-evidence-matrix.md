...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的八格 selector亦已閉合為 compact兩欄四列：↑/↓linear wrap、←/→±4、battle-use拒絕effect type0；renderer geometry/icon/label ABI已釘。remake ring與item shell仍是partial approximation；`0x19953` 是另一 selector | partial | command/item indexed opening/closing renderer、item Enter transaction、end-turn entry、DOSBox visual diff |
