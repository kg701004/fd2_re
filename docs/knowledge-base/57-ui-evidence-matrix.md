...
| UI-12 save/load | F5/F9 global path；save package 自有 schema；原版 `FD2.SAV` file boundary=`0x59cb` bytes、4×logical `0x1450` records at `+0x312b`、each roster prefix=`0xa00`; `0x30550` 明確為 4-slot（0..3）、↑↓ bounded、Enter/Space confirm、Esc cancel；empty-slot oracle `docs/figures/load-empty-original-dosbox.png` 證實 1–4 四列框與第一列 cursor | partial | transform/record remainder、valid-record marker、delete/overwrite、successful-load restore、remake 四槽 model 與畫面差分 |
