...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、geometry/icon/label ABI與12-frame三區clip schedule已釘；official IDA閉合 `0x17eef/0x17fc0`，完整 indexed base+bar/digit/icon/FDTXT compositor已有玩家資產 oracle。remake ring與item shell仍未使用它；`0x19953` 是另一 selector | partial | indexed compositor→Ebiten與12-frame player、item Enter transaction、end-turn entry、DOSBox visual diff |
