...
| preparation | 35–45% | 舊「兩欄文字核取方塊」斷言已失效。`0x318ad/0x31e80` 的原版三區背景、二位數計數、10 欄角色格、游標、彩色／灰色繪法、四向輸入、游標角色 `0x17fc0` 完整狀態，以及 `0x1297d` 的可見 `0,1,2,1` 待機週期已接正式路徑。`0x31d3c` 已依 `0x1956b→0x19953→0x197e5→0x2d31b` 接上 6＋4＋穩定脈動＋4＋5＋還原的 Draw 確認生命週期；三張整備證據圖均為 E1 原始資源合成，不是 DOSBox 實機。仍缺跨畫面初始相位、晚期存檔與同狀態實機差分 |
