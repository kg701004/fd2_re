...
| UI-10 church | `0x2d7bd` 左右四項循環、Enter/Space confirm、Esc cancel；`0x3072f` raw dispatch `0→0x2ffa5`、`1→0x2f8ea`、`2→0x30dc3`、`3→0x31385`。class path `0x31385→0x31793→0x311DC→0x19953` 已閉合 state/input。`0x31019` 已接 FDICON、name/current class/FDTXT593/target class、palette201/205；FDOTHER#14 entry16=310×86，`0x1974c` opening 6 幀、`0x2d31b` closing 5 幀＋source restore均由 Draw acknowledgement 推進。`0x19953` FFFC=name substitution、FDOTHER#2 choice 4-open/4-close與BIOS pulse已接；其後 dialogue 5-close＋source restore才執行 mutation／返回。`0x3072f` exact mixed-codec stable scene（FDOTHER#5 raw dialogue grid＋four-mode digits、#14 entry1、DATO#131、FDTXT585/586）及`0x2d669/0x2d85f`四幀開關、source restore、兩-tick selected pair已接runtime並保存original-resource artifact | partial/fail-closed | raw index0 status/command overlay、inventory flag export、FD2.SAV與DOSBox E2 visual diff |
