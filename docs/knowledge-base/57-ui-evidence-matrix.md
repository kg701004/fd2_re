...
| weapon/item/secret shop | 90% | 69個shop節點已用variant 1/3/5啟用indexed owner；四項service production owner均已接。ch02 secret variant5四個service、Right/Left wrap及Escape→town selection5均達原版DOSBox與production remake整幀AE=0；E2修正service pulse雙重`/2`與返回selection0錯誤。仍缺variant1/3同狀態E2 |
...
| UI-09 shop | purchase、sell、standalone equip與transfer均有original-resource regression及production owner；strict adapters在raw projection不完整時fail-closed。secret gate保存23筆normal selection與Shift/Ctrl/Alt-F1..F10 BIOS scan；chord只揭露selection5，後續confirm才進variant5。ch02 variant5四個service、Right/Left wrap及Escape closing→town selection5均與同gold remake整幀AE=0；E2修正production double-`/2` pulse及返回selection0 bug | partial（E1 + ch02 variant5 menu/return E2） | variant1/3同狀態DOSBox diff；各service child panel E2 |
