...
| UI-03 action menu | Docker Capstone `0x18890` + `0x18d8c`：↑0 attack、←1 spell、→2 item、↓3 wait/field interaction；native command grid每欄四列。item branch `0x1bbdc` 的compact兩欄四列input、geometry/icon/label ABI與12-frame三區clip opening/closing schedule已釘。remake ring與item shell仍是partial approximation；`0x19953` 是另一 selector | partial | indexed source/buffer→Ebiten adapter、item Enter transaction、end-turn entry、DOSBox visual diff |
