...
| UI-07 postbattle | `campInput` battle result 約 2394；campaign node 可表達 post node；`campaign_full` 30 戰 transition matrix 已逐列展開。主迴圈直接指令、scenario `chNN→map(N-1)` 與 handler `chNN_post→set_chapter(N+1)` 共同證實玩家戰鬥 N 使用 raw `ch(N-1)_post`。13個既有同號錯接已全數清除：ch04/05/09/11/12/19/25 安全移接，ch08/10/13/18/24/29 撤回並失敗即關閉；連同較早修正的 ch14/15，目前稽核為9 active、13 blocked、2 mapping complete但未啟用。raw ch24 handler `0x24df2` 的 dialog/PAN/spawn/ACT75 現屬 `postbattle_ch25→town_ch26`；raw ch25 的 branch/layout/ACT77–80 應屬 `postbattle_ch26→town_ch27`，仍待 activation gate。raw `ch15_post` 實屬玩家第16戰，IDA 證實 JOIN18 只在 `word42>=0x140` 巢狀分支，constructor 對 map15 形成76-slot入口；仍缺一般玩家原版runtime capture、JOIN-time persistent record及town17/save回歸，故 `postbattle_ch16` 維持unbound。IDA 另固定 `0x2CAD7` 子流程 raw 0=內部重複；直接整備／option2 的非零結果使 gate 回傳0，其餘 option 的非零結果使 gate 回傳1；目前只保存 raw outcome，不命名退出原因 | partial | 以原版 handler offset／DOSBox input 差分核對每章是否進 town/shop/rest/preparation/ending；ch06/ch07/ch16 等仍 fail-closed |
