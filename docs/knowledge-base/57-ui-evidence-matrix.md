...
| preparation | 35–45% | 舊「兩欄文字核取方塊」斷言已失效。`0x318ad/0x31e80` 的原版三區背景、二位數計數、10 欄角色格、游標、彩色／灰色繪法、四向輸入、游標角色 `0x17fc0` 完整狀態，以及 `0x1297d` 的可見 `0,1,2,1` 待機週期已接正式路徑。`0x31d3c` 的 DATO #75 對話框、FDTXT `0x292`、`0x16559(0)` 肖像覆蓋順序與 FDOTHER #2 Yes／No 已成穩定 E1 合成；[`preparation-roster-compositor-partial.png`](../figures/preparation-roster-compositor-partial.png)／[`preparation-confirmation-compositor-partial.png`](../figures/preparation-confirmation-compositor-partial.png) 都不是 DOSBox 實機。仍缺確認開框／脈動／關框、跨畫面初始相位、晚期存檔與同狀態實機差分 |
...
| UI-11 preparation | `0x318ad` 每次進入清除 30 旗標；`0x31a7c..0x31b08` 固定左右 ±1、上下 ±10；`0x31e80` 固定 10 欄座標、FDOTHER #1 descriptor0 游標、FDICON pose0/cycle、選取 `0x4deda` 與未選 `0x4de56`。背景由 FDOTHER #5 第20項 223×86 放 `(92,7)`、第21項 310×99 放 `(5,94)`、第137項 86×86 經 `0x16886→0x4e63d` 放 `(5,7)`；計數用第31～40項。游標角色的同一 0x50 位元組記錄已交給既有 `RenderNativeItemPanelData` 重現 `0x17fc0`。`0x1297d` 的有號計時邊界與 `NativeFrameIndex` 3→1 已由共用介面接入。`0x31d3c` 穩定確認接 `0x1956b(0x4b)`、FDTXT `0x292` 於 `(95,119)`、`0x16559(0)` 最後覆蓋及 `0x19953` 的 Yes／No；原始圖像索引、記錄或資源缺值即退回 | partial（E1） | 補確認六幀開框、選項脈動、`0x197e5→0x2d31b` 關框；查明跨戰場／城鎮的行程全域初始相位；取得合法晚期存檔，以同一狀態做 DOSBox／重製像素差分。`0x1f42d` 已更正為戰場進入演出，不屬此選人視窗 |
