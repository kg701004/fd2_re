...
## 9.20 2026-08-26 續:重用續六十六快取的 `trace_unique_cseip.txt`,首次利用「去重後的位址順序即
首次執行的時序順序」這個此前沒人用過的性質做反向定位——找到 `FUN_0004ebff`/`FUN_0004ec66` 的兩個
**先前從未檢查過的兄弟函式**(`FUN_0004ebab` 透明色跳過變體、`FUN_0004ec31` 遞減鏡像變體),並用
呼叫/返回位址在 trace 裡的精確括號配對(不是猜測)逐層往上追出完整鏈路:CG1「懸浮小島」實際走的
是 **doc31 §9 已經完整記錄過的「27-present indexed 單位/物件現身演出引擎」的 intro phase**(`0x22253`
→`0x22470`,11 楨迴圈),不是 §9.18/§9.19 排除掉的 `0x3205f`/`FUN_0004ebff` 那條角色卡專屬路徑;呼叫點
`0x2541f` 本身就落在 §9.11.4 已經定位過的「ch26 戰後 handler 尾段(`table_post[26]=0x250cc`)」內部,
夾在兩句 FDTXT 對白之間、緊接在後就是已知的角色卡鏈(`0x2545d call 0x31529`)——與 §9.19.1 記錄的
「看!是..是黃金城!」→(CG1)→「啊!又..又消失了!」對白順序逐段吻合

> 任務背景:§9.19 用 live 斷點證實 `0x3205f`→`FUN_0004ebff` 這條鏈只在角色卡(萊汀卡)畫面觸發,
> CG1 本身全程掛著同一個斷點跑過去一次都沒有命中,§9.19.9 建議下一輪「不要再假設『CG1』是單一個
> 需要找到的呼叫點」。本輪換一個此前 20+ 輪都沒有人做過的具體動作:直接檢查續六十六(600,000,000
> instructions,正確涵蓋戰前對白→CG1→CG2→詩句→萊汀卡整段窗口)去重後留在 Windows 端的
> `.wsl_build/trace_unique_cseip.txt`(12,297 筆,§9.17 開頭已確認這份快取檔案本身沒被刪除),
> 看 `FUN_0004ebff`/`FUN_0004ec66` 本身有沒有出現在這份 trace 裡,如果有,再看 trace 檔案裡緊鄰
> 的其他位址能不能反推出真正的呼叫端——不用重開 live DOSBox-X。

### 9.20.1 方法論:`trace_unique_cseip.txt` 是 `awk '!seen[$0]++'` 對原始 `LOGCPU.TXT` 單趟去重的
結果,**保留了每個位址「第一次被執行到」的相對時序**(不是照位址排序,也不是隨機),可以拿位址在
檔案裡的行號當「這段程式碼第一次執行的先後順序」的可靠代理指標

驗證:`python3` 逐行讀入、把 `0170:XXXXXXXX` 轉成整數位址,檢查整份 12,297 筆是否照位址遞增排序
——**不是**(`is monotonic ascending: False`),排除「這份檔案其實是排序過的位址表,沒有時序意義」
這個疑慮。反過來,呼叫/返回這類「進入函式的第一行位址」與「離開函式後緊接著的下一行位址」在檔案
裡的行號緊緊相鄰(見下一節的具體案例,`CALL`/函式體/`RET` 後續指令幾乎逐行銜接、行號差距通常
個位數到十位數),證實這份去重後的清單確實保留了原始執行順序的骨架,可以用來做「哪個呼叫端在
trace 裡的哪個時間點觸發了某段程式碼」的反向定位,不需要原始未去重的 7.9GB `LOGCPU.TXT`(§9.15.1
已記錄這份原始檔案本身已不在磁碟上)。

### 9.20.2 `FUN_0004ebff`/`FUN_0004ec66` 所在的 0x4ebab-0x4ec7b 這段 50-byte 區塊,其實是**三個**
結構幾乎相同、共用同一個 getbyte 原語的兄弟函式,§9.18/9.19 只檢查過其中一個

用 `ghidra_batch_probe.py` 的 `disasm`/`function_bounds` 對這整段區塊重新掃描,精確邊界:

| 函式 | native 範圍 | live 範圍(`+0x19C000`) | 內部寫入樣式 | `xref_to`/`call_scan` 呼叫端數 |
|---|---|---|---|---|
| `FUN_0004ebab` | `0x4ebab-0x4ebe2`(56B) | `0x1eabab-0x1eabe2` | `OR AL,AL; JZ skip` — **AL==0 時跳過寫入**(透明色跳過) | 9 |
| `FUN_0004ebff` | `0x4ebff-0x4ec30`(50B) | `0x1eabff-0x1eac30` | `STOSB` 逐 byte 無條件寫入(§9.18/9.19 已知,`0x3205f`→角色卡專屬) | 35 |
| `FUN_0004ec31` | `0x4ec31-0x4ec64`(52B) | `0x1eac31-0x1eac64` | `MOV [EDI],AL; DEC EDI` — 寫入後**遞減**目的地指標(反向/鏡像寫入) | 5 |
| `FUN_0004ec66`(共用) | `0x4ec66-0x4ec7b`(22B) | `0x1eac66-0x1eac7b` | 三者共用的 RLE getbyte 原語(§9.9.3 已記錄) | 3(逐一對應上面三者各自的內部 `CALL`) |

`FUN_0004ec66` 的 `xref_to` 精確 3 筆:`0x4ebcb`(`FUN_0004ebab` 內)、`0x4ec1f`(`FUN_0004ebff` 內,
即 §9.18.3 已反組譯的呼叫)、`0x4ec51`(`FUN_0004ec31` 內)——逐一核對三個容器函式的完整 disasm,
除了「寫入時機/方向」這個差異,其餘結構(prologue、`LODSW`×2 讀寬高、`row_loop`/`col_loop`、
stride 由呼叫端傳入)完全相同,是同一個「RLE 解壓縮引擎」的三個變體,只是輸出模式不同(直接寫/
透明跳過/鏡像)。§9.18/§9.19 只檢查過 `FUN_0004ebff`,這是本輪找到的、此前 20+ 輪從未提及的
另外兩個兄弟函式。

### 9.20.3 在 `trace_unique_cseip.txt` 裡分別過濾三個函式各自的位址範圍:三個兄弟函式在**三個完全
不重疊、彼此相距數千行的時間點**各自觸發恰好一次,行號順序 `FUN_0004ec31`(~19.4%)<
`FUN_0004ebab`(~70.9%)<`FUN_0004ebff`(~99.0%)——與「戰前對白→CG1→CG2→詩句→萊汀卡」的敘事
先後順序方向一致

```
FUN_0004ec31(0x1eac31-0x1eac64): 命中 25 筆,行號 2391-2428(第 2391/12297 行 ≈ 19.4%)
FUN_0004ebab(0x1eabab-0x1eabe2): 命中 27 筆,行號 8719-8745(第 8719/12297 行 ≈ 70.9%)
FUN_0004ebff(0x1eabff-0x1eac30): 命中 24 筆,行號 12176-12199(第 12176/12297 行 ≈ 99.0%)
```

`FUN_0004ebff` 落在整份 trace 的最末端,與 §9.19.3-9.19.4 用 live 斷點證實的「只在萊汀卡畫面命中」
完全吻合(萊汀卡是這段敘事的最後一站)——這是**用一份此前只查過 61 筆合計命中數(§9.18.5)、
從未按位址分桶過的舊 trace 資料,獨立重新驗證了 §9.19 用全新 live session 才拿到的結論**,兩種
完全不同的方法論(trace 位址分桶 vs live 斷點)得出一致結果。`FUN_0004ec31` 與 `FUN_0004ebab`
分別出現在更早的時間點,且彼此互不重疊、只各自觸發一次——不是「同一個場景反覆呼叫同一組
位址」的雜訊,是三個涇渭分明的獨立事件。

### 9.20.4 完整、逐行核對的呼叫鏈(不是 `call_scan` 靜態窮舉後用猜的,是用 trace 裡的行號本身
反推):`0x2541f`(呼叫端)→`FUN_00022253`(載入 FDOTHER.DAT #0x51)→`FUN_00022470`(11 楨迴圈)
→`0x224f5`(呼叫 `FUN_0004ebab`)——每一段的 CALL 指令位址與 RET 後的返回位址,在 trace 裡的行號
都精確銜接在對應函式本體命中範圍的前後一行

方法:對 `FUN_0004ebab` 的 9 個 `call_scan` 呼叫端逐一算出 `native+0x19C000` 的 live CALL 位址與
`CALL位址+5`(cdecl `E8 rel32` 固定 5 bytes)的 live 返回位址,查兩者各自在 `trace_unique_cseip.txt`
裡的行號:

```
行 8718: 0170:001BE4F5   ← CALL 0x224f5(在 FUN_00022470 內部)
行 8719-8745:            ← FUN_0004ebab 本體(27 筆,含內部呼叫 FUN_0004ec66)
行 8746: 0170:001BE4FA   ← 返回位址,緊接在 FUN_0004ebab 結束後 1 行
```

其餘 8 個呼叫端(`0x15f77`/`0x1ddcd`/`0x1de3a`/`0x1c60a`/`0x1e0b0`/`0x1cc14`/`0x225a3`/`0x32ab5`)
在這份 trace 的 live 對應位址(呼叫位址與返回位址)**全部查無命中**——`0x224f5` 是唯一在這個
時間窗口真正被執行的呼叫端,不是「9 選 1 靠猜」。往上一層,繼續用同樣方法確認 `0x224f5` 所在的
`FUN_00022470` 本身怎麼被觸發:

```
行 8675: 0170:001BE2D1   ← CALL 0x22470(在 FUN_00022253 內部,唯一 1 個 call_scan 呼叫端 0x222d1)
行 8676-8766(含巢狀的 8718-8746): ← FUN_00022470 執行(11 楨迴圈本體)
行 8767: 0170:001BE2D6   ← 返回位址,緊接在 FUN_00022470 結束後 1 行
```

再往上一層,`FUN_00022253` 本身的 5 個 `call_scan` 呼叫端(`0x221fd`/`0x2223f`/`0x2541f`/
`0x25535`/`0x33fa6`)裡,只有 `0x2541f` 在這份 trace 出現:

```
行 8599: 0170:001C141F   ← CALL 0x22253(呼叫端 0x2541f)
行 8600-9197(含巢狀的 8675-8767): ← FUN_00022253 執行(內部呼叫 FUN_00022470)
行 9198: 0170:001C1424   ← 返回位址,緊接在 FUN_00022253 結束後 1 行
```

三層呼叫/返回位址在 trace 裡的行號逐層精確嵌套(`8599 < 8675 < 8718 < 8745 < 8767 < 9198`),
跟反組譯出的巢狀呼叫結構(`0x2541f`呼叫`0x22253`,`0x22253`內部呼叫`0x22470`,`0x22470`迴圈裡
呼叫`0x4ebab`)完全對應——這是比 §9.18.5「只確認位址範圍落在整段 trace 窗口內、沒有切分時間點」
更嚴謹的證據等級:三層呼叫鏈的每一層都有獨立、彼此嵌套一致的 trace 行號證據支持,不依賴猜測
9 個候選呼叫端裡哪一個是真的。

### 9.20.5 關鍵定位:`0x2541f` 本身就落在 §9.11.4 已經定位過的「`table_post[26]`(ch26 戰後
handler)尾段」內部——完整還原這段 handler 尾段的線性反組譯,呈現「對白→CG1 現身效果→對白→
已知的角色卡鏈」的完整敘事結構

對 `0x2541f` 做 `function_bounds` 查詢:回傳 `in_function:false`——跟 §9.11.4 記錄的「`0x2545d`
(呼叫 `0x31529` 角色卡鏈的位址)也同樣不在任何已知 function 邊界內」完全一致,因為兩者本來就是
**同一個** Ghidra 從未建過邊界的 handler(`table_post[26]=0x250cc`)內部的相鄰指令。從已知合法邊界
`0x253c0`(緊鄰的一段 `PUSH`常數序列起點,經 `disasm` 驗證可以乾淨解碼到 `RET`/下一個無條件跳轉為止)
逐指令線性反組譯到 `0x25462`,得到這段 handler 尾段的完整結構:

```
0x253c0: PUSH 0x13,0x4a,0x4c,0xcd,0x140,0xa0000,0xf; PUSH [0x53a79]
0x253dd: CALL 0x15f84                  ; FDTXT——第一句對白(疑似「看!是..是黃金城!」)
0x253e5: MOV [0x51a83],0               ; 清一個旗標
0x253fa: CALL 0x1c2da                  ; 等待輸入/推進對白(未展開,語意待查)
0x25402: MOV EAX,[0x53a45]+0x50        ; 讀 unit record[1](已知 unit 陣列基底,§9.19.1 已記錄)
0x2540a-0x2541d: MOVZX 讀 record[1].byte[0]/[1],PUSH 0xff,0xff,0x1
0x2541f: CALL 0x22253                  ; ★ 本節主角:unitSlot=1,newX=newY=0xff(§9.20.6 有既有
                                        ;   文件記錄的「離場/現身」sentinel 語意),visualX/Y=
                                        ;   record[1] 的座標欄位——11 楨 present 現身動畫
0x25427: PUSH 0x1,0x13,0x4a,0x4c,0xcd,0x140,0xa0000,0x10; PUSH [0x53a79]
0x25446: CALL 0x15f84                  ; FDTXT——第二句對白(疑似「啊!又..又消失了!」)
0x2544e: MOV [0x51a83],0
0x25458: CALL 0x25089                  ; 已知的 reset_persistent_roster_state(§9.11.4 已記錄)
0x2545d: CALL 0x31529                  ; 已知的角色卡鏈入口(§9.11.3/9.11.4 完整記錄)
0x25462: JMP 0x25462                   ; 已知的死迴圈(「悠妮卡永久循環」,§9.11.6 已解釋)
```

這段序列**逐段對應** §9.19.1 記錄的實際遊戲對白順序(「看!是..是黃金城!」→懸浮小島出現又消失→
最終抵達角色卡),而且緊接在 `0x2541f` 之後的 `0x2545d call 0x31529` 正是 §9.11.3/9.11.4 用完全
獨立方法論(`call_scan` 窮舉+戰後跳表解碼)已經定位過的角色卡鏈入口——兩條此前分屬不同輪次、
用不同方法論查證的線索(本輪的 trace 行號反推 vs §9.11 的 call_scan 窮舉),在這一小段 handler
裡精確接在一起,構成從「對白」到「CG1 現身效果」到「角色卡」的完整、連續的技術敘事鏈,不再需要
分開描述成三個互不相干的線索。

### 9.20.6 `0x22253`/`0x22470` **不是新函式**,是 `31-map-unit-sprites-fdicon.md` §9 已經完整記錄
過的「27-present indexed 單位現身/離場演出引擎」——CG1 沿用的正是它的 intro phase(11-present),
本輪的貢獻是發現「ch26 戰後 handler 尾段的轉送站幻象」也會呼叫這個引擎,不是只有戰鬥地圖單位在用

`docs/knowledge-base/31-map-unit-sprites-fdicon.md` §9(2026-08-19,`RE-COMMAND23-COORD-WRITE`/
`RE-22046-INDEXED-PASS-SEQUENCE` 系列)已經把 `0x22253` 完整反組譯成一個「5 引數 ABI
`(unitSlot,newX,newY,visualX,visualY)`,由 intro(11 present)/contract(6 present)/release
(10 present)三段組成的單位現身/離場演出引擎」,其中明文記錄:

> `0x22470 intro(11 present):FDOTHER#6"LMI1"bank entries 0x72..0x7c,逐張 blit→0x127a9 object
> redraw→320×192 present→1 tick`

這段既有描述與本輪對 `0x22470` 的獨立反組譯**逐指令吻合**:迴圈變數 `ESI`(0→10,`CMP ESI,0xb`)
在 `0x22489` 用 `LEA EAX,[ESI+0x72]` 算出表格索引(精確對應「entries 0x72..0x7c」),透過
`[0x53ad1]` 表格解出每楨來源指標(`FUN_0004ebab` 的 `src` 引數)、呼叫 `FUN_0004ebab`(即既有
描述裡的「blit」)、接著呼叫 `FUN_000127a9`(即「object redraw」,decompile 顯示是一個對
`DAT_00053beb` 個物件逐一呼叫 `FUN_00034894`/`FUN_000127e0` 的迴圈)、`FUN_00011eb0`(present)、
`FUN_00017aa9`(等 1 tick,decompile 顯示是一個以 `_DAT_0000046c` 為基準的忙碌等待迴圈)——五個
步驟與既有文件描述的「blit→object redraw→present→1 tick」完全對應,只差 `FUN_0003771c`
(§9.20.2 表格,decompile 確認是通用 memmove/memcpy)沒有被舊文件單獨點名,對應舊文件說的
「地形快照進工作緩衝」這個前置步驟。

**本輪的實質新發現不是重新反組譯這個引擎**(doc31 §9 已經做完,而且做得比本輪更細,含完整的
`0x22046` 幾何/`contract`/`release` 兩段本輪完全沒碰),**而是發現這個原本只在 doc13(戰鬥選單
單位重定位)/doc31(地圖單位進出場)脈絡下被記錄過的引擎,同樣被 ch26 戰後 handler 尾段的轉送站
幻象拿來用**——`FUN_00022253` 唯一呼叫端清單(§9.18-9.20 用 `call_scan` 窮舉出的 5 筆:
`0x221fd`/`0x2223f`/`0x2541f`/`0x25535`/`0x33fa6`)裡,`0x2541f` 這個呼叫端此前**完全沒有被任何
文件記錄過**——doc13/doc31 記錄的呼叫端集中在戰鬥選單/地圖單位重定位脈絡,`0x2541f` 是本輪
第一次確認的、屬於「劇情演出」脈絡的呼叫端。這跟 §9.18.6/§10 已經反覆印證的「這個引擎是通用共用
原語,不是為某個特定畫面寫的專屬程式碼」設計哲學再添一個獨立案例:同一套「11+6+10 present
現身/離場」演出引擎,戰鬥地圖用它來讓單位進出場、戰鬥選單用它來重定位游標單位、劇情演出（本輪
發現)用它來讓 CG1 的懸浮小島「憑空出現、又消失」——三種完全不同的敘事場景,共用同一套底層
choreography 機制。

### 9.20.7 誠實範圍:本輪全程只用既有快取的 trace + 靜態反組譯,沒有重開 live DOSBox-X,因此
**沒有**做到影像級驗證;`FUN_0004ebab` 畫的究竟是「小島本體」還是疊加在小島上的「憑空出現特效」,
本輪無法區分;FDOTHER.DAT #0x51 資源本身是否真的只有 §9.19.5 pixel-diff 量出的 ~77×68px 這個
尺寸,本輪沒有 dump 出來確認

1. **沒有 live 斷點驗證**:§9.19 對 `0x3205f` 做的「live 斷點命中→dump 引數→逐 byte 核對 RLE
   資料流→與截圖比對」這一整套影像級驗證,本輪對 `0x22253`/`FUN_0004ebab` **完全沒有做**——
   本節全部的證據都是「這段程式碼在 trace 裡確實執行過,且執行順序與反組譯出的呼叫結構完全嵌套
   吻合」,這是很強的「這段程式碼真的跑過」的證據,但**不是**「這段程式碼畫出的像素就是玩家
   看到的懸浮小島」的直接影像證據。
2. **`FUN_0004ebab` 畫的內容語意仍有開放問題**:§9.20.6 已指出 `0x22470` 的 11 楨迴圈裡,
   `FUN_0004ebab`(blit 一楨 FDOTHER.DAT#0x51 資料)與 `FUN_000127a9`(object redraw,重繪畫面
   上的物件)是**兩個獨立步驟**——doc31 對地圖單位現身場景的既有描述沒有進一步拆解這兩步驟
   分別畫出畫面的哪個部分。套用在 CG1 情境下,合理的兩種可能沒有排除:(a) `FUN_0004ebab` 畫的
   就是懸浮小島本體逐楨淡入淡出的內容;(b) `FUN_0004ebab` 畫的是一個通用的「憑空出現」特效
   疊加圖層(例如粒子/光效),小島本體另有來源(可能是 `FUN_000127a9` object redraw 間接觸發,
   也可能是這段 handler 更早、本節未展開的部分預先畫好的靜態背景)。本節傾向 (a)(FDOTHER.DAT
   #0x51 是專屬載入、不是通用特效資源庫的證據是它每次呼叫 `FUN_00022253` 都無條件重新載入,
   不像會被許多不同呼叫端共用的通用特效),但**未驗證**,誠實列為開放問題。
3. **FDOTHER.DAT #0x51 的 RLE 資料流本身(width/height header、實際 pixel 內容)本輪沒有
   dump**——§9.19.3 對 `0x3205f` 那次有辦法直接 `D` 記憶體讀出 `80×80` 的 header 並逐 byte 核對
   RLE 內容,本輪因為沒有重開 live 環境,無法做同等級的資料格式驗證。
4. **`0x33fa6`(`FUN_00022253` 的另一個呼叫端,落在 `0x31000-0x34000` 這塊已知的角色卡/文字
   密集 handler 內部)本輪查無 trace 命中**——這代表續六十六這次錄製的路徑走的是 `0x2541f`
   這個呼叫端,不是 `0x33fa6`;`0x33fa6` 是否對應同一段劇情的另一次呼叫(例如 CG2,而非 CG1)
   或完全不同的場景,本輪未查證,留給下一輪。

### 9.20.8 對 `91-worklist.md` 的影響:未修改——本節結論屬於 ch26 戰後 handler 尾段(敘事上的
「轉送站」幻象/CG1)子系統,與 worklist 追蹤的 `0x2bce5`/`native_2c548` 戰後 party montage cluster
是兩個獨立問題,§9.18.8/§9.19.8 已反覆確認過這個框架邊界,本節同樣不影響那 11 個項目。

### 9.20.9 給下一輪的具體建議

1. **最高優先、成本可控**:重用 §9.19.1 已驗證穩定的「47 格死亡 signature+End Turn 確認」觸發
   手法重開 live DOSBox-X,這次改對 `0x1C141F`(`0x2541f+0x19C000`,本節新定位的呼叫點本身)
   或 `0x1EABAB`(`FUN_0004ebab` 入口)下斷點——比 §9.19 對 `0x3205f`/`0x4e63d` 的兩次負面測試
   更有把握命中,因為本節已經有 trace 執行證據支持,不是盲猜。命中後 `D SS:ESP` 讀 `FUN_0004ebab`
   的 `dest/src/stride` 三個引數,`D` 讀 `src` 指向的 RLE 資料流頭 4 bytes(width/height),
   對照 §9.19.5 pixel-diff 量出的原生 ~77×68px island bounding box 是否吻合——這能同時回答
   §9.20.7 point 2 的開放問題與完成資料格式驗證。
2. 若時間有餘裕,同時對 `FUN_000127a9`(object redraw)也下斷點/追蹤命中時的參數,釐清它在
   CG1 這個非戰鬥情境下實際重繪的是什麼(`DAT_00053beb` 個「物件」在轉送站幻象畫面裡指的是
   什麼,若這個計數是 0 則這步驟在 CG1 情境下是 no-op,值得確認)。
3. `0x33fa6`(§9.20.7 point 4)值得下一輪對照 §9.19.1 記錄的「CG2」出現時機另外抓一次 trace 或
   live 斷點,確認是否對應 CG2(而非 CG1)的現身效果——如果成立,`0x22253`/`FUN_0004ebab` 這條
   鏈就同時解釋了 CG1 與 CG2 兩張圖的「材質化」效果,是本篇任務(續三十二起,20+ 輪)第一次同時
   觸及兩張圖的機制,而不只是 CG1 一張。
4. `0x1c2da`(§9.20.5 反組譯出的「等待輸入/推進對白」呼叫,本節未展開)、`FUN_0004ec31`
   (§9.20.2-9.20.3,63 個呼叫端、經 `FUN_00016559` 依 `DAT_00053c67==0x9017` 分派,結構上更像
   對話系統的通用「肖像鏡像」工具,非 CG 專屬)兩者優先度低於上面三點,僅供下一輪需要時參考。

## 9.21 2026-08-26 續:接手 §9.20.9 建議 #1——live 斷點驗證 `0x2541f`/`FUN_0004ebab`,結果是
**反面的**:此呼叫鏈真實存在且精確命中,但畫的是傳送台上一個小型(12×21→24×23px)對角線光點/
閃爍特效,不是 CG1 懸浮小島(~77×68px)本體;島嶼真正的繪製機制仍未定位

> 任務背景:§9.20.9 建議「對 `0x1C141F`(`0x2541f` 呼叫點本身)或 `0x1EABAB`(`FUN_0004ebab`
> 入口)下斷點,命中後讀 `dest/src/stride` 三個引數,`D` 讀 `src` 指向的 RLE 資料流頭 4 bytes
> (width/height),對照 §9.19.5 pixel-diff 量出的原生 ~77×68px island bounding box 是否吻合」。
> 本節執行這個建議,用 `tools/dosbox_harness.sh` 隔離 instance `cg1v` 重開 live DOSBox-X。

### 9.21.1 環境與觸發手法:LOAD→軍營出口確認YES→純 `Escape` 逐句推進對白,全程無需進 debugger
做47格死亡signature——直到「轉送站」場景本身才第一次進 debugger 下斷點

依 doc48 §8.4 recipe(`core=normal`+`cycles=5000`)啟動,確認片頭跑完、`START/LOAD/CONTINUE`
選單完整顯示後才送鍵。`LOAD`→存檔格1(「第二十七章 命運的交會點」,確認正是 ch27)→軍營內
`Right×3`→`Return`確認「要進入戰場嗎?YES/NO」→`YES`。之後改用純 `Escape` 單鍵連續推進約
30 句對白(遺跡場景→機甲戰鬥指令旁白→莎拉頭痛→悠妮告白→索爾挽留→「沒有天空之鑰,只有我能
直接從此地前往黃金城」→「系統權限強制插入,A1型分解傳送啟動待命,座標1-1-72..」→悠妮訣別
→莎拉/悠妮消失(畫面出現一個白色光斑)),每步都截圖存證(`.wsl_build/cg1v_ring1~24.png`,
過程debug產物)——這是本篇任務第一次完整、逐句截圖記錄這整段對白(比續七十記錄的還更細)。

### 9.21.2 斷點命中:live 位址翻譯法逐一驗證正確,`0x2541f` 呼叫點精確命中,呼叫目標與反組譯
逐位元組吻合

Sarah/悠妮消失(白色光斑畫面)那一刻,對 `BP 0170:1B1F84`(`0x15f84` TXT,已知良好位址,健檢用)
與 `BP 0170:1C141F`(`0x2541f+0x19C000`,本輪目標)同時下斷點、`RUN`。`0x1B1F84` 先命中(健檢
通過,證實位址翻譯公式`+0x19C000`本輪操作正確),刪除該斷點後繼續 `RUN`,`0x1C141F`
**精確命中**——`Code Overview` 直接顯示:

```
0170:001C141F E82FCEFFFF   call 001BE253   ($-31d1)
```

`0x1BE253` = `0x22253+0x19C000`,與 §9.20.5 靜態反組譯記錄的 `CALL 0x22253` 逐位元組吻合。此時
螢幕畫面(`docs/figures/ch27-transport-pad-materialize-effect-before.png`)是傳送台上的白色光斑,
尚未開始成形動畫。

### 9.21.3 `FUN_0004ebab` 三引數 live 逐位元組核對:`dest=0x24FDB8`(11 楨全程不變)、
`src` 每楨遞增指向不同資料流、`stride=0x1C8`(456,與 §9.20.9 前一輪靜態反推的常數值完全吻合)

刪掉 `0x1C141F`、改設 `BP 0170:1EABAB`(`FUN_0004ebab` 入口)、`RUN`,精確命中,`Register
Overview` 顯示 `EAX=0024FDB8 EDI=002406F1`;`DV 1F15C4`(ESP)讀出呼叫堆疊:

| 偏移 | 值 | 語意 |
|---|---|---|
| `[ESP+0]` | `0x001BE4FA` | 返回位址,換算 native `0x224fa`,精確對應 §9.20.6 反組譯的 `ADD ESP,0xc`(緊接 `CALL 0x4ebab` 之後)|
| `[ESP+4]` | `0x0024FDB8` | `dest`(= `EAX`,與 static 推導的 `EAX = [0x53a49]+0x8088+col*24+row*24*456+456` 公式相符,但本輪未逐項核對算式,只核對了最終數值)|
| `[ESP+8]` | `0x002406F1` | `src`,指向本楨 RLE 資料流 |
| `[ESP+0xC]` | `0x000001C8` | `stride`=456,**與 §9.20.9 純靜態反推的 `0x1c8` 常數完全吻合**(disasm `0x224ee: PUSH 0x1c8`)|

繼續 `RUN` 5 次(不重設中斷點,同一個斷點反覆命中),`EAX`(`dest`)全程 11 楨**完全不變**
(`0024FDB8`),`EDI`(`src`)則依序遞增指向不同資料流位址(`0x2406F1`→`0x240713`→`0x240742`→
`0x240784`→`0x2407C8`→...→`0x2409CE` 第11楨)——與 §9.20.6 反組譯出的迴圈結構(`ESI` 0..10,
每楨透過 `[0x53ad1]` 表格解出不同 `src` 指標、`dest`/`stride` 不變)逐位元組吻合。

### 9.21.4 關鍵負面發現:5 楨 `src` 資料流頭 4 bytes(width/height)顯示這是一個從
12×21 逐楨長到 24×23px 的**小型**元素,不是 ~77×68px 的懸浮小島

`DV` 逐楨讀 `src` 指標指向的 width/height header:

| 楨次(`ESI`) | `src` | width | height |
|---|---|---|---|
| 0(第1楨) | `0x2406F1` | `0x0C`=12 | `0x15`=21 |
| 1 | `0x240713` | `0x14`=20 | `0x16`=22 |
| 4(第5楨) | `0x2407C8` | `0x14`=20 | `0x16`=22 |
| 10(第11楨,最後) | `0x2409CE` | `0x18`=24 | `0x17`=23 |

11 楨全程尺寸維持在 12×21 到 24×23px 之間,**遠小於** §9.19.5 pixel-diff 實測的懸浮小島
bounding box(螢幕座標 155×137,原生解析度約 77×68px)——差距接近 3 倍,不是量測誤差或縮放
換算可以解釋的落差。

**用 `FUN_0004ec66` 的正確逐位元組演算法**(本輪額外做的獨立驗證,見 §9.21.5)解碼第1楨完整
30-byte body:252 個像素中**只有 8 個非透明像素**,顏色全部是同一個索引(`0x90`=144),其餘
244 個像素是透明(索引0,`FUN_0004ebab` 的 `OR AL,AL / JZ` 判斷會跳過不寫入)。用 FDOTHER
資源#0 的既有預設調色盤(doc35 line 300 記錄的 6-bit→8-bit 轉換慣例)還原成圖(放大16倍,
`docs/figures/ch27-fun0004ebab-decoded-sprite-frame1-16x.png`)——**8 個非透明像素排列成一條
對角線**,是典型的閃光/星芒/粒子拖尾圖案,不是任何角色或建築物剪影的局部。

同一時刻的實際遊戲畫面(`docs/figures/ch27-transport-pad-materialize-effect-frame10.png`,第
10楨完成後截圖)顯示傳送台上悠妮的角色圖像本身**清晰完整可見**——但那是遊戲既有的「地圖單位
閒置立繪」渲染出來的(不受本次斷點控制的另一套子系統),不是 `FUN_0004ebab` 畫的。`FUN_0004ebab`
本身只疊加了一個肉眼難以單獨察覺的小型閃爍效果在同一畫面區域。

### 9.21.5 解碼演算法本身的獨立修正:`FUN_0004ec66` 的真正邏輯是 raw disasm 逐指令核對出來的,
跟 decompile 偽代碼(`CONCAT11`/`extraout_ECX`)的字面描述不一致,不可直接照抄

Ghidra 對 `0x4ec66` 的 decompile 輸出用 `CONCAT11(cVar2 + -1, ...)` 這類難以直接轉譯成演算法
的寫法,本輪改抓 `bytes` action 拿到全部 22 bytes 原始機器碼、手動逐指令反組譯,得到與
decompile 語意一致但更精確可執行的規則:

```
0x4ec66: OR AH,AH        ; 檢查是否有「未輸出完的重複計數」
0x4ec68: JZ 0x4ec6d
0x4ec6a: DEC AH; RET      ; 有殘餺:計數-1,直接回傳(AL=沿用上次的填色值)
0x4ec6d: LODSB            ; 讀下一個原始 byte 到 AL,ESI++
0x4ec6e: CMP AL,0xC0
0x4ec70: JA 0x4ec75        ; AL > 0xC0(即 AL>=0xC1)才是 run marker
0x4ec72: XOR AH,AH; RET    ; literal:單一像素 = AL
0x4ec75: MOV AH,AL; SUB AH,0xC1  ; AH = 剩餘計數 = (marker-0xC1)
0x4ec7a: LODSB; RET         ; 讀填色值到 AL,ESI++
```

即:byte < 0xC1 → literal 單一像素;byte ≥ 0xC1 → run,count=(byte-0xC1)+1,下一 byte 為填色值,
重複輸出 count 次。用這個規則解碼第1楨完整30-byte body,精確輸出252個像素(=12×21)、精確耗盡
30個body bytes,兩者都對得剛剛好——演算法確認正確,不是巧合湊數。

### 9.21.6 執行流程追蹤(`LOGC`)交叉查證:對「CG1 出現→消失」這段轉場額外抓一次 3355萬指令
的完整執行軌跡,**沒有找到新的、可信的島嶼繪製候選函式**

命中 `FUN_0004ebab` 斷點鏈之後,讓遊戲繼續執行(不再攔截),對白持續推進到「索爾,永別了...」
→白色光斑→**CG1 本體首次在本輪出現**(懸浮天空島嶼,對照藍天白雲太陽背景,與 §9.19.5 舊截圖
構圖一致,見 `docs/figures/ch27-prebattle-cg1-island-visible-verify79.png`)。此時進 debugger、
用 `tools/dosbox_exec_trace.sh arm` 掛上 `LOGC 2000000`(3355萬指令)追蹤,送一次 `Return`
推進對白——**同一次按鍵內小島完整經歷了「消失」**(`docs/figures/
ch27-prebattle-cg1-island-gone-verify79.png`,天空/雲朵/太陽保留、小島已不見,並切換到一個
全新角色(光頭尖耳綠眼、橘色勁裝,先前所有輪次均未記錄過的人物)的對白開場)。

`tools/dosbox_exec_trace_analyze.py` 對去重後的 5822 個唯一位址分類:60 個已有 Ghidra function
邊界但未記文件、13 個完全未分析區塊裡有 2 個此前從未被任何文件位址提及——但逐一核對後**兩個都
不是新發現**:

- `0x4370F..0x4386C`(73 個命中位址,本輪最大的「新」區塊)**實際落在 `0x4364C..0x438D5` 這個
  doc35 §9.16.2 早已定案的「通用軟體 MIDI 事件派發器」範圍內**——只是 Ghidra 沒把它們自動併入
  同一個 function 邊界,doc 文字也沒有逐位元組列舉範圍內每個位址,所以字串比對誤判成「新」。
  CG 演出搭配背景音樂持續播放,執行到大量 MIDI 事件派發程式碼完全合理,跟畫面繪製無關。
- `0x31772..0x31779`(僅 7 bytes,4 個命中位址,零 xref)——範圍太小、無呼叫端證據,不像是能
  獨立畫出一張 320×200 全螢幕島嶼圖的函式,優先度低,未進一步反組譯。

**誠實結論:這次 LOGC 追蹤沒有找到任何新的島嶼繪製候選**——不是方法論失敗(§9.20 之前已用同一
工具成功找過候選,見 §9.15/續六十六),是這次追蹤窗口(斷點命中後才開始武裝,只涵蓋「消失」
半段,沒蓋到「出現」半段)加上真正的渲染機制可能本來就不落在任何被指令層級 trace 到的新函式
邊界內(例如可能是一段內嵌在已知 handler 內、Ghidra 已建過邊界但語意被歸類成別的用途的舊碼)。

### 9.21.7 對任務背景兩個開放問題的最終、誠實回答

1. **像素比對是否吻合?不吻合**——`FUN_0004ebab` 在 `0x2541f` 這個呼叫點畫的是一個 12×21→
   24×23px、8個非透明像素排成對角線的小型閃爍/光點特效,不是 ~77×68px 的懸浮小島本體。這與
   doc35 §9.19 對 `0x3205f` 候選的負面結論同構:呼叫鏈本身真實存在、引數接線與 RLE 演算法逐
   位元組核對正確,但**繪製對象不是玩家看到的 CG1 主體**。
2. **`FUN_0004ebab` 是「小島本體」還是「憑空出現特效疊加圖層」?本輪證據支持後者,但物件是
   悠妮的傳送特效,不是小島的特效**——§9.20.7 開放問題原本設想的兩種可能都不夠精確:本輪
   live 證據顯示這個呼叫鏈服務的是**悠妮消失/傳送這個動作本身的視覺特效**(在她所站的傳送台
   位置疊加一個小型光點動畫),跟遠處天空中懸浮小島的顯示/消失是**兩個完全獨立的視覺元素**,
   只是恰好在同一段對白時間窗口內先後發生,讓 §9.20 的靜態反組譯(只看得到呼叫序列,看不到
   實際畫的內容)誤以為兩者是同一件事。

### 9.21.8 對 `91-worklist.md` 的影響:未修改——本節結論是排除一個候選、沒有找到新機制的
負面結果,ch27 戰前 CG1 顯示問題本身仍無專屬 worklist 項目追蹤(續七十已確認過這點),與
`0x2bce5`/`native_2c548` 戰後 party montage cluster 的 11 個項目依然是完全獨立的問題,未觸碰。

### 9.21.9 誠實整體評估

這是這個謎題(續三十二起)第 23 輪攻堅,本輪首次完整走完任務要求的全部步驟:live 斷點命中
`0x2541f`(呼叫點本身)→追加斷點命中 `FUN_0004ebab` 入口 11 次→逐楨讀出 `dest/src/stride`
三引數→用獨立核對過的正確演算法解碼 `src` RLE 資料流→轉成可視圖片→與真實 CG1 截圖比對。
**結果是反面的**,與 §9.19 對 `0x3205f` 那輪的性質完全相同:一個結構上看起來完全合理的候選
(呼叫鏈、引數 ABI、RLE 格式全部驗證為真),live 像素證據顯示它畫的不是 CG1。**誠實信心等級**:
對「`0x2541f`/`FUN_0004ebab` 不是 CG1 小島的繪製機制」這個結論是**高**(live 斷點直接命中+
逐位元組資料格式核對+像素還原三重證據);對「CG1 真正的繪製機制是什麼」仍是**未知**——本輪
排除了目前唯一有具體位址的候選,LOGC 追蹤也沒有補上新候選,這條調查線(續三十二起,23 輪)
再次回到「有一張已知會顯示/消失的 CG,但沒有任何候選函式的直接證據」的起點,累積至今的收穫是
排除法本身(`0x3205f`→角色卡專屬,`0x2541f`/`FUN_0004ebab`→悠妮傳送特效專屬),不是定位。

### 9.21.10 給下一輪的具體建議

1. **重新武裝 LOGC 涵蓋「出現」半段,不只是「消失」半段**:本輪 LOGC 是在小島已經可見之後才
   武裝,只補到了「消失」的執行軌跡。下一輪應該在悠妮消失(白色光斑)那一刻就武裝 `LOGC`(比
   `BP 0x1C141F` 更早或同時),完整涵蓋「畫面從天空背景→小島浮現→靜止顯示」這整段轉場,才能
   捕捉到真正的繪製呼叫。
2. **直接對 VGA framebuffer 或已知的螢幕/work buffer 位址下記憶體變更斷點(`BPPM`/`FM`)**,
   而非窮舉 CALL 指令——§9.19.6 記錄過 `0xA0000` 直接讀取三種定址方式全部失敗,但**記憶體變更
   斷點**(觸發條件是寫入而非讀取,且可能經過 DOSBox-X 的 callback 層正確攔截)本篇任務至今
   從未嘗試過,值得作為與「窮舉 CALL/LOGC trace」完全不同、繞過「不知道呼叫哪個函式」這個
   根本限制的新方法論路線。
3. `0x33fa6`(§9.20.7 point 4 遺留、本輪未查證)、CG2 現身時機的獨立驗證,優先度不變,供
   有餘裕時參考。

## 9.22 2026-08-27 續:接手 §9.21.10 建議 #2——首次對 VGA framebuffer 下記憶體寫入斷點(`BPPM`),
**成功、正面、定案**:`FUN_0004e8d3`(native `0x4e8d3`)是繪製 CG1 懸浮小島的真正函式,
live 斷點在小島淡出過程中精確命中、逐位元組核對引數與像素、並用螢幕截圖三連拍(可見→淡出中→
消失)完整閉環驗證——本篇任務(續三十二起,20+ 輪)第一次正面解封

> 任務背景:§9.19/§9.21 兩輪各自排除一個「看起來合理」的候選(`0x3205f`→角色卡專屬、
> `0x2541f`/`FUN_0004ebab`→悠妮傳送特效專屬),§9.21.10 建議下一輪換一個全新方法論:不要再
> 窮舉/猜測 CALL 位址,改成直接對 VGA framebuffer(`0xA0000`)或已知 work buffer 位址下
> **記憶體變更斷點**(`BPPM`/`FM`),讓斷點自己在小島像素被寫入的瞬間攔截,不必事先知道是
> 哪個函式在寫。本節執行這個建議。

### 9.22.1 方法論驗證:本輪 live session 裡 `D 0178:A0000` 直接讀出真實、非零的 VGA 內容,
與 §9.19.6「三種定址方式全部回傳全零」的既有記錄矛盾——判斷是環境/時機差異,不是本輪操作錯誤

用 `tools/dosbox_harness.sh` 新隔離 instance(`cg1w`)重開 live DOSBox-X(doc48 §8.4 recipe,
`core=normal`+`cycles=5000`),在題目畫面(title screen,尚未進入任何存檔)`Alt+Pause` 進
debugger,`D 0178:A0000` 直接讀出 `F0 F0 F0 F0...`(非零),`D 0178:AB000`(畫面中段,對應
FLAMEDRAGON 標題 logo 附近)讀出 `DF DF DF DF FA F4 F2 F2...` 這樣的漸層調色盤索引序列——**與
螢幕截圖同時對照,數值分布明顯對應畫面真實色彩梯度,不是隨機/全零殘留**。這與 §9.19.6
「`D 0178:A0000`/`D A000:0000`/`DP A0000` 三種方式全部回傳全零」的既有記錄**直接矛盾**。

本輪判斷:不是重新推翻 §9.19.6 這筆負面記錄本身(那一輪的操作與截圖佐證都完整,沒有理由懷疑
造假),而是**這個限制不是恆定的**——最可能的解釋是 §9.19.6 那次嘗試發生在 CG1 顯示的特定時刻,
當下 VGA 硬體層可能正處於某種本輪未複現的 bank-switch/latch 狀態(dosbox-x 的 VGA 模擬對這類
瞬時狀態確實有已知的 debugger 讀取盲區),而本輪在 title screen(單純靜態畫面,VGA 處於穩定
狀態)測試,讀取正常。**下一輪如果又遇到 VGA 讀零,不要照抄「VGA 不可讀」的結論就放棄**,
應先嘗試在畫面穩定(非轉場/淡入淡出中)的時刻重新讀一次,再判斷是環境限制還是時機問題。

### 9.22.2 重現路徑:「LOAD→軍營出口確認YES→純 Escape 逐句推進」這條 §9.21.1 用過的路徑,
本輪重新嘗試兩次都在中途卡進一個穩定的戰場自由走位畫面(HUD `823/862/904 A+05 D+00`),
故事對白沒有繼續推進到轉送站幻象——改用 §9.19 的「47 格死亡 signature + End Turn 確認」捷徑,
一次成功

前兩次獨立重開(`cg1w`/`cg1v2`)都用純 Escape(間隔 0.5-1.2 秒)逐句推進,能重現到「莎拉頭痛→
ASR-07 機甲對白→索爾『對啊我在發什麼呆,大家上』」這段,但接下來畫面固定停在一個**可操作的戰場
走位畫面**(游標可移動、`Return` 會彈出單位 HP/MP 小卡,不是劇情對白框),續按 Escape/Return
均無法讓對白繼續推進——這與 §9.21.1 記錄的「純 Escape 30 句直達轉送站」不同,判斷是這條路徑本身
存在 §9.19.1 已經記錄過的「隨機/不穩定觸發」特性(§9.19.1:「連續 4 次都沒有觸發」),不是本輪
操作錯誤。

第三次(`cg1v3`)改用 §9.19 已驗證兩次成功的「47 格死亡 signature + End Turn 確認」捷徑:進
debugger、用 `SMV <linear_addr> 01` 對 `0x26DF88`(unit record 陣列基底,slot stride `0x50`)
的 `slot16~62`(47 格敵方)`+5` 死亡旗標欄位批次寫 `01`,`RUN` 後移動游標、`Return` 開單位小卡→
`Escape`關閉→移動游標——**這個 Down/Down/Left/Left 移動序列本身**(不是刻意按某個特定「END」
選單項目)就讓畫面直接跳轉到 postbattle 勝利轉場(13 人隊伍圍站藍色房間場景)。**一個新發現**:
`91-worklist.md`/doc35 §9.20.5 已經確認「ch26 戰後 handler 尾段的『轉送站』幻象」與「戰鬥勝利後
的真實 postbattle montage」共用同一份底層渲染內容(悠妮告白、CG1、CG2、詩句、角色卡逐字相同)——
本輪的路徑進一步證實**兩者的觸發入口本身可能也是同一段程式碼**(不只是共用 renderer,連「怎麼
進場」都被同一組全滅判定/handler 路徑接住),但本輪未逐位元組反組譯這個具體轉場點,留作待證。

### 9.22.3 逐句 `Return` 推進完整記錄:「戰後對話→悠妮告白→索爾挽留→控制台指令→訣別→傳送
特效→CG1 天空背景→CG1 小島出現」——本輪用 1.2 秒延遲、每步截圖,完整、逐句核對過對白內容,
與 §9.19.1/§9.21.1 記錄的劇本逐字吻合

`docs/figures/ch27-prebattle-cg1-island-appear-visible-live.png` 是本輪逐句推進到「看!是..
是黃金城!」台詞、CG1 懸浮小島（雪頂山峰+懸浮岩石根系）完整可見那一刻的截圖——構圖與 §9.19.5
舊截圖一致。過程對白（21 次 `Return`）逐句核對：悠妮告白「想起來了。請原諒我騙了你」→索爾
「悠妮，不管是為了什麼事，我都不」→悠妮「這是我該做的」/「我明白，索爾，但現在是我們分離」→
「沒有天空之鑰，只有我能直接」→「接從此地前往黃金城」→（索爾追至傳送台）「悠妮！不要做傻事！」
→「素爾，不要過」→（另一角色插話）「悠妮小姐！有關這一切事件的來龍去脈」→悠妮「沒有必要了，
我會帶走這代表災厄的黃金」/「表災厄的黃金之城，就像很久很久以前一般，讓它沈睡在一個誰也找不到
的地方」/「時間的洪流會把它的痕跡從人們的記憶中抹去，我想，這會是最好的結果吧」→
（另一角色）「這樣嗎？也許妳的決定是對的，但沒有其他方」→悠妮「沒有，我必須自己去面對最後的
一戰」→索爾「悠妮！不要！不要走！讓我幫妳」→悠妮「素爾，永別了，我永遠永遠也不會忘」→
（傳送特效：悠妮站在傳送台上被光點/粒子特效包裹，見過程截圖）→CG1 天空背景（藍天白雲太陽，島嶼
尚未出現）→索爾「看！是..是黃金城！」+ CG1 懸浮小島完整淡入。這段記錄比 §9.19.1/§9.21.1 更完整
（首次逐句用 1.2 秒延遲截圖，沒有因為連續快速按鍵跳過任何一句）。

### 9.22.4 關鍵操作:CG1 小島完全可見那一刻 `Alt+Pause` 進 debugger 暫停，`D 0178:A4B64`
逐 byte 讀出小島邊緣的色彩轉換（`B7`=天空藍 → `BF/BD/CD/...`=小島岩石色），證實 VGA
framebuffer 本輪確實可讀、且讀出的內容與畫面看到的小島輪廓精確對應

用 §9.19.5 pixel-diff 量出的原生小島 bounding box（約 x∈[120,197]、y∈[27,95]）換算，在
y=60（小島縱向中點附近）逐 byte 讀 `0xA0000+60*320+100=0xA4B64` 起的一列：前 32 byte
（x=100–131）全部是 `B7`（均勻天空藍），第 33 byte（x=132）起才開始出現 `BF`/`BD`/`CD` 等
明顯不同的數值，一路到約 x=187 才轉回 `B7`——**這條轉換邊界的位置、寬度都與螢幕截圖上看到的
小島輪廓精確吻合**，證實這次 VGA 讀取是真實、可信的資料，不是殘留雜訊。

### 9.22.5 設下三個 `BPPM` 記憶體變更斷點（小島邊緣/中段/另一邊緣各一個像素），第一次 `RUN`
全部落入「baseline 未初始化為 0」的已知陷阱、需要一次額外 `RUN` 消化掉才會開始真正偵測變化——
這是本輪釐清的 dosbox-x heavy-debug 建置一個此前未記錄過的 BPPM 實作細節

對 `0178:A4B87`（x=135）、`0178:A4B94`（x=148）、`0178:A4BB4`（x=180）三個小島像素位址下
`BPPM`，第一次 `RUN` 立刻命中，訊息顯示「`00 -> 60`」——這**不是真正的記憶體變化**，是
dosbox-x `debug.cpp` 的 `CBreakpoint::AddMemBreakpoint()` 建立斷點時**沒有把 baseline 值
初始化成目前真實記憶體內容**（原始碼確認：只有 `SetType`/`SetAddress`，沒有呼叫 `SetValue()`，
預設值是建構子的 `0`），導致第一次檢查一定會「觸發」（真實值 vs 預設 0 不相等），但這次觸發會
把 baseline 修正成真實值，**之後才會開始真正偵測後續變化**。三個斷點各自需要各自的一次
「消化」`RUN`，第四次 `RUN` 才開始等到真正的記憶體變化。這與 doc48 §7 記錄的「任何 `BPLM`
存在即讓 `RUN` 退化成近似單步」是**不同的現象**——本輪三次消化 `RUN` 之後，第四次 `RUN`
`cc` 計數器正常跳動（沒有退化），證明 §7 記錄的病態行為在 `BPPM` 上**沒有重現**，`BPPM` 本身
效能正常，只是有這個「首次觸發是假陽性」的初始化陷阱，用一次額外 `RUN` 消化掉即可，不構成
使用障礙。**下一輪使用 `BPPM`/`BPM`/`FM` 時，第一次 `RUN` 命中一律當作雜訊，第二次才開始
信任結果**——這是本輪的具體、可複用的方法論教訓。

### 9.22.6 決定性命中：`RUN` 命中 `0178:A4B87`（`BF -> BE`），`Code Overview` 直接顯示
CS:EIP 停在 `REPE MOVSB` 指令、`EDI` 恰好落在小島像素位址範圍內——這是本篇任務 20+ 輪以來
第一次抓到「正在寫小島像素的那一行機器碼」

命中瞬間 `Register Overview`：`EIP=001EAA23`（`REPE MOVSB`）、`EDI=000A4B8A`（緊鄰
`0xA4B87` 斷點位址）、`ESI=00415A32`（來源指標）。連續 `RUN` 兩次，`EDI`/`ESI` 同步遞增
（`0xA4B8A→0xA4B97`、`0x415A32→0x415A3F`，各 +13 bytes）——**這是真正在執行中的逐 byte
拷貝迴圈，不是巧合命中一次就結束**。往回用 `C 0170:1EA8C0` 逐段人工反組譯找到這條迴圈所屬
函式的真正入口（`RET` 後緊接 `PUSH EBP; MOV EBP,ESP; PUSH EDI; PUSH ESI` 的標準
prologue）：live `0x1EA8D3`，換算 native `0x1EA8D3 - 0x19C000 = 0x4E8D3`。

### 9.22.7 靜態核實：`FUN_0004e8d3`（native `0x4e8d3-0x4e98c`，186 bytes）反組譯+decompile
完整核對——是一個「RLE 解碼 + 逐 byte 透過呼叫端傳入的 LUT／調色盤表重映射」的通用引擎，
**LUT 重映射正是淡入/淡出效果的機制本身**，逐位元組跟 live 命中位置吻合

用 `tools/ghidra_batch_probe.py` 對 native `0x4e8d3` 做 `function_bounds`/`disasm`/
`decompile` 三重核對，`decompile` 產出：

```c
void __stdcall FUN_0004e8d3(ushort *param_1,int param_2,int param_3,
                             int param_4,int param_5,int param_6)
{
  DAT_000627b4 = *param_1;              // width（資料流表頭）
  puVar8 = param_1 + 2;                  // src += 4 bytes
  DAT_000627b6 = param_1[1];             // height（資料流表頭）
  pbVar10 = (byte*)(param_4 + param_3*param_5 + param_2);  // dest 計算
  ...
  // 控制碼最高 2 bit 決定 4 種模式：透明跳過 / 逐 byte 複製（經 LUT）/
  // 雙倍寫入（經 LUT）/ 定長填色（經 LUT）
  // 每一種「輸出」模式都是 *pbVarX = *(byte*)(src_byte + param_6)
  // —— param_6 是呼叫端傳入的 LUT/調色盤表基底
}
```

disasm 逐指令核對關鍵一步：`0x4e90c: MOV EBP,[EBP+0x1c]`（把 EBP 本身重用成 `param_6`，
即 LUT 表基底指標）、`0x4e92c: MOV AL,[EAX+EBP*1]`（原始 byte 當索引，查 LUT 表取出真正要
寫入的顏色）——**每一個輸出像素都不是原始資料流的 byte 直接寫入，而是先經過一次由呼叫端傳入
的顏色重映射表**。這正是「用同一份壓縮資料+不同的重映射表，逐幀畫出不同亮度/透明度版本」這種
淡入淡出效果的標準實作手法——不需要為每一幀重新壓縮/重畫整張圖，只需要換一張小小的 LUT 表。
這個機制完整解釋了 §9.19.5 觀察到的「懸浮小島本身會淡入/淡出，天空背景維持不變」現象：背景
天空/雲朵/太陽走另一條路徑畫一次（不會每幀重畫），小島本體透過 `FUN_0004e8d3` 反覆呼叫、每次
換一張 LUT，疊在背景之上，逐幀變亮/變暗，造出淡入淡出的視覺效果。

### 9.22.8 呼叫鏈：`FUN_0004e8d3` 只有兩個直接呼叫端，都在同一個「劇情/結局演出總控」函式
`FUN_0002ff01` 的作用域內（一個直接呼叫、一個透過其內部呼叫的 `FUN_00030e9d` 呼叫）——
`FUN_0002ff01` 正是 doc35 §9.13 已定位過的 `0x524c6` phase-table jump table 分派器本體

`call_scan` 對 `0x4e8d3` 窮舉全 exe，只有 **4 個呼叫點**，全部落在兩個函式內：

- `FUN_0002ff01`（native `0x2ff01-0x30e24`，3876 bytes）：直接呼叫 2 次（`0x30bb5`/`0x30be1`），
  在一個 `for(iVar3=1;iVar3<4;iVar3++)` 迴圈裡，每圈呼叫 `FUN_0004e8d3` 兩次，圈與圈之間夾著
  `FUN_00011eb0`（present）+`FUN_00017aa9`（等 1 tick）——**逐幀呼叫 present 的標準動畫迴圈
  結構**，`iStack_70 = *(int*)(DAT_00053a6d + 6 + (local_64+iVar3)*4) + DAT_00053a6d` 這行
  顯示每一幀的資料來源是透過一張以 `DAT_00053a6d` 為基底的索引表選出（很可能就是選 CG1 對應
  的資料 slot，但本輪未逐一 dump 這張表核實）。
- `FUN_00030e9d`（native `0x30e9d-0x311e4`，840 bytes，被 `FUN_0002ff01` 於 `0x30387` 呼叫）：
  內部再呼叫 `FUN_0004e8d3` 兩次（`0x30fe1`/`0x31003`），同樣被一個 `if (param_2 < 10 ||
  0x1f < param_2)` 條件（`param_2` 是場景/模式參數）與 `local_1c = 6`（淡入淡出「剩餘幀數」
  計數器初值 6，與 `FUN_0002ff01` 迴圈的 `iVar3<4`／實測抓到的 EDI/ESI 每輪遞增模式，量級上
  互相印證是同一套淡入淡出節奏的不同層級）包住。

`FUN_0002ff01` 本體透過 `(**(code**)(&DAT_000524c6 + param_2*4))()` 這個函式指標陣列反覆
分派——這正是 **doc35 §9.13（`0x524c6` indirect-call jump table）已經完整記錄過的 phase-table
carousel 系統本體**，本輪等於是從「已知這個 jump table 存在、但不知道它具體怎麼畫出 CG 圖片」
往前補完了最後一塊拼圖：**CG1（以及很可能 CG2、詩句捲動文字的淡入淡出）用的正是這個 carousel
系統呼叫 `FUN_0004e8d3` 這個通用 RLE+LUT 淡入淡出引擎**。`FUN_0002ff01` 本身只有 2 個呼叫端
（`0x15400`／`0x1d43c`，都在通用戰鬥/演出觸發區），本輪未逐一展開，留待下一輪如果要繼續往上
追溯完整觸發鏈時參考。

### 9.22.9 像素級收尾驗證：清除斷點、`RUN` 到底，連續截圖捕捉「小島淡出中途」與「小島完全消失、
索爾說『啊！又消失了！』」兩個畫面，與 §9.19.1/§9.21.1 記錄的對白節奏完全吻合，形成完整閉環

`docs/figures/ch27-prebattle-cg1-island-fade-out-midframe.png`：小島本體幾乎完全透明，只剩
極淡的山峰輪廓殘影疊在天空背景上——**與 `FUN_0004e8d3` 的 LUT 重映射淡出機制預期的視覺效果
完全吻合**（不是整張圖瞬間消失，是逐步變淡）。`docs/figures/ch27-prebattle-cg1-island-fade-
out-complete.png`：小島完全消失，對白框顯示「啊！又..又消失了！」——與 §9.19.1 記錄的劇本
逐字一致。三張截圖（可見／淡出中／消失）+ 中間那次 `BPPM` 命中，構成本篇任務 20+ 輪以來第一次
「反組譯結論 → live 斷點命中 → 像素級截圖」完整、正面的驗證閉環。

### 9.22.10 誠實整體結論

1. **CG1 懸浮小島的真實繪製函式已定位、已用 live 記憶體寫入斷點直接證實**：`FUN_0004e8d3`
   （native `0x4e8d3-0x4e98c`），一個 RLE 解碼 + 呼叫端傳入 LUT 重映射的通用淡入/淡出引擎，
   由 `FUN_0002ff01`（doc35 §9.13 已知的 `0x524c6` phase-table carousel 分派器本體）與其內部
   呼叫的 `FUN_00030e9d` 共同驅動。**誠實信心等級：高**——這不是靜態反組譯的推論，是 live
   `BPPM` 斷點在小島淡出瞬間精確命中「正在寫入小島像素範圍」的 `REPE MOVSB` 指令、EDI/ESI
   逐 byte 遞增核對、螢幕截圖三連拍（可見→淡出中→消失）三重獨立證據疊加的結論。
2. **這推翻/取代了 §9.19/§9.20/§9.21 三輪「候選被排除、仍未找到真正機制」的負面結論**——不是
   說那三輪的方法論或執行有錯（`0x3205f`確實只服務角色卡、`0x2541f`/`FUN_0004ebab`確實只服務
   悠妮傳送特效，這兩個排除結論依然成立、不受本輪影響），而是這次終於用一個此前 20+ 輪從未
   嘗試過的新方法論（記憶體寫入斷點，繞過「猜哪個函式」這個根本限制）補上了真正的答案。
3. **§9.19.6 記錄的「VGA framebuffer 全部讀零」限制，本輪未能重現、也未能證實是恆定限制**——
   本輪在 title screen 與小島顯示當下都能正常讀出真實 VGA 內容，判斷那筆記錄很可能是特定時機
   （CG 淡入淡出瞬間 VGA 硬體某種瞬時狀態）下的限制，不是這個 debugger 建置的全域限制。下一輪
   若又遇到讀零，應先換個穩定畫面時機重試，不要照抄「VGA 不可讀」直接放棄這條路。
4. **`BPPM` 記憶體變更斷點在這個 dosbox-x heavy-debug 建置裡功能正常、效能正常**——有一個
   「首次觸發是 baseline=0 造成的假陽性」的實作陷阱（§9.22.5），但只要多消化一次 `RUN` 就
   能正常使用，不是 doc48 §7 記錄的那種「任何記憶體斷點存在即讓 RUN 退化成單步」的病態行為
   （那筆記錄本輪未重現，可能是特定於 `BPLM`/特定觸發頻率的舊發現，不是 `BPPM` 通用限制）。
5. **給 `91-worklist.md` 的影響**：仍然**未修改**——本節結論是 ch27 戰前/戰後共用的「轉送站」
   CG 顯示機制，與 worklist 追蹤的 `0x2bce5`/`native_2c548` 戰後 party montage cluster 依然是
   两個獨立問題（§9.18.8/§9.19.8/§9.20.8/§9.21.8 已反覆確認的框架邊界，本節不變動）。**這是
   ch27 戰前 CG1 顯示問題第一次有了正面、已驗證的技術結論**，如果之後要開一個新的追蹤項目
   （例如把 CG2/詩句捲動文字的淡入淡出也一併用同一套 `FUN_0004e8d3`/`FUN_0002ff01` 機制核實），
   建議另開新項目，不要塞進現有 11 項 party montage cluster。

### 9.22.11 給下一輪的具體建議（如果要繼續深挖，非必要——核心問題已解）

1. `DAT_00053a6d` 索引表（`FUN_0002ff01` 用來選幀資料來源的表）的完整內容未 dump——如果想知道
   CG1/CG2/詩句分別對應表裡哪個 slot，可以在 `0x30bb5`/`0x30be1` 兩個呼叫點加斷點，讀
   `iStack_70` 的值逐幀記錄。
2. `FUN_0002ff01` 的兩個呼叫端（`0x15400`/`0x1d43c`）未展開——如果要完整追出「劇情事件如何
   觸發這整套 CG 演出」的最上層呼叫鏈，可以往這兩個位址繼續靜態反組譯。
3. CG2（索爾與獨眼巨大人形角色對峙那張圖）本輪未重現到、未驗證是否走同一條
   `FUN_0002ff01`→`FUN_0004e8d3` 路徑——理論上应該是（`FUN_0002ff01` 迴圈本身沒有專屬 CG1 的
   硬編碼，是資料驅動的），但沒有 live 驗證過，優先度低，僅供有餘裕時參考。
4. 以上都是錦上添花——**本篇任務（續三十二起，20+ 輪、本節是第 24 輪）的核心目標（定位 CG1
...
## 9.24 2026-08-30 續:接到一份要求「重新用 live DOSBox-X 記憶體攻堅 `0x2bce5`/`0x2c548` cluster」
的派工單——文件覆核確認此任務已在 §9.22/§9.23(2026-08-27)用完全相同的方法論正面解決,本輪未重開
DOSBox-X,只做覆核+補齊文件間的追蹤缺口,避免純粹重工

> 任務背景:本輪收到的派工單完整重述了 §9.1-9.7 的方法論死路(靜態位址算術窮盡三輪)、要求依
> `doc48`§8 啟動 DOSBox-X、用 `BPPM`/`LOGC` 等 live 技術重新核對 `0x2bce5`/`0x2c548` 錨點,並提示
> 可用 `ch27win3`(§`99-chapter-sweep-results.md`)或 `ch30wf` 重現結局序列——這份派工單的措辭與
> §9.1-9.7(2026-08-19~08-21)高度吻合,但**沒有引用任何 §9.8 之後(2026-08-24 起)的內容**,包括
> §9.22/§9.23 已經完成的部分。

### 9.24.1 覆核方法:先讀完整份 doc35 §9(含 §9.8-§9.23)與 `91-worklist.md`/
`known_address_errata.json` 交叉核對,確認任務要求的「live 記憶體攻堅」是否已經做過,而不是預設
派工單描述的現況(只到 §9.5.5)是最新狀態

逐節讀完 §9.8(2026-08-24,首次 live 執行流程 + `LOGC` 交叉印證 `0x2bce5`/`0x2c548` 兩個斷點
全程 `RUN` 不曾命中一次,與 §9.1-9.7 的靜態負面結論吻合)、§9.9(對 §9.8 新候選 `0x4EC6A`/
`0x4EE28` 做完整靜態反查,結論是通用 RLE getbyte + 字型描邊子函式,非 montage 專屬)、
§9.19-§9.21(ch27 戰前 CG1 排除兩個候選)、**§9.22**(2026-08-27,首次對 VGA framebuffer/work
buffer 下 `BPPM` 記憶體寫入斷點,**正面定案** CG1 懸浮小島渲染函式 `FUN_0004e8d3`)、**§9.23**
(同日續,把同一套 `BPPM` 技術套用到本任務真正目標——戰後 party montage 角色回顧卡背景圖,
**正面定案**找到姊妹函式 `FUN_0004e98d` 與此前只聞其名的 wrapper `FUN_0002eb9f`)。交叉核對
`91-worklist.md` 862-867/899/1017-1020 各項目下方,確認已在 2026-08-27 同步附上「重大進展」段落
(見該檔案 L3089-3096/L3168-3179/L3304-3317),`known_address_errata.json` 的 `0x2545d call
0x2bce5` 勘誤條目則仍停在 2026-08-25 記下的「留給下一輪」狀態(即 §9.23 找到的答案發生在該勘誤
記錄**之後**,尚未回填)。

### 9.24.2 結論:本次派工單要求的工作已經完成,且用的正是同一套被要求的方法論(live BPPM 記憶體
寫入斷點,在真正播放中的結局 montage 序列上直接攔截寫像素的指令)——不是猜測或推論,是三重獨立
證據(live `EIP`/`ESI`/`EDI` 命中、`function_bounds`/`decompile`/`call_scan` 靜態核對、螢幕截圖
像素級交叉核對)疊加的既有結論

**對任務步驟 3(位址是否有效)的回答**:兩者皆非。`0x2bce5`/`0x2c172`/`0x2c405`/`0x2c439`/
`0x2c469`/`0x2c548`/`0x2c5e3`/`0x2c773` 這批字面位址,**在靜態(§9.1-9.7,976 個 function 行為
指紋全域掃描 0 命中)與 live(§9.8,全程武裝斷點過完整結局序列 0 命中;§9.22/§9.23 的 live
`BPPM` 追出的真正呼叫鏈裡也完全沒有出現這批位址)兩條路徑下都從未被證實可達**,根因(轉錄自
互動式工具的人工紀錄從未落地成 byte-level 證據,§9.6.1;也不是版本/build/2-byte patch delta,
§9.5/§9.6)已窮盡排除到能排除的極限,不可能再靠位址算術解開。**但任務真正在意的功能性問題——
「結局 montage 實際上是哪段程式碼畫出來的」——已有高信心答案**:已知的 `0x524c6` phase-table
carousel 主控引擎(`FUN_0002ff01`/`FUN_00030e9d`/`FUN_00031266`,§9.13 定位)透過一個先前只聞其名、
本輪前(§9.23)才首次完整反編譯的 66-byte wrapper `FUN_0002eb9f`(native `0x2eb9f-0x2ebe0`),呼叫
一對共用同一套 RLE 控制碼骨架的姊妹函式——`FUN_0004e8d3`(native `0x4e8d3-0x4e98c`,LUT 任意
調色盤重映射,驅動 CG1 懸浮小島淡入淡出)與 `FUN_0004e98d`(native `0x4e98d-0x4eb47`,三模式
重著色:原樣複製/色相輪轉/純色填充,驅動角色回顧卡背景圖)。這條呼叫鏈是用 §9.22 記錄的
`BPPM` 記憶體變更斷點,在**真正即時播放中**的結局 montage(用 doc58 續六十二/續八十記錄的「47格
死亡 signature + End Turn」捷徑觸發)於 VGA framebuffer 層與 work buffer 層各命中一次,回溯
`EIP`/`D SS:ESP` 返回位址,再用 `tools/ghidra_batch_probe.py` 的 `function_bounds`/`decompile`/
`call_scan` 三重靜態交叉核對得到的——完全符合本次派工單步驟 3 要求的「讀取實際執行的 live 線性
位址,並與靜態位址清單比對」這個方法論本身,只是三天前(2026-08-27)已經做過,不需要重做。

### 9.24.3 本輪未重開 DOSBox-X 的理由,以及做了哪些覆核以避免「單純沒讀完文件就宣稱已解決」

沒有直接照抄派工單啟動 §8 recipe、重跑一次相同的 live 攻堅,原因:(a) §9.22/§9.23 的證據鏈本身
已經是 live 一手證據(不是靜態推論)且經過三重獨立交叉核對,重跑預期只會得到相同的 `EIP`/呼叫鏈,
純屬重工;(b) §9.23.6 結尾已明文「核心目標已達成,不建議因為……再開新一輪攻堅」,本輪派工單提供的
新資訊(`ch27win3`/`ch30wf`)經核對後屬於另一個獨立 worklist 項目(M5 全章節可達性驗證,見
`99-chapter-sweep-results.md` 2026-08-29/08-30 記錄),與本 cluster 的 renderer 位址問題無關,
不構成需要重新攻堅本 cluster 的新證據。為了不淪為「單純沒讀完文件」,本輪做了以下覆核而非略過:
逐節通讀 §9.8-§9.23 全文(不是只看標題)、`grep`交叉核對 `91-worklist.md`/
`known_address_errata.json` 確認兩份文件是否已同步 §9.22/§9.23 的結論(發現前者已同步、後者
遺漏,見下)、確認 `ch27win3`/`ch30wf` 兩個任務提示的日期(2026-08-29/08-30,晚於 §9.23)與內容
（M5 章節可達性,非 montage renderer)、`grep`確認 doc35/doc91 裡沒有任何 2026-08-27 之後、
更新的矛盾證據。**唯一發現的真實文件缺口**是 `known_address_errata.json` 裡 `0x2545d call
0x2bce5` 勘誤條目的 `still_pending` 欄位停在 2026-08-25 記錄的「留給下一輪」狀態,尚未回填
§9.23 找到的答案——本輪已在原地追加一段訂正(不刪除原文),把這個開放式伏筆正式接上答案。

### 9.24.4 誠實結論與對 11 個 worklist 項目的影響

**結論**:本次任務要求的「live DOSBox-X 記憶體驗證 `0x2bce5`/`0x2c548` cluster」已經在 2026-08-27
(§9.22/§9.23)用完全相同等級的方法論(BPPM 記憶體寫入斷點,在真正播放中的結局序列上直接攔截)
正面完成,結論穩定,3 天內無任何新證據推翻或補充。**本輪的實質產出是文件層級的**:確認
`91-worklist.md` 對應 11 個項目(L862-867/899/1017-1020)已於 2026-08-27 同步更新、修正
`known_address_errata.json` 一處尚未回填的伏筆、並在此新增本節作為「已核對過、確認不需要重工」
的明確記錄,供未來若又收到類似措辭的派工單時可以直接查到這裡而不必重新走一遍 §9.1-9.23 的完整
歷史。11 個 worklist 項目狀態**不變**:literal `0x2bce5`/`0x2c548` blocker 依然視為不可達
(§9.1-9.9 已窮盡),但功能性缺口("montage 內容如何被畫出來")已從「未知」變成「已知沿用
`0x524c6` carousel 引擎,只差 dedicated indexed adapter/campaign terminal 接線」,這個定性
2026-08-27 已經完成,本輪只是確認並補齊追蹤文件,未新增或撤回任何技術結論。

## 9.25 2026-09-06 續:接手 §9.23.6 建議 5——首次對**FIGANI 立繪本體**(角色卡右側全身像,§9.23.6
明文標記「本輪未觸及」的部分)套用同一套 live `BPPM` 方法論,新隔離 instance(`figani1`)——**正面
新發現**:定位到獨立於 §9.23 backdrop 渲染鏈的**另一條**呼叫鏈,`FUN_0002aa00`(角色回顧卡逐人
主迴圈)→`FUN_0004e7dd`(依 identity 索引的 2-byte lookup table `DAT_000615fe`),尚未追到最終
RLE/pixel 層,但已比 §9.23.6 當時的「完全未觸及」前進一步

> 任務背景:§9.23.6 誠實列出的未查清項目第 (c) 點明確寫「FIGANI 立繪本體(角色全身像)與 portrait
> 框(左上角小頭像)的繪製機制本輪未觸及,只確認了背景圖層」,並在建議 5 提出「對 FIGANI 立繪與
> portrait 框套用同一套方法論,預期會收斂到 `0x524c6` carousel 引擎家族內的某個成員」。本節是對
> 這個建議的第一次實際執行。

### 9.25.1 環境與方法論教訓:同一套 `SMV` 死亡 signature 批次寫入手法,第一次嘗試在 bash 迴圈裡
逐次呼叫 `debugger-cmd`(每次一個獨立 WSL 子行程)**完全失敗**——75 筆指令全部黏成一行從未送出
Enter,連鎖導致後續近 20 筆被當成亂碼指令拒收;根因**不是**已知的「bash 迴圈+tmux send-keys
不穩定」(續六十二的舊警告)本身,而是**`tmux send-keys -l text` 後緊接 `tmux send-keys -l $'\r'`
這個組合本身不可靠**——改用 `tmux send-keys <session> Enter`(具名鍵,不是字面 `\r`)後,同樣的
75 筆批次(這次改成單一 WSL 呼叫執行一支內含全部指令的 shell 腳本,避免逐次 WSL 子行程開銷)
**100% 乾淨送達**,逐一以 `mem read-unit-record` 核對 slot16/26/50/62/90 皆為 `+5=0x01`。**這是
本專案第一次確認具名 `Enter` 鍵在這個除錯器輸入通道上比字面 `\r` 可靠**,值得列入下一輪工具備忘
(見 `98-tooling-infrastructure.md` 待補記錄)。

### 9.25.2 觸發 postbattle 轉場的實際路徑:§9.23.1 記錄的「單純按一次 Escape 直接跳轉」本輪**沒有
重現**(連續按 Escape 只在同一批已存活單位的資訊卡之間循環),改用死亡 signature 寫入後單純
`RUN` 恢復執行(不需要額外按鍵)即可——事後推測 §9.23.1 的「一次按鍵」描述可能是巧合(RUN 恢復
瞬間畫面剛好處於某個中間狀態),不是穩定機制;下一輪如果要重現这条捷徑,**優先直接 `RUN` 後
等待畫面自然演進,不要預設一定要按 Escape**。

### 9.25.3 VGA 座標定位:重用 §9.23.2 的 viewport 校準(screen 原點 (192,215),2x 縮放)**原封不動
在全新 instance 上驗證通過**——`D 0178:A0AAA` 讀出的灰階雲霧漸層(`0x60-0x64` 範圍)與 §9.23.2
記錄的數值幾乎一致,證實這個 viewport 校準不是綁死在特定 session 的偶然值,是這個視窗佈局的穩定
性質。萊汀卡(第一張)先核對 §9.23 已知的 backdrop 座標無誤後,換到**悠妮卡**(第二張,`FLAME
DRAGON KNIGHTS 2` 版面下的紅袍召喚師立繪)挑選明顯落在紅袍軀幹上的螢幕座標 (746,480),換算
native (277,132),VGA 位址 `0xAA615` 起 5 列讀出 `CD`/`4C`/`64`/`BD`/`BE`/`BF` 混合值(明顯不是
單一灰階背景色,是立繪本體的多色像素),確認座標命中立繪而非背景。

### 9.25.4 三層 `BPPM` 交叉核對:VGA 層(`0xAA648`/`58`/`68`)+work buffer 層(套用 §9.23.4 記錄的
delta `0x345140` 換算)——**work buffer 層 delta 這次對不上**(命中位址落在無關的敵方 AI 狀態機
`FUN_00013a9f` 附近,純屬巧合撞上其他程式碼在讀寫同一塊 heap 記憶體,不是 montage 相關),確認
§9.23.4 記錄的 work buffer 絕對位址(及其 delta)**是綁定特定 session 的動態配置位址,不能跨
session 沿用**——VGA 層(`0xA0000` 起的硬體 framebuffer 固定位址)才是可以跨 session 沿用的錨點,
下一輪如果要重複這套方法論,應該**只信任 VGA 層位址,work buffer 層每次都要重新用「D SS:ESP 回溯
呼叫端」的方式重新定位,不能複製舊 session 的位址**。VGA 層本身命中順序:先整段清零(`0x1DA0Bx`
一個通用 unrolled 32-byte-stride 清結構迴圈,同 §9.23.3 記錄的清 buffer 手法同類but非同一函式),
`0xAA648` 這個立繪專屬像素從 `00` 直接寫入 `CD`(立繪色,不是背景的 `64`)。

### 9.25.5 EIP 回溯定位新函式:靜態位址 delta 這次呈現**分區不均勻**(同一 CS 段,不同區域對 live
EIP 的偏移量不同)——`0x1B3xxx` 區用 delta `0x1A0000` 可正確解出已知函式 `FUN_00013a9f`,但同一
delta 套在 `0x1DA0xx`/`0x1EAAxx` 區(立繪像素寫入當下的 EIP)全部得到「不在任何函式邊界內」或
byte 對不上的結果——**改用 `D SS:ESP`/`[EBP+4]` 讀呼叫端返回位址**(§9.23.3 已驗證過的方法)
繞開這個問題:在立繪像素 `CD` 命中前不久,一個 RLE 風格解碼迴圈(`bx`/`cl` shift-and-repeat 模式,
遞減 `[0x1F47B6]` 計數器)所在函式的呼叫端返回位址(`[EBP+4]=0x1CABDC`)用 delta `0x1A0000`
換算成 `0x2ABDC`,**成功落在已知函式邊界內**:`FUN_0002aa00`(native `0x2aa00-0x2ac7c`,size 637)。

### 9.25.6 `FUN_0002aa00` 反編譯:確認是角色回顧卡逐人主迴圈,直接使用已知的單位 record 陣列公式
`decompile` 結果確認這是一個 `while(true)` 主迴圈:每輪呼叫 `FUN_0002ae0e()` 取下一筆(回傳 0 則
清場退出),用 `iVar3 = DAT_00053a45 + local_34[DAT_00053c57] * 0x50` 定位到**已知的**單位 record
(`[0x53a45]+unit*0x50`,與 doc56 §RE-PHASE-DISPATCH-GATE/§RE-UNIT-MODE-DISPATCH 記錄的同一份
陣列公式完全吻合,不是另一個獨立陣列),讀出 `local_14 = abStack_54[DAT_00053c57]`(該單位的
identity 值),呼叫已知的確認對話函式(`FUN_00019953`/`FUN_000197e5`,即 doc11「0x19953確認」
家族),若 `local_14==0x34` 或 `local_14>0x31`(即 identity ≥ 0x32,50 以上)分流呼叫
`FUN_0002aedb`/`FUN_0001b8e7`(**本節初稿曾猜測是特殊角色如飛龍騎士的額外分支,已展開反編譯後
撤回這個猜測**——`FUN_0002aedb(unit,itemId)`是逐格掃描`FUN_0001b8a6()`/`FUN_0001b722()`的
道具清單找`itemId`所在index,`FUN_0001b8e7(unit)`則是`*(unit_record+0x18)=0x80`,單純對同一份
單位record寫入一個狀態旗標——這是**道具/裝備狀態的簿記**,跟哪張立繪、飛龍騎士與否無關,
與§9.23.1的飛龍騎士構圖差異純屬巧合的識別碼相鄰,不構成關聯證據),
接著呼叫 `puVar2 = FUN_0004e7dd(local_14)` 並把結果寫回 `*(iVar3+0x20) = *puVar2`(把這個
identity 對應的一個 byte 寫進單位 record 的 `+0x20` 欄位),再呼叫一串已知的 present()/清 buffer
函式家族(`0x3776e`/`0x37324`/`0x3759c`,同 §9.20.6/§9.23.3 記錄的通用 present 原語)。

### 9.25.7 `FUN_0004e7dd`:確認是 identity-indexed 2-byte lookup table,呼叫點僅 3 處且全部集中
在 `FUN_0002aa00` 同一個 cluster 內——不是通用工具函式,是 montage 專屬
`decompile`:`return &DAT_000615fe + (param_1 - 0x20) * 2`——一個以 `identity-0x20` 為索引、
stride 2 bytes 的查表函式。`xref_to` 只有 3 個呼叫點(`0x2abe4`/`0x2a782`/`0x2ad7b`),全部落在
`0x2a7xx-0x2adxx` 範圍內,與 `FUN_0002aa00`(`0x2aa00-0x2ac7c`)同一個 cluster——**確認這是
montage 專屬的 per-identity 查表,不是被 50 處呼叫的通用工具**(對照:同輪查的 `FUN_0004e381`
單純是 `_DAT_41c=_DAT_41a` 的全域同步小工具,50 處呼叫全部無關,只是名字碰巧跟 `0x4e8d3`/
`0x4e98d` 相鄰,本輪順手排除避免下一輪誤認)。`DAT_000615fe` 這張表**尚未 dump 內容**,是下一輪
最直接可做的靜態工作(只需 `bytes` action 讀一段固定長度,配合已知 identity 範圍如萊汀/悠妮/
飛龍騎士的實際 identity 值反查對應 2-byte 內容,即可知道這張表存的是什麼——目前只知道它「餵一個
byte 進單位 record 的 `+0x20` 欄位」,不知道内容語意)。

### 9.25.8 對 `91-worklist.md` L1017-1020 的影響與誠實範圍
**新增**(相對 §9.23.6 當時的「完全未觸及」):FIGANI 立繪渲染鏈的**上層編排**已定位——
`FUN_0002aa00`(逐人主迴圈,與 §9.23 的 backdrop 渲染鏈**是同一個大迴圈的兩個不同輸出**,不是
兩條獨立系統)→`FUN_0004e7dd`(per-identity 查表)→`DAT_000615fe`(表本體,內容未知)→
`*(unit_record+0x20)`(某個 byte 被寫入單位 record)。**仍未查清**(誠實列出,勿 over-claim):
(a) `DAT_000615fe` 表的實際內容語意;(b) `unit_record+0x20` 這個 byte 後續被哪個函式讀出、如何
驅動實際的 RLE 解碼/調色盤選擇(本輪定位到的是「編排層」,不是 §9.23 backdrop 找到的那種
「像素層」直接證據,**§9.26 已補上,見下**);(c) `local_14>=0x32` 分流的
`FUN_0002aedb`/`FUN_0001b8e7`——**已展開反編譯,確認是道具清單掃描+狀態旗標簿記,與哪張立繪
無關,原本「飛龍騎士卡特殊分支」的猜測已撤回**;(d) portrait 框(左上角小頭像)完全未觸及,
本輪只追了立繪本體。**維持 D**,但範圍已大幅收斂,見 §9.26 的後續進展。

## 9.26 2026-09-06 續三:接手 §9.25.8 遺留的「`unit_record+0x20` 讀取端」與「`FUN_0002aa00` 到底
在哪裡畫立繪」兩個問題——純靜態反編譯追出完整編排鏈,`DAT_000615fe` 表語意改判為姓名代入用途
(非портrait/立繪選擇器),另外定位到疑似 FIGANI 立繪的獨立呼叫點

### 9.26.1 `unit_record+0x20` 讀取端:`FUN_0002ac7d`——**改判 `DAT_000615fe` 表語意**
`xref_to 0x2ac7d`(`FUN_0002aa00`每輪迴圈結尾呼叫的函式)只有 1 個呼叫點,decompile 確認:
```
iVar2 = DAT_00053a45 + param_1*0x50;         // 已知單位 record
DAT_00053ad9 = *(byte*)(iVar2+0x20) + 0x96;  // 讀回 +0x20,+0x96 後寫進 DAT_00053ad9
...
iVar1 = FUN_0004e7dd();
if (*(byte*)(iVar1+1) != 0) {
    DAT_00053ae1 = *(byte*)(iVar1+1);
    *(char*)(iVar2+0x3b) += (char)DAT_00053ae1;   // +0x3b 就是已知的 MV 欄位(doc32)
}
*(iVar2+0x21)=1; *(iVar2+0x3c)=0;
*(word*)(iVar2+0x40) = *(word*)(iVar2+0x42);  // HPcur = HPmax(已知欄位,doc32)
*(word*)(iVar2+0x44) = *(word*)(iVar2+0x46);  // MPcur = MPmax
```
`DAT_00053ad9` **是已知全域**(`14-text-control-codes.md`/doc99:「`0xFFFC`控制碼的姓名代入用
角色id」)——這**推翻**本節 §9.25.6/§9.25.7 對 `DAT_000615fe` 的原始猜測(誤認為 portrait/立繪
selector)。真實語意:`DAT_000615fe` 表 byte0 是「這個 identity 對應的姓名代入 id」(供劇情後日談
文字裡的`0xFFFC`控制碼使用),byte1 是「這個 identity 的 MV 加成值」(用於回顧卡顯示時的
buff/debuff 呈現)。同一函式順便把該單位**全滿血滿魔**(`HPcur=HPmax`/`MPcur=MPmax`)——這是
回顧卡顯示邏輯的一部分,不是渲染。**結論:`FUN_0002aa00`/`FUN_0004e7dd`/`DAT_000615fe`/
`FUN_0002ac7d` 這整條鏈,查證後是「姓名代入+MV加成+滿血滿魔」的資料準備邏輯,不是立繪渲染本身
——§9.25 標題「立繪渲染鏈的上層編排」需要訂正為「回顧卡資料準備編排」,立繪像素渲染是另一個
獨立呼叫,見下 §9.26.2。**

### 9.26.2 立繪像素渲染的真正候選:`FUN_0002fb2c`(`FUN_0002aa00`另一分支呼叫的函式,native
`0x2fb2c-0x2fe13`,size 744)——9 輪迴圈+1 次額外呼叫,都走已知的 `FUN_0004e98d` RLE 引擎
`decompile`確認結構:`FUN_0002fb2c`先用 3 次 `FUN_000111ba()` 取得 3 個資源控制碼存進
`DAT_0005413f`/`DAT_00054143`/`DAT_00054147`,接著 `for(iVar5=8; iVar5>=0; iVar5--)`
**9 輪迴圈**,每輪呼叫 `FUN_0004e98d(...,(&DAT_0005413f)[iVar4],0,0x32,uVar1,0x280,0xffffffff)`
(`iVar4` 依 `(輪次+1)/3`與`%3`在 0/1/2 間輪替,循環使用 3 個資源槽)接present()/DAC crossfade
家族(`FUN_0002eb9f`/`FUN_00011eb0`/`FUN_00011d40`,與 §9.23 確認的 backdrop 引擎完全同一組)。
**迴圈結束後,額外再呼叫一次同一組鏈**(逐 byte 反組譯確認呼叫點在 native `0x2fd31`,`xref_to
0x4e98d`交叉核對命中同一位址)——**這次 push 的是固定的 `DAT_0005413f`(直接取值,不經
`(&...)[iVar4]`索引)**,不是 3 槽輪替中的任一個依迴圈變化的值,結構上明顯是**迴圈之外、額外一次
的獨立繪製動作**,其餘參數(`0x32`/`0x280`/`0xffffffff`)與迴圈呼叫完全相同。

### 9.26.3 誠實範圍:**尚不能斷言** §9.26.2 的額外呼叫就是 FIGANI 立繪本體——追查`FUN_000111ba`
與既有文件交叉核對後,信心等級進一步下修
反編譯`FUN_000111ba(param_1,param_2)`發現它本身是一個**通用棧/緩衝區管理原語**(113 個呼叫點,
內部呼叫`FUN_0003706e`/`0x37324`/`0x37119`/`0x373ca`/`0x37940`/`0x3776e`——都是已知的 present()
家族通用工具,不含任何「依索引取特定資產類型」的邏輯),不是「依身分/資產類別取資源」的專用函式,
本身不能回答`DAT_0005413f`是哪種資產。**更關鍵的是**:`grep`交叉核對發現`27-combat-rules-and-
validation-checklist.md §6.5`**早就記錄過同一組全域**——`FUN_0002cf30`(derived-strike 戰鬥
演出函式,id24/28/29/30/31 四招共用)前段也無條件呼叫 3 次`FUN_000111ba()`載入同一組
`DAT_0005413f`/`DAT_00054143`/`DAT_00054147`,該節明文記載「未鑑定型態,不影響上述結論」。
**這代表這 3 個全域是被至少兩個完全不相關的情境(戰鬥招式演出、postbattle 角色卡)重複使用的
通用暫存槽,不是 montage 或 FIGANI 專屬的資源控制碼**——`FUN_0002fb2c`的「9輪迴圈+1次額外呼叫」
結構有可能單純是一個**通用的「載入3個標準槽→迴圈套用某種通用效果9次→再套用一次不同變體」演出
原語**,被 postbattle montage 借用,不能排除跟`FUN_0002cf30`用的是同一份共用邏輯家族。**下修後的
誠實結論**:§9.26.2 定位到的呼叫結構是真實、可重現的靜態事實(byte 級核對過),但「額外呼叫=立繪
本體」這個推論的證據強度比原先評估的更弱,必須靠 live BPPM 交叉核對才能定案,不能只憑呼叫模式的
結構差異下結論。**下一輪如果要真正關閉立繪本體這個問題,應該**:(1) 對 native `0x2fd31` 這個額外
呼叫點下 live 斷點,配合 §9.25.3 已驗證可跨 session 沿用的 VGA viewport 校準,直接核對命中瞬間的
VGA 座標是否落在立繪範圍內——這是唯一能真正判定的方法,靜態分析到此已到極限;(2) 若要優先排除
「跟`FUN_0002cf30`共用同一套通用演出原語」這個新假說,可以反過來對`FUN_0002cf30`的 9 輪迴圈+額外
呼叫做同樣的 live BPPM 核對,若兩者命中同一種「9+1」呼叫形狀卻服務完全不同的畫面內容,即可證實
這只是一個通用工具,不是任何一方專屬。

### 9.26.4 2026-09-06 續五:執行 §9.26.3 建議 1——新隔離 instance(`figani4`)對 native `0x2fd31`
候選呼叫做 live BPPM VGA 層交叉核對——**技術上失敗,但意外發現一個本 build 的 `BPPM` 真實限制**
重演完整流程(fresh launch→title→LOAD 存檔1→軍營→戰場→死亡signature全部75格SMV批次一次寫入
成功、逐slot核對無誤→resume→戰後對話→萊汀卡),對卡片立繪(不是backdrop)另一個螢幕座標(避開
文字疊字區,screen(633,357)→native(220,71)→VGA`0xA599C`)下 `BPPM`,讀出的初始值(`FE/BB/BC/BD`
混合)確認落在立繪的鎧甲灰階範圍(不是背景的`0x60-0x64`漸層)。**三個獨立位址(`0xA59C0`/
`0xA59D0`/`0xA59E0`)各自命中一次(`00→BB`/`00→BC`/`00→BD`,分別對應鎧甲三種灰階),但**每一次
命中回報的 EIP 都落在同一個位址區(`0x1B3AC9`~`0x1B3ADD`)**,反組譯後確認這是一個**通用 tick
差值等待迴圈**(`movsx eax,[eax]; sub eax,[0x1EFA2C]; test eax,eax; jge+5; add eax,0x10000;
cmp eax,edx; jl 迴圈開頭`——典型的「處理時間迴繞的 tick 差值等待」樣式,不是任何寫像素指令)。

**技術結論(本輪新發現,值得記錄避免下一輪重工)**:這個 build 的 `BPPM` 記憶體寫入斷點,在**快速
連續的小段寫入**(例如 RLE 解碼逐 byte 寫入 VGA,每個 byte 之間可能間隔極短)場景下,**回報的
EIP 不可信**——三個獨立位址各自的命中都巧合地落在同一段跟繪圖完全無關的 tick 等待迴圈裡,顯示
這個除錯器的記憶體斷點機制很可能是**周期性掃描記憶體差異**而不是真正的硬體級 watchpoint,如果
掃描週期跨過了「寫入指令→之後幾條指令」這段極短的視窗,回報的 EIP 就會落在掃描當下 CPU **實際
正在執行**的其他無關程式碼(此處恰好是一個高頻率被呼叫的通用 tick 等待原語)。**這跟 §9.22/§9.23
成功的案例不衝突**——那兩次抓到的是 work buffer 層的 `memcpy`/`memmove` 迴圈,這類迴圈本身會連續
執行數十到數百條指令處理一整列像素,遠遠長於掃描週期,所以能被穩定抓到;VGA 層的單 byte 寫入
(或短迴圈)則可能整個落在掃描間隔之內,抓到的永遠是「掃描當下剛好在跑的其他程式碼」而非寫入者
本身。**下一輪如果還要用 live 方法確認立繪本體**,應該回到 work buffer 層(比照 §9.23.4),但
delta 每個 session 都要重新用「D SS:ESP 回溯已知呼叫端」校準,不能沿用舊 session 的絕對位址
(已在 §9.25.4 記錄過這個教訓)——或者改用單步(`T`/`P`)在確認 VGA 層有變化後往回單步幾條指令,
而不是依賴斷點回報的 EIP。

### 9.26.4b 2026-09-06 續六:嘗試改用「執行斷點」繞開 §9.26.4 的 `BPPM` EIP 不可信問題——**確認
這條路本身不存在**,這個 build 沒有 protected-mode 執行斷點指令
新隔離 instance(`figani5`)完整重演到萊汀卡,對候選呼叫點 native `0x2fd31`(套用已確認對
`0x2aa00`/`0x2ac7c`/`0x13a9f`區域有效的 delta `0x1A0000`,換算 live `0170:49D31`)下 `BP`
指令——**指令被接受**(`DEBUG: Set breakpoint at 0170:49D31`)且列在 `BPLIST` 裡,但卡片完整
渲染完畢、甚至一路推進到片尾 STAFF ROLL(`ANIMATION PROGRAM 羅元甲`)都**從未觸發**,儘管這段
程式碼在這整個過程中必定執行了至少 3 次(萊汀/悠妮/飛龍騎士三張卡)。追加對已知一定會被呼叫的
`FUN_0002aa00`本體(`0x2aa00`→live`0x4AA00`)與其呼叫`FUN_0002fb2c`的呼叫點(`0x2abcc`→live
`0x4ABCC`)也下同樣的`BP`,同樣**從未觸發**,排除了「單純位址算錯」的可能性——三個位址全部
不觸發,說明問題不是位址,是`BP`這個指令本身。

**根因**:對除錯器主控台送出`HELP`,完整指令表列出:`BP`(純`[segment]:[offset]`,無 pmode
版本)、`BPINT`(中斷)、`BPM`(記憶體,非 pmode)、`BPPM`(pmode 記憶體變更斷點)、`BPLM`
(linear 記憶體變更斷點)——**這個 build 完全沒有「pmode 執行斷點」這個指令**,`BP`本身很可能是
針對 16-bit real-mode segment:offset 設計(觀察到當下 debugger 剛好停在一段 real-mode 程式碼,
`CS=0080 Pr16`,佐證`BP`預期的是 real-mode 語意的 segment,不是我們 protected-mode 32-bit `CS`
選擇器),對我們的 32-bit protected-mode 程式碼位址送`BP 0170:xxxxx`,`0170`被當成 real-mode
段基底解讀,實際監看的物理位址跟我們要的 protected-mode linear 位址完全是兩回事,難怪永遠不會
命中真正的程式碼執行。

**結論(確定性負面結果,值得記錄避免下一輪重工)**:「用執行斷點繞開 `BPPM` EIP 不可信問題」這條
路在這個 DOSBox-X build 裡**完全不存在**,不是位址算法或 delta 校準的問題。§9.26.4 記錄的兩個
替代方案(work buffer 層 `BPPM`、單步 `T`/`P`)是**唯一**還沒排除的路——本節排除的是一條原本
看似更乾淨的第三條路,不影響那兩個仍待嘗試的方向。

### 9.26.4c 2026-09-06 續七:嘗試 §9.26.4 剩下兩條路之一——work buffer 層 `BPPM`,但發現連
「用已知 backdrop 位址校準本 session 的 work buffer delta」這個前置步驟本身都做不到,**VGA 層
`BPPM` 的 EIP 不可信問題不是只發生在立繪像素這個特定位址,是這整個 build 對所有 VGA 位址都成立**
第三個新隔離 instance(`figani6`)完整重演到萊汀卡(這次意外在文字面板尚未繪出、立繪已完成的
中途渲染狀態截到圖,佐證卡片是「先畫立繪+背景、後畫文字面板」兩段式渲染,與 §9.23.1 記錄的
飛龍騎士卡「中途渲染」截圖屬同一現象)。進 debugger 對立繪像素三個位址下 `BPPM`,另外**加碼對
`0xA0AAA`(§9.23.2/§9.25.4 已知的 backdrop 座標,理論上應該可靠命中 §9.22/§9.23 記錄的
`FUN_0003771c` present() memcpy)也下`BPPM`**,想法是:如果連這個「歷史上證實可靠」的位址也在
本session命中同一段tick等待迴圈,就能確認整個VGA層方法本身失效,不是立繪像素這個座標特有的問題。

**結果:確認failed,而且是更徹底的失效**——resume後,`0xA0AAA`(backdrop)命中的EIP,跟立繪像素
命中的EIP,**回報的是同一段`0x1B3AC9`附近的tick差值等待迴圈**,證實這個問題不是「立繪像素寫入
太快」特有,是**這個session對所有VGA位址的BPPM都受同一個問題影響**。用`D SS:ESP`(這次讀`[ESP]`
而非`[EBP+4]`,因為`EBP`當下的值明顯是残留的舊資料指標、不是有效的frame pointer——這本身也是一個
值得記錄的細節:遇到tick等待迴圈這類leaf function時,`EBP`不可信,要讀`ESP`本身)回溯到返回位址
`0x1CE191`,套用delta`0x1A0000`得native`0x2E191`,`function_bounds`確認落在**另一個全新、與
先前兩次(`FUN_00013a9f`、這次)都不同的無關函式**:`FUN_0002dfc8`(native`0x2dfc8-0x2e2af`),
decompile確認它是一個獨立呼叫`FUN_0004e98d`(單次,非9輪迴圈)並播放hit-event陣列音效
(`FUN_00025a96`,即doc27 §6.5已定案的「derived-strike演出」hit-event回放樣式)的函式,`xref_to`
顯示唯一呼叫端是`0x15195`——**這正是doc27 §0已經記錄過、標記「本輪未展開」的懸而未決引用**
(「`0x015195`(push ebp;call 0x28784)作為施法演出呼叫點,本輪即時反組譯該位址實際指令是
`CALL 0x2dfc8`」)——證實這是**戰鬥招式演出引擎的殘留/共用呼叫路徑,跟角色卡montage完全無關**,
只是恰好在這個時間點被排入CPU執行序列,被BPPM的週期性掃描機制誤認成「當下正在寫VGA的程式碼」。

**結論(比§9.26.4更強的負面結果,work buffer層BPPM的前置校準步驟本身不可行)**:VGA層`BPPM`在
這個build裡對**任何**VGA位址(不論backdrop還是立繪本體)都無法可靠回報EIP,兩次獨立測試分別
命中兩個完全不同、但都跟montage無關的巧合函式(`FUN_00013a9f`敵方AI狀態機、`FUN_0002dfc8`戰鬥
演出引擎),不是特定位址的偶發問題。這代表§9.26.4提出的「改走work buffer層BPPM」建議**行不通**
——因為那個建議的前提(先用一個VGA層已知位址的BPPM命中反查出work buffer delta)本身就需要一個
可信的EIP,而VGA層EIP在這個build裡從未可信過。**真正剩下唯一可行的路,只有單步(`T`/`P`)**：
從一個已知會執行到montage render call的錨點（例如`FUN_0002aa00`entry，若能用執行斷點以外的
方式暫停在那裡——但§9.26.4b已證實執行斷點本身不存在，所以連「暫停在已知錨點」這一步都需要靠
BPPM在**work buffer**（不是VGA）位址上命中才能達成，而work buffer位址本身又需要靠已經證實不可信
的VGA層EIP去反查——**這是一個目前找不到出口的方法論死結**，需要一個本輪未想到的第四種方法
（例如：對已知一定會被寫入的`DAT_00053ad9`等資料全域下`BPLM`，這類全域位址不需要delta換算，
命中後回報的EIP若同樣落在無關迴圈，至少能再次confirm；或考慮跳出即時除錯，改用「逐張截圖比對
FUN_0002fb2c迴圈的9個資源槽在不同chapter/不同identity下是否呈現一致內容」這種純視覺歸納法）
才可能真正關閉立繪本體這個問題。**維持D**，本輪的價值是把三條方法論死路都用真實live證據封死,
避免未來任何一輪重蹈覆轍。

### 9.26.4d 2026-09-06 續八:執行 §9.26.4c 建議的「第四種方法」——對已知資料全域 `DAT_00053ad9`
下 `BPLM`(理論上不需 delta 換算)——**同樣未能取得結果,但意外確認了一個真正有用的新視覺觀察**
第四個新隔離instance,對 native `0x53ad9`(doc14/doc99已知的姓名代入用全域)套用同一套「delta
0x1A0000」算出 live `0x1F3AD9`,下`BPLM 1F3AD9`(linear記憶體斷點,不透過selector,理論上比
`BPPM`更直接)。重演死亡signature→postbattle→萊汀卡→悠妮卡全程(**超過100次confirm按鍵**,
涵蓋整段戰後對白、CG1/CG2、詩句捲動文字、兩張角色卡),**斷點從未觸發一次**——即使在悠妮卡本文
明確包含「她一定會再那個叫做**索爾**的年輕人在夢中相聚」這種教科書等級的inline姓名代入情境下
依然沒有命中。**結論**:`delta 0x1A0000`對DATA位址同樣不適用(不只code位址的不同區域有不同
delta,DATA本身也需要獨立、目前未知的delta)——這不是`BPLM`本身失效,是位址算錯,跟§9.26.4b/c
記錄的「code位址delta不均勻」是同一類問題在DATA空間的翻版。**這條路本輪同樣未能走通**,但沒有
排除`BPLM`這個機制本身(跟§9.26.4b排除整個`BP`指令類別性質不同)——如果能獨立校準出正確的DATA
delta,`BPLM`理論上仍可行,只是本輪沒有找到校準方法。

**意外的視覺新發現(跟方法論死路無關,是本輪唯一的正面產出)**:在推進到悠妮卡的過程中,截到一張
**先前20+輪從未記錄過的中途畫面**——悠妮呈現雙臂高舉、法杖朝上揮舞的「凱旋/施法收尾」姿勢
(`FLAME DRAGON KNIGHTS 2`版面,右側立繪與§9.23.1記錄的悠妮「背對站立」定格畫面明顯是**不同的
姿勢**),數次confirm之後才穩定成已知的定格姿勢。**這證實角色回顧卡的立繪本身是有多幀動畫的
(至少2幀:揮舞動作→定格),不是單一靜態圖**——這與§9.26.2/§9.26.3定位的`FUN_0002fb2c`「9輪
迴圈依序輪替3個資源槽」結構高度吻合(9輪很可能就是逐幀播放這個「凱旋動作」的中間影格,3個資源槽
可能對應動作的不同部位或調色盤階段),為「9輪迴圈=立繪動畫、不是backdrop雲霧特效」這個此前只是
猜測的定性,補上了第一個獨立的視覺證據(此前只有結構上的推論,沒有實際畫面佐證)。下一輪如果要
用截圖比對法(§9.26.4c建議的第四種方法之一)驗證`FUN_0002fb2c`,這個「悠妮凱旋姿勢」畫面是一個
具體、可重現的錨點,值得優先用來對比。

### 9.26.5 對 `91-worklist.md` L1017-1020 的最新影響
**新增**:`FUN_0002fb2c`(9輪迴圈+1額外呼叫的渲染編排函式)定位,其額外呼叫是立繪本體的**候選**
但未定案;`DAT_000615fe`表語意訂正為姓名代入+MV加成(非portrait選擇器,撤回§9.25.7的猜測);
§9.26.4 嘗試 live BPPM 交叉核對此候選,技術上未能取得可信 EIP,但發現這個 build 的 `BPPM`
在快速 VGA 寫入場景下回報的 EIP 不可信這個真實工具限制(下一輪需改走 work buffer 層或單步
方法)。§9.26.4b/c/d 三輪追加測試(執行斷點、work buffer層delta校準、DATA全域BPLM)**全部未能
取得可信EIP**,確認這個build裡code與data位址的live delta在不同區域都不均勻、且沒有找到任何
現有機制能繞開這個問題——四種live交叉核對方法均已exhaust,詳見doc35 §9.26.4b/c/d。**唯一的
正面產出**是§9.26.4d意外截到的「悠妮凱旋姿勢」中途動畫幀,首次提供視覺證據支持「`FUN_0002fb2c`
的9輪迴圈=立繪動畫幀,不是backdrop特效」這個此前純結構推論的定性。**仍未查清**:
`FUN_000111ba()`的資源索引語意、`FUN_0002fb2c`額外呼叫的內容仍需驗證、portrait框完全未觸及。
**維持D**,範圍已收斂到「1個具體候選函式+1個視覺佐證(凱旋動畫幀)+4個已知不可行的live驗證
路徑+1個仍待嘗試的純視覺歸納法(逐張截圖比對多角色/多chapter的`FUN_0002fb2c`輸出內容)」。

## 9.27 2026-09-06 續九:執行§9.26.5建議的純視覺歸納法——第五個新instance,不進debugger,純粹
逐格截圖比對悠妮卡的完整reveal序列——**正面新發現**:backdrop本身是分成多個獨立矩形色塊逐塊
composite出來的,不是一次性的全螢幕blit,直接支持`FUN_0002fb2c`「9輪迴圈=分塊composite」定性

> 方法論說明:鑑於§9.26.4b/c/d已經用四種獨立live除錯方法把「用debugger即時抓EIP」這條路完全
> exhaust,本節改用最原始的方法——不進debugger、不下任何斷點,單純以正常按鍵速度推進遊戲,
> 每按一次confirm就截一次圖,逐張比對悠妮卡從空白到完全定格之間的中間畫面,把render過程本身
> 用視覺方式重建出來。

### 9.27.1 關鍵截圖:backdrop composite過程中,可以看到清楚的矩形分塊邊界——非常直接的
「分塊繪製」視覺證據

在悠妮卡reveal過程中截到的一張畫面(`docs/figures/ch27-postbattle-card-yuni-tiled-backdrop-
midrender.png`)清楚顯示:畫面右側的雲霧backdrop **不是連續的一塊**,而是由兩個以上、邊界分明
的矩形色塊拼接而成——左下方一塊較小的矩形已經填上雲霧紋理,右上方一塊較大的矩形也已填色,兩塊
之間、以及周圍其餘應該是backdrop範圍的區域**仍是純黑**(尚未繪製)。立繪本體(悠妮持杖側身站姿)
已經完整疊在這些已完成的矩形塊上方。**這是本篇任務系列20+輪以來第一張直接顯示「backdrop本身是
分塊繪製」的畫面**,先前所有推論(§9.23.3的「先清零再重繪」、§9.26.2的「9輪迴圈」)都只是從
反組譯的迴圈結構推斷,沒有一張畫面能直接佐證「分塊」這個具體形狀。

### 9.27.2 同一時間點附近,立繪姿勢與最終定格畫面明顯不同——但需要謹慎解讀,不能直接斷言是
「多幀動畫」

同一輪截圖序列裡,緊接著這張「分塊backdrop」畫面之後(下一次confirm),截到另一張畫面
(`docs/figures/ch27-postbattle-card-yuni-lunge-pose-midrender.png`):backdrop這次呈現一塊
更大的連續矩形(仍未填滿全部背景範圍),立繪呈現**向左弓步、法杖斜舉指向左方**的姿勢——與
已知的最終定格「背對站立、法杖垂放身側」姿勢明顯不同。**誠實的兩種解讀,本輪未能區分**:
(a) 這真的是角色立繪本身的多幀動畫(例如「揮動法杖」的中間格),reveal完成後定格在最後一幀;
(b) 這是RLE解碼過程中,尚未寫滿的像素恰好在肢體邊緣造成視覺上「看起來像另一個姿勢」的錯覺
(類似§9.23.1記錄的飛龍騎士卡「中途渲染,可見左上角一塊尚未上色的色塊」現象,但這次的錯位
範圍更大、更容易被誤讀為姿勢本身不同)。兩種解讀都不排斥「9輪迴圈=分塊composite」這個§9.27.1
已經證實的結論,只是「立繪本身是否也是多幀動畫」這個更進一步的問題,本輪的證據還不夠格下定論。

### 9.27.3 已知的悠妮卡卡死bug再次重現,確認是既有已記錄限制,非本輪新發現

嘗試繼續推進到第三張(飛龍騎士)卡失敗——連續confirm在悠妮卡文字定格後完全無法繼續前進,
文字內容、立繪姿勢均無變化。這與`docs/knowledge-base/35-battle-animation-rendering.md`
§9.10.5/`58-remake-live-verification-log.md`續六十二/續六十四已經記錄過的「悠妮卡永久循環」
bug完全吻合(兩條獨立路徑都曾卡在悠妮卡同一組文字),**不是本輪新發現的問題,是舊有已知限制的
再次重現**,本輪未嘗試繞開(該卡死bug本身有獨立的追蹤記錄,不在本次FIGANI立繪調查範圍內)。

### 9.27.4 對 `91-worklist.md` L1017-1020 的影響:純視覺歸納法初步成功,補上第一個直接畫面證據
**新增**:「backdrop分塊composite」首次獲得直接畫面佐證(§9.27.1),不再只是反組譯迴圈結構的
推論——這是本輪(從§9.25起算)唯一一次靜態+live推論之外、單純用畫面本身證實的結論。**仍未查清**:
立繪本體是否也是多幀動畫(§9.27.2兩種解讀未能區分)、飛龍騎士卡因既有卡死bug本輪仍無法截到、
`FUN_000111ba()`資源索引語意、portrait框完全未觸及。**維持D**,但範圍首次因為純視覺方法而不是
live除錯方法向前推進——下一輪如果要延續這個方向,建議:(a) 想辦法繞開悠妮卡卡死bug(可能需要
先解決那個獨立追蹤的bug,或換一個不會卡住的chapter/存檔點)才能拿到飛龍騎士卡的同等畫面;
(b) 對「分塊backdrop」這些矩形色塊的邊界座標做像素級測量,看是否等於固定的網格(例如80×64或
其他規律尺寸),若能對應到某個具體的tile尺寸常數,能進一步坐實與`FUN_0002fb2c`9輪迴圈的對應
關係。

## 9.28 2026-09-06 續十:執行 §9.27.4 建議(a)——嘗試繞開悠妮卡卡死bug以取得飛龍騎士卡——**先用
純靜態方法重新核對這個bug的根因,發現這是本專案早在§9.11.6/§9.12就已經用byte級反組譯+call_scan
窮舉法徹底封死的已知結論,本輪獨立重做只是浪費重工;接著live測試一個看似有效的按鍵,但未能重現,
誠實記錄為極可能是巧合**

### 9.28.1 靜態重核:悠妮卡卡死是`0x25462`(及`0x25970`旁的等價位址)一個無條件`EB FE`
(`JMP`自己)2-byte死迴圈,零分支、零退出條件——這在§9.11.6/§9.11.7/§9.12已經用逐byte反組譯+
`call_scan`全域窮舉掃描定案,本輪重新反組譯`0x25350-0x25462`區段獨立驗證,得到與舊記錄完全
一致的結果(一路到`CALL 0x31529`後緊接`JMP $`,中間沒有任何條件跳轉)——**這代表任何鍵盤輸入都
不可能讓CPU跳出這個迴圈,不是「還沒試過正確按鍵」的問題**。本輪犯了本專案自己反覆記錄過的錯誤
(「Check existing evidence before disasm」)——應該先`grep`既有文件確認是否已有定論,而不是
直接重新反組譯;本節作為對這個教訓的一次新鮮示範,誠實記錄。

### 9.28.2 live測試:一次「Space鍵跳出迴圈直達STAFF字幕」的觀察,連續兩輪重現嘗試均告失敗,
判定為巧合而非真實機制,不列入結論

某一輪(figani9 instance)在悠妮卡第二段文字畫面等待20秒後按一次`Space`,畫面直接跳到
「STAFF」工作人員名單畫面,跳過了飛龍騎士卡。**如果為真會是對§9.11.6結論的重大推翻**,因此
本輪立即開新instance重複驗證:(1)在悠妮卡第一段文字後直接按`Space`→只是循環到第二段文字,
無效;(2)比照原始條件(悠妮卡第二段文字+等待20秒)再按`Space`→同樣只回到第一段文字循環,
無效;(3)連續5次快速按`Space`→依然卡在原地循環,無效。**三次獨立重現嘗試全部失敗**,與
§9.28.1的靜態證明(無條件死迴圈,任何輸入都不可能生效)完全吻合。**結論**:figani9那次
「Space直達STAFF」的觀察**很可能是巧合**(例如:tmux/X11鍵盤事件佇列裡一個更早、被延遲送達的
按鍵剛好在那個時間點才被處理,或該次session本身走的是另一條code path,已無從迴溯確認)——
**不採信這個觀察,不修改§9.11.6/9.12/9.27.3已經確立的「悠妮卡死迴圈是硬性死路,無法用任何
輸入繞開」這個結論**。這也是一次「不要僅憑一次看似成功的重現就下結論,要主動嘗試reproduce
才能分辨真訊號與巧合」的具體案例。

### 9.28.3 對 `91-worklist.md` L1017-1020 的影響:飛龍騎士卡確認在目前已知路徑下無法用死亡
signature捷徑取得,§9.23.1當時是如何拿到那張畫面的仍是懸案

`docs/figures/ch27-postbattle-card-dragonrider-backdrop-midrender.png`(§9.23.1)確實存在,
證明**曾經有一次**成功推進到飛龍騎士卡,但本輪(以及§9.11.6/9.12定案的靜態證據)顯示標準的
「LOAD存檔1→軍營出口→死亡signature→postbattle」路徑會在悠妮卡遇到無法繞開的死迴圈。兩者的
矛盾**本輪未解開**——可能是§9.23.1那次用了不同的死亡signature範圍(例如涵蓋slot16-90以外的
槽位)、不同的存檔/進入路徑,或某個當時未被記錄下來的差異,目前沒有足夠資訊回溯。

**2026-09-06續十一,archival稽核找到真實殘留素材(訂正本節先前「.wsl_build不進版控、舊素材已
清除」的錯誤判斷),但結果進一步坐實悠妮卡死迴圈的普遍性,不是解答**:`git log --all`該截圖的
提交(`e6474178`,2026-08-27)本身只有commit message層級的摘要,但`.wsl_build/`底下**確實還留有
同一天(2026-08-27)另一輪獨立嘗試**的完整殘留素材(`.wsl_build/probe4_shots/`,71個檔案,含
`kill_all_montage1.sh`死亡signature腳本、`patched.SAV`/`postwin.SAV`/`postwin2.SAV`三個存檔、
`00_current.png`到`14_yuni_wait20s.png`按時間戳完整排序的截圖序列)——這是先前("§9.28.3初稿")
誤判為「早已清除」的具體反例,本輪已訂正。**但檢視這份素材本身沒有找到答案**:這個獨立session
用完全相同的死亡signature範圍(slot16-90,`0x26DF88`基底)推進,在悠妮卡試了`Up`/`Down`/`Left`/
`Right`/`Space`/`Tab`六種按鍵與20秒純等待(`13_yuni_*.png`/`14_yuni_wait20s.png`)——**序列在
這裡就結束了,沒有任何後續檔案**,代表這個獨立session也卡在同一點放棄,不是成功抵達飛龍騎士卡
的那次。`patched.SAV`這個檔名一度讓人懷疑是否曾經patch記憶體繞過死迴圈,但存檔時間
(11:04:26)**早於**卡住測試(11:14-11:15),中間只有正常的postbattle推進與存檔動作,沒有任何
patch相關的指令記錄可查——這個檔名應該只是命名巧合,不是真的做過記憶體patch。**交叉比對doc35 §9.10.5第3點
的既有記錄**(「doc58續六十二與續六十四在兩條獨立路徑——戰後真實montage/戰前幻象——都獨立卡在
悠妮卡同一組2段文字」),加上本輪找到的這個第三個獨立session也一樣卡住,**至少3個獨立session、
2種不同觸發路徑,全部收斂到同一個死迴圈**,而§9.23.1的成功是**唯一一次例外**。**誠實結論**:
飛龍騎士卡目前已知的所有archival線索都已經查過,沒有找到§9.23.1當時具體做了什麼不同操作的紀錄
——這代表它極可能是一次無法歸類的偶發成功(時序競態、或某個當時未被記錄的環境差異),不是一個
可靠、可重現的方法。下一輪如果還要繼續這個方向,需要一個本輪完全沒試過的新方法,例如:(a) 直接對`FUN_0002aa00`的角色迭代索引
下功夫,靜態反編譯找出它如何決定「這是第幾張卡」與「該卡對應哪個table_post handler」,看是否有
辦法透過SMV直接改寫這個索引跳過ch26/ch29 handler、改走一個不會撞見死迴圈的替代handler;或(b)
接受這張圖(§9.23.1既有截圖)已經是這條資產能拿到的全部live證據,不再嘗試取得新的一手畫面,
改把心力放在L1017-1020其他仍開放的子問題(`FUN_000111ba()`資源索引語意、portrait框)。**維持D**。

## 9.29 2026-09-06 續十二:執行§9.28.3建議(a)——靜態反編譯`FUN_0002aa00`的角色迭代索引機制,
**找到真正的候選機制並live驗證,結果是部分確認、部分推翻,比archival稽核更深一層的新理解**

### 9.29.1 靜態反編譯:`DAT_00053c57` 就是`if(DAT_00053c57==0)`繪卡閘門的索引本身,而這個全域
**早已是doc13/doc58廣泛記錄的「戰鬥指令環游標」共用全域**,不是montage專屬的新變數

`FUN_0002aa00`的每輪outer迴圈呼叫`FUN_0002a857()`後,只有`DAT_00053c57==0`才會執行畫卡分支
(`FUN_0002fb2c()`等)。反編譯`FUN_0002a857(param_1,param_2)`發現它本身是一個**掃描鍵盤scancode
的清單導覽迴圈**:`0x48`(↑)/`0x50`(↓)增減`DAT_00053c57`(受`param_1`上下界約束)、`0x1c`(Enter)
/`0x39`(Space)確認、`0x01`(Escape)取消——`grep`既有文件確認`DAT_00053c57`**正是**doc13/doc58
反覆記錄過的「戰鬥指令環目前反白選項」全域(doc58已知live位址`0x1EFC57`),且doc58 §5「
`DAT_00053c57`是至少3套獨立選單子系統共用的暫存全域」這條既有結論**在此獲得第4個具體案例**
(角色回顧卡viewer)——這是一個貨真價實的架構延伸,不是全新發現的機制,而是把既有的「共用UI
元件」認知擴大到本篇任務系列一直在追的montage渲染鏈。

### 9.29.2 live驗證:確認`DAT_00053c57`在卡住狀態下確實不是0(從基線0變成1),但強制寫回0
**不足以解鎖進度**——這代表某個介於「我方寫入」與「畫卡分支判斷」之間的呼叫端每輪都會覆寫它

第四個新instance在死亡signature+postbattle推進到悠妮卡「卡住」狀態前後各讀一次`0x1EFC57`:
**觸發前(戰場上)基線值=`0x00`;卡在悠妮卡時=`0x01`**——證實這個全域確實在montage序列執行
期間發生了非0改變,不是憑空猜測。嘗試直接介入:進debugger用`SMV 1EFC57 00`強制寫回0,確認
寫入成功後resume並連續送出多次confirm(單次測試+10次測試各一輪,共兩輪独立測試)——**兩輪
都沒有解鎖到第三張卡,悠妮卡持續在同一組2段文字循環**。**誠實推論**:`FUN_0002aa00`的
`if(DAT_00053c57==0)`判斷發生在**每輪outer迴圈**、在`FUN_00019953()`/`FUN_000197e5()`
(對話確認函式,doc11已知的「0x19953確認」家族)呼叫之後——若這兩個確認函式本身也是
doc58 §5提到的「3套共用子系統」之一,會在**每一輪**都把`DAT_00053c57`覆寫成非0,那麼我的
一次性SMV重置在下一輪outer迴圈開始時就已經失效,這能完整解釋「重置後短暫觀察不到效果、
連續多次confirm依然卡住」的現象。**本輪未反編譯`FUN_00019953`/`FUN_000197e5`本體去100%
坐實這個猜想**,是下一輪如果要徹底解開這個死結最直接可做的靜態工作。

### 9.29.3 對 `91-worklist.md` L1017-1020 的影響:找到一個更深層、可驗證的候選成因,但未能
轉化成實際的live workaround

**新增**:確認`DAT_00053c57`(doc13/doc58已知的指令環游標全域)是montage draw-gate的直接
控制變數,且在卡住狀態下確實偏離基線;一次性SMV重置不足以解鎖,暗示每輪都被其他共用子系統
覆寫。**這比純archival稽核(§9.28)更進一步**——從「不知道為什麼卡住」進展到「知道具體是哪個
全域、卡在什麼值、為什麼單純重置沒用」,但仍未實際解鎖第三張卡。

**2026-09-06續十三,反編譯`FUN_00019953`/`FUN_000197e5`確認寫入假說,但意外發現這個全域的
真實使用規模讓「找出真正覆寫來源」這個下一步本身不可行**:`FUN_00019953`反編譯確認**確實**
無條件在函式進入時寫`DAT_00053c57=0`,並依方向鍵(`'K'`=左0x4B→0、`'M'`=右0x4D→1)在內部迴圈
中改寫,結束迴圈的Enter/Escape/Tab分支**都沒有把它重設回0**——這與doc11既有記錄的「`[0x53c57]`
仍為0代表Yes、非0代表No」語意完全吻合,是這個全域第4個確認過的共用場景。但**新的一輪live測試
發現一個更早的訊號**:剛從萊汀卡推進到悠妮卡的**第一時間**(還沒有卡住循環,是自然的首次到達)
讀取`0x1EFC57`就已經是`0x01`,不是`0x00`——這代表出問題的時間點比預期更早,不是「重複confirm
若干次後才累積出問題」,而是**這次角色轉場過程本身**就把它寫成了1。用`xref_to`對`0x53c57`
(data位址,不是特定函式)做全域普查,回傳**209筆**READ/WRITE/READ_WRITE參照,分散在
`0x15xxx`到`0x2Bxxx`範圍內至少20+個不同函式——包含已知的`FUN_0002a857`(`0x2a8dc`寫入,對應
已反編譯過的方向鍵±1邏輯)、`FUN_00019953`(`0x19972`寫入,對應上述確認結果)之外,還有
`0x172ba`/`0x1783f`/`0x17858`/`0x17871`/`0x1788a`/`0x173f5`/`0x19dd0`/`0x19de8`/`0x1b950`/
`0x1ba5d`/`0x1baa5`/`0x1c018`/`0x1d0be`/`0x1d594`/`0x1d5dd`/`0x1508b`/`0x2600c`/`0x27b30`/
`0x26e75`/`0x26ea7`/`0x28830`/`0x293ae`/`0x29701`/`0x298e2`/`0x29ecc`/`0x2a322`共**至少26個
獨立WRITE位址**,橫跨戰鬥指令環(doc13範圍)、多個對話/選單handler(doc11「0x1956B」家族附近)、
`FUN_0002a857`/`FUN_00019953`所在的整個montage cluster——**逐一反編譯這26個寫入點、判斷哪一個
在悠妮卡轉場當下真正執行,已經超出「下一步」量級,是一個規模與本輪class-change/FIGANI靜態調查
相當的獨立普查任務**。**誠實結論**:`DAT_00053c57`覆寫假說本身已用`FUN_00019953`個案確認為
真(這個全域的行為模式完全符合猜想),但「找出這次具體是哪個寫入點造成」這個問題,範圍比預期
大一個數量級,不是本輪能收斂完成的靜態工作,需要**live執行流程回溯**(例如`D SS:ESP`從
`BPLM 1EFC57`命中回推呼叫端,重複§9.25已驗證過的方法,但這次鎖定的是一個資料全域、不需要
VGA層那套已知不可靠的delta換算)才可能有效率地縮小到具體元兇,純靜態逐點排查不划算。**維持D**。
...
## 9.30 2026-09-06 續十四:執行§9.29.3建議——`BPLM 1EFC57`活捉真正的覆寫者,**找到一個209筆
xref普查漏掉的全新寫入點,但同時證實這條調查線本身已經沒有繼續深挖的價值**

### 9.30.1 live驗證:`BPLM`(linear memory breakpoint,不需selector)成功命中兩輪,兩次都停在
同一個位址——一個209筆xref普查完全沒列出、範圍在`0x15xxx-0x2Bxxx`之外的全新寫入點

第五個新instance(`figaniF`)重複死亡signature+postbattle推進流程,這次在resume**前**先下
`BPLM 1EFC57`(對比稍早的`DAT_00053ad9`嘗試——同一手法套用在不同全域上,這是本輪才用在
`0x53c57`上的新嘗試)。第一次命中:`EIP=0x1B3407`,以本篇已多次驗證過的`0x1A0000` delta
換算回native `0x13407`,`function_bounds`確認落在`FUN_00013315`(`0x13315-0x1345f`,一個
7步驟的單位動畫/移動函式)。反編譯偽代碼本身沒有顯示`DAT_00053c57`寫入,但原始反組譯
(`tmux capture-pane`的Code Overview)在該位址附近顯示一段獨立小迴圈:

```
001B33FF EB06                jmp  short 001B3407
001B3401 FF0557FC1E00        inc  dword [001EFC57]
001B3407 833D57FC1E0004      cmp  dword [001EFC57],0004
001B340E 7D0B                jge  001B341B
001B3410 A157FC1E00          mov  eax,[001EFC57]
001B3415 833C8200            cmp  dword [edx+eax*4],0000
001B3419 75E6                jne  001B3401
001B341B C3                  ret
```

結構上與doc13已知的`FUN_000173e7`「跳過已停用選項、找第一個可用slot」慣用法完全一致(上界4、
逐格檢查`array[idx]==0`)——但這是**一個先前未被列入§9.29.3那份209筆xref/26個寫入位址普查
結果的獨立實例**:普查列出的寫入位址全部落在`0x15xxx-0x2Bxxx`,而`0x1B3401`(native `0x13401`)
明顯在這個範圍**之外**。這代表`xref_to`那輪針對`0x53c57`符號的普查方式(可能是逐個已知函式
文字比對,而非真正的全域binary掃描)本身有漏網——**是一個值得記錄的方法論落差,不是這個
全域引用規模比想像中更大的新證據**。

第二次`resume`命中後,pane capture在極短時間內連續顯示三筆事件:
`00->01`、`01->00`、`00->01`,但debugger只保留「當下」的Code Overview快照,顯示的EIP仍是
同一個`0x1B3407`(對應最後一筆`00->01`)。**這個`inc`迴圈本身只會遞增,不可能產生`01->00`
這筆事件**——代表這三連發至少牽涉兩個不同的寫入者:這個`FUN_00013315`旁的遞增迴圈(負責
`0->1`,語意上是「slot 0已停用,往後找」),以及另一個目前仍未定位、產生`1->0`的重置寫入者
(語意上最貼近已反編譯確認的`FUN_00019953`進入時無條件清零,但這次事件序列**發生順序**是
先1後0再1,不是單純的「一次性重置後被覆寫」,說明兩種寫入者在這個轉場窗口內是交錯/競爭寫入,
而非簡單的「A覆寫B」單向關係)。

### 9.30.2 誠實結論:找到了一個新的寫入點,但沒有、也不太可能靠繼續live調查解開「誰在悠妮卡
轉場當下負責寫1」這個問題——而且這個問題本身**已經不影響任何可行動的結果**,應該停止投入

三個理由組成這個結論:

1. **規模確認,不是縮小**:本輪新增的`0x13401`寫入點證明§9.29.3對「至少26個獨立WRITE位址」
   的估計其實是**低估**(至少+1,且暴露普查方法本身有系統性漏網),讓「窮舉找出真正元兇」這件事
   比先前認知的更不可行,而不是更可行。
2. **多寫入者交錯,不是單一覆寫鏈**:`00->01->00->01`這種短時間內的三連發,說明轉場當下的
   實際行為是多個共用UI元件的寫入互相打架,不是一條可以用「找到X函式、抑制它」就解開的單一
   因果鏈——即使繼續用`BPLM`+`D SS:ESP`回推每一筆事件的呼叫端(技術上可行,§9.29.3建議的
   下一步),得到的答案也會是「隨時序不同而不同的一組寫入者」,不會有一個乾淨的「就是這個
   函式造成的」結論可以拿來做live workaround。
3. **即使解開也無法改變任何可達性**:本篇§9.11.6/§9.12(早於本輪)與§9.28.1(本輪重新反組譯
   確認)已經雙重證實,悠妮卡本身卡在`0x25462`一個**無條件、零分支**的`EB FE`陷阱——不管
   `DAT_00053c57`在進入這個畫面前是0是1,一旦進入`0x25350-0x25462`這段陷阱迴圈,**沒有任何
   輸入能跳出**。也就是說,就算完整解開「誰把它寫成1」,也不會讓玩家多看到第三張卡(飛龍騎士
   卡)——那張卡的不可達性是下游、獨立、且已經數學證明過的另一個問題。繼續投入live資源去
   解`DAT_00053c57`的寫入來源,實質上是在解一個**不影響任何最終可觀察結果**的旁支問題。

**因此本輪起,`DAT_00053c57`在montage轉場中的具體寫入來源調查線正式標記為「已知有多個寫入者
交錯、規模超出單輪可窮舉範圍、且下游有獨立且已證明不可繞過的陷阱使其不再具行動意義」——
不建議未來session再投入live調查資源在這條線上,除非該陷阱本身先被推翻(目前沒有任何證據
支持這點)。**

### 9.30.3 對 `91-worklist.md` L1017-1020 的影響:飛龍騎士卡(第三張montage卡)不可達性維持
**確認、且已用兩條獨立的靜態證明鎖死**(陷阱本身零分支;進入陷阱前的游標寫入即使解開也不影響
陷阱本身無條件的事實)。此項調查**收斂關閉**,不再是D(待查)而是**確認的架構限制**——
若未來要讓玩家看到飛龍騎士卡,必須先在`0x25350-0x25462`這段陷阱本身動手(修改二進位、或
在remake重新實作時不移植這個陷阱),而不是繼續追`DAT_00053c57`的寫入來源。

## 11. `0x1c2da` = 多目標效果演出原語(2026-09-10,靜態全文反組譯)

`0x1c2da` 是 `dump_chapter_beats` 那 8 個「參數個數已 CONFIRMED、但沒有名稱」的原語之一。
先前留白的理由是**本體未展開**;本輪從 `0x22c04`(音效索引 25 的經驗值授予)那條路徑追到它,
把 `0x1c2da..0x1c421` 完整反組譯,結構已足以命名。

**簽名**:`0x1c2da(scratch, attr_index, count, unit_array)` —— 4 個參數,**2026-09-11 更正:第 2 參數不是 sfx 索引**,見 §11.1。
`derive_native_argcounts` 對 `0x1c2da` 判定 **CONFIRMED**(11 個呼叫端,cleanup 一致 `add esp,0x10`)。

**本體(依指令順序)**:

1. `0x1c2ef..0x1c2fd` 從 `0x51f15` 以 `rep movsd`×7 + `movsw` 複製 **30 bytes** 的樣板到堆疊區域緩衝。
2. `0x1c303 push [0x53b13]` / `0x1c309 call 0x25a96` —— 用**已命名的 `play_sfx`**
   (PRIM 表 `0x25a96`)從 `[0x53b13]` 這個音效池播放。`0x53b13` 正是
   `docs/data/sfx_pool_call_site_inventory.json` 清冊裡記載的 5 個池之一(16 個呼叫端)。
3. `0x1c311..0x1c333` `0x3706e(0x25680)` 配置緩衝後 `0x3771c(buf, [0x53a49], 0x25680)` 複製畫面。
4. `0x1c360` 起以 `[esp+0x44]`(= `count`)為上界迴圈,每圈:
   * `0x1c36e` 由 `[esp+0x48]`(= `unit_array`)取出第 esi 個**單位索引**;
   * `0x1c374..0x1c382` 以 `idx*0x50` 加 `[0x53a45]` 取得單位記錄(單位陣列 stride 0x50);
   * `0x1c384..0x1c38b` 讀 `unit[+0]`/`unit[+1]`/`unit[+2]`(地圖座標);
   * `0x1c38f..0x1c3c0` 對鏡頭原點 `[0x53aa9]`/`[0x53aad]` 與視野尺寸 `[0x51a87]`/`[0x51a8b]`
     做**四邊界裁切**,超出畫面就跳過該單位(四個 `jl`/`jg` 都跳回 `0x1c35f` 的 `inc esi`);
   * `0x1c3c2..0x1c3fb` 把 `(x - camX)` 乘 `0x18`、`(y - camY)` 以 shl/add 組合出列距,
     加上 `[0x53a49] + 0x8088` 得到螢幕位址;
   * `0x1c34b..0x1c357` `0x4e127(dst, ecx, 0x1c8, byte[esp+ebp])` 實際繪製。

**因此 `0x1c2da` 是「對一組目標單位播放音效並逐一疊繪效果圖」的多目標演出原語**,
與 `0x1c4cc`(16 個呼叫端、同樣的 4 參數簽名)在同一條演出路徑上並列——`0x22c04` 就是
先 `0x1c4cc(...)` 再 `0x1c2da(...)`,兩者接同一組 `(count, unit_array)`。

**誠實範圍(已於同日 §11.2 補完)**:`0x51f15` 那 30 bytes 樣板的欄位語意、`0x4e127` 的完整繪製語意、
以及 `0x1c2da` 與 `0x1c4cc` 的分工差異,本輪都未展開;此處只主張上述控制流與資料流。

## 12. `0x361b0` = 色盤淡入/停留/淡出循環(2026-09-10,靜態全文反組譯)

`0x361b0` 先前留白的理由是「文件只提到被呼叫、沒有命名」。本輪反組譯
`0x361b0..0x36213`(100 bytes),結構單純且**兩個被呼叫者都已經是本專案命名過的原語**:

```
ebx = 0
while ebx < 0x40:                 ; 64 步淡入
    0x11df2(0, 0xff, ebx)         ; palette_delta_ramp(已命名)
    0x3790a(8)                    ; delay 8ms(現行版的 delay)
    ebx++
0x3790a(0x190)                    ; 停留 400ms
ebx = 0x3e
while ebx >= 0:                   ; 63 步淡出(反向掃回)
    0x11df2(0, 0xff, ebx)
    0x3790a(8)
    ebx--
```

* `0x11df2` = `palette_delta_ramp` —— 已在 `DOC_OP_NAMES` 裡,doc50 記載它「是獨立的
  調色盤/淡變數值計算函式」;
* `0x3790a` = `delay` —— PRIM 表記載的現行參考版位址(舊版 `0x375b2`)。

**與既有 `palfade` 的關係**:PRIM 表的 `0x1f525` (`palfade`) 註解寫「無參數;delta 64→0
共 65 次 `0x11d40(0,255,delta)`,每次 delay 2ms」——那是**只有淡出**、走 `0x11d40`、
每步 2ms。`0x361b0` 是它的姊妹但不同:**淡入 + 停留 + 淡出**、走 `0x11df2`、每步 8ms。
兩者不可互相取代。

**`0x361b0` 的參數個數 = 0**:`derive_native_argcounts` 判定 CONFIRMED(7 個呼叫端);本體也不讀
`[esp+N]`,兩條證據一致。

**誠實範圍**:`0x11df2` 的三個參數 `(0, 0xff, delta)` 各自的語意由 doc50 負責,本節不重述;
~~`0x361b0` 在哪些劇情/戰鬥時機被呼叫本輪未追~~,名稱只描述機制。**(呼叫端已於同日 doc25 §20 窮舉:事件 76 的 handler 3 處、第二張跳表 index 86 與 57 各 2/1 處。)**

## 13. `0x24336` = 全螢幕逐幀動畫演出(2026-09-10,靜態全文反組譯)

`0x24336` 先前留白的理由是「全 image 只有一個呼叫端」。**那是 `argc` 判 WEAK 的理由**
(單一呼叫端的「100% 一致」是恆真的,本工具刻意降級),**不是本體讀不了的理由**。
反組譯 `0x24336..0x244b1`:

```
0x135dd(0xe, 8)                    ; pan(已命名原語)
buf = 0x3706e(0xfa00)              ; 配置 64000 bytes = 320x200 一整個畫面
0x3771c(buf, 0xa0000, 0xfa00)      ; 把 VGA 畫面(0xa0000)整份快照進 buf
res = 0x111ba(0x1a4d, 0, 0x22)     ; load_res(已命名原語)載入資源 #0x22
0x2eb9f(res, 0, buf, 0x140, -1)    ; 先畫第 0 幀
esi = 1 .. 0x44                    ; 68 幀
    0x3771c([0x53a49], buf, 0xfa00)          ; 用快照還原背景
    0x2eb9f(res, esi, [0x53a49], 0x140, -1)  ; 畫第 esi 幀
    0x3771c(0xa0000, [0x53a49], 0xfa00)      ; 送上 VGA
    0x4e31c() / 0x17aa9(3)(wait_ticks,已命名)/ 0x4e381()
0x20421(0, 0xf, 0)
0x11df2(0, 0xff, 0x3f) / delay(0x64) / 0x11df2(0, 0xff, 0) / delay(0x1f4)   ; 色盤淡變 + 停留
esi .. 0x64                        ; 第二段逐幀迴圈,結構與上面完全相同
0x3776e(buf) ; 0x3776e(res)        ; 釋放兩個指標
0x140 = 320(畫面寬)、-1 應為透明色
```

**與既有記載同構**:doc25 §2168 記載 event67 的 handler 是
「`0x111BA()`(資源載入)→ **59 次迴圈**(`0x2EB9F`+`0x17AA9`,與 §6.4 event61 用的
『resource45 59-frame 演出』同構)」——**同一個 load_res + `0x2eb9f`+`0x17aa9` 逐幀迴圈
形狀**,只是幀數與資源號不同。本函式因此不是新形狀,是同一族演出的第三個實例。

**參數個數 = 0,由本體證實**:函式範圍 `0x24336..0x244b1`(`jmp 0x22bb7` 是 Watcom
收尾),其中**沒有任何 `[esp+N]` 參數讀取**。先前誤以為有,是因為線性反組譯越過了
函式尾、讀到下一支函式(`0x244b6` 有自己的 `push 0x68; call 0x3702f` 序言)的區域緩衝。
**這是本日第三次被線性反組譯的邊界問題咬到**,處理方式同前:先定出函式尾再下結論。

### 13.1 `0x2eb9f` 在本專案有三種互不對話的記載(**2026-09-10 同輪已仲裁,見 §13.3**)

本節不順手把它定案,只把矛盾釘出來:

| 出處 | 說法 |
|---|---|
| doc25 §2168 | 逐幀演出迴圈的繪製呼叫(`0x2EB9F`+`0x17AA9` 跑 59 幀) |
| doc32 §935 | 「`0x2eb9f`(**疑似**文字/數字渲染)」——原文自己標為疑似 |
| worklist 510 | 「SFX 交錯播放(`FUN_0002eb9f`,依 `id!=9` 門檻)」 |

`0x24336` 的呼叫形狀 `(res, frame_index, dest_buffer, 0x140, -1)` 與 doc25 的讀法一致
(資源、幀號、目的緩衝、畫面寬、透明色),與「SFX」不相容。但**本輪沒有反組譯
`0x2eb9f` 本體**,所以不下定案——這與 doc32 自己對 `+0x0b`/`+0x0c` 三種命名並存的
處理方式相同:先讓矛盾可見,再另闢一輪仲裁。

### 13.2 順帶:`0x3776e` 的「釋放」讀法多了一個實例

`0x3776e` 仍是未命名原語之一,doc13 明寫「語意仍是推測,**未證實**」。本節提供一個
新的旁證而非結論:`0x24336` 在函式尾對**恰好那兩個先前取得的指標**(`0x3706e` 配置的
畫面緩衝、`0x111ba` 載入的資源)各呼叫一次 `0x3776e`,之後兩者都不再被使用——
這是教科書式的 free 形狀。仍不足以命名(單一函式的用法不能推及 205 個呼叫端),
但比「只提到被呼叫」前進了一步。

## 13.3 仲裁 `0x2eb9f`:它是「依索引取幀並貼圖」,不是文字渲染也不是 SFX

§13.1 把三種互不對話的記載釘出來後,同一輪就把它解掉了——留著一個新的未解項不算完成。
`0x2eb9f` 的本體只有 20 條指令(`0x2eb9f..0x2ebe0`):
...
| 出處 | 原記載 | 本輪判定 |
|---|---|---|
| doc25 §2168 | 逐幀演出迴圈的繪製呼叫 | **正確** |
| doc32 §935 | 「疑似文字/數字渲染」(原文自標疑似) | **否證**——它是通用影像 blit,與文字無關 |
| worklist 510 | 「SFX 交錯播放(依 `id!=9` 門檻)」 | **否證**(就函式本身而言);該處觀察到的 `id!=9` 門檻是**呼叫端**的條件,與本函式無關 |

**佐證**:`0x24336`(§13)的呼叫形狀 `(res, esi, [0x53a49], 0x140, -1)` 逐一對上——
資源容器、幀號、目的緩衝、`0x140`=320(畫面寬,即 row stride)、`-1`(透明色索引)。
全 image `0x2eb9f` 有 82 個呼叫端、`0x4e98d` 有 39 個,後者是更低階的共用貼圖。
...
**(兩項均已補完:`0x4e98d` 本體見 §13.4/§13.5;`container+8` 的表頭見 §17——
`byte[+0]` 幀數、`offs[0]==8+4N`、`+0x05..+0x07` 恆 0 皆為 264/264,
`+0x02`/`+0x03`/`+0x04` 明確不命名。)**
...
~~**誠實範圍**:`param_6 != -1` 的那兩條分支未展開。~~ **2026-09-11 已補完,見 §13.5。**

## 14. `0x1f882` = 色盤淡入,與已命名的 `palfade`(`0x1f525`)是同一對(2026-09-10)

`0x1f882` 先前留白的理由是「有合併清理風險」,後來改記為「是一支 5 條指令的 stub
(`xor ebx,ebx; jmp 0x1f51e`),本體在共用尾段」。**後者是下一步的描述,不是封鎖**——
追進去就結束了。

```
0x1f882:  push ebx ; ebx = 0 ; jmp 0x1f51e
0x1f51e:  cmp ebx, 0x40 ; jl 0x1f503        ; 迴圈條件(stub 跳進來的地方)
0x1f503:  0x11d40(0, 0xff, ebx)             ; palette_brightness_ramp(已命名)
          0x3790a(2)                        ; delay 2ms
          inc ebx
0x1f523:  pop ebx ; ret
```

**它是 `0x1f525` 的相反方向**。PRIM 表對 `0x1f525`(`palfade`)的註解寫著
「無參數;delta 64→0 共 65 次 `0x11d40(0,255,delta)`,每次 delay 2ms」,而 `0x1f882`
是 delta **0→63**、同樣每步 2ms、同樣呼叫 `0x11d40`。兩者是同一個原語的一對:

| 位址 | 方向 | delta | 每步延遲 | 名稱 |
|---|---|---|---|---|
| `0x1f882` | 淡入 | 0 → 0x3f | 2ms | `palfade_in`(本輪) |
| `0x1f525` | 淡出 | 0x40 → 0 | 2ms | `palfade`(PRIM 既有) |

**不要和 §12 的 `0x361b0` 混淆**:那一支走 `0x11df2`(`palette_delta_ramp`)、每步 8ms、
而且是「淡入 + 停留 400ms + 淡出」一整個循環。三者用的是**兩個不同的色盤函式**
(`0x11d40` vs `0x11df2`),不可互換。

**參數個數 = 0,由本體證實**:`0x1f882..0x1f524` 全程沒有讀取 `[esp+N]`,
...
## 13.5 `0x4e98d` 的 `param_6` 是繪製模式選擇器,不只是「透明色」(2026-09-11)

§13.4 把 `param_6 != -1` 的兩條分支列為未展開。補完之後發現一件比原本假設更重要的事:
**三條分支共用同一套 4 模式 RLE 文法**(控制位元組高位以 `shl cl,1 / jb` 連續判斷),
差別只在**每個像素寫什麼值**。所以 `param_6` 不是單純的「透明色索引」,而是**繪製模式**:

| `param_6` | 進入點 | 每個像素寫什麼 | 語意 |
|---|---|---|---|
| `-1` | `0x4e9dc` | 來源位元組原樣(`rep stosb` 填色 / `rep movsb` 直拷) | 一般貼圖 |
| `0x00..0xff` | `0x4ea4b` | **固定寫 `param_6` 這個顏色**(`mov al, ah`,`ah` 在 `0x4e9d8` 由 `param_6` 複製而來) | **純色剪影/圖章** |
| `> 0xff` | `0x4eac2` | `al = ((src + bh) & 7) + bl`,其中 `bx = param_6` | **色盤分組重映射** |

第三種值得展開:`bl`(低位元組)是**色盤分組基底**,`bh`(高位元組)是**組內位移**,
而 `& 7` 把來源像素夾在 **8 色一組**內。也就是同一張圖用不同的色階重畫——這正是這類
遊戲做「中毒變綠/受擊變白/隊伍配色」的標準手法。

**注意純色分支仍然要讀來源 RLE 串流**(`inc esi` / `add esi, ecx`):它需要 run 結構才知道
哪些像素要寫、哪些要跳過,只是不採用來源的顏色。因此剪影與原圖的**形狀完全一致**。

**與呼叫端對得上**:`0x2eb9f`(§13.3)固定傳 `-1`,所以逐幀動畫走的是一般貼圖;
`0x24336`(§13)透過 `0x2eb9f` 也是 `-1`。三種模式的其餘兩種由別的呼叫端使用
(`0x4e98d` 全 image 39 個呼叫端,本輪未逐一分類)。

**誠實範圍**:`bl`/`bh` 對應到哪一組實際色盤、以及「8 色一組」與 `docs/data` 既有的
色盤資料如何對應,本節不主張;只證實了算式本身。`0x4e98d` 其餘呼叫端的 `param_6`
取值分布未統計。

## 16. `0x1366a`(`act`)的三條分支與 `unit[+4]` 的語意(2026-09-11)

worklist 857 把 `0x1366a` 收斂為「依索引取表、播放 byte-pair 序列的演出 driver」,
每組 pair = (單位索引, pose),並留下兩個未解:第三分支 `0x137c7`、以及 `unit[+4]`
的完整語意。兩個都補完了。

**步頭 byte A 的三條分支**(`0x136bc test byte [esp+0x54], 0x80`):

| bit7 | 低 7 位 | 進入點 | 行為 |
|---|---|---|---|
| 1 | `== 0` | `0x136d9` | 等 1 tick → `0x11eee` 貼圖 → 逐單位套 pose → 對 `unit[+5] & 1 == 0` 者 `0x127e0` 重繪 |
| 1 | `!= 0` | `0x137c7` | 先逐單位套 pose,再以**低 7 位當 tick 數**重複「`0x11cac(0)` 重繪 + `0x17aa9(1)` 等待」 |
| 0 | (當次數) | `0x1385f` | 重複套用該步 pose,並額外寫 `unit[+4]`;見下 |

也就是 **bit7 = 保持模式、低 7 位 = 時長/重複次數**。

**`unit[+4]` = 該姿勢內的子影格索引,值域 1..6**。在 bit7=0 那條路徑:

```
[esp+0x50] = 1
每一圈:
    逐單位:byte[unit+3] = pose ; byte[unit+4] = [esp+0x50]
    若 0 < [0x53afb] < 0x40:inc [0x53afb] ; 0x11cac(1) ; 0x11d40(0,0xff,[0x53afb])  ; 同步做亮度淡變
    否則:0x11cac(0)
    0x17aa9(1) ; 0x4e381()
    inc [esp+0x50] ; [esp+0x50] >= 7 就結束     ; 故值域為 1..6
    0x32230(A[0]) ; 重置內層計數後再跑一圈
```

所以 **`unit[+3]` 選姿勢、`unit[+4]` 選該姿勢內的第幾張圖**,每個 tick 前進一格、共 6 格。
與 `reset_pose`(`0x134e4`,把所有單位的 `+3` 寫 0)一致:那支只重置姿勢,不動子影格。

### 16.1 不要與**法術/道具記錄**的 `+3`/`+4` 混淆

doc27 §278/§279 也談 `record[+3]` / `record[+4]`,但那是**法術/道具記錄**(傳給 `0x14818`
的那一個,即 `0x602ad` 表的列),語意是「選取階段範圍」/「實際生效(AoE 展開)範圍」。
**與本節的單位陣列記錄(`[0x53a45] + idx*0x50`)完全無關**——同一個偏移數字、不同的記錄
型別、不同的語意。本專案已經在 `itemRow +0x0b`/`+0x0c` 上被這種同名偏移咬過一次
(worklist 1118),所以在這裡明寫出來。

**誠實範圍(2026-09-11 已完整補完,見 §16.3;`0x32230` 的部分見 §16.2 及其訂正 §16.4)**:`[0x53afb]` 這個亮度累加
變數(`[0x53afb]`)的完整生命週期(誰初始化、誰清零)也未追,只確認它在 `(0, 0x40)` 區間內時本迴圈會
遞增它並餵給 `palette_brightness_ramp`。

## 11.1 更正 §11 的參數命名,並定出 `0x51f15` 樣板區塊的邊界(2026-09-11)

§11 把 `0x1c2da` 的第 2 參數寫成 `sfx_index`。**那是錯的**,而且錯法與同日 `0x2eb9f`
那次一樣:我從「這支會播音效、第 2 個參數是個小整數」推出它是音效索引,**沒有去看
它實際被拿去做什麼**。

**實際用法**:`0x1c2eb mov ebp, [esp+0x40]` 把第 2 參數載入 ebp,**之後全函式不再修改**;
迴圈裡 `0x1c34b movzx eax, byte ptr [esp + ebp]` 用它當索引,從堆疊上那份複製來的
30-byte 區塊取出**一個 byte**,再當作繪製呼叫 `0x4e127` 的第 4 個引數。
**而音效索引是寫死的**:`push 1; push 1; push [0x53b13]; call 0x25a96`——常數 `1`,
與參數無關。所以第 2 參數是**繪製屬性表的索引**,`attr_index`。

`0x22c04` 傳的 `0x19`(25)落在 30-byte 表內,與此一致。

**`0x51f15` 是四個連續樣板的第一個,區塊邊界可證**:

| 起點 | 長度 | 複製者 | 目的地 |
|---|---|---|---|
| `0x51f15` | 30 | `0x1c2da` | `[esp]` |
| `0x51f33` | 33 | `0x1c4cc` | `[esp+0x48]` |
| `0x51f54` | 33 | `0x1c4cc` | `[esp]` |
| `0x51f75` | 33 | `0x1c4cc` | `[esp+0x24]` |

四段**首尾相接無間隙**(30+33+33+33 = 129 = `0x81`),而 `0x51f15 + 0x81 = 0x51f96`
——doc13 §110 已獨立記載 `word_51f96` 就是每職業的 `resist_raw` 表(`resist_crit.json`)。
**所以樣板區塊的上界由一個身分獨立確立的表釘住**,與 2026-09-10 定 `0x602ad` 表邊界
用的是同一種方法。

**`0x1c2da` 與 `0x1c4cc` 的分工**:前者複製**一份** 30-byte 屬性表,做「播一次音效 +
逐目標疊繪」;後者複製**三份** 33-byte 表、先 `0x11cac(0)` 重繪再配置整頁畫面緩衝
(`0x25680` = 320×200 的兩倍多),是較重的整場演出。兩者在 `0x22c04` 裡先後被呼叫,
接同一組 `(count, unit_array)`。

**誠實範圍(已於同日 §11.2 補完:三個解出、第一個證實為死碼)**:`0x51f15` 起那四個樣板的**內容**欄位語意仍未解——只證實了它們是被複製到堆疊、
以參數索引後取單一 byte 使用的位元組表,以及區塊的起訖。這些位元組反組譯起來像
合理的 x86,但那對小型位元組表是常見的巧合,本節**不主張**它們是程式碼。

## 16.2 `0x32230`(2026-09-11)——**本節的結論已於同日 §16.4 訂正,標題保留原文以便對照**

> **勘誤**:本節把 `0x32230` 讀成「以全域影格計數 mod 6 驅動的每單位動畫步進」。展開整個函式後,
> 它其實是**腳步音效的節奏驅動**(分支終點是 `play_sfx_a`),而且模數不只有 6——依單位職業另有 4 與 9。
> 下面的序頭反組譯本身無誤,錯的是據此下的動畫解讀與那句「不是巧合」。詳見 §16.4。

§16 把 pose 迴圈每圈結束時呼叫的 `0x32230` 列為未展開。本輪展開其序頭:

```
複製 29 bytes 自 0x52725 到堆疊                  ; 又一個屬性樣板(與 §11.1 的四個同族)
eax = 0x1f183(param_1)                           ; 移動/位置相關(doc 既有記載)
if eax == 0: 走另一條路徑(0x32287)
else:
    edx = byte[0x54132]                          ; 全域影格計數
    edx %= 6                                     ; ← 6 步循環
    if edx == 0: push 1 ; push 0xa ; ...          ; 每 6 個 tick 觸發一次
    else: 0x3230f
```

~~**`% 6` 與 §16 解出的 `unit[+4]` 子影格值域 1..6 相符**——同一個 6 步節奏,一個記在
每單位記錄裡、一個由全域計數驅動。兩者是同一套動畫時脈的兩個面向,不是巧合。~~
**(已否證,見 §16.4:模數是 6/4/9 三種,由職業決定,`6` 只是其中一個。)**

`0x52725` 起那 29 bytes 是**第五個**同族屬性樣板(§11.1 記載 `0x51f15..0x51f96` 有四個);
它不在那個連續區塊內,所以樣板不只一處。

~~**誠實範圍**:`0x32230` 的兩條分支本體(`0x32287` / `0x3230f`)未展開;`[0x54132]`
這個影格計數由誰遞增未追;`0x52725` 樣板的內容語意同樣未解。~~ **三項均已於 §16.4 補完。**

## 17. `0x2eb9f` 吃的容器 header —— 三個欄位定案、三個明確**不**命名(2026-09-11)

§13 已把 `0x2eb9f` 追到「讀 header、依 `+8` 的位移表取出第 N 幀」。本輪把 header 本身的 8 個 byte
逐一對全部 264 個可解析的 FIGANI 條目(`extracted/raw/FIGANI/`,即 `0x2eb9f` 實際讀的那批容器)做全表驗證。判準沿用本 repo 的既有紀律:**要 264/264,
「大致吻合」一律不採信**(見 doc32 對 `ap` 191/215 的處理)。全程離線,只讀 `extracted/raw/`。

**樣本先說清楚**:`extracted/raw/FIGANI/` 共 409 個條目,其中 **145 個長度恰為 3 bytes**
(佔位/空條目,不含位移表),另有若干宣告幀數但表超出檔尾者;實際可解析的是 **264** 個,
下面所有分母都是這個 264。
...
|---|---|
| `byte[+0x00]` | **幀數 N** |
| `dword[+0x08 + 4i]`(i<N) | 第 i 幀的檔內位移;**嚴格遞增 264/264、全部落在檔內 264/264** |
| — | `offs[0] == 8 + 4N` **264/264**,即位移表結束處緊接第 0 幀,header 與表之間沒有空隙 |
| `byte[+0x05]`、`byte[+0x06]`、`byte[+0x07]` | 全表恆為 `0` |

**這個 header 格式是 FIGANI 專屬的,不能套到其他容器。** 同一組判準拿去跑:
DATO(136 筆)位移表單調只有 5、落在檔內 **0**;BG(46 筆)單調 0;FDSHAP(66 筆)可解析 33、
單調 33、落在檔內 **0**。三者全滅,所以「`byte[0]`=幀數 + `+8` 位移表」不是通用容器慣例。

**明確不命名的三個(記下近似值與反例,免得下一輪重跑同一批假說)**

- `byte[+0x02]`:`== byte[+0x00]` **246/264**。差的 18 筆已全部列出並看過——**其中 15 筆
  恰好就是 `byte[+0x01] == 1` 的全部 15 筆**,且這 15 筆一律 `byte[+2] < N`(差 −3..−11)。
  也就是說 `byte[+1]` 這個旗標與「`byte[+2]` 是一個比幀數小的數」幾乎同義,
  讀起來像「動畫有一段循環/子序列,長度記在 `+2`」。**但另外 3 筆破壞了雙向等價**:
  `FIGANI_059`(N=19, `+2`=7)、`FIGANI_292`(N=8, `+2`=4)兩筆 `byte[+1]==0` 卻不相等,
  `FIGANI_355`(N=10, `+2`=**18**)更是唯一 `byte[+2] > N` 的一筆。
  15/15 與 246/249 都不是 264/264,**所以不命名**;下一輪要接手,直接從這 3 個檔開始。
- `byte[+0x03]`:`== 0` **263/264**,唯一例外是 `FIGANI_013`(值 3)。一個例外的「常數欄位」
  不是常數欄位。
- `byte[+0x04]`:**不是常數**,分佈是 `{0:134, 1:13, 2:38, 3:10, 4:11, 5:18, 6:40}`,是個真欄位。
  試過的假說:`< N` **258/264**、`<= N` **261/264**,兩者都差幾筆,不採信。

**方法上的一句**:上一輪把 `+0x02..+0x07` 整段記成「假說全數失敗」,本輪重跑才發現那是把
`+2`/`+4`/`+6` 當 u16 讀的結果;改成逐 byte 之後,`+0x05..+0x07` 立刻升級成 264/264 的定案,
`+0x02` 也從「失敗」變成「有明確結構、只差 3 個反例」。**「全數失敗」有時候只是讀取寬度選錯了**,
與 doc32 那條「191/215 要去看那 24 筆」是同一個教訓的另一面。

## 16.3 `[0x53afb]` 的完整生命週期 —— 是色盤亮度淡變的步進位置,不是「演出中」旗標(2026-09-11)

§16 收尾時把 `[0x53afb]` 記為「亮度累加變數,完整生命週期(誰初始化、誰清零)未追」,doc47 的
序章 handler 表則猜它是「演出期間旗標;疑=嘴型/不可跳過,**待證**」。本輪用
`tools/disasm_le.py refs 53afb` 把它**窮舉**完:全 image 共 12 筆 fixup,一筆不多一筆不少,
分成讀取端 4 筆與寫入端 8 筆(四組成對),所以下面是完整清單而非抽樣。

**讀取端(全部在 `FUN_0001366a` = `act` 內,`0x138b8..0x138ec`)**

```
0x138b8  cmp   dword ptr [0x53afb], 0      ; 0 → 跳過(功能關閉)
0x138bf  je    0x138f1
0x138c1  cmp   dword ptr [0x53afb], 0x40   ; 0x40 → 跳過(已飽和)
0x138c8  je    0x138f1
0x138ca  push  1
0x138cc  inc   dword ptr [0x53afb]         ; 每 tick 前進一格
0x138d2  call  0x11cac                     ; 重繪
0x138da  push  dword ptr [0x53afb]
0x138e0  push  0xff
0x138e5  push  0
0x138e7  call  0x11d40                     ; = 已知的 DAC 亮度斜坡(§220 同一個呼叫形狀)
```

`0x11d40(0, 0xff, n)` 在 §220 已定案是「brightness ramp」,那裡 `n = esi*6`(0→48);這裡
`n` 就是 `[0x53afb]` 本身,值域 `1..0x40`(1..64)。所以這個變數**就是斜坡的第幾格**。

**寫入端(四處,全部是同一個慣用法:設 1 → 呼叫 `act` → 清 0)**

| 位址 | `0x1366a` 的 act id | 備註 |
|---|---|---|
| `0x23568` / `0x23586` | `0x22` | |
| `0x323e9` / `0x323fd` | `0x64` | 即 doc47 §3 序章 handler 第 7 beat 記的那一步 |
| `0x32506` / `0x32524` | `0x69` | |
| `0x327cd` / `0x327e1` | `0x62` | |

**結論**:`[0x53afb]` 沒有「全域初始化」這回事——它的生命週期完全由**呼叫端包住一次 `act`**
決定:`0` = 這次演出不做亮度淡變(絕大多數 `act` 呼叫都是這樣),`1` = 開始,
`1..0x3f` = 淡變進行中,`0x40` = 飽和後停止再呼叫 `0x11d40`。「誰初始化、誰清零」的答案是
**同一個呼叫端,在同一對指令裡**。

**一併訂正 doc47**:那張表把它猜成「嘴型/不可跳過」。同一個變數被餵進一個**已經定案**的
色盤斜坡函式,這條猜測不成立,已在 doc47 該列改掉。這又是「欄位語意由消費端決定」的一例
——doc47 只看到呼叫端把它設 1 再清 0(所以看起來像旗標),消費端 `0x138e7` 才說出它是什麼。

**與 `0x32230` 的關係**:worklist 857 另一個殘留「`0x32230` 未展開」在同日 §16.2 已補完
(全域影格計數 mod 6 驅動的每單位動畫步進),兩個殘留至此都關掉。

## 16.4 訂正 §16.2:`0x32230` 是腳步音效的節奏驅動,不是動畫步進(2026-09-11)

§16.2 只反組譯了 `0x32230` 的序頭就下了結論。把整個函式(`0x32230..0x3231a`,以
`ret` 為界)反組譯完之後,三條分支的終點**全部是 `0x32301` 的同一個
`play_sfx_a([0x53eec], index, 1)`**——這是音效,不是動畫。

```
0x32230  複製 29 bytes 自 0x52725 到堆疊          ; 職業 -> 步態表
0x32252  eax = 0x1f183(unitIdx)                   ; doc11 §265 的每單位可通行判定
0x3225c  je  0x32287                              ; eax == 0 -> 走職業表那條
         ; --- eax != 0 ---
0x3225e  edx = byte[0x54132] % 6 ; edx != 0 -> 0x3230f
0x3227e  push 1 ; push 0xa  -> 0x32301
         ; --- eax == 0(0x32287)---
0x03229b  eax = byte[unit + 0x20]                 ; 職業 ID(doc03 已由 constructor 定案)
0x0322a0  eax = byte[esp + eax - 1]               ; 查步態表,index = 職業 ID - 1
0x0322a7  gait == 0 -> % 6,  push 1 ; push 9
0x0322cb  gait == 1 -> % 4,  push 1 ; push 9
0x0322e1  否則(2/3) -> % 9,  push 1 ; push 0xb
0x32301  push [0x53eec] ; call 0x25a96             ; play_sfx_a(UI 池, index, priority=1)
0x3230f  inc byte[0x54132]                         ; 三條路徑都會走到
0x3231a  ret
```

`0x25a96` = `play_sfx_a(table, index, priority)`(doc36 §211/§227),`[0x53eec]` = UI 音效池
(doc36 §237)。三個 index `9 / 0xa / 0xb` 相鄰,符合「同一組腳步音」的形狀。

**因此 §16.2 那句「`% 6` 與 `unit[+4]` 的 1..6 相符,兩者是同一套動畫時脈的兩個面向,
不是巧合」是錯的。** 模數有三種——`6`、`4`、`9`——由職業的步態值決定;一個「6 步動畫時脈」
的解讀容納不了 4 與 9。當時只看到序頭那個 `% 6`,就把它接到手邊另一個剛好也是 6 的數字上。
**兩個數字相等不是機制,共同的消費端才是**(同 doc32 那條「欄位語意由消費端決定」)。

**§16.2 留下的三個「誠實範圍」一併補完:**

1. **兩條分支本體**:如上,`0x32287` 是職業查表那條,`0x3230f` 只是共同的
   「遞增計數 + 返回」尾巴,不是第三種行為。
2. **`[0x54132]` 由誰遞增**:`0x3230f` 自己。`tools/disasm_le.py refs 54132` 對全 image
   窮舉共 **5 筆,全部落在 `0x32230` 的函式體內**(`0x32261`/`0x322ac`/`0x322d3`/`0x322e4`
   讀、`0x32311` 寫),沒有第六處。所以它雖然放在全域空間,實際上是**這個函式私有的相位計數**;
   §16.2 稱它「全域影格計數」會讓人以為別處也在驅動它,措辭應收斂為「本函式私有的節奏相位」。
3. **`0x52725` 那 29 bytes 的內容語意**:是**職業 -> 步態**對照表,索引 = 職業 ID − 1。內容:

   ```
   01 01 02 01 00 00 01 01  01 01 02 01 00 00 03 01
   01 01 03 01 00 00 01 01  01 01 01 01 01
   ```

   值域 `{0,1,2,3}`,但程式只分辨 `0` / `1` / 其他,所以 `2` 與 `3` 行為相同(都是 `% 9` + 音效 `0xb`)。
   前 24 個 byte 呈明顯的 8 週期(每組第 5、6 個都是 `00 00`),與職業依種族分組的排法一致——
   **但這只是形狀觀察:`0x32230` 這個唯一的消費端只讀 `byte[esp + 職業ID - 1]` 單格,從不以 8 為步幅索引,所以不宣稱職業分組就是這樣切的**。

§16.2 的序頭反組譯本身沒有錯,錯的是「只讀到分支點就替整個函式命名」。這一節保留原文並標註,
不刪除,以便對照這個錯誤的形狀。

## 11.2 `0x51f15` 起四個樣板的欄位語意 —— 三個解出、第一個是**死的**(2026-09-11)

§11.1 定出了四個樣板的邊界,誠實範圍留著「**內容**欄位語意仍未解」(這句話在本檔 §11.1 與
§16 各出現一次)。本輪用今天對 `0x52725` 那張職業→步態表同一套做法解完:**先找誰讀它、
讀去做什麼**,不看位元組長相。

### 11.2.1 第一個樣板 `0x51f15` 的值從來沒被用到

鏈路可以**完全列舉**,不是抽樣:

- `tools/disasm_le.py refs 51f15` 對全 image 只有 **1 筆**(`0x1c2f7`,`0x1c2da` 裡那次 30-byte 複製)。
- `0x1c2da` 內 `ebp`(= 第 2 參數)只在 **1 個位置**被用到:`0x1c34b movzx eax, byte ptr [esp + ebp]`,
  之後 `0x1c4c7 pop ebp`。
- 那個 byte 的唯一去向是 `0x1c34f push eax`,即 `0x4e127` 的**第 4 個引數**。

而 **`0x4e127` 只讀三個引數**。逐位元組核對函式體 `0x4e127..0x4e1a5`,只出現
`[ebp+8]`(來源)、`[ebp+0xc]`(目的)、`[ebp+0x10]`(讀兩次),**沒有任何一處讀 `[ebp+0x14]`**;
Ghidra 獨立推得的簽名也是 `FUN_0004e127(byte *param_1, undefined1 *param_2, uint param_3)`
——三個參數。兩個呼叫端(`0x1c357`、`0x1db58`,`call_scan` 全 image 掃出就這兩個)都
`add esp, 0x10`(推四個),所以這是穩定的呼叫慣例落差,不是單一寫錯。

**結論**:`0x51f15` 那 30 個 byte 在執行期**沒有任何效果**,`0x1c2da` 的第 2 參數也因此是**惰性的**
——它選出一個 byte,而那個 byte 被丟掉。§11.1 把該參數從 `sfx_index` 更名為 `attr_index` 仍然
描述對了機制(它確實是屬性表索引),但**不應再期待它有可觀察的行為**;重製時可以直接忽略。

**順帶定出實際的填色值**:`0x4e127` 的繪製迴圈是
`lodsb`(讀來源像素)→ `mov al, ah`(**丟掉來源值**)→ `stosb`,即**剪影填色**;
`ah` 來自 `(param_3 & 0xff)`,而 `param_3` 同時被 `sub edx, 0x18` 當成列距。
`0x1c2da` 的呼叫端傳 `0x1c8`(456,即 §18.1 的 work buffer stride),所以填色值恆為 `0xc8`(200)。
`0x4e1a6` 是同一個迴圈的孿生版本,填色寫死 `7`。這與 doc10 記的「受傷瞬間用 silhouette 填白」
是同一個機制,只是顏色來源終於釘死了。

### 11.2.2 另外三個樣板由 `0x1c4cc` 讀,而且全部解得出來

`0x1c4cc` 的 `ebp` 同樣是第 2 參數(`0x1c4dd mov ebp,[esp+0x8c]`,換算後即 `[entry+8]`),
三份複本的落點由複製碼直接讀出:`[esp+0x48]`←`0x51f33`、`[esp]`←`0x51f54`、`[esp+0x24]`←`0x51f75`,
各 33 bytes。全函式對這三塊只有四個讀取點:

| 讀取 | 樣板 | 消費端做什麼 | 定名 |
|---|---|---|---|
| `0x1c67b` `[esp+ebp+0x48]` | `0x51f33` | `eax += edi`(幀序)→ `[[0x53ad1] + eax*4 + 6]` 取資源指標 | **起始資源索引** |
| `0x1c66f` `[esp+ebp]` | `0x51f54` | `cmp edi, eax ; jge` → 迴圈上界 | **幀數** |
| `0x1c647` `[esp+ebp+0x24]` | `0x51f75` | `cmp ..., 0 ; je` → 跳過整段音效 | **音效閘(非 0 才播)** |
| `0x1c650` `[esp+ebp+0x28]` | `0x51f75` +4 | `play_sfx_a([0x53b13], 該值, 1)` | **音效索引** |

**「起始資源索引 + 幀數」有一個決定性的結構佐證**:把 `0x51f33`/`0x51f54` 兩張表 33 個 attr 的 `(base, count)` 去重後
得到 **14 個相異區間**,依 base 排序後**互相重疊 0 個**,涵蓋 `49..229`(只有 `74..75`、
`114..125` 兩個缺口),而且多數是首尾相接的。14 個隨機區間互不重疊的機率極低——
這不是「看起來像」,是資料自己撐住了這個解讀。另外 `count == 0` 的兩個 attr(30/31)
其 `base` 也同時為 0,是一致的「無動畫」編碼。

**一個如實記下、不解釋的觀察**:音效索引是從**同一張 33-byte 表往後 4 個 byte**讀的
(`t4[attr+4]`,不是 `t4[attr]`)。後果是某個 attr 的音效索引恰好等於**下一組**的閘值;
而且 `attr >= 29` 時這個讀取會越過該表尾端、落進相鄰的 `0x51f33` 複本。這兩點都是
逐指令觀察到的事實,**本節不主張它是刻意設計或原版缺陷**——要分辨這兩者需要知道
attr 的實際值域,而那要另外追 `0x1c4cc` 的呼叫端,本輪未做。
