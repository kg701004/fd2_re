...
### 9.26.3 誠實範圍:**尚不能斷言** §9.26.2 的額外呼叫就是 FIGANI 立繪本體
理由:(a) `DAT_0005413f`(3 槽之一)實際指向什麼資源(TAI/FIGANI/FDOTHER 哪一種容器)本輪未查證
——只確認它是`FUN_000111ba()`的回傳值,這個函式本身的語意(依什麼索引取什麼資源)未展開;
(b) 9 輪迴圈本身也可能就是在畫立繪的分層/漸層效果(例如逐層淡入),不能單純假設「迴圈=backdrop、
額外一次=立繪」這個二分法一定成立;(c) 沒有 live BPPM 交叉核對這個額外呼叫瞬間的 VGA 命中座標
是否精確對應立繪範圍(§9.25 已建立的 VGA 座標校準可以直接沿用,只是本輪沒有重新開 live
instance)。**下一輪如果要真正關閉立繪本體這個問題,應該**:(1) 對 native `0x2fd31` 這個額外呼叫
點下 live 斷點(`BP 0170:<0x2fd31對應的live位址,需重新用D SS:ESP或已知anchor校準>`),配合
§9.25.3 已驗證可跨session沿用的 VGA viewport 校準,直接核對命中瞬間的 VGA 座標是否落在立繪範圍
內;(2) 反編譯 `FUN_000111ba()`,搞清楚它依什麼索引取得哪個資源控制碼,才能知道 `DAT_0005413f`
究竟是哪個資產類別。
