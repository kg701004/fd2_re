...
全域、卡在什麼值、為什麼單純重置沒用」,但仍未實際解鎖第三張卡。**仍待驗證**:反編譯
`FUN_00019953`/`FUN_000197e5`確認是否真的覆寫`DAT_00053c57`;若確認,下一步是用`BPPM`盯住
`0x1EFC57`本身(不是VGA/work buffer,這類全域位址不需要delta換算,理論上比§9.26.4系列的VGA
層BPPM更可靠),抓出真正的覆寫來源,再決定是否要在**每輪**outer迴圈的關鍵時間點重複寫入0
(而不是只寫一次)來真正測試這個修復假說。**維持D**。
