...
方法)。**仍未查清**:`FUN_000111ba()`的資源索引語意、`FUN_0002fb2c`額外呼叫的內容仍待 work
buffer 層或單步交叉核對、portrait框完全未觸及。**維持D**,範圍已收斂到「1個具體候選函式+
1個已知不可行的驗證路徑(VGA層BPPM)+2個待試的替代驗證路徑(work buffer層BPPM、單步)」。
