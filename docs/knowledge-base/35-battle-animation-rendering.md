...
「像素層」直接證據);(c) `local_14>=0x32` 分流的 `FUN_0002aedb`/`FUN_0001b8e7` 這條特殊分支
(飛龍騎士卡)完全未展開;(d) portrait 框(左上角小頭像)完全未觸及,本輪只追了立繪本體。
**維持 D**,但範圍已從「完全未知從哪裡下手」收斂到「已知編排層 3 個具體位址,下一輪從
`DAT_000615fe` dump + `unit_record+0x20` 的讀取端 xref 兩個方向continue即可,不需要重新從零
摸索」。
