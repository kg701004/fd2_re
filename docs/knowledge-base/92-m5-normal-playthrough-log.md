...
## 2026-09-02 續十五：用戶要求「請繼續測試」——首次用真正的中斷點攔截「按上+Enter」，確認Attack分支真的有被執行到候選掃描

先派Ghidra headless agent完整反組譯`0x18d8c`(指令環本體)跟`0x18890`(外層dispatcher)：確認這不是
jump table，是對全域`DAT_00053c57`(0=攻擊/1=法術/2=道具/其他=待機)的if-else鏈，Attack分支入口
`0x18f2f`，完整呼叫鏈是`0x3706e`(暫存配置)→`0x14818`(候選掃描)→**`0x115b6`(目標確認迴圈，真正
會擋在這裡讀鍵的地方)**→確認後才到`0x2e2b0`(真正的傷害/命中orchestrator)。Wait分支(`0x19077`)
只有3個呼叫，遠比Attack簡單。

**Live執行**：新開`bpchase`，重演到部署，移動亞雷斯到敵人旁邊(ring開啟)。用新工具`mem
find-signature`重新校準這次的delta(**第4次獨立確認`0x19c000`**)，算出`0x18f2f`/`0x14818`/
`0x115b6`三個live位址，用`debugger-cmd "BP ..."`各設一個中斷點，`BPLIST`確認3個都設好。`resume`
恢復執行，按「上」(選攻擊)+`Enter`(確認)——**EIP精確落在`0x1B0818`(候選掃描函式`0x14818`的live
位址)，不是巧合，是真正的中斷點命中**：這是整個Attack調查以來第一次用「攔截執行中的CPU」而非
「事後比對記憶體快照」的方式，直接證實**Attack分支真的被dispatch、真的執行到候選掃描這一步**，
不是卡在更早的地方沒被呼叫到。

**過程中意外撞到並修好一個`resume`工具本身的真bug**：續十七剛寫好的`resume`指令這次連續呼叫兩次，
結果送出的兩次`RUN`文字疊在debugger console同一行還沒送出(`I-> U 0170:1B4F2FRUNRUN_`)，導致
`resume`誤以為恢復了但其實遊戲還在暫停——手動清行(Ctrl+U)+改用tmux具名Enter鍵才恢復。**但這極可能
是這個專案本身反覆撞過的tmux/xdotool按鍵時序既有問題(doc58續七十~續七十七)，不是這個送法本身
的缺陷**(`dosbox_harness.sh`的`cmd_debugger_cmd`整個session都用完全相同的送法沒出過問題)——因此
沒有貿然改成具名Enter鍵，而是採用這個工具箱一貫的「settle/verify」哲學：`cmd_resume`(.sh)送RUN
前先清行(Ctrl+U)去掉任何殘留輸入；`resume --verify`(.py)從單純警告改成**自動重試最多3次**，把
偶發的按鍵時序問題從「需要人工介入」變成「大多數時候自己就恢復」。修好後live重新驗證：同一個
instance上`resume --verify`第一次呼叫就正確回報`OK...attempt 1/3`。

**後續嘗試**（誠實記錄，沒有完全解開）：修好resume後繼續嘗試追蹤到`0x115b6`——但發現候選掃描
`0x14818`只在「真的按下確認鍵那一刻」執行一次，不是持續空轉的迴圈（連續截圖比對確認遊戲在兩次
按鍵之間是真的在跑，不是卡在中斷點反覆命中），這代表要抓到`0x115b6`需要更精細的「每按一鍵就立刻
檢查一次」節奏，而不是這次這種夾雜大量除錯器手動操作、時序被打亂的嘗試——第二次補按Enter後直接
跳到裝備狀態卡（回到已知的卡住畫面），沒有再命中任何中斷點，這輪沒能追到`0x115b6`內部實際發生
了什麼。**用`mem read-unit-array`在卡住當下重新讀一次亞雷斯的記錄**：camp/acted/gate3三個gate
依然全部「通過」，跟續十六完全一致，再次獨立確認gate3假說不成立。

**結論**：本輪最大的價值是方法論(第一次證實斷點式追蹤在這個環境可行、抓到真正的執行點)跟一個
真的工具修好，而不是解開最終謎團——`0x115b6`內部到底為什麼沒有讓亞雷斯進入真正的目標選擇畫面，
仍待下一輪用更嚴謹的「單鍵單檢查」節奏重試，這次已經證明中斷點技巧本身是可行的，下一輪可以直接
沿用，不需要重新驗證delta/位址正確性。

**清理**：`bpchase`已`teardown`，`status`確認清空。程式碼異動：`fd2_dosbox_live_helper.sh`/`.py`
的`resume`可靠性修法（見doc98對應章節）。

## 2026-09-02 續十六：用戶要求「自動繼續進行後續所有測試」——重大轉折：確認裝備卡完全不經過doc13記載的任何一個已知函式

延續續十五的斷點技巧，這次改成更精確的節奏：新開`tracechase`，重演到部署，移動亞雷斯到敵人旁邊，
用`mem find-signature`重新校準delta（**第5次獨立確認`0x19c000`**）。逐步擴大中斷點覆蓋範圍，
每加一個就完整重測一次「按Enter確認攻擊」，全程用screenshot-diff(而非tmux pane文字)判斷真假暫停：

1. `0x18f6f`(候選掃描`0x14818`呼叫後的下一行，正常return必經之路)、`0x115b6`(目標確認迴圈入口)、
   `0x18f97`(目標確認呼叫後下一行)——**三個都沒命中**，畫面直接跳到裝備卡。
2. 直接讀`DAT_00053c57`(ring選取哪個方向的全域)：按鍵前就已經是`0`(攻擊)，排除「選錯方向」。
3. 加回`0x14818`本身的入口(`0x1B0818`，續十五確實命中過的那個)——**這次沒命中**。
4. 加`0x18d8c`本體最開頭讀`DAT_00053c57`的那一行(`0x18f22`，指令環真正的起點)——**沒命中**。
5. 加`0x17aed`(doc13記載的「非互動替代畫面」)——**沒命中**。
6. 加`0x117e7`(整個系統最外層的entry-gate/dispatch函式)——**沒命中**。

**連續4次獨立的「按Enter→畫面跳裝備卡」重現，全程搭配這6個涵蓋doc13整條已知呼叫鏈(從最外層
entry gate到指令環本體到候選掃描到目標確認到non-interactive fallback)的中斷點，沒有一次命中
任何一個**。這不是「某個分支沒走到」，是**doc13記載的整套battle-menu-system(`0x117e7`/
`0x18890`/`0x18d8c`/`0x17aed`)在這個「按Enter跳裝備卡」的瞬間完全沒有被呼叫**。

**結論(推翻續十五的暫時性解讀，這是本輪最重要的修正)**：續十五那一次在`0x1B0818`(`0x14818`入口)
的命中，現在看來很可能是**單一、當時情境特有**的結果(例如那次剛好走了不同的cursor/選取路徑)，
不能代表「攻擊分支正常dispatch」是這個「按Enter跳裝備卡」現象的常態解釋——本輪用更廣、更系統性
的中斷點覆蓋，四次獨立重現全部指向同一個結論：**這個裝備卡畫面，是doc13完全沒有記載過的另一條
程式碼路徑**，不是`0x117e7`三個gate中任何一個失敗導致的`0x17aed`替代畫面，也不是`0x18d8c`指令環
內部某個分支的邏輯錯誤。

**新的候選方向(未驗證，留給下一輪)**：
1. 這整個「部署階段」可能是完全獨立於doc13記載的正式戰鬥指令系統之外的另一套UI/輸入處理——doc92
   Round 1最早記錄的「自動逐一顯示各隊員完整狀態卡」本身就暗示這是一個獨立的、可能是純劇本/計時
   驅動的展示機制，我按的Enter鍵未必真的被這套機制當成「選擇動作」在處理。
2. 需要找到「按Enter之後、畫面變成裝備卡之前」真正被呼叫的函式——這需要用一個更暴力但更可靠的
   方法：對整個code segment設一個範圍很寬的中斷點掃描（例如每隔0x1000 bytes設一個臨時斷點，
   逐步縮小範圍），或者換一個完全不同的抓取角度：不追code位址，改追「裝備卡本身的繪製函式」
   （已知的道具面板渲染函式`RenderNativeItemPanelResources`對應的native位址，這個repo的
   `remake/internal/battle`底下應該有線索可查，不需要從零反組譯）。
3. 索爾/悠妮的「待機成功」是否也繞過了doc13記載的系統，只是because Wait剛好走到了某個共用的、
   非ring-specific的成功路徑——這一點本輪沒有機會驗證(嘗試導覽到索爾時畫面卡住不動，可能是這個
   instance在大量手動debugger操作後input出狀況，非遊戲邏輯問題，已捨棄該instance)，是下一輪
   應該優先補上的對照組。

**清理**：`tracechase`已`teardown`，`status`確認清空。本輪沒有程式碼異動，純調查+文件記錄。

## 2026-09-02 續十七：用戶要求「繼續深挖」——決定性對照組實驗，證實斷點方法論完全正確、亞雷斯的異常是真的

派研究agent查`remake/internal/battle/native_item_panel_data_render.go`：確認裝備狀態卡是由
`0x17aed`(整個642-byte的非互動orchestrator，doc13已完整反組譯)組合`0x17eef`(面板/立繪版面)+
`0x17fc0`(數值/名稱覆蓋層)+`0x184c0`(裝備清單)渲染而成，而且**找到doc13標註「無關」但從沒測過的
第三個呼叫來源`0x29656`**——這是續十六唯一還沒覆蓋到的候選缺口。

**Live執行(`controltest`)**：重演到部署，這次改成對**索爾的待機**(已知成功案例)做對照組測試，
用同一套delta校準(**第6次獨立確認`0x19c000`**)，設5個中斷點：`0x117e7`、`0x18f22`(指令環本體
第一行)、`0x19077`(待機分支入口)、`0x17aed`、新找到的`0x29656`。索爾選中→開ring→按「下」
(待機)+Enter——

**EIP精確落在`0x1B4F22`，正是`0x18f22`（`0x18d8c`指令環本體的第一行指令）**——這是本次投入這麼
多輪斷點式追蹤以來，第一次同時滿足「真的命中」+「對照組本身是已知成功案例」兩個條件的乾淨結果。
繼續resume，索爾正確變灰階（待機成功執行），全程用screenshot-diff跟`resume --verify`的自動重試
機制確認每一步真假暫停，沒有依賴tmux pane文字。

**結論(本輪最重要的方法論確認)**：**索爾的合法待機操作，確定會經過`0x18d8c`這個記錄在案的函式，
而且斷點在正確的位址上真的抓得到**——這代表續十六「亞雷斯的Attack完全不經過doc13記載的任何已知
函式」不是delta算錯、位址算錯，或中斷點技巧本身有問題，是一個**真實、已經雙重驗證過的異常**：
同一套方法論、同一個delta、同一批位址，對索爾能抓到、對亞雷斯抓不到。

**尚待驗證**：新找到的`0x29656`(第三個`0x17aed`呼叫來源)這次也設了中斷點，但這輪的操作重心在
索爾對照組上，沒有機會針對它重新對亞雷斯測一次——這是下一輪最值得優先驗證的候選，如果`0x29656`
在亞雷斯的案例被命中，就直接解開「裝備卡到底從哪裡來」這個續十六留下的核心懸案。

**清理**：`controltest`已`teardown`，`status`確認清空。本輪沒有程式碼異動，純調查+文件記錄，但這是
整個Attack調查(續九~續十七)第一次拿到「方法論被獨立驗證為正確」的堅實結論。

## 2026-09-02 續十八：用戶要求「繼續深挖」——測試續十七留下的`0x29656`候選，過程中意外發現移動流程
本身操作有誤，修正後首次觀察到亞雷斯的ring真的命中`0x18f22`（doc13已知函式），部分推翻續十六

**背景**：本輪一開始是要直接執行續十七留下的任務——對亞雷斯測`0x29656`。但重新開一個全新instance
(`aresleadtest`，因為上一輪的`aresleadtest`背景進程已經因為session邊界而消失，且發現WSL預設
distro被切成`kali-linux`，实际harness環境在`Ubuntu`，這次都用`wsl -d Ubuntu`明確指定)、重演部署後，
**發現先前所有輪次(續一~續十七)的「Enter→方向鍵→Enter確認移動」操作，在這次逐鍵+screenshot確認
下，其實有一步走錯**：

1. 游標移到亞雷斯身上、按Enter：畫面顯示「亞雷斯 LV.01 HP048 MP000」的資訊卡(這其實同時已經是
   移動預覽游標的啟動狀態，不是需要另外dismiss的獨立畫面)。
2. **直接接著送方向鍵(不要再按第二次Enter)**：這次確認方向鍵真的會移動一個獨立的「目的地預覽
   游標」(地形有淡淡的範圍高亮)，跟純瀏覽游標視覺上很像但語意不同。
3. 移到敵人相鄰格、按Enter確認：**這次亞雷斯真的移動了**(先前很多輪因為多按了一次不必要的
   Enter、或送出方向鍵前後間隔太久，導致這個「目的地預覽」狀態提前跳回純瀏覽模式，方向鍵變成
   移動普通瀏覽游標，最後的Enter落在空地上觸發「要結束本回合的行動嗎?」而非確認移動——這個
   誤觸的YES/NO對話框本身也是本輪意外發現，已用Escape安全取消，確認「那麼就不要了」的取消文字)。
   移動成功後，ring**正確自動在新位置開啟**，四圖示：上(紅/劍)、左(紅/杖)、右(藍/袋)、下(藍/甲+2)。

**斷點設置**：用doc58續十三delta校準法（**第7次獨立確認delta=0x19C000**，signature在DS選擇子`0178`
下、linear`0x100000`起找到唯一命中，offset`0xAD912`），設4個斷點：`0x117e7`→`0x1AD7E7`、
`0x18f22`→`0x1B4F22`、`0x17aed`→`0x1B3AED`、`0x29656`→`0x1C5656`(過程中也踩到一個純操作失誤：
python工具`mem find-signature`的CLI慣例是**不帶`0x`前綴**的裸十六進位字串，帶了`0x`前綴會讓
`MEMDUMPBIN`拿到無法解析的參數、靜默產生0-byte檔案——不是工具bug，是本輪自己沒看`--help`範例)。

**★ 核心結果**：resume後對亞雷斯送「上(攻擊方向)+Enter」，screenshot-diff確認真的暫停了——
**EIP精確落在`0x1B4F22`，正是`0x18f22`(`0x18d8c`指令環本體第一行)**。這是整個Attack調查
(續九~續十八)**第一次**在亞雷斯身上真正命中doc13記載的任何一個已知函式(先前續十六用4個獨立
重現、6個涵蓋全鏈路的斷點，一次都沒命中過)。`0x29656`、`0x117e7`、`0x17aed`三個這次都沒命中。

繼續resume後，畫面从「上」按下的狀態變成一個**新的4圖示子選單**(頭盔/上半身盔甲/全身盔甲人形/
神殿柱子圖示，配色全藍，跟外層紅/藍雙色的主選單視覺上明顯不同)，看起來像裝備欄位選擇畫面。
但緊接著再次resume+screenshot，畫面又變回外層的4圖示主選單(紅/紅/藍/藍)——這個子選單狀態
一閃即逝，用`DAT_00053c57`(`0x1efc57`)讀值嘗試核對「上」對應的ringSel index，**兩次讀到的值都是
`0x02`且送出Left後值沒變**，不足以確認語意，判斷是本輪還沒抓對觸發子選單重繪的確切時間點，
或這個global不是這個特定ring instance在用的選擇變數——**這部分留待下一輪用更密的斷點/截圖節奏
釐清，不勉強下結論**。

**結論(修正續十六，但不是全盤推翻)**：
1. **續十六「亞雷斯的Attack完全不經過doc13記載的任何已知函式」這個結論不成立**——本輪用修正過
   的、真正驗證過的移動流程，亞雷斯的「上+Enter」確實會進入`0x18d8c`(經`0x18f22`確認)。續十六
   4次重現全部沒命中的真正原因，本輪懷疑(未證實)是續十六的移動流程本身可能也踩到同一個
   「移動預覽狀態提前逾時→方向鍵變成瀏覽游標→最後Enter沒有真的確認移動」陷阱，導致「移動」
   從未真正發生、ring也從未在新位置真正打開過，所以自然不會進到任何後續函式——這跟續十/續十二
   反覆記錄的「移動後才會卡」現象並不矛盾(因為續十/續十二用迷你狀態卡等其他方式間接確認過確實
   移動成功)，但續十六本身沒有針對「這次移動是否真的生效」做同等嚴格的確認，值得懷疑。
2. **「裝備卡到底從哪裡來」依然沒有最終答案**——這次雖然命中了`0x18f22`，但後續進入的畫面
   (子選單、又快速跳回主選單)跟先前續十/續十二/續十四記錄的「完整全螢幕HP/AP/裝備清單卡、
   Enter完全沒反應、只有Escape能退」不是同一個症狀——**這代表本輪根本沒有重現出先前那個
   「真正卡住」的bug**，只是走到了一個目前還看不懂、但至少是真實doc13函式覆蓋範圍內的畫面。
3. 下一輪的優先任務：(a)用這輪修正過的、確認可靠的移動+ring操作流程，刻意重新嘗試多次「上/左/
   右/下」+Enter，看能不能真正重現續十/續十二那個「完整卡死、Enter沒反應」的畫面，還是這個bug
   本身在「移動流程被走對」的前提下根本不會發生(這將是對整個投資這麼多輪的調查最根本的一次
   證偽測試)；(b)如果卡死畫面仍可重現，這次要在**卡死的那一刻**(不是攻擊按下的瞬間)重新設斷點，
   因為本輪證實了`0x18d8c`本身沒問題，問題(如果還存在)一定在`0x18d8c`往後的某個分支。

**清理**：`aresleadtest`已`teardown`，`status`確認清空。本輪程式碼異動：無。文件異動：本節。

## 2026-09-02 續十九：直接執行續十八留下的證偽測試——「卡死」bug真的重現了，而且斷點直接抓到
組成畫面的兩個子函式，同時排除了`0x17aed`本體與`0x29656`

新開`stuckretest`，重演到部署，用續十八修正過、確認可靠的流程移動亞雷斯到敵人相鄰格(這次一次
`send-keys`批次送出`Return Right Right Down Return`就順利成功，ring正確在新位置自動開啟，證實
這個修正過的流程本身穩定可重複，不需要逐鍵screenshot才能成功)。

**★ 卡死bug確認真的重現**：按「上」+Enter，畫面先短暫顯示一個全藍4圖示子選單(頭盔/上半身盔甲/
全身盔甲人形/神殿柱子)，**再按一次Enter**，畫面變成續十/續十二/續十四記錄過的那張完整全螢幕卡：
「亞雷斯／人類 騎士／LV.01／HP048/048／MP000/000／DX002／MV07／HIT092／AP026／EV002／DP006／
刺矛+AP020／布衣+DP002」——數值與續一/續十完全吻合。**再按第三次Enter：畫面完全沒反應，
確認卡死**。這是本次整個調查第一次在「確認移動流程本身正確」的前提下，乾淨地重現原始bug，
直接證偽了續十八結尾提出的「bug可能只是移動流程操作錯誤的副產物」這個猜測——**bug是真的，
不是操作誤差**。

**斷點定位「卡死那一刻」**：Escape退回全藍子選單(確認Escape仍然有效)，進debugger，delta校準
(**第8次獨立確認`0x19C000`**，signature在linear`0x100000`唯一命中於offset`0xAD912`)，這次除了
沿用的4個(`0x117e7`/`0x18f22`/`0x17aed`/`0x29656`)，新增5個瞄準續十七研究agent找到的
「`0x17aed`組合`0x17eef`(面板/立繪版面)+`0x17fc0`(數值/名稱覆蓋層)+`0x184c0`(裝備清單)」這三個
子函式、以及`0x18f2f`(Round19已知的Attack分支入口)、`0x14818`(候選掃描)——共9個斷點，覆蓋
doc13已知鏈路的幾乎所有節點。resume後，這次改成直接對著卡死畫面前那個關鍵Enter(全藍子選單
→完整卡死卡的那一次)下手：

1. 第一次resume後送Enter，screenshot-diff確認真暫停，**EIP精確落在`0x1B3EEF`＝`0x17eef`
   (面板/立繪版面子函式)本身的第一行**——這是全新結果，先前沒有任何一輪命中過這個位址。
2. 繼續resume，**EIP落在`0x1B44C0`＝`0x184c0`(裝備清單子函式)第一行**——第二個子函式也命中。
3. 再繼續resume，EIP只前進到`0x1B44C5`(`0x184c0`內部+5 bytes，不是新斷點，可能是`resume`
   工具的screenshot-diff把某個動畫幀誤判成「真的還在跑」提前又暫停，沒有深究)。

**★ 核心結論(這是本輪最重要的發現)**：`0x17eef`與`0x184c0`——`0x17aed`所組合的三個子函式中的
兩個——**確實被呼叫，且順序符合續十七研究agent的分析**，這證實「卡死卡」畫面的內容真的是
`0x17aed`那套非互動替代演出的渲染結果。但**`0x17aed`自己的入口斷點(`0x1B3AED`)、`0x117e7`、
`0x18f22`、`0x18f2f`、`0x14818`、`0x29656`全部沒有命中一次**——換句話說：**畫面內容是`0x17aed`
的產物，但呼叫路徑不經過`0x17aed`自己的已知入口位址，也不經過doc13記載的任何其他候選(含續十七
新找到、這兩輪都測過的`0x29656`)**。這代表存在一個**doc13、續十七研究agent都沒找到的第四條
呼叫路徑**，直接呼叫`0x17eef`/`0x184c0`(可能跳過`0x17fc0`，這次沒命中，也可能只是這條路徑
不需要那個子步驟)，繞過`0x17aed`本體。

嘗試用`MEMDUMPBIN`讀取`ESP`指向的返回位址來反推真正呼叫者，撞到一個已知的DOSBox-X上游bug
(`MEMDUMPBIN`回報成功但檔案沒真的寫出，見`58-remake-live-verification-log.md`續四十九/續五十)，
`D`指令的替代讀法這次沒能成功讀到值(顯示還是`na`，可能是指令格式問題，本輪沒有進一步排查)，
反推呼叫者這件事本輪未完成，留給下一輪。

**與續十八的關係**：續十八結尾把「原始卡死bug是否還會重現」列為「最根本的證偽測試」，本輪
的答案是明確的**「會重現，bug是真的」**——續十八「或許只是移動流程走錯」的猜測不成立。但
續十八「亞雷斯的ring能真正走到`0x18d8c`」這個發現依然成立(只是這次的repro路徑裡，那個
`0x18f22`命中沒有再次出現在通往卡死卡的這條特定路徑上——可能`0x18d8c`只在「上」鍵那次
Up+Enter組合的第一層outer ring處理時被呼叫過一次，而卡死卡是第二層「全藍子選單再按Enter」
另外觸發的、走全新路徑)。

**下一輪優先任務**：
1. 找到真正呼叫`0x17eef`/`0x184c0`的第四個呼叫點——建議改用更暴力的方法：對整個`0x17000`~
   `0x1a000`範圍內、以`0x1000`為間隔設一整排臨時斷點，逐步縮小到真正的CALL指令位址，或修好
   `D`指令讀取返回位址的方法直接反推。
2. 確認`0x17fc0`(數值/名稱覆蓋層)這次為何沒命中——是這條路徑本身跳過它，還是斷點位址算錯。
3. 只要找到真正的呼叫點，這個投資了19輪的調查就能真正結案：那個呼叫點的邏輯(可能是某個
   「移動後的入口判斷分支」)就是根因所在。

**清理**：`stuckretest`已`teardown`，`status`確認清空。本輪程式碼異動：無。文件異動：本節。

## 2026-09-02 續二十：不用即時DOSBox-X，改用Ghidra headless靜態分析追出真正呼叫者——19輪
調查正式結案：「卡死」根本不是Attack的bug，是Item指令對「無消耗品」單位的正常後備畫面

延續續十九留下的返回位址(live `0x1B3E8C`)，重開一個新instance(`callertest`)重演到同一個
「卡死卡前一刻」，這次修正續十九踩到的`D`指令語法錯誤(正確格式是**`D <seg> <off>`用空格分隔，
不是`D <seg>:<off>`冒號分隔**，doc48表格早就寫過，這輪自己沒查對)，成功讀出`[ESP]`＝
`8C 3E 1B 00`＝live`0x1B3E8C`，減delta(`0x19C000`，**第9次獨立確認**)得ghidra位址`0x17e8c`附近，
用`D 0170 1B3E80`讀出該位址前幾bytes原始碼，確認`0x1B3E87`（ghidra`0x17e87`）處確實是一條
`E8 63 00 00 00`(`CALL rel32=+0x63`)指令，目標算出來精確等於`0x1B3EEF`——**逐byte確認這是一條
真正的CALL指令，不是猜測**。

**改用Ghidra headless靜態分析**(不需要即時DOSBox-X，用`FD2Analysis3`既有分析資料庫做
`getFunctionContaining`/`getReferencesTo`查詢，比一個個設中斷點快很多)，一路往上追呼叫鏈：

1. `0x17e87`所在的函式其實不是`0x17aed`本體(`0x17aed`函式體是`[0x17aed,0x17d6e]`，長度剛好
   吻合續十七研究agent說的「642 bytes」)，而是一個**先前完全沒人記錄過的獨立函式
   `FUN_00017e0b`**(`[0x17e0b,0x17eee]`，227 bytes)，它依序：配置一塊記憶體(`0x3702f`)、
   算3個座標值存進`0x53c5b/0x53c5f/0x53c63`、畫2條分隔線(`0x3771c`)、**呼叫`0x17eef`**(面板/
   立繪版面)、**呼叫`0x184c0`**(裝備清單)、再跑一個迴圈畫邊框(`0x18409`)、最後呼叫`0x4e381`
   收尾——這正是續十七研究agent形容的「非互動orchestrator」的實際內容，只是它不叫`0x17aed`。
2. `FUN_00017e0b`有3個呼叫者：`0x17afc`(在`0x17aed`函式體內，doc13記載的那條路徑)、
   `0x1b948`、`0x1c010`——**後兩個都不在`0x17aed`範圍內**，是全新位址。
3. `0x1b948`在`FUN_0001b932`(`[0x1b932,0x1b9dd]`)裡，`0x1c010`在`FUN_0001bffe`
   (`[0x1bffe,0x1c141]`)裡。兩個函式的程式碼**幾乎一模一樣**：都是先呼叫`0x17e0b`，
   接著**把`[0x53c57]`(doc13記載的ringSel全域)直接寫死成`0`**，再呼叫共用的`0x1b9de`。
4. `FUN_0001b932`本身又被6個地方呼叫，其中4個(`0x1bce1`/`0x1bea0`/`0x1bf5a`/`0x1bfd0`)都在
   同一個更大的函式`FUN_0001bbdc`(`[0x1bbdc,0x1bffd]`，1057 bytes)內部——這是一個內容豐富、
   多分支的函式，明顯是**一整套獨立的「裝備/欄位」ring UI狀態機**，跟doc13記載的戰鬥指令環
   完全是不同的程式碼。
5. **關鍵一步**：`FUN_0001bbdc`本身只有**一個**呼叫者——`0x19056`。而`0x19056`剛好落在
   `0x18d8c`本體函式範圍內(`[0x18d8c,0x190ab]`)，**只比續十九/Round19文件記載的「Item分支
   入口`0x19055`」多1 byte**！反組譯`0x18f22`附近確認`0x18d8c`一開頭就是
   `CMP [0x53c57],0x0 / JNZ 0x19005`——如果`[0x53c57]`(ringSel)是`0`(Attack)才會往下走
   `0x18f2f`那條候選掃描鏈路，否則跳到`0x19005`繼續比對Spell(1)/Item(2)/Wait(3)。

**★★★ 最終結論(19輪投資，正式結案)**：這整個「亞雷斯Attack卡死」的前提**從一開始就理解錯了**。
續十八/續十九反覆確認過`[0x53c57]`在這個repro路徑裡讀到的值是**`0x02`**——這正是doc13/
`main.go`註解記載的**Item(物品)的索引**，不是Attack(索引應該是`0`)。也就是說：**這整個調查
從續九開始，每一輪對亞雷斯按「上」鍵，實際上執行的都是「物品」指令，不是「攻擊」指令**——
可能是這個特定ring狀態下方向鍵到選項的對應關係跟外層戰鬥指令環的「↑攻擊/←法術/→物品/↓待機」
不同(例如殘留了上一次的選取狀態、或者這個ring的方向映射本身就不一樣，本輪沒有進一步查證
「為什麼按上鍵卻選到物品」這個更細節的子問題，留待需要時再查)，但**「物品」指令選中後走到的
`FUN_0001bbdc`→`FUN_00017e0b`→`0x17eef`/`0x184c0`這整條鏈路，渲染出來的畫面正是亞雷斯的
裝備清單(刺矛+AP020/布衣+DP002)**——這完全合理：**亞雷斯的消耗品欄位是空的，「物品」指令
在沒有可用道具時，很可能就是設計成顯示裝備清單作為後備唯讀畫面，Enter沒有反應、只有Escape
能退出，是因為這根本不是一個需要選擇目標的互動畫面，是一張純資訊卡**。這不是bug，是
**正常的、對「無消耗品單位」的Item指令後備行為**。

**這也完整解釋了為何前18輪(續九~續十九)反覆卡在同一個現象、且從未在候選掃描(`0x14818`)/
目標確認(`0x115b6`)/`0x17aed`本體/`0x117e7`任何一個位址命中過斷點**——因為執行流根本沒有
進入Attack分支(`0x18f2f`)，一直都在Item分支(`0x19055`)，所以doc13記載的Attack鏈路上所有
位址自然都不會被命中，跟`0x18d8c`本身或亞雷斯這個單位是否「壞掉」完全無關。

**真正還沒查的、優先度很低的後續問題**：
1. 為什麼「上」鍵(直覺應該對應Attack)反而選到了Item(索引2)？可能跟`[0x53c57]`是殘留狀態
   (上次選單裡帶過來的，比如通過先前的「左→Enter→Escape」測試序列殘留成2)有關，也可能是
   deployment階段這個特定ring的方向映射本身就跟戰鬥指令環不同。要查的話：在按任何方向鍵之前
   先讀`[0x53c57]`初始值，然後每按一次方向鍵就重讀一次，而不是像續十八/續十九那樣只在
   Enter後才讀。
2. 真正嘗試「左」(法術，doc13索引1)、「下」(待機，索引3)在亞雷斯身上是否也一切正常——如果
   這個理論正確，這兩個應該都不會卡（除非同樣因為某種原因選到Item）。
3. 對Sol/Yuni也測一次「物品」指令，確認他們是否也會因為消耗品清單為空而看到同樣的裝備後備卡
   (如果會，這進一步證實這是通用機制而非亞雷斯專屬)。

這三點都是「錦上添花」的驗證，不是「找根因」——根因(`FUN_0001bbdc`鏈路 + Item分支 + 空消耗品
清單後備行為)已經用靜態反組譯+返回位址+函式呼叫圖三重交叉確認，證據链完整、每一步都有具體
位址可查證，不依賴任何猜測。

**清理**：`callertest`已`teardown`，`status`確認清空。本輪程式碼異動：無(Ghidra分析用
`-readOnly`，未寫回專案)。文件異動：本節。
