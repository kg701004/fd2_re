...
| 反組譯與 SDD | FD2.EXE 的戰役狀態機、事件處理器、戰鬥原始介面、存檔外層、物品類型 5–24 與操作證據持續收斂；敵方物理落點、評分、原版路徑方向及無可用行動時的備援鏈已分段閉合。FDFIELD 的 `b17/b18/b19` 也已固定為執行期 `+0x34/+0x35/+0x36` 來源，33 張地圖的原始模式分布已資料化；模式 2／11 的精確分支與 `0x13FD4` 五分之一 HP 回復亦已閉合。全域事件表已由錯誤的 58 筆更正為 90 筆；格子事件與全部 33 圖寶物格／16 槽獎勵表均已接入可編輯資產，map25 的 event58 五選一寶物及 event59–61 格子規則也已解碼，event59/60 已依原版向左格步驟提交點接入。FDTXT_026 已更正為 63 個實際顯示單位，事件 61 的文字 2／3／4 可精確映射；第 26 章也不再讓渥德的 group 1 提前進場。另已發現先前只有 map0 帶原版索引 renderer 輸入，現以固定雜湊的 FDFIELD／FDSHAP 補齊全部 33 圖 | AI 各低四位已有可觀察矩陣，但玩法名稱、事件 82 的一般玩家觸發與完整回合執行期仍未閉合。event61 的文字、59 幀 adapter、D0／entry12／JOIN31 交易已接正式 wait 路徑；map25 view 已由 ch25 pre-handler 的 PAN(9,39)→FOCUS_UNIT(0) 與游標狀態機推導，不再依賴測試注入，但尚缺同狀態 DOSBox 視覺比較，不能宣稱 E2。其餘特殊事件寶物、索引效果繪製器、完整戰後／城鎮順序仍有 `[~]`／`[ ]` 項 |
...
selection0 原版幀；初次並排與像素差圖把剩餘差異定位到共用字型 shadow
...
pulse frame。selection5 後 Enter 進入 ch02 secret variant5 的 service0
phase0/2 亦已取得兩組 raw RGB 整幀相同 E2，並抓出及修正 production pulse
雙重除以二。四個 service、Right/Left wrap 與 Escape 返回 hidden selection5
亦已逐幀配對。ch02 武器店variant1、道具店variant3與秘密商店variant5的service0 selected
phase目前也都與原版全幀相同；shop剩餘主缺口已轉為四項service的child panel
與其他章節狀態，而不是三種主選單背景。service0 Enter 後的 ch02 武器購買
...
| ch02 隱藏 selection5 再確認後的秘密商店 service0／selected pulse（左原版 DOSBox、右 source-built remake；gold0、320×200 raw RGB 整幀相同） | ![secret shop original and remake](docs/figures/secret-shop-ch02-original-vs-remake.png) |
| ch02 秘密商店四個 service 與 Escape 返回 hidden town（上排原版 DOSBox、下排 remake；每格320×200全幀AE=0） | ![secret shop services and return](docs/figures/secret-shop-ch02-services-return-original-vs-remake.png) |
...
| ch01 原版戰場 HUD（原版錄影 434.5 秒的較晚狀態；camera `(1,13)`、absolute cursor `(8,15)`、visible cursor `(7,2)` 與下列 remake 對齊；FD2.SAV 初始快照另為 `(1,13)/(8,17)/(7,4)`，且此圖非同 roster/event/tick 的完整像素差分） | ![original map HUD](docs/figures/native-map-ch01-original-video.png) |
