...
目前 `campaign_full.json` 有 121 個 story/cutscene 節點：9 個 direct script、42 個 handler-bound；
其餘 70 個分成 30 個 retreat、23 個 rumor、13 個 unbound postbattle、4 個 generic story fallback；
13 個 unbound postbattle 另有 generated binding skeleton（全節點共 24 個，含 11 個 active handler 的對照檔），
但未經 override/compile gate 不算 active handler。
工具不會依章號自動套劇本，避免把 pre/post/分支 scene 誤接。
逐項缺口可用唯讀 `python3 tools/audit_postbattle_binding_gates.py` 檢查；目前 13 個仍是
blocked；ch04/ch05/ch08/ch09/ch10/ch11/ch12/ch18/ch19 已以 compiler regression 驗證並提升為 active handler；ch05/ch08/ch09/ch11/ch19 的 acting resources 由 Docker exporter 逐幀解碼，其餘 mapping-complete 檔仍不會自動寫回 `handler_binding`。
