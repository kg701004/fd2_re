...
> **2026-09-02 使用者指示：`remake/`（Go／Ebiten 重製引擎原始碼）已整個移除。**
> 原因：remake 側 live 測試被判定為不可信的驗證來源（「remake驗證過的資料本身就有問題，
> 驗出來的也會有問題」），連帶已 commit 的相關修復也一併回退。本文件其餘段落中提到
> `remake/` 的內容（畫面比對、進度表、路徑連結）反映的是移除前的歷史狀態，並非目前倉庫
> 現況；程式碼本身可從 git 歷史還原（`git log --all -- remake/`）。反向工程（RE）相關的
> 文件、資料、工具（`docs/`、`tools/`、`extracted/`、`org_game/`）不受影響，正常維護。
