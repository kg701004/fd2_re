...
其餘 74 個分成 30 個 retreat、23 個 rumor、17 個 unbound postbattle、4 個 generic story fallback；
17 個 unbound postbattle 另有 generated binding skeleton（全節點共 24 個，含 7 個 active handler 的對照檔），
...
blocked；ch09/ch10/ch12/ch18/ch19 已以 compiler regression 驗證並提升為 active handler；ch09/ch19 的 acting resources 由 Docker exporter 逐幀解碼，其餘 mapping-complete 檔仍不會自動寫回 `handler_binding`。
