...
| 反組譯與 SDD | FD2.EXE 的戰役狀態機、事件處理器、戰鬥原始介面、存檔外層、物品類型 5–24 與操作證據持續收斂；敵方物理落點生成、逐列順序與 `0x14237` 評分已分段閉合，並與 `0x1548E` 執行鏈分離 | AI 無攻擊方案的移動、原始輔助欄位語意及完整回合執行期尚未閉合；索引效果繪製器、完整戰後／城鎮順序仍有 `[~]`／`[ ]` 項 |
